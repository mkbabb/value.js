...
const colorToVec = (color: RGBColor<number>): Vec3 => {
    return [color.r, color.g, color.b];
};

const rgb = (r: number, g: number, b: number) => new RGB<number>(r, g, b);
const hsl = (h: number, s: number, l: number) => new HSL<number>(h, s, l);

const RGBColorToHSLColor = (color: RGBColor<number>): HSLColor<number> => {
    const result = rgb2hsl(color);
...
function hueRotate(color: RGBColor<number>, angle: number = 0): RGBColor<number> {
...
function grayscale(color: RGBColor<number>, value: number = 1): RGBColor<number> {
    const matrix: Mat3 = [
...
function sepia(color: RGBColor<number>, value: number = 1): RGBColor<number> {
    const matrix: Mat3 = [
...
function saturate(color: RGBColor<number>, value: number = 1): RGBColor<number> {
    const matrix: Mat3 = [
...
function applyColorMatrix(color: RGBColor<number>, matrix: Mat3): RGBColor<number> {
    const result = transformMat3(colorToVec(color), matrix);
    return rgb(clamp(result[0], 0, 255), clamp(result[1], 0, 255), clamp(result[2], 0, 255));
}

function brightness(color: RGBColor<number>, value: number = 1): RGBColor<number> {
    return linearFilter(color, value);
}

function contrast(color: RGBColor<number>, value: number = 1): RGBColor<number> {
    return linearFilter(color, value, -(0.5 * value) + 0.5);
}

function linearFilter(color: RGBColor<number>, slope: number = 1, intercept: number = 0): RGBColor<number> {
    const vec = colorToVec(color);
    return rgb(
        clamp(vec[0] * slope + intercept * 255, 0, 255),
        clamp(vec[1] * slope + intercept * 255, 0, 255),
        clamp(vec[2] * slope + intercept * 255, 0, 255),
    );
}

function invert(color: RGBColor<number>, value: number = 1): RGBColor<number> {
    const vec = colorToVec(color);
    return rgb(
        clamp((value + (vec[0] / 255) * (1 - 2 * value)) * 255, 0, 255),
        clamp((value + (vec[1] / 255) * (1 - 2 * value)) * 255, 0, 255),
        clamp((value + (vec[2] / 255) * (1 - 2 * value)) * 255, 0, 255),
    );
}
...
function spsa(
    ...
    values: number[],
    iters: number,
    target: RGBColor<number>,
    targetHSL: HSLColor<number>,
...
function solve(target: RGBColor<number>): { values: number[]; loss: number; filter: string } {
...
export function rgb2ColorFilter(color: RGBColor<number>): { values: number[]; loss: number; filter: string } {
    return solve(color);
}
