 ...
 * `difference.ts` (`xyzToICtCp` / `ictcpToXYZ` — the BT.2100/BT.2124 matrices +
 ...
 * exactly what `xyzToICtCp` consumes); the ICtCpColor carries [0,1]-normalized
...
import { ictcpToXYZ, xyzToICtCp } from "../difference";
...
export function xyz2ictcp(xyz: XYZColor): ICtCpColor {
    const [I, Ct, Cp] = xyzToICtCp(xyz.x, xyz.y, xyz.z);
...
export function ictcp2xyz(color: ICtCpColor): XYZColor {
    ...
    const [x, y, z] = ictcpToXYZ(I, Ct, Cp);
