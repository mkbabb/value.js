// The abstract `Color<T>` base class + the `ColorChannel<T>` phantom brand +
// the typed channel accessors (`ch` / `channelOf` / `setChannel`).
//
// S.W1 W1-8 (god-module class-hierarchy split): the base class is lifted OUT of
// `index.ts` (the barrel) into this LEAF module so the 15 subclasses in
// `spaces.ts` can `extends Color` from a module that is guaranteed fully
// evaluated before they are. The wave census asked for base to stay in
// `index.ts`, but that is empirically infeasible in this cyclic graph: a barrel
// that both declares `Color` AND re-exports `spaces.ts` (which extends it) TDZs
// the subclass `extends` clause ("Class extends value undefined") because ES
// re-exports evaluate the subclass module before the barrel body declares
// `Color`. A dedicated leaf is the only cycle-robust home. `index.ts` remains
// THE barrel — every consumer's `import { Color, ch, RGBColor } from "."` is
// preserved via the barrel's re-export; nothing outside this subgraph changes.
//
// LEAF INVARIANT: this module imports ONLY leaves (utils/clone, constants types,
// the leaf `serialize.ts`). It must NEVER transitively import `dispatch.ts` /
// `spaces.ts` — that would re-form the eval cycle. `serialize.ts`'s color2/
// gamutMap are late-bound (registered by dispatch) precisely to keep this leaf
// clean; see `serialize.ts`'s header.

import { clone } from "../../utils";
import type { ColorSpace, WhitePoint } from "./constants";
import {
    convertColorSpaceDenorm,
    formatAnimationColor,
    formatAnimationNumber,
    formatColor,
    formatNumber,
    writeAnimationScratch,
} from "./serialize";
...
export abstract class Color<T = number> {
    ...
    toAnimationString(digits: number = 4, outputSpace?: ColorSpace): string {
        ...
        // B1 — write each channel into the shared (serialize-private) scratch, no
        // intermediate array. `Number(channel)` coerces both a raw number and a
        // parsed `ValueUnit` (numeric `valueOf`) in the no-convert path; `none`/NaN
        ...
        return formatAnimationColor(space, count, alpha);
    }
