...
//
// X.P.W6.b — the WPT cases, READ from the vendored files rather than retyped. Ported from X.P.W5.c
// (`docs/tranches/X/parse-that/evidence/W6/retired-seam/typescript/test/css-color5/wpt-cases.mjs`).
//
...
// literal the file writes yields exactly the cases a browser runs — no case is chosen by this
// repository's author.
...
export const WPT_FILES: Readonly<Record<string, string>> = Object.freeze({
...
export type WptCase = Readonly<{ file: string; kind: string; input: string; expected: string | undefined }>;
 ...
 */
export function loadWptCases(file: string): WptCase[] {
    ...
    const cases: WptCase[] = [];
    const record = (kind: string) => (...args: string[]) => {
