...
import type { Palette, User, ProposedColorName } from "@lib/palette/types";
import type AdminUsersPanel from "@components/custom/palette-browser/AdminUsersPanel.vue";

export function useAdminOperations(deps: {
    ...
    const filteredApproved = computed(() => {
        const q = deps.searchQuery.value.toLowerCase();
        if (!q) return approvedColors.value;
        return approvedColors.value.filter(
            (item) => item.name.toLowerCase().includes(q) || item.css.toLowerCase().includes(q),
        );
    });
