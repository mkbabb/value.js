...
**The gamut-ink netting facility (`@lib/gamut-ink`, S.W5-8 / owner-ruling §1.2).** The truth-line inks above are *tokens*; their PAINT primitives live in ONE module — `@lib/gamut-ink` — so the netting idiom is never copied per-plate. It owns: `createInkProbe` (a hidden span in the plate's own cascade whose computed style resolves the four `--gamut-*` tokens, `color-mix` and all, to concrete canvas inks — cached per scheme); the `WEBBING` table (period 6px / 45° / 1px weight) + its derived `HATCH_STEP`; `drawHatch` (the lattice line-painter); the `SECOND_NET` paper-underlay + dashed-ink treatment (§1.2b) for sRGB-excess runs under wide interpolation spaces, with `drawSecondNet`; and the shared `DPR_CAP`. Two consumers today — the picker's spectrum-plate overlay (`gamutOverlayPaint.ts`) and the gradient page's perceived-space plate (`perceivedSpacePaint.ts`); geometry stays library-owned (`@src/units/color/boundary`), scheduling the consumer's.
