...
**Name**: W3 — The material ladder · neutrals · ink (D1 deployed as the ONE material system;
D6 as the ONE ink contract; D9's shadow-palette grammar; population-gated)
**Opens after**: T.W1 (round 2 — runs parallel with T.W2; disjoint surfaces: materials vs boot
chain, the §3.2 law).
**Spec of record**: `audit/SYNTHESIS.md §3` T.W3 rows (items W3-1..W3-5) · §2 D1 (THE FOUR-RUNG
MATERIAL LADDER) + D6 (INK-ON-TIER) + D9 (SHADOW-PALETTE STATE GRAMMAR) · §1.1 R2/R7/R8 (what
retires vs what SURVIVES — E-3 must not over-delete) · Q4/Q9 outcomes from `T.md §12` · the
C2/C3/C5 reconciliations · §6.1 rows O-7/O-9/O-11/O-18 · the §6.2 CSS tripwire
[AMENDED-AT-PASS-2].
...
ONE material system + ONE ink contract, population-gated. (SYNTHESIS §3 T.W3 Goal, verbatim.)
The D1 thesis governs: *glass earns its blur by floating over live content; a surface that sits
IN a plate is a tone of the plate, not a second pane of glass.*

## §Completion criterion

Every W3 item gate green (per-item gates transcribed; T.md §3.1 summary): O-7 census oracle
green + t-mobile F-8 390-frame · O-9 all-cases · O-7 row for the search seat · O-11 gates 1-6 ·
O-18 population contrast census both schemes at the owner's reference color · CSS gz ≤120 KiB
at the wave gate.

---

## §Scope (SYNTHESIS §3 T.W3 item table, transcribed verbatim — gates intact; anchors + evidence appended)
...
tone-step of the plate, NO backdrop-blur — the markdown `bg-muted` pattern generalized; producer
rung P3) · **3 CHROME** (true floating glass, as shipped) · **4 STAGE** (named near-black pair
`--stage`/`--on-stage-chrome`). RETIRES: S-20 `bg-card/75` as species (R8 — the one-species GOAL
survives, re-grounded at the picker's rung) · the 6 parallel well mints · raw
`bg-black`/`bg-white` · the 7th bespoke header recipe (CC-3). Corollary (T-24): with rungs 1-2
in place every in-view surface ≥ ~0.7 effective alpha of ONE neutral family — the hue fork dies
by construction. **The C3 LAW rides this wave**: color appears only as color DATA;
chrome/material/type are neutral; the complete sanctioned-exception ledger (ramps, dots, strips,
netting, field, blob, Palettes-rainbow, admin-gold) is encoded in DESIGN.md at W3-1. Rung
assignments needing taste sign-off ride **Q4** (gradient perceived-space plate → well ·
eyedropper overlay stays chrome · PaletteCard = well · the sliders console = rung-2 WELL, the
owner-word re-cut surfaced for ratification). Mobile frame: every rung judged at 390 over the
aurora's brightest band (t-mobile F-8 — the stricter reading sets the floor).

**D6 (transcribed — the contract)**: *the referent is a property of the surface the text sits
on, never a global constant.* De-emphasis becomes a floor-clamped rung of certified ink, never
post-hoc opacity (the guard-then-alpha class dies). Producer stake: tiers publish effective
lightness (P3/P5 contract row) — the demo threads what exists now and consumes the published
values at W7.
...
- **bounds expansion**: any `../glass-ui` write (the WELL rung, the rest-floor/feather knobs,
  and tier-lightness publication are P3 — interim demo mints are sanctioned ONLY as booked
  swaps); any write to the picker knot files (W4's) beyond reading settled tokens; any `src/`
  write (the guard family `safeAccentColor`/`wcagContrastRatio` already takes `bgL` — a missing
  library seam is a W1/src defect filing, not a W3 improvisation);
- **non-local gate failures**: the O-7 census finding a surface that fits NO rung (the ladder
  is incomplete — a Q4-class taste row, route to ratification, never mint an off-ladder
  material); O-11 gate 4 showing layout-track work (the F3 fork survived somewhere — halt,
  the compositor-only law is structural); O-18 failing on a site whose tier lightness is
  unpublishable (the P3 contract row gap — record the interim referent + book the swap);
  CC-1 reproducing after P8 lands (the packet premise refuted);
- **loop halt**: the third iteration of any rest-floor eye-bracket tuning loop halts and
  routes (the bracket [0.45, 0.65] is the ratified frame — landing outside it is ratification
  material).
...
| Item | Files | Access |
|---|---|---|
| W3-1 (core) | the tier tokens (style.css/theme token blocks) · the 9 pane-card tier stamps · the 6 well-mint sites (die) · `--well-bg` interim · STAGE tokens · DESIGN.md (C3 law + ledger) | modify |
| W3-2 | `ShadowPalette` NEW (colocated with the card family per E-1) · the three host classes | create/modify |
| W3-3 | the search seat sites (`.input-bar` consumers on panes) | modify |
| W3-4 (core) | `PaneHeader.vue` (material + veil; W4 later takes GEOMETRY only) | modify |
...
## §Hard gate (verbatim-faithful to SYNTHESIS §3 T.W3 + §6.1/§6.2)

1. **W3-1**: O-7 card-material census green — every pane + named fixture resolves to its ONE
   rung (MEMBERSHIP, not fixed alpha), both schemes, all panes + fixtures; the t-mobile F-8
   390-frame judged over the aurora's brightest band; the 6 well mints dead (grep); raw
   `bg-black`/`bg-white` dead; the C3 law + exception ledger in DESIGN.md.
2. **W3-2**: O-9 all-cases — zero-result ⇒ ghost present + `aria-hidden` + caption carries AT
   ("never a bare text plate"), per host class; error states UNTOUCHED (plain register — R7's
   surviving semantics: error ≠ empty, never announce work that isn't happening).
3. **W3-3**: the O-7 row for the seated field green; judged beside dashed-well + card in-frame;
   mono is opt-in content-class only.
4. **W3-4**: **O-11 gates 1-6** green (rest floor ∈ [0.45, 0.65] of the veil · no band · no
   double-exposure · compositor-only with the CDP layout track FLAT · engine/PRM coherence ·
   one grammar — no in-card sticky above `--z-header`); swell completes ≤64px; the supersession
...
7. PP-8 repo-wide sweep · PI-1 Lighthouse delta recorded · `npm run lint` 0 · `npm run
   typecheck` 0 · `npm test` green · e2e green (incl. the O-7/O-9/O-11/O-18 mints) · clean
   `git status`.

## §No-workaround prohibitions (binding)

- **NO off-ladder material mints** (Q9's rejected alternative; the T-CM-4 class) — every
  surface lands ON a rung; a surface that fits no rung routes to ratification.
- **Color only as DATA** (C3) — exceptions ONLY per the encoded ledger; no new chromatic
  chrome.
- **NO per-site contrast guards, NO guard-then-alpha, NO opacity de-emphasis** (D6 retires the
  class, not instances).
- **CC-1 is producer-root (P8)** — no demo cascade arms-race re-painting bare `.glass-wash`;
  the two sites (`GradientStopEditor.vue:171`, `GradientCodeEditor.vue:93`) get their rung when
  the packet lands (consume note recorded).
- **Interim mints are sanctioned ONLY with booked swaps** (`--well-bg` · search seat · feather)
  — each carries its P3 row by name.
- **E-3 must not over-delete**: R2/R7/R8's SURVIVES columns are binding (the feather, the
  scroll modulation, the aria split, the one-species GOAL).
...
Saved at close (cited in `PROGRESS.md`): the O-7 census table (site → rung → both schemes, +
the 390-frame captures); the O-9 per-host captures; the O-11 six-gate record (incl. the CDP
layout-track trace); the O-18 census table (selector → ground → ratio, both schemes); the CSS
gz number at gate; the retirement greps (well mints, bg-black/white, BG_LIGHTNESS constants);
the DESIGN.md ledger diff; π paired frames (both schemes × 3 viewports) toward the W8 package;
per-item commit hashes.

## §Commit plan

W3-1 (tokens + rung adoption, may split tokens/panes; the C3 DESIGN.md encode as its own docs
commit); W3-2 (ShadowPalette, own commit with body citing R7's survives column); W3-3 (search
...
- **W3-1 interim `--well-bg` → the P3 WELL rung** — booked swap; fires at W7 (or earlier if
  the file:-pin surfaces it).
- **W3-3 interim search seat → the P3 seated field-chrome rung** — booked swap.
- **W3-4 demo feather carried → the P3 rest-floor + bottom-feather knobs** — booked swap; the
  F3 layout-animation fork retires onto the producer scroll grammar.
- **CC-1 consume note** — the two `.glass-wash` sites paint their rung the day P8 lands.
- **The P3/P5 tier-lightness contract row** — W3-5 threads today's known composited values;
  the published-values consume fires at W7.

## §Evidence packets consumed

`audit/lanes/t-card-material.md` §1-§3 · `audit/lanes/t-card-color-census.md` (§4 deployment
table + CC-2/CC-3/CC-5/G1) · `audit/lanes/t-shadow-palette.md` · `audit/lanes/t-search-tabs.md`
§1-3 · `audit/lanes/t-header-shading.md` (F1-F6, §3/§4) · `audit/lanes/t-a11y-contrast.md`
(F-1..F-4) · `audit/lanes/t-mobile-audit.md` (F-7/F-8) · `audit/lanes/t-contradictions.md`
(C2/C3/C5) · `audit/lanes/t-gradient-surfaces.md` §1 (the netting's future re-judge surface —
W6-1 reads this wave's plate).

## §Hand-off

T.W4 opens on this wave (+W2): the console is authored as a rung-2 WELL against settled tier
tokens (C5); the certified-ink contract prices the letter rail and channel strip (O-18 rungs);
the veil GEOMETRY (W4-7) re-derives against a header whose MATERIAL is settled here. W6-1
re-judges the netting on THIS wave's gradient plate (R5 note); W6's surfaces inherit the rung
grammar. The booked P3 swaps fire at W7; the O-7/O-18 censuses join the standing suite the W8
critique passes re-run.
