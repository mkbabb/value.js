...
## §Lane structure (the amended single-writer map, transcribed verbatim — total over all 8 items)
...
- **bounds expansion**: any `src/` write (W6-2 consumes W1-src's sampler — ZERO new demo math; a
  missing/wrong export is a **W1 defect** and the lane HALTS on it, never improvises); any
...
- **loop halt**: the third iteration of any netting-intensity or chip-proportion tuning loop
  halts and routes (taste brackets belong to W8/the owner).

## §File bounds · disjointness · worktrees

| Lane | Files (pre-move anchors; re-derive via the W1 MOVE-MAP) | Access |
|---|---|---|
...
legs); W6-6's banner-mount removal (one line, `App.vue:115` pre-move) lands through W5's App.vue
queue (recorded in both wave logs). `Dock.vue` is Lane D's; W5's R7 row (PKT-3-gated) coordinates
through Lane D's queue if it fires in-round. `GradientStopEditor.vue` is Lane G's; W5's R9 retime
rides inside W6-2's re-authored paint stack (recorded in both wave logs).

Do NOT touch: `src/`, `../glass-ui`, `../keyframes.js`, `docs/precepts/`, the boot chain (W2's,
closed), the tier tokens + `PaneHeader.vue` (W3's, closed), the W4 picker composite
(`ColorSpaceSelector.vue:54-57` is a read-only precedent cite). Each lane runs in its own sibling
worktree cut from the wave head.

## §Hard gate (verbatim-faithful to SYNTHESIS §3 T.W6)

1. **O-13 slims in the SAME COMMIT as the T-10 excise** (the census's one AT-RISK row — else it
...
- **The register law**: rings on WatercolorDots ride the dot's own silhouette or do not exist —
  no interim geometric ring re-mint at the seal (Q12 ABROGATE; a future fitted ring is P5's
  solid-ring register, producer-side).
- **T-29 is DEMO-owned** — no portal/teleport workaround for the Tools clip; the release is the
  settle-stamped `overflow` cure at `Dock.vue`, whole-shadow preserved.
- **Never a lying preview** — a chip that approximates the library output is FORBIDDEN (O-14 is
  byte-identity, not resemblance).
- **REDESIGN, never excise** for W6-5 — the Regenerate verb is re-seated, not deleted.
...
Saved at close (cited in `PROGRESS.md`): per-lane before/after visual evidence (both schemes;
390 frames for W6-1/W6-4); the O-13 same-commit hash; the O-14 byte-identity output; the O-19
luma-delta measurements; the O-21/O-17/O-20 spec outputs; the O-22 variant matrix + the W0-1
contract verification record; the O-15a/b captures; the Q5/Q12 ratified-outcome records; per-lane
commit hashes.

## §Commit plan

Per-lane item-scoped commits (G: netting recal / envelope plate / rail re-author; E: easing
...
- **W6-3 regex-driven autoplay → booked DELETION** the day P7's declarative `autoplay`/`playing`
  door lands (T.md §7.2; re-verified at W7).
- **P5 solid-ring register** = the standing law for any future WatercolorDot ring (opened by
  W6-7's abrogation; the fitted-ring alternative consumes it at W7 only if Q12 rules that arm).
- **P7 EasingPicker v2** producer half — the demo composition lands now; the v2 consume fires at
  W7 (booked swap).
- **`.underline-tabs` + A6 MARKER retire-checks** — NOT this wave's (W7 walks them; legacy-sweep
  F6/F7).
- Boundary note: the ShadowPalette species grammar (D9) is **W3-2's**, landed in round 2 — W6
  consumes its registers where surfaces meet, never re-mints (no double-write).

## §Evidence packets consumed

`audit/lanes/t-gradient-surfaces.md` §1-§6 · `audit/lanes/t-easing-pane.md` ·
`audit/lanes/t-nav-dropdowns.md` · `audit/lanes/t-misc-elements.md` F2/F3 ·
`audit/lanes/t-outline-dropdown-clip.md` §1/§2 · `audit/lanes/t-api-state.md` (the T-9 strata) ·
`audit/lanes/t-card-color-census.md` CC-4/CC-5 · `audit/lanes/t-mobile-audit.md` F-6/F-9 ·
`audit/SYNTHESIS.md` §1.1 R1/R5/R9/R10 · `letters/GLASSUI-T-ASKS.md` P5/P7/P8.

## §Hand-off

W8's critique passes judge these surfaces against the doctrine + the owner's verbatim lines;
W7 fires the booked swaps (P7 consume, the P5 register law) and walks the MARKER retire-checks.
The O-13-slimmed view-accents oracle, the O-14 truth law, and the W6-7 register law join the
standing slate; W9 re-runs the repo-wide sweeps these excisions must survive.
