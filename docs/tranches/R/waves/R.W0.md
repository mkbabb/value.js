...
**Status**: SPECED — awaits the Q-table pass (Q4 rides this wave; Q3 is closed on the record).

---

## §Goal criterion

The tree tells the truth before any R work stands on it: the untracked design treatments are committed (they are the R.W3/R.W5 specs); the probe scratch is gone and self-ignoring; the proof-purge tail is committed; the ten published-but-untagged versions carry annotated tags; `master` carries what its released tags reference; the P/Q/N close records exist; the docs say true things (parse-that pin, the `/math` load-bearing leaf, the pin policy); and the pass-2 worktree evidence is safely in-tree before the worktrees die.
...
|---|---|---|---|---|
| W0-1 | `docs/frontend-design/` treatments | `?? docs/frontend-design/` untracked; `color-picker.md` 25 KB, `hero-lab.md` 32 KB (sizes were **swapped** in R1-ASBUILT) | **COMMIT both**, then **apply the overlay-amendment packet P1–P10** (`audit/pass2/overlay-amendment.md §5`, incl. the pass-3 P8 `:140` clause) to `color-picker.md` as a **second commit** — the merged treatment is the R.W3 spec | both tracked; amendment commit present |
...
- **Q4 — ratify or flip** (pin policy; speced default encoded here): **keep `file:../glass-ui` + `file:../keyframes.js` deliberately** and RECORD it in CLAUDE.md at W0-12 — registry pins during active co-development are theater ("3.13.0" and "BA 4.0.0" both went stale before mattering); the protecting discipline is the adopt-event BOOKS + the by-name MIGRATION tables + **`boot-smoke` cold** as the named-export drift catch-all. The alternative (registry-pin at 5.0.0) makes every glass-ui break an explicit version event at the price of twice-disproven pin staleness. W0-8's KEEP is the same policy applied to the peer-provision chain.
...
Clean `git status`; `master` carries 1.2.0; `git tag` == registry (≥ v0.6.0); P/Q/N records exist; treatments committed + amended; keyframes devDep kept + recorded; docs spot-check true; pass-2 evidence hoisted (already done — verify tracked) + worktrees cleaned.
...
R.W1 consumes `seeds/kf1.patch`; R.W2 consumes `seeds/vite-config-boot-cure.patch` + `vite.config.cured.ts`; R.W3 consumes the amended `docs/frontend-design/color-picker.md` (W0-1's second commit); R.W5 consumes `hero-lab.md`. The SHIM-oracle wording (W0-2) is owed to the R.W2/R.W3 gate text — this wave discards the files, never the knowledge.
