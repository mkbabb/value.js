...
| **R.W2** | FUNCTIONAL TRUTH — boot cure, Tabs→SegmentedTabs, N.W10 rows, PRM, hero-lab artifact deletion (Q1), kill-list | `waves/R.W2.md` | 1 | **EXECUTING** — first half landed; completion lane re-dispatched (rate-limit death); boot/e2e held RED by the producer-side D8-1 recurrence (escalated) | — |
...
| Book | Trigger | Status |
|---|---|---|
| D8-1 no-shim verify | glass-ui rebuilds dist with `layer(components)` (ask dispatched to the live BG agent at pass-2 time; **ESCALATED 2026-07-03**, glass-ui `1599c230` — the 15:10 BG rebuild re-emitted the unlayered import at `dist/styles/index.css:266` AND moved `/styles` onto the emitted tree, so the defect now blocks value.js boot at every ≥lg viewport) | **ARMED + ESCALATED** — BG live; R.W2's boot/e2e gate clauses held RED by it |
