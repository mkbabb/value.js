...
await st("after-canvas-tab"); await page.screenshot({ path: OUT + "m-light-probe2-0-collapsed.png" });
...
for (const w of [300,900,2500]) { await page.waitForTimeout(w); await st("after-tap-dock+"+w); } log.push("hit@eq-center " + JSON.stringify(await page.evaluate(() => { const e = document.querySelector("[aria-label='Equation']"); const b = e.getBoundingClientRect(); const h = document.elementFromPoint(b.x + b.width/2, b.y + b.height/2); return { hit: h?.closest("[aria-label]")?.getAttribute("aria-label"), tag: h?.tagName, eqVisible: e.checkVisibility?.({ opacityProperty: true, visibilityProperty: true }), dockOverflow: getComputedStyle(document.querySelector(".controls-dock-anchor .glass-dock")).overflow, dockW: document.querySelector(".controls-dock-anchor .glass-dock").scrollWidth + "/" + document.querySelector(".controls-dock-anchor .glass-dock").clientWidth }; }))); await page.screenshot({ path: OUT + "m-light-probe2-1-expanded.png" });
const eq = page.locator("[aria-label='Equation']");
log.push("eq count " + await eq.count());
if (await eq.count()) { const eb = await eq.first().boundingBox(); log.push("eq bb " + JSON.stringify(eb)); await page.touchscreen.tap(eb.x + eb.width / 2, eb.y + eb.height / 2); await page.waitForTimeout(1500); await st("after-tap-eq"); await page.screenshot({ path: OUT + "m-light-probe2-2-eq.png" }); }
writeFileSync(OUT + "probe-mobile2.log", log.join("\n") + "\n");
