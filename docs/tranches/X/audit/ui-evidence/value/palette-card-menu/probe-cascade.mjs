...
const out = await page.evaluate(() => {
  ...
  const del = [...menu.querySelectorAll('[role="menuitem"]')].find((e) => e.textContent.includes("Delete"));
  const rules = [];
  const walk = (list, layer) => { for (const r of list) { try {
    ...
    if (r instanceof CSSStyleRule) {
      for (const [el, tag] of [[del, "del"], [menu, "menu"]]) {
        try { if (el.matches(r.selectorText) && (r.style.color || r.style.fontFamily || r.style.fontSize || r.style.getPropertyValue("--font-mono") )) rules.push({ tag, layer, sel: r.selectorText.slice(0, 140), color: r.style.color, ff: r.style.fontFamily.slice(0, 80), fs: r.style.fontSize }); } catch {}
      }
    } } catch {} } };
  for (const s of document.styleSheets) { try { walk(s.cssRules, "(unlayered)"); } catch {} }
  return { delClass: del.className, menuClass: menu.className, delColor: getComputedStyle(del).color, rules };
});
