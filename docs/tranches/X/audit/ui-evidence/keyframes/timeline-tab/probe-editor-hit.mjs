...
await page.keyboard.press("End"); res.afterEnd = await page.evaluate(() => document.activeElement.className.toString().slice(0,40)); await page.keyboard.type(" oklch(", { delay: 30 }); await page.waitForTimeout(400); res.midType = await page.evaluate(() => document.activeElement.className.toString().slice(0,40)); await page.waitForTimeout(1200);
...
await page.screenshot({ path: OUT + "40-probe-css-error-1440-light.png" });
writeFileSync(OUT + "probe-editor-hit.json", JSON.stringify(res, null, 2)); console.log(JSON.stringify(res, null, 1));
await b.close();
