...
// route engine-fusion COPY of value.js src/css/bbnf/math.ts — ONLY tokenQuantity's number/unit split differs.
//
...
const NUMERIC_PREFIX = /[+-]?(?:\d*\.\d+|\d+)(?:[eE][+-]?\d+)?/y;
export function tokenQuantity(token: string): Numeric {
    // route engine-fusion: the grammar already delimited the token; the number/unit split is one
    // sticky prefix test (no match array). Same split as `NUMERIC` for every token the grammar yields.
    NUMERIC_PREFIX.lastIndex = 0;
    const cut = NUMERIC_PREFIX.test(token) ? NUMERIC_PREFIX.lastIndex : -1;
    const digits = cut < 0 ? "" : token.slice(0, cut);
    const rawUnit = cut < 0 ? "" : token.slice(cut);
