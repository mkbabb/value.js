...
// <define:__LEVERS__>
var define_LEVERS_default = { stockAsciiDispatch: true };
...
function positionalRules() {
  if (typeof __POSITIONAL__ === "undefined" || !__POSITIONAL__) return void 0;
  ...
  if (p === void 0) {
    ...
    if (t.compiled === void 0) {
      ...
      const opts = {
        ...
        levers: typeof define_LEVERS_default === "undefined" ? void 0 : define_LEVERS_default
      };
...
function textOf(value2) {
  if (Array.isArray(value2)) return value2.map(textOf).join("");
  return typeof value2 === "string" ? value2 : "";
}
var isTag = (value2) => typeof value2 === "object" && value2 !== null && !Array.isArray(value2);
function tagsOf(value2) {
  if (isTag(value2)) return [value2];
  return Array.isArray(value2) ? value2.flatMap(tagsOf) : [];
}
function tag(value2, key) {
  return tagsOf(value2).find((t) => key in t)?.[key];
}
function attachStylesheetActions(rules) {
  ...
  const text = (key) => (value2) => ({ [key]: textOf(value2) });
  const trimmed = (key) => (value2) => ({ [key]: textOf(value2).trim() });
  const items = (value2) => tagsOf(value2).filter((t) => "item" in t).map((t) => t.item);
  for (const run3 of ["commaRun", "semiRun", "spaceRun", "argRun", "syntaxPart"]) on(run3, trimmed("item"));
