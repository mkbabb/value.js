  ...
  switch (expr.type) {
    ...
    case "alternation": {
      ...
      for (const alt of alts) {
        cs.union(exprFirstSet(alt, firstSets, nullable2, ast));
      }
      return cs;
    }
  }
...
function buildDispatchTable(alternatives, firstSets, nullable2) {
  ...
  for (const alt of alternatives) {
    if (exprIsNullable(alt, nullable2, /* @__PURE__ */ new Map())) return null;
    const cs = exprFirstSet(alt, firstSets, nullable2, /* @__PURE__ */ new Map());
...
var emptySet = () => new CharSet2();
var fullSet = () => {
  const s = new CharSet2();
...
// docs/tranches/X/parse-that/evidence/W7-research/judge/harness/synth/emit-aot.ts
...
`;
      ...
      case "regex": {
        const re = e.value;
        const run2 = singleClassRun(re);
        if (run2 !== null && run2.all) {
          return `if (${pos} >= s.length) ${out} = -1; else { ${keep ? `V = s.substring(${pos});` : ""} ${out} = s.length; }
`;
        }
        if (run2 !== null) {
          const T = constName(run2.tbl), C = constName(new RegExp(run2.cls.source, run2.cls.flags + "y"));
  ...
  if (${q} - ${pos} < ${run2.min}) ${out} = -1; else { ${keep ? `V = ${q} > ${pos} ? s.substring(${pos}, ${q}) : undefined; ` : ""}${out} = ${q}; }
}
`;
        }
...
`;
  ...
  while (queue.length > 0) {
    ...
    if (a !== void 0 && mode === "v") {
      const A = opts.aot ? (aotActs.push(name), `AK${aotActs.length - 1}`) : constName(a.fn);
...
`;
    return { rule: () => {
      throw new Error("aot: load the module");
    }, value: () => void 0, source, moduleSource };
  }
...
// docs/tranches/X/parse-that/evidence/W7-research/judge/harness/synth/load-gen.ts
...
var OPTS = { stripFb3: false, stripped: 0 };
function stripCaseWorkaround(text) {
  return text.replace(/\/((?:\\.|\[(?:\\.|[^\]])*\]|[^/\\\n[])+)\/i\b/g, (lit, body) => "/" + body.replace(/\[([a-zA-Z])([a-zA-Z])\]/g, (cls, a, b) => a !== b && a.toLowerCase() === b.toLowerCase() ? (OPTS.stripped++, a.toLowerCase()) : cls) + "/i");
}
function compileGrammar() {
  let text = Object.values(GRAMMAR_MODULES).join("\n");
  if (OPTS.stripFb3) {
    OPTS.stripped = 0;
    text = stripCaseWorkaround(text);
  }
...
function run() {
  throw new Error("generation shim: no parsing");
}
function emitModule(rules, o) {
  ...
  const own = o.positional ? new Set(BBNFToASTWithImports(stylesheet_default)[1].rules.keys()) : /* @__PURE__ */ new Set();
  return emitGrammar(ast, {
    actions,
    positional: (n) => own.has(n),
    levers: o.stockAscii ? { stockAsciiDispatch: true } : void 0,
    aot: true,
    header: o.header
  }).moduleSource;
}
