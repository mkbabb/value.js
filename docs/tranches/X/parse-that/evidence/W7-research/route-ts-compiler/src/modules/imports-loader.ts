...
// route-ts-compiler (F-b-1 root cure): no `node:path`/`node:fs` — module IDs resolve through the pure
// `posix-path` module, and every read goes through the HOST reader the caller supplies (a files map
// in a browser bundle, `fs.readFileSync` in a node host). Otherwise bbnf-lang 0.1.4's loader verbatim.
import * as path from "./posix-path.js";

import type { Expression } from "../../vendor/bbnf-0.1.4/types.js";
 ...
 * use `path.resolve`.
 */
function canonicalize(filePath: string, _useRealFs: boolean): string {
...
export function loadModuleGraphSync(
    ...
    const useRealFs = false;
    if (readFileSync === undefined) throw new Error("bbnf: loadModuleGraphSync needs a host reader (no filesystem is assumed)");
...
export async function loadModuleGraph(
    ...
    const reader =
        readFile ??
        ((_p: string): Promise<string> => { throw new Error("bbnf: loadModuleGraph needs a host reader (no filesystem is assumed)"); });
