...
## §18 — X.P.W7.gap: three valid forms the retired parser refused, cured in BBNF and rowed beside F-b-4 (SERVED MODEL: claude-opus-5-5 · 2026-09-25) — DATED, BESIDE (E-3)

Ruled by W7.md ADDENDUM (h) (COHESION §0ee): value.js is mirror-primary, so product correctness outranks retired parity; the new mismatches against the retired parser on exactly these three forms are a ruled delta, like F-b-4 (§16) and CP-CASE (§17).

| field | reading |
|---|---|
| **classes** | **GAP-ADJ** adjacent functions with no whitespace (`rotate(0)scale(1.3)`) · **GAP-URL** an unquoted `url(…)` (`url(./x.svg)`) · **GAP-GRP** a `()` group inside a math function (`calc(50% - (1em / 2))`). |
| **spec** | GAP-ADJ: css-syntax-3 §4 (`)` then a `<function-token>`), css-transforms-1 `<transform-list> = <transform-function>+`. GAP-URL: css-syntax-3 §4.3.6 `<url-token>` (whole token; §4.3.14 `<bad-url-token>` refused). GAP-GRP: css-values-4 §10.1 `<calc-value> = … \| ( <calc-sum> )`. |
| **cure (value.js `src/`)** | `src/css/grammar/value.bbnf`: `termSep` gains `(?<=\))(?=[a-zA-Z_-][\w-]*\()` (only a function token may stand against a `)`); `urlCall`/`urlBody` (the url token) with `urlHead` excluding the unquoted form from `call`; `mathCall` (the css-values-4 math function names) over `mathComma`/`mathSlash`/`mathSpace`/`mathTerm`, where `mathTerm = group \| valueTerm` and `group = ( mathComma )` — a group stands only inside a math function. Actions in `src/css/bbnf/value.ts` (`urlValue`, `groupValue` = a call with the empty name; the math lists reuse `listOf`). Re-emitted by the pinned emitter (bbnf-lang `f0059db14`). Pinned by `test/css/bbnf-gap.test.ts` (14; RED 11 on the prior bytes). |
| **oracle rows** (`bench/paired/equiv.mjs`) | 163 of 1,376,531, none non-ASCII: GAP-URL 79 (18 sources: `url(/)` family and sheets with `@import url(x.css)` / `@namespace svg url(…)`; 8 now accepted, 71 still refused with the diagnostic moved to the first unread component or to `syntax_mismatch` in `coerceToSyntax`) · GAP-GRP 83 (8 distinct sources: real calc/min/max/gradient/transform values from the real corpus, and the sheets carrying them incl. bulma declaration 946; 15 accepted, 68 moved) · GAP-ADJ 1 (the monaco sheet, now accepted whole). The standing 92 (88 F-b-4 + 4 CP-CASE) are unchanged row for row. |
| **value differential** (`bench/css-equivalence`) | `w6-classes.ts` gains GAP-URL (repair: the url spelled `url("…")`) and GAP-GRP (repair: each outermost group spelled `0`), mechanism tests: assay `parseCssValue(s)` GAP-URL 4, real GAP-GRP 6; MIRROR-DEFECTS 0. No GAP-ADJ cell exists in either corpus, so no class is written for it. |
| **consumer direction** | **CHANGES a value:** the three forms are accepted; `url(a b)` (a bad url the product used to read as a two-item list) is refused; a math group is `{ kind: "call", name: "" }`. CHANGELOG `[Unreleased]` records it. |
| **gate reading** | V-1 / L-G2 reads 255 = 88 F-b-4 + 4 CP-CASE + 163 GAP; equal-work prefixes 528 rows, 0 mismatches (the retired arm still refuses these forms, so the cut does not move; E-3). |

## §18-A — X.P.W7.gap2: three more valid forms the retired parser refused, cured in BBNF and rowed beside §18 (SERVED MODEL: claude-opus-5-5 · 2026-09-25) — DATED, BESIDE (E-3)

Ruled by W7.md ADDENDUM (i) (COHESION §0ee), under ADDENDUM (h)'s law: the new mismatches against the retired parser on exactly these three forms are a ruled delta, like §18.

| field | reading |
|---|---|
| **classes** | **GAP-LN** grid line names (`[rail] 1fr [end]`) · **GAP-URL0** the empty `url()` · **GAP-MUL** `*` with no whitespace inside a math function (`calc(1px*2)`). |
| **spec** | GAP-LN: css-grid-2 §7.2 `<line-names> = '[' <custom-ident>* ']'` (a `[]` simple block, css-syntax-3 §5.4.8). GAP-URL0: css-syntax-3 §4.3.6, a `<url-token>` with empty contents. GAP-MUL: css-values-4 §10.1 (only `+` and `-` need whitespace; `/` was already the slash list's separator). |
| **cure (value.js `src/`)** | `src/css/grammar/value.bbnf`: `lineNames` (one leaf: `[`, whitespace-separated identifiers only, `]`) as a `valueTerm` before `operator`; `urlCall`'s body optional (`( urlBody ? )`) with `urlHead` no longer excluding `url()` from itself; `mathSpace` separated by `mathSep = termSep \| /\s*(?=\*)\|(?<=\*)/`. Actions in `src/css/bbnf/value.ts`: `lineNamesValue` (a keyword spelled canonically `[a b]`), `urlValue` (no argument → `url` call with no args). Re-emitted by the pinned emitter (bbnf-lang `f0059db14`), sha256 `96c3fa63…`. Pinned by `test/css/bbnf-gap.test.ts` (25; the 11 new tests RED 8 on the prior bytes, the 3 stay-refused guards passing both sides). |
| **oracle rows** (`bench/paired/equiv.mjs`) | 141 new of 1,376,531, none non-ASCII: GAP-URL0 65 (5 sources `url()` `url( )` `url(  )` `url(\t)` `url(\n)`: 10 accepted in the two value entries, 55 moved to `syntax_mismatch` in `coerceToSyntax`) · GAP-LN 63 (3 real sources `[label] auto [value] 1fr`, `[rail] var(--rail-track) [stage] minmax(0, 1fr)`, `[top] auto [stage] 1fr [bottom] auto`: 6 accepted, 33 `syntax_mismatch`; and 12 sheet texts handed to `parseCssValue`/`parseCssValues` (24 rows) whose leading `[data-x]` now reads as line names, the refusal moving from `[data-x]` to the `{` after it) · GAP-MUL 13 (`color-mix(in hsl, red calc(var(--v)*1%), blue)`: 2 accepted as a generic call, 11 `syntax_mismatch`). **One §18 row moved:** `collect:parseStylesheet(sheet)` on the keyframes-js-index sheet (a GAP-URL row), whose refusal moves from `[rail]` to `[top]auto[stage]1fr[bottom]auto` (line names juxtaposed with no whitespace, not cured here). The other 254 of the prior 255 (88 F-b-4 + 4 CP-CASE + 162 §18) are identical row for row. |
| **value differential** (`bench/css-equivalence`) | `w6-classes.ts` gains GAP-URL0 (repair: `url("")`) and GAP-LN (repair: each block spelled the ident `x`), mechanism tests: assay `parseCssValue(s)` GAP-URL0 5, real GAP-LN 3; MIRROR-DEFECTS 0. No GAP-MUL cell exists in either corpus, so no class is written for it. |
| **consumer direction** | **CHANGES a value:** the three forms are accepted. A line-names block is a keyword (`[a b]`), so `coerceToSyntax(…, "<custom-ident>")` accepts a lone block as it already accepts a string or an operator keyword (pre-existing looseness, not widened in kind). CHANGELOG `[Unreleased]` records it. |
| **gate reading** | V-1 / L-G2 reads 396 = 88 F-b-4 + 4 CP-CASE + 163 §18 + 141 §18-A; equal-work prefixes 528 rows, 0 mismatches (the retired arm still refuses these forms; E-3, the corpus is not regenerated). |
