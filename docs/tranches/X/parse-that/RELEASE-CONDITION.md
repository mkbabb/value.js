...
**ADDENDUM 2026-09-23 — the RC-P re-read of record (X.P.W6.x, dated beside; E-3; SERVED MODEL: claude-opus-5-5).** Authority: W6.md §Units RC-P line; COHESION §0by. This re-read replaces nothing above; it states the predicate as it now evaluates.
- **The seam** is value.js's own BBNF grammar: `src/css/grammar/{tokens,math,color,value,stylesheet}.bbnf`, compiled by `@mkbabb/bbnf-lang` onto `@mkbabb/parse-that`. The hand-rolled `src/css/grammar.ts` was deleted at `7e60d700`, with no shim; the stylesheet layer's own scanners retired at `dff875e8`. value.js now reads CSS text only through that grammar.
- **Conjunct 4 `ADMITTED(V)` RETIRES.** Its subject, a `.wasm` lowering of the AC-1 algebra, no longer exists: the algebra retired at X.P.W6.r, and no value.js release ships or depends on a Wasm target. The §2.4 dependency-closure addendum above is superseded with it. `RC-P(V)` is the conjunction of the other five: 1 PUBLISHED · 2 TOTALITY · 3 EQUIVALENCE · 5 BAR-DISCHARGED · 6 ROUTED.
- **Conjunct 3 `EQUIVALENCE(V)` reads the instrument that measured the swap.** That instrument is value.js `bench/css-equivalence/`: the six-entry differential and the `parseStylesheet`/timeline differential, both against the retired parser read back from git at `2155142b`. It needs MIRROR-DEFECTS 0 and STYLESHEET DEFECTS 0, and every divergence must be ruled (DIVERGENCE-LEDGER §15, the SH-1 class). It is read against `V`'s packed `/css`.
- **Conjunct 5 `BAR-DISCHARGED`** reads the bench of record the D-23 bar names: value.js `bench/css-parse.bench.ts`, BBNF grammar vs the retired hand parser, recorded at `bench/records/`. Whether it passes is still the owner's OC-1 ruling. This addendum does not invent a bar.
- **KF.W3 stays gate-keyed** on this re-read predicate at X-W11's coordinate. It is never scheduled.
