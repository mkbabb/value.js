...
// X.W12U.s2 · UIA-V-133 · V-331 · V-333 · V-334 · V-335 · V-336 frame (SHOT=<png>); same flow as probe-migrate.mjs, stops before Cancel. 6 saved
...
if (open) {
    ...
    after = await read();
    if (after && !process.env.SHOT) { await p.locator("[role=dialog]").getByRole("button", { name: "Cancel" }).click(); await p.waitForTimeout(600); cancelled = (await read()) === null; }
}
