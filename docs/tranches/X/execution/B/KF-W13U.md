...
## Check 5 — FRESH ADVERSARIAL CHECK (L-20, pass 2 on Close 2 / Repair 3)

SERVED MODEL: claude-opus-5-5 · 2026-09-23 · Track B · VERIFY-ONLY (0 kf / glass / product bytes). The dispatch calls this "Check 2" of the round. It is named "Check 5" because `## Check 1..4` already stand above (E-3). Read: the spec whole (`KF-W13.md`, 395 L), the record header through `## Unit plan`, the RESUME 2 plan, `## Close 2`, `## Check 4` and `## Repair 3`. Crash-recovery: kf holds the two untracked `VALUEJS-INBOUND-*` letters only. The value.js dirty paths (`CARRY-LEDGER.md`, `scripts/dev/dev.sh`, `demo/**`, `package*.json`, …) belong to sibling seats, and none is in this seat's set (the record and the LEDGER). **0 inherited paths.** kf HEAD = `origin/master` = `9bdcdad5`. Load ⟨`uptime`⟩ at open was 69.7 / 39.0 / 37.0.

### Axis 1 — claimed GREENs re-run by this seat (kf `9bdcdad5`, dev `http://localhost:5173/` HTTP 200, headed Chromium, the banked probes copied to scratch)

| gate | ⟨cmd⟩ | this seat | reproduces |
|---|---|---|---|
| `.t2` | ⟨`grep -rn isAnimStarted demo test \| wc -l`⟩ | `0` | YES |
| `npm run check` | ⟨`npm run check`⟩ | `proof:structure — PASS`, EXIT 0 · EXIT 0 | YES |
| vitest demo | ⟨`npm run test:demo -- --no-file-parallelism`⟩ | `66 passed (66)` · `518 passed (518)`, EXIT 0, ×2 (one run concurrent with the e2e build, at load ≈ 70–160) | YES (Repair 3: 518) |
| G-KFW13U-t drag limb, the banked probe | ⟨`node probe-t1.mjs`⟩ (`t/probe-t1.mjs`, unchanged) | paused 4949.5 / 4782.3 → mid `2300` → late `3800` → afterUp `3800`; cube `rotateX(155.273deg)` → `(317.237deg)`; errs `[]`, ×2 | YES — C4-3 CURED |
| G-KFW13U-t drag limb, forced race | ⟨`node probe-t1-burst.mjs`⟩ (`repair-c4/`) | late `3950` · afterUp `3950`, cube `rotateX(327.499deg)`, stillPaused 2, errs `[]`, ×2 | YES (pre-cure 5000 per Repair 3) |
| the R-close-1 witness | contained in the test:demo 518 | the `R-close-1 (KF.W13U repair)` case passes | YES |

Cited from Check 4 and not re-run, because 0 bytes have moved on their surfaces since (`9bdcdad5` touches `PlaybackRibbon.vue` and its contract test only): the G-KFW13U-w, -e, OA-32 and OA-33 Share-row GREENs (7/7 at Check 4). Claimed GREENs re-run by this seat: **5 gates, all reproduce; 0 fail.**

### Axes 2–9

- **(2) Bounds.** ⟨`git -C keyframes.js show --stat 9bdcdad5`⟩ → `PlaybackRibbon.vue` +26/−4 · `playback-ribbon-contract.test.ts` +36. Both are in `.t`'s row (`demo/components/playback/**` · `test/demo/**`). ⟨`git show --stat 21b8f5c2`⟩ (value.js receipt) → this record, `LEDGER.md` +1 and `evidence/W13U/repair-c4/probe-t1-burst.mjs`, all in bounds. ⟨`git log --format=%h 8868e3d8..HEAD -- scripts/dev/dev.sh \| wc -l`⟩ → `0`. The seven Close-2 shas were audited in bounds by Close 2 and Check 4. **In bounds.**
- **(3) Masking.** The `9bdcdad5` diff was read whole. It has no try/catch, skip, allowlist, settle, producer byte or reka reach. The cure is a gesture-owned rail model (`railT = gestureT ?? currentT`), cleared at the drag seam's `onEnd`, with the read-back as the one authority at rest. The witness is RED on the pre-cure bytes (`expected '4800' to be '1000'`, Repair 3). **0 masking.**
- **(4) Families.** `9bdcdad5` is one meaning in one sha (the cure + its witness). **Unsplit.**
- **(5) E-3.** ⟨`git diff --stat 8868e3d8..HEAD -- KF-W13.md registry/adjudicated/`⟩ → `KF-W13.md \| 17 +` only. Those lines come from the orchestrator's addenda commits (`f0f7218d` · `55dc67ea` · `df281b84` · `15409ece`, append-only), and none is a KF.W13U sha. The registry diff is empty. **Held.**
- **(6) Mail.** The INBOX's latest sweep lines (15:1x F.W14 OPEN; X-W7R OPEN) record 0 unrowed. I-40 is Track C (out of scope), and I-41..I-44 are READ or FOLDED. **0 UNREAD in scope.**
- **(7) Four-verb.** Close 2 holds IMPLEMENTED = PARTIAL and VERIFIED = NO, and Repair 3 did not stamp. **Lawful.**
- **(8) Goal at the bytes.** The owner's docket: the cube animates (YES); the timeline is live at boot and a press-then-drag scrubs the playhead and the cube (**YES**, now including the forced race); easing label and dropdown (YES); living icons (YES); the mbabb collapse and Share keyboard (YES; the theme row is producer). **Not met:** the scene switch still changes the dock width and the pane surface set more than once on first visits (ESC-d3-1). The close clause "kf e2e GREEN" is not met.
- **(9) Published figures.** Repair 3's figures reproduce: 518/518; banked probe `2300 → 3800` with cube 317.237°; burst `→ 3950` with cube 327.499°; e2e 4/6 with the same two failures.

### Axis 10 — honest-RED adjudication

This seat re-read the kf e2e: ⟨`npm run gh-pages && KF_PLAYWRIGHT_DIR=<value.js> npm run demo:correctness -- --workers=1`⟩ (load 162.1 → 62.1) → **`passed: 4/6`, EXIT 1**. It fails ✗ live-session on S4 alone (`ringPainted:false`, with `enterToggled` true; S5 PASS) and ✗ live-session-mobile on M1 SCROLL alone (`scrollTop=0; 765px content in a 704px body`). subject-animates PASS.

A relief search found no ruling. ⟨`grep -n 'QUIET-FOCUS-RING\|DRAWER-DETENT-REACH\|ESC-d3-1' docs/tranches/X/COHESION.md`⟩ → **0 hits**. COHESION's newest addendum is §0cc (OA-50, fourier admin, unrelated). `KF-W13.md` has no addendum after §0cb, and the honest-RED list at the §0br supplement still carries only the five ids.

| RED gate | relief at the spec bytes? | owner named? | reading |
|---|---|---|---|
| G-KFW13U-d two-owners / morph blur | YES: `.d` names `DOCK-MORPH-ROOT` (O-56) / `DOCK-SCROLL-MORPH` (O-55) | glass BL → KF.W13R `.d` | **HONEST-RED** |
| OA-33 theme-row keyboard | YES: §0br `.d4` names `DARK-MENU-ITEM` (O-61 R-3) | glass BL → KF.W13R `.v` | **HONEST-RED** |
| `GLASS-SURFACE-PAINT-CONTAIN` · `KF-TIMELINE-FILL` | YES: the §0br honest-RED list | glass BL → KF.W13R `.v` | **HONEST-RED** |
| close clause "kf e2e GREEN": S4 ring (ESC-x-1 `QUIET-FOCUS-RING`) · M1 (ESC-x-2 `DRAWER-DETENT-REACH`) | **NO.** §0br grants `.x` to "cure usability · S4 · S5 · M1 at their causes", and neither id is in the relieved list. The proposed owner, KF.W13R, opens only after this wave CLOSES, which is circular. No dated relief has been issued since Check 4 / Repair 3. | proposed, not ruled | **UNRELIEVED, HIGH** (carried from C4-1) |
| `.d3` ESC-d-1 change-once (width, surface set) | **NO.** It is a consumer gate; its causes are consumer bytes outside the grant; there is no id and no successor routing. No grant has been issued since Repair 3. | none named | **UNRELIEVED, HIGH** (carried from C4-2; 0 bytes on its surface since, so the measured RED stands as cited) |
| G-KFW13U-t drag limb (R-close-1) | — | — | **CURED** (reproduced above) |
| R-close-2 `[real-cube]` intermittent | — | KF.W13V `.k` | MINOR, mitigated (PASS again here: 4 PASS of the last 5 reads) |
| R-close-3 `probe-sharp` confound | — | KF.W13R `.v` | LOW |

### Successor "Opens after" conjuncts

- **KF.W13R**: "KF.W13U CLOSED" is **RED**, so it is lawfully blocked.
- **KF.W13V**: "KF.W13R CLOSED" is RED and "KF.W13U CLOSED" is RED; the two audit registers are GREEN (Check 4). It is lawfully blocked.
- The deadlock Check 4 named still stands. ESC-x-1/-2's only named cure is in KF.W13R, which cannot open before this close. Breaking it is the orchestrator's act, by a dated relief naming S4-ring and M1 with owner KF.W13R, or by re-sequencing KF.W13R `.m` ahead of this close. A check seat cannot relieve it.

### Register

| # | severity | claim | receipt | cure |
|---|---|---|---|---|
| C5-1 | HIGH | Close clause "kf e2e GREEN" is RED at 4/6 (S4 `ringPainted:false`, M1 touch scroll 0), and no spec or COHESION relief names `QUIET-FOCUS-RING` / `DRAWER-DETENT-REACH` | this seat's roster run, EXIT 1; COHESION grep 0 hits | orchestrator: a dated COHESION relief naming S4-ring and M1 with owner KF.W13R (glass 7.0.0 → 10.0.1), or re-sequence KF.W13R `.m` ahead of this close; never a consumer ring/drawer copy |
| C5-2 | HIGH | `.d3` ESC-d-1 change-once limbs are RED, with no grant or relief issued | Close 2 / `.d3` probe-switch10 ×6; 0 bytes on `App.vue` / ChromeDock since | orchestrator: a dated grant over `App.vue` `:248-250` / `:309-315` / template `:38-65` plus a ruling on the Scene label, then cure at the route-keyed reads |
| C5-3 | MINOR | R-close-2 `[real-cube]` intermittent | PASS here | KF.W13V `.k`, mitigated |
| C5-4 | LOW | R-close-3 `probe-sharp` is confounded by the living glyphs | Close 2 masked variant | KF.W13R `.v` |
| C5-5 | INFO | C4-3 / R-close-1 is verified CURED at `9bdcdad5`, at its cause, with a born-RED witness | probe-t1 ×2 `→ 3800`; burst ×2 `→ 3950`; 518/518 ×2 | none |
...
### KF.W13U.d5 — the one commit point for the scene swap's chrome (SERVED MODEL: claude-opus-5-5)

Spec: `KF-W13.md` seventh addendum §2 (`:404-415`) · COHESION §0cd (`:3140-3144`, the file's last addendum) · cause map = `.d3` receipt step 5 (this record `:642-700`: (a) route-keyed `storedControls` pick projected against the held set, (b) EditorShell `channels`/`super-key`/`#tabs-content`, (c) the route-bound Scene label).

1. **Crash-recovery.** ⟨`git -C keyframes.js status --porcelain`⟩ → the two untracked `docs/tranches/V/coordination/VALUEJS-INBOUND-2026-07-{24,27}-*.md` only (no writable set; untouched). value.js: `evidence/W13U/d5/` absent; record + LEDGER clean (dirty `X-W7R.md` / `X-P-W6.md` are Tracks A/D's). **0 inherited paths.** kf HEAD `9bdcdad5` = origin.
2. **BEFORE ×2 (dev, the bytes `9bdcdad5`).** Probe copied ⟨`sed 1s… d3/probe-switch10.mjs > d5/probe-switch10.mjs; diff`⟩ → line 1 (the SERVED-MODEL comment) only; logic unchanged. ⟨`node probe-switch10.mjs http://localhost:5173/ before-dev{1,2}`⟩ (load 48.7 · 55.8) → ⟨`node sum.mjs before-dev1 before-dev2`⟩: width `1122111111` ×2 · reversals `0001000000` ×2 · surface-set `2222111110` ×2 · Controls-label `0012111110` ×2 · empty 0 · pageerrors 0 — **RED ×2, reproduces the brief's BEFORE and `.d3`'s figures exactly.** Scene-label vs Controls-label flip frame: square>spring `68/76`, spring>easing `68/75` (cause (c): two width steps).
3. **Anchors at the true bytes (kf `9bdcdad5`).** `App.vue`: ChromeDock `:current-scene-id` `:20`; EditorShell `:channels` `:39` + `:super-key` `:40` + `:stage-mode` `:44`; `#tabs-content` `:63`; `#ribbon-content` `:67-71`; `currentChannels` `:248`; `storedControls` `:250` (keyed on the route's `currentSuperKey`); `sceneBoundToCurrent` `:287-291`; `dockSelectedControl` `:309-313`. `ChromeDock.vue`: Scene `<Select :model-value="currentSceneId">` `:446` (the trigger label is its `<SelectValue/>`), `currentIcon` `:139`, the rows' `@pointerenter` warm `:471`. `scenes.ts` `warmScene` `:113`. The brief's `App.vue:311`/`:41`/`:64` sit within ±3 lines of these; INTENT applied at the true bytes. The addendum's `demo/shell/ChromeDock.vue` is `demo/app/dock/ChromeDock.vue` at the bytes (the only ChromeDock; the writable set names it).
4. **The cure (kf `febb3bcd`, pushed ⟨`git push origin master`⟩ → `9bdcdad5..febb3bcd  master -> master`; 3 paths, +77/−26).**
   - `App.vue`: `sceneBoundToCurrent` hoisted above the stored-controls key (same predicate, unchanged); ONE `resolvedScene` computed `{ id, superKey, stageMode, api }` using Vue 3.5's previous-value computed arg: while `!sceneBoundToCurrent` it returns the HELD value (same identity → no downstream trigger), and the moment the predicate turns true it re-projects the route + `sceneRef` — every read flips together in the resolve flush. Boot (no held value) projects the route. Readers moved onto it: ChromeDock `:current-scene-id` (the Scene trigger label, glyph, bold row) · `storedControls` key (so `dockSelectedControl` is the bound scene's own pick against the bound scene's own set — the destination pick is never projected against the held source set) · `currentChannels` · EditorShell `:super-key` (its `AnimationControlsGroup :key` — no route-keyed remount while pending) · `stageMode` · `#tabs-content` + `#ribbon-content`. The route (`currentSceneId`/`currentSuperKey`) remains the authority for the machine feed, the shell binding, the scene host key and `ACTIVE_SCENE_KEY` (the write gate that already suspends the leaving host's store writes — it SHOULD read the route).
   - `ChromeDock.vue`: the `currentSceneId` prop documented as the resolved scene (the Scene trigger reads the prop, not the route; the App feeds it `resolvedScene.id`); scene rows `@focus="emit('warmScene', scene.id)"` beside `@pointerenter` (warm on intent, latency only).
   - `scenes.ts`: `warmScene` docblock → "on INTENT — pointer-enter or keyboard focus". No logic change.
   - Untouched (FORBIDDEN list): `selectedSurfaceFrom` (⟨`git -C keyframes.js diff 9bdcdad5 febb3bcd --stat`⟩ → the 3 paths only; `demo/state/controlSurfaces.ts` absent) · no per-scene surface table · 0 glass bytes. Adjacent edits: **none**.
   - Witness: none landed (`test/demo/**` is outside this unit's grant; the gate is the served-page probe). Stated, not masked.
5. **Cold leg.** `evidence/W13U/d5/probe-switch10-cold.mjs` = `probe-switch10.mjs` with ONE change: the pick is a route write (`location.hash = '#/'+dest`), so no scene row is hovered or focused and `warmScene` never fires — every first visit fetches its chunk cold in a fresh context. Measured defect of the first cold form, cured in the PROBE before any banked run: with no menu open the dock's pointer-idle collapse (~2.5 s) landed inside the sampling window (a scratch dev run read width `1111112111` with a `60` px plateau — the collapsed circle — and one scratch run started a switch collapsed); the cold probe now holds the dock expanded as the open menu does in the menu probe (a 1 px pointer jiggle on the dock every 417 ms). The two scratch reports were deleted, their readings recorded here.
6. **AFTER (the committed bytes `febb3bcd`).** Dev `http://localhost:5173/` (HTTP 200); gh-pages = ⟨`npm run gh-pages`⟩ EXIT 0 (`✓ built`, after the commit) served static ⟨`python3 -m http.server 4195`⟩ from `dist/gh-pages/`. Headed Chromium `--ignore-gpu-blocklist`, 1440×900. Figures read back from the saved reports by ⟨`node sum.mjs <tags>`⟩, double-run (identical):

| run | load (1-min) | width changes | reversals | surface-set changes | empty frames | Scene/Controls label same frame | pageerrors |
|---|---|---|---|---|---|---|---|
| before-dev1 / dev2 | 48.7 / 55.8 | `1122111111` ×2 | `0001000000` ×2 | `2222111110` ×2 | 0 | 5 of 7 (square>spring 68/76, spring>easing 68/75) | 0 |
| **after-dev1** | 25.98 | `1111111111` | `0000000000` | `0011111110` | 0 | 7 of 7 | 0 |
| **after-dev2** | 34.57 | `1111111111` | `0000000000` | `0011111110` | 0 | 7 of 7 | 0 |
| **after-gh1** | 34.14 | `1111111111` | `0000000000` | `0011111110` | 0 | 7 of 7 | 0 |
| **after-gh2** | 28.43 | `1111111111` | `0000000000` | `0011111110` | 0 | 7 of 7 | 0 |
| **cold-dev1** | 63.97 | `1111111111` | `0000000000` | `0011111110` | 0 | 7 of 7 | 0 |
| **cold-dev2** | 40.15 | `1111111111` | `0000000000` | `0011111110` | 0 | — | 0 |
| **cold-gh1** | 22.77 | `1111111111` | `0000000000` | `0011111110` | 0 | 7 of 7 | 0 |
| **cold-gh2** | 28.21 | `1111111111` | `0000000000` | `0011111110` | 0 | — | 0 |

   Reading the surface-set `0`s: cube>amiga and amiga>square hold the IDENTICAL signature at source and destination (`11|Controls|div.` — same triad, same selected surface, same pane panel), so the honest count is 0 changes (BEFORE's `2` there was a blank-and-return, the pane's active panel absent for the pending frames); square>cube (the tenth) is the `0` "as today". Every switch whose set differs changes exactly once, and never more than once. **G-d5 GREEN ×8** (dev ×2 + gh-pages ×2 + cold dev ×2 + cold gh-pages ×2).
...
## Check 7 — FRESH ADVERSARIAL CHECK (L-20 pass 1 on Close 3 / RESUME 3; the orchestrator's "Check 1" of this sitting)

SERVED MODEL: claude-opus-5-5 · 2026-09-23 · Track B · VERIFY-ONLY, 0 kf / glass / product bytes. Read: the spec whole (`KF-W13.md`, incl. the §0cd seventh addendum `:397-417`), this record's header through `## Unit plan`, `## RESUME 3` whole (the `.d5` receipt), `## Close 3` whole; commits `febb3bcd` (kf) · `6d9171e1` · `e60f980f` · `09049bc7` (value.js). Numbered 7 because `## Check 1` already exists above (E-3: never overwritten). Instrument: served dev `http://localhost:5173/` (HTTP 200) + a fresh ⟨`npm run gh-pages`⟩ (`✓ built in 2.79s`, EXIT 0) served at `http://127.0.0.1:4196/`; headed Chromium `--ignore-gpu-blocklist`; banked probes copied to this seat's scratch (0 bytes in `evidence/`). Load (1-min) across the sitting 20.9–60.3.

### Axes

| # | axis | reading |
|---|---|---|
| 1 | claimed GREENs reproduce | **YES** — table below (12 gates). |
| 2 | bounds | ⟨`git show --stat febb3bcd`⟩ → `demo/app/App.vue` · `demo/app/dock/ChromeDock.vue` · `demo/app/scene/scenes.ts` (3 files +77/−26) = the §0cd whole-file grant (the addendum's `demo/shell/ChromeDock.vue` is the only ChromeDock, at `demo/app/dock/`; `scenes.ts` defines `warmScene`); `demo/state/controlSurfaces.ts` absent (forbidden list held); `git diff --check` clean. value.js `6d9171e1` = record + LEDGER + INBOX (+1 sweep line) · `e60f980f` = record + LEDGER + `evidence/W13U/d5/**` · `09049bc7` = record + LEDGER. `scripts/dev/dev.sh`: dirty by standing arrangement, last commit `85cfea2c` (T era) — untouched. **Clean.** |
| 3 | masking | ⟨`git diff 9bdcdad5 febb3bcd \| grep -c 'try {\|catch\|\.skip\|\.only('`⟩ → 0; no allowlist, no producer selector, no node_modules byte; `selectedSurfaceFrom` untouched, no per-scene table, no super-key remount (EditorShell `:super-key` reads `resolvedScene.superKey`). The cure is a Vue 3.5 previous-value `computed` holding the source projection while `!sceneBoundToCurrent` — the ruling's one commit point at its cause. **Clean.** |
| 4 | families | one kf sha for `.d5` (one meaning); the value.js receipt/close each one sha. **Unsplit.** |
| 5 | E-3 | ⟨`git diff --stat 6d9171e1~1..HEAD -- docs/tranches/X/keyframes/waves/ docs/tranches/V/megatranche/registry/adjudicated/`⟩ → `KF-W13.md` +23/−0 only, carried by orchestrator addenda `2b373908` (§0ce) + `ff9160bd` (§0cf) — append-only, not this wave's commits; none of this wave's 3 value.js shas touches the spec, registry or conformance paths. **Held.** |
| 6 | mail | ⟨`grep -nE '\| *UNREAD *\|' INBOX.md`⟩ → 0 rows; Close 3's sweep (0 unrowed) stands. **Clean.** |
| 7 | four-verb line | AUDITED/SPECIFIED YES · IMPLEMENTED YES (Close 3) · VERIFIED → this check. **Lawful.** |
| 8 | goal at the bytes | the §0cd ruling: every scene-derived dock/pane read (Scene trigger `:current-scene-id`, `storedControls` key → `dockSelectedControl`, `currentChannels`, `:super-key`, `stageMode`, `#tabs-content`/`#ribbon-content`) now reads `resolvedScene`, which holds the source whole while pending; rows warm on `@pointerenter` + `@focus`. Measured: Scene label and Controls label change in the same frame, surface set once per differing switch, in all 9 of this seat's served runs. **MET.** |
| 9 | published figures | Close 3's G-d5 figures, check, test:demo 66/518, vitest 1780, e2e 4/6 (S4 + M1), w/t/e/OA-32/OA-33 all reproduce (table). **Reproduce.** |

### Gates re-run by this seat (kf `febb3bcd` = origin/master)

| gate | this seat | reading |
|---|---|---|
| G-d5 change-once, dev (⟨`node probe-switch10.mjs http://localhost:5173/ dev{1,2}`⟩ + ⟨`node sum.mjs`⟩) | load 30.9 · 51.0: width `1111111111` ×2 · rev `0000000000` ×2 · surf `0011111110` ×2 · empty 0 · sceneLbl `1111111111` · pageerrors 0 | **GREEN ×2** |
| G-d5 change-once, gh-pages (`:4196`, gh1..gh4) | load 60.3 · 37.9 · 26.9 · 21.8. gh2, gh4: identical to dev. **gh1**: width `1211111111`, rev `0100000000` — amiga>square plateaus `410.4 → 418.3 → 60`: the 60 px plateau is the COLLAPSED dock (`ChromeDock.vue:417` `:collapse-delay="2500"` landing inside the probe's 2500 ms post-click window under load) — an instrument confound, the one change 410.4→418.3 holds. **gh3**: width `1121111111`, rev `0010000000` — square>spring `413.5 → 400.4 → 401.5`, the SAME signature as Close 3's gh2 (R-close3-1): a 1.1 px spring undershoot inside one width change. surf `0011111110`, empty 0, sceneLbl `1111111111`, pageerrors 0 in all four | commit-point limbs GREEN ×4; the "no reversal" limb reads GREEN on gh2 + gh4 (the twice the gate asks) and carries the R-close3-1 undershoot on 1 run (2 of 8 gh runs across Close 3 + this seat). The morph is the producer's: 0 consumer width transitions (⟨`grep -rn 'transition.*width' demo/app/dock demo/styles`⟩ → 0), the width animation lives in installed glass 7.0.0 `dist/dock.js` → **HONEST-RED `DOCK-MORPH-ROOT`** (O-56), owner KF.W13R `.d` (MINOR C7-1) |
| G-d5 cold (⟨`probe-switch10-cold.mjs`⟩ gh) | load 23.8: width `1111111111` · rev 0 · surf `0011111110` · empty 0 · pageerrors 0 | **GREEN** |
| `npm run check` | EXIT 0 ×2 (`proof:structure — PASS … 0 violations`) | **GREEN** |
| test:demo ⟨`npm run test:demo -- --no-file-parallelism`⟩ | 66 passed (66) · 518 passed (518) ×2 | **GREEN** |
| vitest all ⟨`npx vitest run`⟩ | 181 passed + 5 skipped files · 1780 passed + 2 expected-fail + 14 skipped | **GREEN** (= Close 3) |
| eslint (Close 3's command) | EXIT 0 | **GREEN** |
| `.t2` ⟨`grep -rn isAnimStarted demo test \| wc -l`⟩ | 0 · 0 | **GREEN** |
| kf e2e ⟨`KF_PLAYWRIGHT_DIR=<value.js> npm run demo:correctness -- --workers=1`⟩ | run 1 (load 20.9 → 28.5) **4/6** · run 2 (29.1 → 42.8) **4/6**; ✗ live-session = S4 `ringPainted:false` only (`enterToggled` and `spaceToggled` true, S5 PASS, error budget 0) · ✗ live-session-mobile = M1 SCROLL only; subject-animates `[real-cube]` PASS ×2 | **GREEN-WITH-HONEST-RED** for exactly `QUIET-FOCUS-RING` + `DRAWER-DETENT-REACH` (§0cd) |
| G-KFW13U-w (⟨`probe-w1.mjs`⟩ dev + gh) | `autoPlaying:true`; cube `rotateX(278.1°)`→ … and `(250.0°)`→ … at rest, paused frames equal; gh errs 0; dev 1 console 404 resource line, 0 pageerrors (as Close 3) | **GREEN** |
| G-KFW13U-t (⟨`probe-t1.mjs`⟩ dev + gh) | cubeAfterUp `rotateX(317.237deg)` both, stillPaused 2, errs 0 | **GREEN** |
| G-KFW13U-e (⟨`probe-e1.mjs`⟩ dev + gh) | trigger `ease-in-out` + `curve-glyph` (truth m 5e-4) · rows 29 withPath 29 distinct 29 · named 27 | **GREEN** |
| OA-32 (⟨`probe-icon.mjs`⟩ dev + gh) | diffInside 27–230 · diffOutside 0 · bboxDelta 0 · pageerrors 0 | **GREEN** |
| OA-33 (⟨`probe-kbd.mjs`⟩ dev + gh) | `enterOnShareRowOpens 1` ×2 · `enterOnDarkRowFlips false` ×2 | Share **GREEN** · theme **HONEST-RED `DARK-MENU-ITEM`** (O-61) |

### Register (severity · claim · receipt · cure)

- **C7-1 · MINOR (mitigated, owner-named)** — G-d5's "no reversal" limb carries a 1.1 px dock-width undershoot on square>spring in 2 of 8 gh-pages runs (Close 3 gh2; this seat's gh3), signature `413.5 → 400.4 → 401.5`. Receipt: gh3 report above; 0 consumer width transitions; the width morph is installed glass 7.0.0 `dist/dock.js`. The one-commit-point limbs (width-change count, Scene/Controls label same frame, surface-set once, empty 0) hold in every run, and dev reads 0 reversals ×4 across both seats. Cure: none consumer-side (a local spring copy is a forbidden mask); relieved under the spec's own honest-RED `DOCK-MORPH-ROOT` (O-56; the `.d` unit routes dock-morph jitter to it), owner KF.W13R `.d` (dock motion at 10.0.1, whose gate forbids size overshoot) as R-close3-1 — Close 3 names it.
- **C7-2 · INFO** — the switch probe's 2500 ms post-click window equals the dock's `:collapse-delay="2500"`; under load the idle collapse lands in-window and is counted as a reversal (gh1, plateau 60 px). Cure: the next seat to run `probe-switch10.mjs` (KF.W13R `.d`) holds the dock expanded as the cold probe already does (the 417 ms jiggle), or stops sampling before the collapse; no product defect.
- **C7-3 · INFO** — no vitest witness landed for `.d5` (the grant carries no `test/demo/**` row; the ruling's gate is the served probe). Stated in the receipt, not masked.

**BLOCKER 0 · CRITICAL 0 · HIGH 0 · MINOR 1 · INFO 2.**

### Honest-RED set (each gate's relief at the spec bytes)

| id | gate left red | relief | owner |
|---|---|---|---|
| `QUIET-FOCUS-RING` | kf e2e S4 `ringPainted` | §0cd §1 (`KF-W13.md:401`, `:403-404`), named by id | KF.W13R `.m` / `.v` |
| `DRAWER-DETENT-REACH` | kf e2e M1 SCROLL | §0cd §1 (`:402-404`), named by id | KF.W13R `.m` / `.v` |
| `DOCK-MORPH-ROOT` | G-KFW13U-d two owners per property; C7-1 undershoot | `.d` unit (`:327`) + §0br carried ids (`:363`); producer (O-56) | glass BL → KF.W13R `.d` |
| `DARK-MENU-ITEM` | OA-33 theme-row keyboard | §0br `.d4` (`:361`), named by id | glass (O-61 R-3) → KF.W13R |
| `DOCK-SCROLL-MORPH` · `GLASS-SURFACE-PAINT-CONTAIN` · `KF-TIMELINE-FILL` | carried, not turned by this sitting | §0br (`:363`), named by id | glass BL → KF.W13R `.d`/`.v` |

Carried residuals with owners (not reds of this close): `R-close-2`/C6-3 `[real-cube]` intermittent → KF.W13V `.k` (KFA-17; PASS ×2 this seat) · `R-close-3` → KF.W13R `.v` · `R-d2-1` `R-d2-2` `R-x-1` → KF.W13V `.k` · `R-d2-3` → KF.W13V `.u`.

### Successors

- **KF.W13R** — "Opens after: KF.W13U CLOSED" (`:367`): **GREEN on this check's status move**. Lawfully open.
- **KF.W13V** — "Opens after: KF.W13R CLOSED + the two registers" (`:367`, amending `:340`): the KF.W13U conjunct GREEN; KF.W13R CLOSED **RED** (not yet opened) → **lawfully blocked** on KF.W13R, not on this wave.

### Verdict

**CONFORMANT-HONEST-RED** — 0 BLOCKER/CRITICAL/HIGH; every claimed GREEN reproduces (12 gates); the remaining reds are each relieved by id at the spec bytes and owner-named: `QUIET-FOCUS-RING` · `DRAWER-DETENT-REACH` · `DOCK-MORPH-ROOT` · `DARK-MENU-ITEM`. LEDGER status → CLOSED 2026-09-17 (honest-RED).

### Self-count

kf shas audited 1 (`febb3bcd`) · value.js shas audited 3 (`6d9171e1` `e60f980f` `09049bc7`) · served runs: switch10 dev 2 + gh 4 + cold 1 · w 2 · t 2 · e 2 · icon 2 · kbd 2 · e2e 2 · check 2 · test:demo 2 · vitest-all 1 · eslint 1 · grep 2 · register rows 3 (MINOR 1 · INFO 2) · honest-RED rows 5 (ids 7).
