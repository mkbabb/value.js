...
SERVED MODEL: claude-opus-5-5 · kf `5cf0f58a` → **`a939e7d6`** (one commit family, not pushed) · evidence `keyframes/evidence/W13V/s2/` (value.js).

#### Acts, in order

1. **Crash-recovery.** ⟨`git -C keyframes.js status --porcelain`⟩ → the 2 standing untracked inbound letters only; ⟨`git -C value.js status --porcelain | grep -E 'W13V/s2|KF-W13V.md'`⟩ → 0. Nothing inherited.
2. **Census first (BEFORE, headed, dev).** ⟨`node evidence/W13V/s/census.mjs http://localhost:5173/ 1 s2/before/census-dev-1.json`⟩ → 1440 · 390 · 360: home 0 · cube 0 · amiga 0 · square 2 · easing 0 · spring 1 · **sequence 6** (`Re-time row 1..5 start offset` + `Scrub the sequence master clock`); Sequence dock `Controls(off) · Keyframes(off) · Timeline(off) · Scene facet(off)`. = Seat 0's banked ×2. Run 2 ran while this seat's first bytes landed under HMR (its 390/360 rows read `Timeline` enabled) — kept, renamed `census-dev-2.MIDEDIT.*`, not counted. BEFORE frames from a gh-pages build of `5cf0f58a` (scratch worktree, `127.0.0.1:4177`): ⟨`node s2/pane.mjs http://127.0.0.1:4177/ {1440x900,390x844} before-head before 0`⟩ → item `disabled:true` · stage 6 · pane lanes 0 · scrub on the stage (`s2/before/before-head-sequence-*.png`).
3. **Anchors at the bytes.** `SequenceTarget.vue:151` handles and `SequenceScrubber.vue:32` scrub — as the record's `:139`. The pane host needs an **animated** channel: `ControlsPaneWrapper.vue` `controlHosts` filters `c.animation`, so a light channel mounts no pane body; the Sequence mode's mount is therefore a §0bt adjacent edit in that file (below). Engine placement: `Sequence.entries[].at` (public, set only by `add`).
4. **Born-RED.** `test/demo/scenes/sequence-timeline-pane.test.ts` (5 cases: Timeline surface derived + item enabled · lanes = the engine's entries' `at` · a lane handle re-times the child's `at` · the master scrub is the pane's playhead · the stage's bytes hold no slider/scrubber). ⟨`npx vitest run --project demo …/sequence-timeline-pane.test.ts`⟩ at `5cf0f58a` → **5 failed (5)** (`expected [] to deeply equal [ 'timeline' ]` · `expected undefined to be defined` · `Cannot find module …SequenceLanes.vue` ×2 · `not to match /role="slider"/`) — `s2/logs/born-red.txt`.
5. **The cure (one family, `a939e7d6`).**
   - `useSequenceDemo.ts`: the one light channel declares `surfaces: ["timeline"]` and carries `sequence: SequenceTimelineSource` — `lanes()` (index · `at` · span · tone), `duration()`, `atMax`, `progress()`, `reseat` → `reseatRow` → `buildSequence` → `seq.add(child, at)` (placement truth unchanged; no child `delay`), `scrub`, `setScrubbing`/`setScrubDir` (the stage cascade still reads them), `reset`. `surfacesFor` derives `["timeline"]`; `selectedSurfaceFrom` untouched; no per-scene table.
   - `timeline/timelineTypes.ts`: `SequenceTimelineLane` / `SequenceTimelineSource`. `scene-facility/index.ts`: `ChannelHandle.sequence?`.
   - `timeline/SequenceTimeline.vue` (the pane card: `N items · D ms` + the re-time's Reset) + `timeline/components/SequenceLanes.vue` (glass-free leaf: one grid, the master scrub row = the pane's playhead control, one lane per item with its bar `[at, at+span]` and its re-time handle, a playhead line through every lane; keyboard/pointer verbs carried from the stage; a source guard names the contract).
   - Stage: `SequenceTarget.vue` loses the 5 handles, the row-drag code, the Reset button (moved with the handles) and `<SequenceScrubber/>`; `SequenceScrubber.vue` deleted; `.seq-handle` CSS removed; `ROW_TONES` moved to `sequenceMotion.ts` (the stage rows and the lanes share it). Stage = header (title · stagger · clock Metric · reel · status) + storyboard rows, travellers, axis, playhead.
   - Tests re-seated on the moved control, clauses unchanged: `sequence-scrubber-mount.test.ts` (10 cases, now on `SequenceLanes` with a stub source; the provider guard → the source guard), `sequence-instrument-truth.test.ts` (N-14 on the pane leaf); `dock-surface-items.test.ts` sequence fixture → the live shape `["timeline"]` (Controls/Keyframes/facet disabled, Timeline enabled).
   - Oracles re-seated (property unchanged): `scripts/observe/demo/live-session.mjs` sequence trigger `null` → `"Timeline"`; `live-session-mobile.mjs` A3 — the railless clause reads home only (sequence is now paned), the ghost-column clause still reads both.
   - **Adjacent edits (§0bt):** `demo/components/instrument/transport/controls-pane/ControlsPaneWrapper.vue` (+`sequenceHosts` mount of `SequenceTimeline` beside the ChannelControls hosts; the RibbonBar — whose Timeline actions address one Animation's keyframes — not rendered for a Sequence host) · `demo/components/instrument/transport/transportSource.ts` (`TransportChannel.sequence?`). Reason: the shared pane's only body host is there; without it the Timeline item would open an empty pane.

#### Gates — BEFORE → AFTER (served headed; dev `localhost:5173` ×2 + gh-pages build of `a939e7d6` at `127.0.0.1:4176` ×2; ⟨`s2/run-gates.sh`⟩; logs `s2/logs/`; load 32.20 · 23.29 · 21.93 · 63.32)

| gate | BEFORE | AFTER (dev-1 · dev-2 · gh-1 · gh-2) | verdict |
|---|---|---|---|
| **G-W13V-s1** census ⟨`census.mjs <url> 1`⟩ | Sequence stageEditors **6** at 1440/390/360 | Sequence **0** at 1440/390/360 on all four runs; home/cube/amiga/easing 0; square 2 (`Horizontal/Vertical position`); spring 1 (`Spring target`); kfBlock false everywhere | **GREEN** (judgement below) |
| **G-W13V-s2** at Sequence ⟨`pane.mjs <url> {1440x900,390x844} … 1`⟩ | Timeline item `disabled:true`, pane lanes 0 | Timeline item enabled; dock `Controls(off) · Keyframes(off) · Timeline · Scene facet(off)`; the shared pane shows the Sequence mode: lanes **5**, master scrub ✓, playhead ✓, stage editors 0 — at 1440 (desktop rail) and 390 (the sheet at peek; the lanes sit below the peek fold, as every scene's pane body does at peek — SHEET-POSITION stays glass BL F-21) | **GREEN** ×4 |
| **re-time falsifier** (drag lane 2's handle +120 px in the pane; then the pane's Reset) | not constructible (no pane mode) | 1440: stage row 2 `@260ms → @1079ms → @260ms`, traveller x `782 → 1012 → 782`, lane aria-valuenow `260 → 1079 → 260`; 390: `@260ms → @1450ms → @260ms`, x `152 → 247 → 152` — moved=true · returned=true on all 8 reads | **GREEN** ×2 dev + ×2 gh |
| **byte proxy** ⟨`grep -rlE 'Re-time row\|Scrub the sequence master clock' keyframes.js/demo/scenes \| wc -l`⟩ ×2 | 2 · 2 | **0 · 0** (the names live in `components/instrument/timeline/components/SequenceLanes.vue`) | **GREEN** |
| **floor** ⟨`npm run check`⟩ ×2 | EXIT 0 ×2 (banked) | **EXIT 0 · EXIT 0** (vue-tsc app + test projects 0, proof:structure 0 violations) — `logs/check-{1,2}.txt` | GREEN |
| **floor** ⟨`npx vitest run --project demo`⟩ ×2 | 76 files / 560 tests | **77 / 565 · 77 / 565** (the new 5-case file; every re-seated case kept) — `logs/vitest-demo-{1,2}.txt` | GREEN |
| unit test ⟨`vitest … sequence-timeline-pane.test.ts`⟩ | 5 failed (5) | 5 passed (5) | born-RED → GREEN |

**Judgement — Square 2 / Spring 1 against §0cw ("0 inline editors … on every scene").** Square's two are `sr-only` `role=slider` spans INSIDE the draggable box (`SquareScene.vue:79-96`): the accessible projection of dragging the subject itself, not a surface that edits the animation's data. Spring's `Spring target` (`SpringTarget.vue:106-124`) is the rail the subject's spring chases — re-seating it IS the demo's interaction with the subject; the spring's parameters are the Physics pane's. Neither edits keyframes, timing or placement, so neither is an inline editor in §0cw/OA-46's sense (the ruling's object is the keyframes/timeline/controls editors); they stay, and the census counts them as the canonical row `.s` banked. Sequence's 6 were editors of the Sequence's data (the items' `at` and the master clock) and are the ones this unit moved.

Frames: BEFORE `s2/before/before-head-sequence-{1440x900,390x844}-{landing,timeline}.png` (HEAD build); AFTER `s2/after/{dev,gh}-{1,2}-sequence-{1440x900,390x844}-{landing,timeline,retimed}.png` (24).

#### Residuals

- **R-s2-1** — at 390 the pane opens at the sheet's peek detent, so the lanes are below the fold until the sheet is raised (the same for every scene's pane at peek); SHEET-POSITION (glass BL F-21) unchanged, no consumer lift added.
- **R-s2-2** — `pane.mjs`'s `paneHost` reads `rail` at 390 because the sheet hosts the same `.controls-pane` body (reusable template); informational only.
- **R-s2-3** — prose-only mentions of the deleted `SequenceScrubber` remain in `demo/composables/useDragScrub.ts:18` (outside the writable set) and in historical test docblocks; no code reference.
