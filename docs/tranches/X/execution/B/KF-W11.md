...
**Unit**: `KF.W11.j` — close (phase 4, serial, last). Spec: `KF-W11.md` §Agent Units `:273-277` ·
§Gates G-KFW11-10 `:307` · §Commit plan 7 `:382` · §Format and lint cadence / §Verification
artefacts `:384-386` · the ADDENDUM 2026-09-19 `:408-417` part 3 (the ratchet's close clause).
**Status: PARTIAL** — nine of ten gates GREEN at this seat's own double-run commands; **G-KFW11-4
and G-KFW11-10 are honest-RED, both tracing to ONE test case whose cure is a keyframes.js byte
outside this unit's writable set** (KF11-E(j1)). Every other close act is DONE: 69 commits audited to
§Bounds, the ratchet read and stated, eight producer rows relayed, the E13 sweep run, the LEDGER
moved, fourteen escalations tabled.
**Evidence**: `docs/tranches/X/keyframes/evidence/W11/j-close-2026-09-19.md` — every transcript,
the full commit audit, the diagnostic homing table, the relay entries and the sweep, in full.
**This seat wrote ZERO keyframes.js bytes and ZERO glass-ui bytes.**
...
docs/tranches/X/keyframes/evidence/W11` → **empty**. **Zero inherited hunks on any path this unit may
write — nothing to finish, nothing to rewrite.** ⟨cmd⟩ `git -C ../keyframes.js status --porcelain` →
**2 untracked** value.js-delivered `VALUEJS-INBOUND-*` mail packets, outside every KF.W11 path, not
touched. ⟨cmd⟩ `git status --porcelain` (value.js, whole tree) → **2 modified**:
`docs/tranches/V/reformation/CARRY-LEDGER.md` (a sibling seat's) and `scripts/dev/dev.sh` (unowned) —
**neither touched, `dev.sh` never staged**. Open sha `bf4a9a9c`.

#### Act 1 — the ten gates, re-run at this seat's own commands, double-run

Evidence §2 carries the full table and the pasted failing transcript.

| gate | born-RED (seat 0, `dd28da55`) | this seat (`bf4a9a9c`), run 1 · run 2 | verdict |
|---|---|---|---|
| **G-KFW11-1** | `No test files found` ×2 | **17 passed (17)** · **17 passed (17)** | **GREEN** |
| **G-KFW11-2** | `No test files found` ×2 | **5 passed (5)** · **5 passed (5)** | **GREEN** |
| **G-KFW11-3** | `No test files found` ×2 | **9 passed (9)** · **9 passed (9)** | **GREEN** |
| **G-KFW11-4** | `No test files found` ×2 | **1 failed \| 11 passed (12)** · same | **RED** — one case |
| **G-KFW11-5** | `No test files found` ×2 | **18 passed (18)** · **18 passed (18)** | **GREEN** |
| **G-KFW11-6** | `No test files found` ×2 | **12 passed (12)** · **12 passed (12)** | **GREEN** |
| **G-KFW11-7** | `No test files found` ×2 | **7 passed (7)** · **7 passed (7)** | **GREEN** |
| **G-KFW11-8** | `No test files found` ×2 | **11 passed (11)** · **11 passed (11)** | **GREEN** |
| **G-KFW11-9** | `No test files found` ×2 | **9 passed (9)** · **9 passed (9)** | **GREEN** |
| **G-KFW11-10** | RED by construction; 39 files / 286 tests | **1 failed \| 47 passed (48)** files · **1 failed \| 414 passed (415)** tests, both passes | **RED** — same case |

**Every byte clause re-measured, double-run, 17 of 17** (evidence §2): G-1 latch **1 → 0 · 0** · G-1
`variant:` **4 → nothing · nothing** · G-2 word **0 → 1 · 1** · G-2 css **1 → 0 · 0** · G-3 rail
**1 → 0 · 0** (the disposition `.c` stated) · G-4 clock **0 · 0** (the route stated against it) ·
G-5 clamp **1 → 0 · 0** · G-6 aria **0 → 3 · 3** · G-7 pacing **1 → 0 · 0** · G-7 pause
**`SquareScene.vue:1` → nothing · nothing** · G-7 docblock **1 → 0 · 0** · G-7 `viewBox` **0 · 0**
(still 0, as N-SQ-4 orders) · G-7 witness **0 → 1 · 1** · G-9 acquire ×2 **1 · 1** each · G-9
`pointerId` **1 → 4 · 4** (the mention plus the latch's declaration, admission test and write).

#### The one RED, named at its cause — ESCALATION KF11-E(j1)

```
 FAIL  test/demo/scenes/spring-trace-truth.test.ts > (4b) the floor the heatmap declares is not
       below the one the plot pins
...
This is **`.e`'s residual e-R3 firing exactly as `.e` wrote it** (*"if `.f` renames it, the test
fails EXPLICITLY with the re-bind instruction in its message; that is the enforcement L-14 asked
for, and `.j` should read such a red as a re-bind, never a skip"*) and **`.f`'s KF11-E(f1) returned
exactly as `.f` wrote it**. The coupling L-14 asks for still holds; what moved is the witness's
anchor, because `.f`'s contract commit re-homed `DAMPING_MIN` as the exported `DAMPING_AXIS.min` —
one home for the coordinate space the field and the sliders share. **`.f` kept no byte in
`SpringHeatmap.vue` to satisfy a sibling's regex**, which would have been a workaround aimed at a
test.

**This seat did not cure it, and states why rather than reaching.** The cure is a write to
`test/demo/scenes/spring-trace-truth.test.ts` — `.e`'s §Bounds `:131` create row, a keyframes.js
byte — and this unit's writable set is **value.js paths only**. The standing law: *"any write outside
it is an ESCALATION — stop and return it."* **Returned as KF11-E(j1)** with the cure shape ready,
stronger than the regex it replaces because it reads the shipped export:
`import { DAMPING_AXIS } from ".../SpringHeatmap.vue"` then
`expect(DAMPING_AXIS.min).toBeGreaterThanOrEqual(PLOT_DAMPING_FLOOR)`.

**No `test.skip`, no `.only`, no widened timeout, no weakened assertion, no allowlist** was used to
turn G-KFW11-10 green. That is L-18 base (iv), the masking class, and the gate's own text forbids it.
**The wave closes with the red carried honestly and attributed.**

#### The skip/only clause reads 1 — the literal and the semantic form, both printed

⟨cmd⟩ `git diff dd28da55..HEAD -- test | grep -c 'test.skip\|it.skip\|\.only('` → **1 · 1**. The
single hit, located by ⟨cmd⟩ the same pipeline with `-n`, is
`test/demo/scenes/spring-derby-truth.test.ts:21` — **a docblock sentence that names the masking class
in order to forbid it** (the self-trip class `.b` recorded three times against its own prose).
The semantic form is **0**: ⟨cmd⟩ `grep -rnE '\b(it|test|describe)\.(skip|only)\(' test/` → **no
match**; ⟨cmd⟩ `npx vitest run --project demo 2>&1 | grep -E 'skipped|todo'` → **no line**. **Zero
skipped or exclusive tests exist in the tree.** Recorded as a literal-RED with the semantic GREEN
beside it, not rounded to zero and not cured — the rewording is `.c`'s byte, outside this set.

#### Act 2 — the ratchet's close reading (§0u part 3)

⟨cmd⟩ `npx vue-tsc --noEmit -p tsconfig.json 2>&1 | grep -c 'error TS'` → **24 · 24** (file
histograms byte-identical across both passes).

**The delta the brief asks for**: against the spec's banked **54**, **−30**; against `.r`'s
CI-faithful floor **31** (KF11-E5 — the banked 54 is not reproducible, and the 24-diagnostic
`OrbitalDrag.vue` block does not exist under the lockfile's pins), **−7**. **THE COUNT DID NOT RISE
under either reading. §0u part (2) is MET.** §0u part (3) — 0 at KF.W13's close — is not this wave's
to assert and is not asserted.

**All 24 homed, none unowned-and-unnamed** (evidence §3.1): **KF.W12 §B.2 13** · **KF.W13 §B.2 5** ·
**KF11-E3 2** (`useEasingDemo.ts`) · **KF11-E4 1** (`EditorShell.vue:175`) · **KF11-E2 3** (`src/**`,
unwritable). **KF.W11's own §Bounds rows contribute ZERO** — every unit zeroed its own rows with the
cure that owned them, and **no cast, no `@ts-expect-error`, no `// eslint-disable` was written as a
cure anywhere in the wave**.

#### Act 3 — 69 commits audited against §Bounds by `git show --stat`

⟨cmd⟩ `git log --oneline dd28da55..HEAD | wc -l` → **69**; the union of every sha's `--name-only`
→ **59** distinct paths. **All 59 fall inside §Bounds `:96-137` plus the §0u `.r` grant. ZERO
out-of-bounds writes** (evidence §4.1's per-unit table).

**The Do-NOT-touch list, measured EMPTY**: ⟨cmd⟩ `git diff --stat dd28da55..HEAD -- src node_modules
package.json vitest.config.ts CubeAxisLines.vue PlaybackRibbon.vue instrument/timeline
useKeyframeOps.ts gestureSelectSuppression.ts DESIGN.md` → **0 lines**. Zero `src/**` bytes · zero
`node_modules` · `vitest.config.ts` unmoved (KF11-E7 returned rather than patched, by four seats
independently) · `useKeyframeOps.ts` byte-unchanged (L-2 booked LANDED-BY `d7f68225` under R.2) ·
glass-ui never written. ⟨cmd⟩ the union `| grep -c 'dev.sh'` → **0** — `scripts/dev/dev.sh` in **0 of
69** commits.

**The MUST-NOT-SPLIT families, each verified ONE sha by `git show --stat`**: the four phantom
`variant:` sites **`dc3f0900`** (CubeScene.vue AND SpringScene.vue) · ST-1's word + deletion
**`f4249cb0`** (both files, L-9) · **the guard family in BOTH composables `4ab4c4db`** (useDragScrub
AND useDragCapture — **L-18 base (iii) cannot be raised**) · tether-truth **`b9b5476c`** · MISS-3's
tour AND its assertion **`8c0122e1`** · the spring-plot core **`e683d9a1`**.

**The four §Goal-criterion BLOCKER cores are all LANDED**: CubeTarget #53 `bc5cc128` · SquareScene
L-2 **LANDED-BY `d7f68225`** (R.2, library arm first, born witness `1233e42e` passing) · AmigaScene
D-1 `73b424da` · SpringTarget C-1 `8dcf5882`. **The criterion's own failure mode — hygiene tails
landed over an un-cured BLOCKER — did NOT occur.**

**Shas the successors open on, printed for them**: KF.W12 AXISLINE-UNIT → **`fff7232c`** · KF.W12
KFED-UNIT → `useKeyframeOps.ts` byte-unchanged, the `:58-69` carve unspent · KF.W13 →
`useDragCapture`'s **exported surface unchanged** (0 changed lines in its declarations;
`PlaybackRibbon.vue` never edited).

#### Act 4 — the KF-AV-28 discharge alphabet, stated positively in words

**ZERO discharge receipts of the KF.W7 SWAP-verdict kind exist in this record. The set is EMPTY and
always was.** KF.W7's verdict was KEEP-BESPOKE on all six surfaces with zero swaps (`4c03ceda`), so
no surface of this wave was ever eligible to carry one; `.i` stated the KEEP verdict before its first
byte and emitted none, and no other seat wrote one. The gate's grep — run with its pattern
deliberately broken by a character class so the command cannot self-trip the very file it reads —
returns **0 · 0**. **This clause of G-KFW11-10 is GREEN.**

#### Act 5 — §Format and lint cadence at close

| ⟨cmd⟩ | reading |
|---|---|
| `npm run check` | **exit 2** at leg 1 on the 24 rows above — **none of them KF.W11's**; legs 2 and 3 unreached |
| `npm run test:demo` | **1 failed \| 47 passed (48)** files · **1 failed \| 414 passed (415)** tests, both passes |
...
| `@ts-expect-error` / `eslint-disable` added in the wave diff | **0** / **0** |
| casts added in **product** bytes | **0** (the one grep hit is the prose *"what was absent was any"*, verified in context) |
| casts added in **test** bytes | **9**, all in `spring-derby-truth.test.ts`'s deterministic rAF harness and three spring-state stubs — **recorded, not rounded to zero**; none cures or suppresses a diagnostic |

#### Act 6 — the BH relay: eight producer rows, all routed, none worked around

Evidence §7 carries each row whole. **W11-P1** `registerShortcut`'s combo chip renders a `code`
spelling verbatim (`.a`, KF11-E6) · **W11-P2** the Chip's selectable filter strips `role`/
`aria-pressed` — SPF-4's banked constraint (`.f`) · **W11-P3** ToggleGroup single-mode ships
`aria-pressed` buttons in a `role="group"`; the radio half needs the producer (`.f`) · **W11-P4**
**the Boolean-cast `default: undefined` class the brief names** — `ToggleGroupProps.modelValue` typed
without `undefined` while the runtime accepts it (`.f`) · **W11-P5** the ToggleGroup ROOT reaches the
DOM through reka's `as-child` without the consumer's scope attribute (`.f`) · **W11-P6** LabeledSlider
ships no `valueCommit` seam (`.f`) · **W11-P7** the window shortcut registry checks neither
`defaultPrevented` nor composite-widget ownership (`.f`) · **W11-P8** glass `accessibility.css` keys
forced-colors/prefers-contrast state indication on six selectors and on **nothing** for
`[aria-expanded]`, so a correct disclosure button gets no vendor state indication (`.g`) — one
selector-list addition cures it for every consumer.

**CubeScene D-2/C-3's `variant`, which the brief names: measured and resolved CONSUMER-SIDE, with NO
producer ask owed.** Installed glass-ui 7's `ButtonProps` carries no `variant` at all, so all four
sites were phantom props falling through to the DOM — not a producer gap. Deleted in one sha
(`dc3f0900`), byte clause reads nothing twice. **Recorded so a later seat does not mint a row that
is not owed.** `.c` and `.h` stated positively that their packets surfaced none.

**The rows are written whole into `INBOX.md`.** The COHESION §4a mirror is **routed to the
orchestrator (residual j-R2)**: §Bounds `:141` names the register among `.j`'s modify rows, but
`COHESION.md` is not in this seat's hard-bound writable set and the standing law makes a write
outside it an escalation returned — the same disposition KF.W0's close took for the same mirror
(`LEDGER.md:45`, *"COHESION §4a mirror → orchestrator"*).

#### Act 7 — E13 Step-0, the four-path sweep at this seat's clock

Status read by **cell position and leading verb**, never a bare `grep -i unread` (the token appears
in four rows' Status prose — O-20, I-31, I-32, O-39 — and is the leading verb of **none**);
`INBOX.md` self-excluded. (1) `V/coordination/` newest → the five 2026-09-18 `value-4.1` letters,
**ours and rowed**. (2) glass-ui: BK re-confirmed newest tranche dir; newest letter
`glass-outbound-2026-09-18-valuejs-o26-reply.md` = **I-35, rowed**, UNMOVED. (3) keyframes newest
inbound-grammar = `VALUEJS-INBOUND-2026-09-17-o8-o11-amendment-addendum.md` = **O-21, ours**.
(4) atlas newest = **O-12, ours**, path UNMOVED. ⟨cmd⟩ `grep -cE '^\| [IO]-[0-9]+[a-z]? \|'
INBOX.md` → **79** rows; the positional leading-verb scan → **0 · 0** UNREAD.

**ZERO unrowed letters · ZERO UNREAD · ZERO new `I-n` minted.** Tail **I-35 / O-39** (O-39 was rowed
by X.F.W8 `e` since seat 0's sweep; ours, SENT, carrying no obligation on this wave). **No wave
closes with UNREAD mail in scope; none is.** A dated sweep line is appended at `INBOX.md`'s end.

#### Act 8 — the LEDGER

KF.W11's own cells moved by **minimal in-place replacement**, the file re-read immediately before the
edit and the landed bytes verified after the commit (four tracks write it concurrently). The row is
shared with KF.W12 and KF.W13; **only the KF.W11 portion of the status cell and the commits cell were
touched** — W12's and W13's BLOCKED-ON text is byte-unchanged. An event line is appended.

#### Escalations — fourteen tabled: two discharged, eleven inherited-open, one minted here

**DISCHARGED**: KF11-E1 (ruled at §0u as the RATCHET) · KF11-E5 (the banked 54 unreproducible —
consumed by every unit's re-derivation and by Act 2). **OPEN, each with its owner**: **KF11-E2** the
three `src/**` TS6133 rows → KF.W5/KF.W8; **sole cause of CI's `library gates` failure**, and **§0u
part (3) cannot be met by KF.W11–W13 alone** · **KF11-E3** `useEasingDemo.ts` ×2 → KF.W12 ·
**KF11-E4** `EditorShell.vue(175,10)` → the shell seam's wave · **KF11-E6** → SS-6 (relayed as
W11-P1) · **KF11-E7 the wave's largest coverage cap** — the demo vitest lane cannot import any SFC
that transitively loads glass-ui's spring chunk; raised independently by `.a`, `.b`, `.d`, `.g`, and
**no seat patched `vitest.config.ts`, mocked the producer, patched `node_modules` or skipped a case**
· **KF11-E(c1)** M-4→M-3's bytes are `.e`'s file · **KF11-E(d1)** the loop-seam family needs
`useSweepScene.ts` → KF.W12; nothing of it landed and the family was not split · **KF11-E(f1)** →
re-raised as KF11-E(j1) · **KF11-E(i1)** KF-SCR-5's dead-API deletion needs `SquareScene.vue` in the
same sha · **KF11-E(i2)** C·C-1's sequence half needs `useSequenceDemo.ts` · **`.h` C-20**
`checkerboard.jpg` dead, census clean, delete not taken. **MINTED HERE**: **KF11-E(j1)**, the (4b)
re-bind.

#### Residuals of this seat

**j-R1** — **67 keyframes.js commits are unpushed** (HEAD `bf4a9a9c`, `origin/master` `2b649a1d`;
only `.r`'s two are published). The begin-word authorises push, but push is not among this unit's
enumerated acts and this seat's lock is **value.js paths only** — named, not taken; routed to the
orchestrator with its exact command. **j-R2** — the COHESION §4a mirror (Act 6). **j-R3** — the
skip/only literal (above). **j-R4** — `TransportDock.vue(95,34)` TS2322 is present at close and homed
to KF.W13, where seat 0's open histogram listed only two rows for that file; recorded as a homing
observation, not a rise — the whole-tree count **fell**.

#### What did NOT happen — stated positively, because L-18 will look for it

**No `src/**` byte** (base v) · **no geometry cure off a banked figure** — `.b`, `.d` and `.h` each
re-derived at their own open sha before the cure (base ii) · **the drag-seam guard in BOTH
composables in ONE sha** (base iii) · **the three decisions written before their patches**, as three
value.js commits preceding their families — `532f8985` (`.d`'s canonical domain), `ec4b7eff` (`.e`'s
N-1 fork), `f6d05a5c` (`.f`'s field encoding) (base vi) · **no producer defect cured demo-side**
(base viii) · **no BLOCKER core slipped to a residual** (base i) · **no masking construct anywhere**
(base iv). **E-3 held**: no dated spec, adjudicated registry row, conformance artefact or prior
evidence file was edited — every value.js act was an append or a new file.

#### Inherited-paths clause

**None.** Zero uncommitted hunks on any path in this unit's writable set at open, in either repo.
Zero writes outside it: no keyframes.js byte in any verb, no `COHESION.md` byte, no sibling seat's
path, no `CARRY-LEDGER.md`, **`scripts/dev/dev.sh` never staged**. No stash, no `reset --hard`, no
force-push, no `--amend` (c-R1's and `.g`'s lessons honoured — every correction is a follow-up
commit). Every commit used `git add <exact paths>` and `git commit … -- <the same exact paths>`.

#### SELF-COUNT

**10** gates re-run, each **twice** · **8 GREEN · 2 RED**, both tracing to **one** case · **17** byte
clauses re-measured twice · **69** commits audited, **59** distinct paths, **0** outside §Bounds ·
**6** MUST-NOT-SPLIT families verified unsplit · **4** BLOCKER cores LANDED · **24** diagnostics
standing, **all** homed, **0** in KF.W11's rows · **8** producer rows relayed from **4** units ·
**14** escalations tabled (2 discharged · 11 inherited-open · 1 minted) · **4** residuals of this
seat · **79** INBOX rows swept, **0** UNREAD · **0** SWAP-discharge receipts · **0** figures
published that were not read from settled bytes.

**The unit's verdict: KF.W11 is IMPLEMENTED-with-one-carried-RED — `complete_with_misses`. Nine of
ten gates GREEN at a second seat's own double-run clock, every BLOCKER core landed, every commit in
bounds, the ratchet down 30 from its banked figure and never risen, zero masking constructs, and the
single RED named, attributed to its sibling-seam cause, and returned with its cure rather than
hidden.**
...
**Seat**: the close of **X.KF.W11**, taken a second time by a fresh `claude-opus-5[1m]` that authored
no unit cure, no unit receipt and no byte of `### KF.W11.j`. **VERIFY-ONLY — this seat cured nothing,
wrote ZERO keyframes.js bytes and ZERO glass-ui bytes.** Its whole product is this section, the
LEDGER's own cells, and **the push `.j` named but could not take** (residual j-R1).
**Spec**: §Agent Units `:273-277` · §Gates `:285-307` · §Format and lint cadence / §Verification
artefacts `:384-386` · the ADDENDUM 2026-09-19 `:408-417`. **Wave open sha** `dd28da55` · **head at
close `bf4a9a9c`** · value.js close commits on `tranche-u`.

**Verdict: `complete_with_misses` — PARTIAL with its remainder named.** Eight of ten gates GREEN at
this seat's own double-run commands; **two honest-RED, both tracing to ONE test case** (KF11-E(j1));
all four §Goal-criterion BLOCKER cores LANDED; 69 of 69 commits inside §Bounds; the ratchet **24 · 24**,
fallen and never risen; **zero masking constructs**. Every figure below was read from the settled
bytes at this seat's own clock and double-run; nothing is transcribed from `### KF.W11.j`, and where
this seat's reading and `.j`'s agree that agreement is an independent reproduction, not a copy.

#### Crash-recovery (standing law, first act)

⟨cmd⟩ `git status --porcelain` (value.js) → **2 modified**: `docs/tranches/V/reformation/CARRY-LEDGER.md`
(a sibling track's) and `scripts/dev/dev.sh` (unowned by standing arrangement). ⟨cmd⟩
`git -C ../keyframes.js status --porcelain` → **2 untracked** `VALUEJS-INBOUND-*` letters at their end.
**Not one of the four is inside this seat's writable set** (`execution/B/KF-W11.md` ·
`execution/LEDGER.md` · `V/coordination/INBOX.md` + the SS-6 register · `keyframes/evidence/W11/**`),
which measured **empty**. **Inherited paths: NONE** — nothing to finish, nothing to rewrite. Nothing
was stashed, nothing restored, `scripts/dev/dev.sh` never staged, and no sibling seat's dirty path was
touched. *(Disclosed: value.js `HEAD` moved from `8366f8ad` to `2a29323d` during this sitting — three
concurrent tracks' commits on the shared branch. No byte of theirs was staged by this seat; the push
at Act 5 publishes already-committed history on the shared branch, which is the act the brief names.)*

#### Act 1 — every unit's commits exist, and touch only that unit's writable set

⟨cmd⟩ `git log --oneline dd28da55..HEAD | wc -l` → **69**. Per unit, ⟨cmd⟩
`git log --oneline dd28da55..HEAD --grep="X.KF.W11.<u>" | wc -l`:

| unit | .r | .a | .b | .d | .c | .h | .g | .e | .f | .i | Σ |
|---|---|---|---|---|---|---|---|---|---|---|---|
| commits | 1 | 9 | 11 | 13 | 9 | 10 | 3 | 5 | 5 | 3 | **69** |

⟨cmd⟩ `git log --format='' --name-only dd28da55..HEAD | sort -u` → **59 distinct paths**, each read
against §Bounds `:96-141` and the §0u `.r` grant: **ZERO out of bounds.** The two rows that need their
warrant stated rather than assumed, both stated: **`package-lock.json`** is `.r`'s own §0u grant
(ADDENDUM `:414`), not the forbidden `package.json`; **`demo/scenes/easing/EasingScene.vue`** is `.r`'s
unowned-remainder cure — it is **not** a KF.W11–W13 §Bounds row (KF.W12's easing carve is
`EasingSidebar.vue`'s seat rows), which is exactly the condition `:414` sets.

**The Do-NOT-touch list, measured EMPTY at this seat**: ⟨cmd⟩ `git diff --stat dd28da55..HEAD -- src
node_modules package.json vitest.config.ts demo/scenes/cube/CubeAxisLines.vue
demo/components/playback/PlaybackRibbon.vue demo/components/instrument/timeline
demo/components/instrument/keyframes/composables/useKeyframeOps.ts
demo/utils/gestureSelectSuppression.ts demo/DESIGN.md` → **no output, 0 lines**. ⟨cmd⟩ the same union
`| grep -c 'dev.sh'` → **0** — `scripts/dev/dev.sh` appears in **0 of 69** commits.

**The six MUST-NOT-SPLIT families, each verified ONE sha by `git show --stat` at this seat**: the four
phantom `variant:` sites **`dc3f0900`** (CubeScene.vue 4 ± · SpringScene.vue 2 −, one commit) · ST-1's
one word + one deletion **`f4249cb0`** (SequenceTarget.css + .vue, L-9) · **the guard family in BOTH
composables `4ab4c4db`** (`useDragCapture.ts` 112 ± AND `useDragScrub.ts` 128 ± — **L-18 base (iii)
cannot be raised**) · tether-truth **`b9b5476c`** (D-1+D-6+D-16+N-SQ-4) · MISS-3's settle-paced tour
AND its assertion **`8c0122e1`** (4 files, one sha) · the spring-plot core **`e683d9a1`**
(D-1+N-1+N-2+C-2/L-3, one file, one sha).

**The four §Goal-criterion BLOCKER cores, each LANDED and read at its own sha**: CubeTarget #53
**`bc5cc128`** (*"the die's only pre-start orientation writer paints"*) · SquareScene L-2
**LANDED-BY `d7f68225`** (KF.W5 `.d`, the library arm — R.2 GREEN-BEFORE-CURE, its born witness
`1233e42e` recorded passing at the demo end) · AmigaScene D-1 **`73b424da`** · SpringTarget C-1
**`8dcf5882`**. **The criterion's own failure mode — a hygiene tail landed over an un-cured BLOCKER —
did NOT occur.**

**value.js side**: **18** commits carry the `x-kf-w11` grammar (11 unit receipts + the three decisions
written before their patches + the close set), and **11 evidence files** stand at
`keyframes/evidence/W11/` (`r` · `a`–`j`). Every value.js act is an append or a new file — **E-3 held**:
no dated spec, adjudicated registry row or conformance artefact was edited.

**Landed-wrong findings: NONE.** No commit of this wave writes a path outside its unit's writable set,
no declared family is split, and no `src/**`, `node_modules`, `package.json`, `vitest.config.ts`,
`CubeAxisLines.vue`, `PlaybackRibbon.vue`, timeline or glass-ui byte exists in the range.

#### Act 2 — the ten gates, BEFORE → AFTER, at this seat's own commands, each run twice

BEFORE = the wave's own Baseline, banked READ-ONLY at `dd28da55` (§Baseline, `:581-629`); AFTER = this
seat, at `bf4a9a9c`, ⟨cmd⟩ `npx vitest run --project demo <file>` for G1–G9 and ⟨cmd⟩ `npm run test:demo`
for G10.

| gate | BEFORE (born-RED, `dd28da55`) | AFTER (this seat, `bf4a9a9c`) run 1 · run 2 | verdict |
|---|---|---|---|
| **G-KFW11-1** cube | `No test files found` ×2 | **17 passed (17)** · **17 passed (17)** | **GREEN** |
| **G-KFW11-2** sequence | `No test files found` ×2 | **5 passed (5)** · **5 passed (5)** | **GREEN** |
| **G-KFW11-3** spring | `No test files found` ×2 | **9 passed (9)** · **9 passed (9)** | **GREEN** |
| **G-KFW11-4** spring-plot | `No test files found` ×2 | **1 failed \| 11 passed (12)** · identical | **RED** — one case |
| **G-KFW11-5** facet | `No test files found` ×2 | **18 passed (18)** · **18 passed (18)** | **GREEN** |
| **G-KFW11-6** artifact-truth | `No test files found` ×2 | **12 passed (12)** · **12 passed (12)** | **GREEN** |
| **G-KFW11-7** square | `No test files found` ×2 | **7 passed (7)** · **7 passed (7)** | **GREEN** |
| **G-KFW11-8** amiga | `No test files found` ×2 | **11 passed (11)** · **11 passed (11)** | **GREEN** |
| **G-KFW11-9** drag-seam | `No test files found` ×2 | **9 passed (9)** · **9 passed (9)** | **GREEN** |
| **G-KFW11-10** close | RED by construction; 39 files / 286 tests | **1 failed \| 47 passed (48)** files · **1 failed \| 414 passed (415)** tests, both passes | **RED** — the same case |

**The one RED, at its cause, reproduced verbatim at this seat:**

```
 FAIL  |demo| test/demo/scenes/spring-trace-truth.test.ts > SpringTrace — the ceiling's coupling to
       the ζ floor (L-14, documented AND enforced) > (4b) the floor the heatmap declares is not below
       the one the plot pins
Error: SpringHeatmap.vue no longer declares `DAMPING_MIN = <number>` — L-14's coupling anchor moved;
       re-bind the plot's PLOT_DAMPING_FLOOR witness to the new declaration
```

**48 test files − 1 = the whole demo lane but this one case; 415 − 414 = one assertion.** The failure is
`.e`'s own designed message firing as `.e` wrote it (residual e-R3) because `.f`'s contract commit
re-homed the constant as the exported `DAMPING_AXIS.min` — **the L-14 coupling still holds; what moved
is the witness's anchor.** `.f` kept no byte in `SpringHeatmap.vue` to satisfy a sibling's regex, which
would have been a workaround aimed at a test. **This seat did not cure it either**: the cure is a write
to `test/demo/scenes/spring-trace-truth.test.ts`, a keyframes.js byte, and this seat's writable set is
value.js paths only — the standing law makes a write outside it an ESCALATION returned. **KF11-E(j1)
stands, with its cure unchanged and stronger than the regex it replaces**: import `DAMPING_AXIS` from
the shipped module and assert `DAMPING_AXIS.min >= PLOT_DAMPING_FLOOR`.

**No `test.skip`, no `.only`, no widened timeout, no weakened assertion, no allowlist** was used by any
seat, and none by this one, to turn G-KFW11-4 or G-KFW11-10 green. **The wave closes with the red
carried honestly and attributed** (L-18 base (iv) refused twice).

#### Byte clauses — all 17, re-measured at this seat, double-run, identical on both passes

G-1 latch **0 · 0** (was 1) · G-1 `variant:` **nothing · nothing** (was `CubeScene.vue:2` ·
`SpringScene.vue:2`) · G-2 word **1 · 1** (was 0) · G-2 css **0 · 0** (was 1) · G-3 rail **0 · 0** ·
G-4 clock **0 · 0** (the route stated against it, unmoved) · G-5 clamp **0 · 0** (was 1) · G-6 aria
**3 · 3** (was 0) · G-7 pacing **0 · 0** (was 1) · G-7 pause **nothing · nothing** (was
`SquareScene.vue:1`) · G-7 docblock `sed -n '29p' … | grep -c ':161'` **0 · 0** (was 1) · G-7 `viewBox`
**0 · 0** (STILL 0, as N-SQ-4 orders — the cure that adds one fails by construction, and none was
added) · G-7 witness **1 · 1** (was 0) · G-9 acquire ×2 **1 · 1** each · G-9 `pointerId` **4 · 4**
(was 1: the mention plus the latch's declaration, admission test and write). **17 of 17 read what
§Gates requires.**

#### Act 3 — §Verification artefacts (`:384-386`) at this seat

| ⟨cmd⟩ | reading |
|---|---|
| `npm run check` | **exit 2** at leg 1 (`vue-tsc`) on the 24 rows below — **none of them in KF.W11's cured surfaces**; legs 2 and 3 unreached |
| `npm run test:demo` | **1 failed \| 47 passed (48)** files · **1 failed \| 414 passed (415)** tests, both passes |
| `npx eslint demo/scenes demo/composables demo/components/playback demo/components/instrument/transport/composables` | **exit 0, no output — the §Format scope is CLEAN** |
| `git diff --check dd28da55..HEAD` | **no output — clean** |
| `git diff dd28da55..HEAD -- test \| grep -c 'test.skip\|it.skip\|\.only('` | **1 · 1** literal; **semantic 0** — ⟨cmd⟩ `grep -rnE '\b(it\|test\|describe)\.(skip\|only)\(' test/` → **0 matches**. The single literal hit is prose in `spring-derby-truth.test.ts:21` that *names the masking class in order to forbid it*. Recorded as `.j` recorded it, not rounded to zero, not cured (the rewording is a keyframes byte outside this set) |
| `git diff dd28da55..HEAD \| grep -c '^+.*@ts-expect-error'` / `'^+.*eslint-disable'` | **0** / **0** |
| `grep -c 'DISCHARGE[D] by KF.W7 SWAP verdict' execution/B/KF-W11.md` (the pattern deliberately broken by a character class, as `.j` broke it, so the command cannot self-trip the very file it reads — the first draft of this cell quoted the string whole and read **1**; corrected at the settled bytes before commit, WRITE-THEN-MEASURE) | **0 · 0** — the KF-AV-28 discharge alphabet is EMPTY and always was; KF.W7 ruled KEEP-BESPOKE ×6 (`4c03ceda`), so no surface here was ever eligible to carry one. **This clause of G-KFW11-10 is GREEN.** |

#### The ratchet (§0u part 2) at close

⟨cmd⟩ `npx vue-tsc --noEmit -p tsconfig.json 2>&1 | grep -c 'error TS'` → **24 · 24** (both passes'
file histograms byte-identical). Against the spec's banked **54**: **−30**. Against `.r`'s
CI-faithful floor **31** (KF11-E5): **−7**. **THE COUNT DID NOT RISE UNDER EITHER READING; §0u part (2)
is MET.** Part (3) — 0 at KF.W13's close — is not this wave's to assert and is not asserted here.

**All 24 homed at this seat's own histogram, none unowned-and-unnamed** (Σ = 24):
`useTimingFunctionEditor.ts` 3 · `TransportDock.vue` 3 · `useEasingDemo.ts` 2 · `KeyframesStringControls.vue`
2 · `KeyframesEditor.vue` 2 · `MbabbMenu.vue` 2 · `EasingSidebar.vue` · `TimingFunctionPanel.vue` ·
`LayerConfigPanel.vue` · `ChannelOptions.vue` · `ChannelControls.vue` · `EditorShell.vue` ·
`useKeyframeOps.ts` · `src/animation/physics/smooth.ts` · `src/animation/group/waapi.ts` ·
`src/animation/group/composite/compositor.ts` (1 each). **KF.W12 §B.2 13 · KF.W13 §B.2 5 · KF11-E3 2
(`useEasingDemo.ts`) · KF11-E4 1 (`EditorShell.vue(175,10)`) · KF11-E2 3 (`src/**`, unwritable).**

**KF.W11's own cured surfaces contribute ZERO** — no `cube/`, `sequence/`, `spring/`, `square/`,
`amiga/`, `useDragScrub.ts`, `useDragCapture.ts` or `AnimationVisualizer.vue` row appears. **The one
row inside a KF.W11 §Bounds path is stated precisely rather than waved past**:
`useKeyframeOps.ts(80,13)` TS2322 sits **outside** the `:58-69` `updateFromString` carve this wave was
granted — the carve was never spent (L-2 landed library-first under R.2) and the byte is byte-unchanged,
so the diagnostic is KF.W12's KFED-UNIT row where `.j` homed it. **No cast, no `@ts-expect-error`, no
`eslint-disable` was written as a cure anywhere in the wave.**

#### Act 4 — E13 Step-0, the four-path sweep re-run at this seat's clock

Status read by **cell position and leading verb**, never a bare `grep -i unread`; `INBOX.md`
self-excluded. (1) `docs/tranches/V/coordination/` — **24 entries**, newest non-self the five
2026-09-18 `value-4.1` letters, **ours and rowed**. (2) `../glass-ui/docs/tranches/BK/coordination/` —
**BK re-confirmed the newest glass tranche dir** (⟨cmd⟩ `ls -t docs/tranches/` → `BK · BJ · BI`); **9
entries**; newest `glass-outbound-2026-09-18-valuejs-o26-reply.md` = **I-35, rowed, READ + CONSUMED
WHOLE** (`INBOX.md:115`), UNMOVED. (3) `../keyframes.js/docs/tranches/V/coordination/` — **13 entries**,
newest inbound-grammar letter = `VALUEJS-INBOUND-2026-09-17-o8-o11-amendment-addendum.md` = **O-21,
ours**. (4) `../sci-report/atlas/docs/tranches/P/coordination/` — **28 entries**, newest mtime
2026-08-03, UNMOVED. ⟨cmd⟩ `find <each path> -maxdepth 1 -type f -newermt "2026-09-19 00:00"` → exactly
**two**, both self-or-ours (`INBOX.md`, self-excluded; keyframes' own `INBOUND-LEDGER.md`, clean in git,
carrying no new letter). ⟨cmd⟩ `grep -cE '^\| [IO]-[0-9]+[a-z]? \|' INBOX.md` → **79** rows; the
positional leading-verb scan → **0 UNREAD**. **ZERO unrowed letters · ZERO UNREAD · ZERO new `I-n`
minted** (tail unmoved at **I-35 / O-39**). **No wave closes with UNREAD mail in scope; none is.**

#### Act 5 — the push (residual j-R1 DISCHARGED)

`.j` named 67 unpushed keyframes.js commits and declined to take the act because push was not among its
enumerated acts and its lock was value.js paths only. **This seat's brief enumerates it, and the owner's
2026-09-17 begin-word authorises it.** Taken, in order, never forced:

- ⟨cmd⟩ `git -C ../keyframes.js fetch origin` → `origin/master` still **`2b649a1d`** (no concurrent
  producer write to race).
- ⟨cmd⟩ `git -C ../keyframes.js push origin HEAD` → `To github.com:mkbabb/keyframes.js.git ·
  2b649a1d..bf4a9a9c · HEAD -> master`, **exit 0**. ⟨cmd⟩ `git status -sb` after → `## master...origin/master`
  — **ahead 0. All 69 of the wave's keyframes.js commits are published**, where before only `.r`'s two
  were.
- ⟨cmd⟩ `git push origin HEAD` (value.js, `tranche-u`) → `e112514b..2a29323d`, **exit 0**; `## tranche-u...origin/tranche-u`,
  ahead 0. *(The shared branch carried three concurrent tracks' commits at that moment; publishing
  already-committed history on a shared branch is the act the brief names, and this seat staged no byte
  of theirs.)* A second value.js push follows this commit and the LEDGER's.

**No force, no `--amend`, no stash, no `reset --hard`, no branch moved.**

#### Residuals — each with its named owner

| # | residual | owner |
|---|---|---|
| **j-R1** | 67 unpushed keyframes.js commits | **DISCHARGED at Act 5** — `origin/master` = `bf4a9a9c` |
| **j-R2** | the COHESION §4a mirror of the SS-6 accretion | **the orchestrator** — `COHESION.md` is in no close seat's hard-bound writable set; the same disposition KF.W0's close took (`LEDGER.md:45`). The eight rows themselves are written whole into `INBOX.md` and are not waiting on the mirror |
| **j-R3** | the skip/only literal at `spring-derby-truth.test.ts:21` (prose naming the masking class) | **`.c`'s byte** — a keyframes.js write; semantic form measured **0**, so the gate's intent is met and only its literal grep reads 1 |
| **j-R4** | `TransportDock.vue(95,34)` TS2322 present at close | **KF.W13** — a homing observation, not a rise; the whole-tree count fell |
| **j-R5** *(this seat)* | G-KFW11-4 / G-KFW11-10 stay honest-RED until KF11-E(j1)'s one-line re-bind lands | **the orchestrator → a keyframes-side seat** (`test/demo/scenes/spring-trace-truth.test.ts`, `.e`'s create row); the cure is written out and costs one import and one assertion |

#### Escalations — fourteen, unchanged by this seat; none blocks a sibling wave's dispatch

**DISCHARGED**: **KF11-E1** (ruled at COHESION §0u as the RATCHET) · **KF11-E5** (the banked 54
unreproducible — consumed by every unit's re-derivation and by the ratchet reading above; this seat's
**24** is measured against `.r`'s settled lockfile tree, the same tree CI installs).
**OPEN, each with its owner**: **KF11-E2** the three `src/**` TS6133 rows → **KF.W5/KF.W8**; still the
sole cause of CI's `library gates` failure, and **§0u part (3) cannot be met by KF.W11–W13 alone** ·
**KF11-E3** `useEasingDemo.ts` ×2 → **KF.W12** · **KF11-E4** `EditorShell.vue(175,10)` → **the shell
seam's wave** · **KF11-E6** → **SS-6** (relayed as W11-P1) · **KF11-E7** the demo vitest lane cannot
import an SFC that transitively loads glass-ui's spring chunk — raised independently by `.a`, `.b`,
`.d`, `.g`, and **no seat patched `vitest.config.ts`, mocked the producer, patched `node_modules` or
skipped a case** · **KF11-E(c1)** · **KF11-E(d1)** the loop-seam family needs `useSweepScene.ts` →
**KF.W12** (nothing of it landed; the family was not split) · **KF11-E(f1)** → re-raised as
**KF11-E(j1)**, which this seat reproduces and returns unchanged · **KF11-E(i1)** · **KF11-E(i2)** ·
**`.h` C-20** (`checkerboard.jpg` dead, census clean, delete not taken).

**Shas the successors open on, re-printed**: KF.W12 AXISLINE-UNIT → **`fff7232c`** · KF.W12 KFED-UNIT →
`useKeyframeOps.ts` **byte-unchanged**, the `:58-69` carve unspent · KF.W13 → `useDragCapture`'s
**exported surface unchanged**, `PlaybackRibbon.vue` never edited.

#### The four verbs, moved only as §State permits

| verb | state at close | basis |
|---|---|---|
| **AUDITED** | **YES** (unmoved) | the nineteen home records, as the spec banked them |
| **SPECIFIED** | **YES** (unmoved) | `KF-W11.md`, the spec of record |
| **IMPLEMENTED** | **YES — `complete_with_misses`** | the spec's own condition is the gates after the sequencing head; the head is ruled a RATCHET (§0u) and read **24 · 24, fallen and never risen**; **8 of 10 gates GREEN** at a second seat's own double-run clock with **both REDs on ONE named case**, all four BLOCKER cores landed, 69 of 69 commits in bounds, zero masking constructs. The qualifier travels with the verb and is never dropped |
| **VERIFIED** | **NO** | §State `:19` — stamped only at a successor close; **this wave takes no verb from KF.W10's close and this seat stamps none** |

#### SELF-COUNT

**10** gates re-run, each **twice** · **8 GREEN · 2 RED**, both tracing to **1** case · **17** byte
clauses re-measured twice, **17** reading as §Gates requires · **69** commits audited, **59** distinct
paths, **0** outside §Bounds, **0** landed-wrong · **10** per-unit commit counts published, summing to
**69** · **6** MUST-NOT-SPLIT families verified one sha each · **4** BLOCKER cores LANDED · **24**
diagnostics standing, **all** homed, **0** in this wave's cured surfaces · **18** value.js `x-kf-w11`
commits · **11** evidence files · **79** INBOX rows swept, **0** UNREAD, **0** new `I-n` · **0**
SWAP-discharge receipts · **2** pushes taken, **0** forced · **5** residuals (**1** discharged here) ·
**14** escalations (**2** discharged · **12** open with owners) · **0** keyframes.js bytes and **0**
glass-ui bytes written by this seat · **0** figures published that were not read from settled bytes.

**The close's verdict: X.KF.W11 is IMPLEMENTED `complete_with_misses` — eight of nine packets landed
whole, every BLOCKER core cured, every commit in bounds and now PUBLISHED, the ratchet down thirty and
never risen, and the single carried RED reproduced at a second seat, named at its sibling-seam cause,
and returned with its cure rather than hidden or skipped.**
...
**Seat**: FRESH ADVERSARIAL CHECK (L-20, pass 1) of X.KF.W11's close. **VERIFY-ONLY** — an independent
`claude-opus-5[1m]` that authored **no** byte of the wave's cures, of any unit receipt, of `### KF.W11.j`
or of `## Close`. This seat wrote **ZERO** keyframes.js bytes, **ZERO** glass-ui bytes and no cure of any
kind; its whole product is this section, the LEDGER's own cells and its event line. Every figure below
was read at this seat's own commands, from the settled bytes at keyframes.js **`bf4a9a9c`**
(= `origin/master`, verified `ahead 0`) and value.js `tranche-u`.

**VERDICT: CONFORMANT-HONEST-RED** — **0 BLOCKER · 0 CRITICAL · 0 HIGH · 3 MINOR (all mitigated) ·
3 INFO**. **All ten gate verdicts reproduce** at this seat's own commands (8 GREEN · 2 RED, both on ONE
case) and **17 of 17 byte clauses** re-measure exactly as `## Close` published them. Honest-RED set:
**{G-KFW11-4, G-KFW11-10}**, owner-named, substance verified holding at the bytes, no masking anywhere.

### Axis 1 — every claimed GREEN reproduces (10 of 10 verdicts)

⟨cmd⟩ `npx vitest run --project demo <file>` per gate, and ⟨cmd⟩ `npm run test:demo` for G10, run at
this seat:

| gate | `## Close` published | THIS SEAT | verdict |
|---|---|---|---|
| G-KFW11-1 | 17 passed (17) | **17 passed (17)** | reproduces GREEN |
| G-KFW11-2 | 5 passed (5) | **5 passed (5)** | reproduces GREEN |
| G-KFW11-3 | 9 passed (9) | **9 passed (9)** | reproduces GREEN |
| G-KFW11-4 | 1 failed \| 11 passed (12) | **1 failed \| 11 passed (12)** | reproduces **RED** |
| G-KFW11-5 | 18 passed (18) | **18 passed (18)** | reproduces GREEN |
| G-KFW11-6 | 12 passed (12) | **12 passed (12)** | reproduces GREEN |
| G-KFW11-7 | 7 passed (7) | **7 passed (7)** | reproduces GREEN |
| G-KFW11-8 | 11 passed (11) | **11 passed (11)** | reproduces GREEN |
| G-KFW11-9 | 9 passed (9) | **9 passed (9)** | reproduces GREEN |
| G-KFW11-10 | 1 failed \| 47 passed (48) files · 1 failed \| 414 passed (415) tests | **identical, both figures** | reproduces **RED** |

**Byte clauses, all 17 re-measured at this seat**: G-1 latch **0** · G-1 `variant:` **nothing** · G-2
word **1** · G-2 css **0** · G-3 rail **0** · G-4 clock **0** · G-5 clamp **0** · G-6 aria **3** · G-7
pacing **0** · G-7 pause **nothing** · G-7 `:29` docblock **0** · G-7 `viewBox` **0** (still) · G-7
witness **1** · G-9 acquire ×2 **1 · 1** · G-9 `pointerId` **4**. **17 of 17 identical to the close's
table.** The ratchet: ⟨cmd⟩ `npx vue-tsc --noEmit -p tsconfig.json 2>&1 | grep -c 'error TS'` → **24 ·
24**, and the per-file histogram is byte-identical to the close's 16-file list (Σ = 24) with **zero rows
in any KF.W11 cured surface**; `useKeyframeOps.ts(80,13)` confirmed **outside** the unspent `:58-69`
carve and the file confirmed **byte-unchanged** (⟨cmd⟩ `git diff --stat dd28da55..HEAD -- …useKeyframeOps.ts`
→ no output).

### Axis 2 — no write outside §File Bounds

⟨cmd⟩ `git log --format='' --name-only dd28da55..HEAD | sort -u | wc -l` → **59**; ⟨cmd⟩
`git log --oneline dd28da55..HEAD | wc -l` → **69**. Every one of the 59 read against §B.2 `:96-141`
and the §0u `.r` grant: **ZERO out of bounds.** The Do-NOT-touch probe over `src`, `node_modules`,
`package.json`, `vitest.config.ts`, `CubeAxisLines.vue`, `PlaybackRibbon.vue`, `instrument/timeline`,
`instrument/keyframes`, `gestureSelectSuppression.ts`, `DESIGN.md`, `TransportDock.vue`, `demo/styles`,
`demo/app` → **no output, 0 lines**. `scripts/dev/dev.sh` in **0 of 69** commits and never staged.
Serial disjointness verified per shared path: `SpringScene.vue` = `.a`'s `dc3f0900` **then** `.c`
(correct order) · `SequenceScrubber.vue` = `.d` only · `SpringTrace.vue` = `.e` only ·
`SpringHeatmap.vue` = `.f` only · `StartingStyleTarget.vue` = `.g` only · `useSpringDemo.ts` = `.c` only
· `AnimationVisualizer.vue` = `.i` only, and its carve read whole = **KF-AV-16's machine-seam decision
alone**. The `.i` hand-off claim verified at the bytes: `useDragCapture`'s exported surface
`{ isDragging, onPointerDown }` and the `DragCaptureHandlers` interface are **byte-identical** before and
after, so `PlaybackRibbon.vue` (KF.W13's) is untouched in form as well as in bytes.

### Axis 3 — no masking fallback anywhere in the diff

⟨cmd⟩ `git diff dd28da55..HEAD | grep -c '^+.*@ts-expect-error'` → **0** · `'^+.*eslint-disable'` → **0**.
Semantic skip/only ⟨cmd⟩ `grep -rnE '\b(it|test|describe)\.(skip|only)\(' test/` → **no match** (the one
literal hit in the diff is prose at `spring-derby-truth.test.ts:21` that *forbids* the practice).
**Three `try/catch` blocks are added and all three were read whole**: two guard `setPointerCapture`
(which genuinely throws on an inactive pointerId / disconnected element; the gesture is driven by the
document/window listeners either way) and one is amiga **D-8's own ruled cure** — a WebGL build failure
becomes a state the scene paints instead of an exception that kills the mount. **None masks a defect of
this wave**; each carries its rationale at the site. The ten `as any` are **nine in test scaffolding**
(a rAF stub and three fabricated refs in `spring-derby-truth.test.ts`) and **one is a false positive** —
the word "any" inside a `useDragScrub.ts` comment. No allowlist, no copied producer selector, no
`node_modules` patch. **Assertions were extended, never narrowed**: ⟨cmd⟩ over the five extended
witnesses, `it(` 6→10 · 11→13 · 4→6 · 7→12 · 5→10 and `expect(` 15→26 · 20→24 · 12→19 · 12→27 · 13→29,
with ⟨cmd⟩ `git diff dd28da55..HEAD -- <the five> | grep '^-.*expect('` → **no output — zero assertions
removed**.

### Axis 4 — commit families per §Commit plan, not split

All six MUST-NOT-SPLIT families verified ONE sha each by `git show --stat` at this seat: `dc3f0900`
(CubeScene 4 ± + SpringScene 2 −, the four `variant:` sites in one edit) · `f4249cb0` (ST-1's word +
the css deletion, L-9) · **`4ab4c4db` (the guard family in BOTH composables — `useDragCapture.ts`
112 ± AND `useDragScrub.ts` 128 ±, so L-18 base (iii) cannot be raised)** · `b9b5476c` (tether-truth
D-1+D-6+D-16+N-SQ-4) · `8c0122e1` (MISS-3's settle-paced tour AND its assertion, 4 files one sha) ·
`e683d9a1` (the spring-plot core D-1+N-1+N-2+C-2/L-3, one file one sha). **§Sequencing 8 — the decision
precedes the patch, three times — verified by clock**: `.d` `532f8985` 04:13:44 → `36ec392e` 04:21:45 ·
`.e` `ec4b7eff` 06:03:10 → `e683d9a1` 06:09:21 · `.f` `f6d05a5c` 06:11:59 → `065008e9` 06:21:10.
**Two declared families did NOT land, and neither was split**: KF11-E(c1) (M-4→M-3's bytes are
`SpringTrace.vue`, §Bounds `:118` = `.e`'s) and KF11-E(d1) (the loop seam needs
`demo/composables/scene-runtime/useSweepScene.ts`, a path in **no** §B.2 row). Both are correct
ESCALATIONS under the standing law, not misses of will — this seat independently confirms both paths
are outside the escalating units' writable sets.

### Axis 5 — E-3 held

⟨cmd⟩ `git log --format='' --name-only --grep='x-kf-w11'` over value.js → **13 distinct paths**, every
one an append or a new file under `execution/B/KF-W11.md`, `keyframes/evidence/W11/**` or
`V/coordination/INBOX.md`. ⟨cmd⟩ `git show --stat <each> -- docs/tranches/V/megatranche/registry/adjudicated
docs/tranches/X/keyframes/waves docs/tranches/X/COHESION.md docs/tranches/X/EXECUTION-RUNBOOK.md
docs/precepts` → **no output on any commit**. The two LEDGER commits (`8366f8ad`, `5f3694f7`) touch
`LEDGER.md` **alone**. **No dated spec, adjudicated registry row, conformance artefact or sibling spec
was edited by this wave.**

### Axis 6 — mail clean

⟨cmd⟩ `grep -cE '^\| [IO]-[0-9]+[a-z]? \|' INBOX.md` → **79**; status read by cell position and leading
verb → **0 UNREAD**. Four-path entry counts re-measured: **24 · 9 · 13 · 28**, identical to the close's.
⟨cmd⟩ `find <each> -maxdepth 1 -type f -newermt "2026-09-19 07:00"` → **only `INBOX.md` itself**
(self-excluded). Tails unmoved at **I-35 / O-39**. The eight producer rows **W11-P1..P8 are present and
whole** at `INBOX.md:280-287`. **No UNREAD mail in scope.**

### Axis 7 — the four-verb line

AUDITED **YES** (unmoved) · SPECIFIED **YES** (unmoved) · IMPLEMENTED **YES —
`complete_with_misses`**, the qualifier never dropped in the record or the LEDGER · VERIFIED **NO**,
correctly unstamped (§State `:19` reserves it to a successor close). See MINOR-3 for the one reservation.

### Axis 8 — the spec's own goal criterion, MET at the bytes

The criterion's **named failure mode did not occur**: all four BLOCKER cores are landed and each was
read at its own sha — CubeTarget #53 `bc5cc128` · AmigaScene D-1 `73b424da` · SpringTarget C-1
`8dcf5882` · SquareScene L-2 **LANDED-BY `d7f68225`** (verified an ancestor of `dd28da55` and a genuine
KF.W5 `.d` library commit touching `compile-bridge.ts` / `css/animation.ts` / `playback.ts`, so R.2
GREEN-BEFORE-CURE is correctly booked rather than claimed as this wave's cure; its demo witness
`1233e42e` passes). The criterion's own worked example is cured to the letter: ⟨cmd⟩
`grep -c 'setTimeout(step, 520)' demo/scenes/square/useSquareKeyboard.ts` → **0**, the file carries **no
`setTimeout` at all**, its docblock now says *"paced by the spring's own settle … each corner waits for
the chase to arrive"* and names its own former lie, and the assertion landed in the **same sha**
(`8c0122e1`). **A docblock asserting an invariant is an assertion, and this one is now met.**

### Axis 10 — HONEST-RED ADJUDICATION (the two RED gates)

**The set: {G-KFW11-4, G-KFW11-10}. Both trace to ONE case**, `spring-trace-truth.test.ts (4b)`,
reproduced verbatim at this seat.

**Adjudicated at the spec's bytes, weighing both ways:**

1. **It is NOT relieved by any of the three canonical reliefs.** It is not producer-owned; the spec
   routes it to no successor; and the spec names no honest-RED by id here. **The cure's file
   (`test/demo/scenes/spring-trace-truth.test.ts`) is `.e`'s own create row in §B.2 `:131`** — i.e.
   inside this wave's writable set. This seat states that plainly rather than laundering it.
2. **Its substance is verified GREEN at the bytes by this seat, independently.** The invariant case (4b)
   guards is L-14's coupling — *the heatmap's declared ζ floor is not below the one the plot pins.*
   Measured: `SpringHeatmap.vue:157` `DAMPING_AXIS: ParamAxis = { min: 0.2, … }` and `SpringTrace.vue:236`
   `PLOT_DAMPING_FLOOR = 0.2`. **0.2 ≥ 0.2 — the coupling HOLDS.** What broke is the witness's *anchor*:
   `.f`'s contract commit re-homed the constant from a bare `DAMPING_MIN = <number>` (which `.e`'s regex
   reads) to the exported `DAMPING_AXIS.min`. The close's characterisation is therefore **true and
   independently confirmed**, not a soft framing.
3. **G-KFW11-4's own spec-stated GREEN-when clauses both PASS.** §Gates `:295` makes the gate green when
   *"the extracted builder (L-10) is unit-tested against the engine's own samples"* and *"the axis unit
   matches the N-1 decision"* — verified passing at this seat as cases **(1)** and **(5a)**. The failing
   (4b) is a case `.e` **minted beyond** the gate's stated acceptance form, to enforce a row the spec
   wrote as *"L-14 (document **or** clamp the coupling)"*. G-KFW11-10's clause *"every `test/demo/**`
   file passes"* is the one literally unmet clause, and it is unmet solely through (4b).
4. **Nothing was masked to hide it.** `.f` kept no dead byte in its own file to satisfy a sibling's
   regex; no seat skipped, widened, narrowed or allowlisted; the failure fires as `.e`'s **own designed
   diagnostic message**, which is the opposite of concealment.
5. **Owner named, cure written out**: **KF11-E(j1) / residual j-R5** → the orchestrator → a
   keyframes-side seat; the cure is one import + one assertion (`import { DAMPING_AXIS }` and
   `expect(DAMPING_AXIS.min).toBeGreaterThanOrEqual(PLOT_DAMPING_FLOOR)`), which is **stronger** than the
   regex it replaces.

**Conclusion**: the RED is real and in-bounds — recorded as **MINOR-1** below, not dismissed — but its
weight under the spec is low (a stale witness anchor over an invariant that holds, outside the gate's own
stated acceptance form), it is disclosed rather than laundered, and it carries a named owner and a
written cure. It does not rise to HIGH and it does not block.

### Successor "Opens after" conjuncts, measured against this wave

| successor conjunct | state at this seat |
|---|---|
| KF.W12 ← **"KF.W11 `.a`'s OD latch-family commit"** | **GREEN** — `fff7232c`, landed and published; AXISLINE-UNIT opens on it |
| KF.W12 ← **"KF.W11 `.b`'s `useKeyframeOps.ts` L-2 commit"** | **satisfied without a sha** — the carve was never spent (L-2 landed library-first at `d7f68225`, R.2) and the file is **byte-unchanged**, so the serially shared path is free and KFED-UNIT is **NOT blocked**. The conjunct as worded names a commit that does not exist; KF.W12's seat must read this statement instead of hunting a sha (INFO-3) |
| KF.W12 / KF.W13 ← **G-KFW4-1 GREEN** | still RED at the raw count (24 ≠ 0), but **lawfully relieved** by the ADDENDUM `:410` — OP-0 is a RATCHET and *"KF.W12 and KF.W13 read their own OP-0 rows under the same ruling"*. Neither successor is blocked **by this wave** |
| KF.W13 ← `useDragCapture`'s exported surface | **GREEN** — verified byte-identical (surface and handler interface); `PlaybackRibbon.vue` never edited |

**No successor is blocked by KF.W11.**

### Register — severity · claim · receipt · cure

| # | severity | claim | receipt | cure |
|---|---|---|---|---|
| **MINOR-1** | MINOR (mitigated) | **Two gates close RED and the cure is inside this wave's own §Bounds** — `spring-trace-truth.test.ts (4b)`'s regex witness for `SpringHeatmap.vue`'s `DAMPING_MIN` is stale after `.f` re-homed the constant; no relief of the three canonical kinds applies | ⟨cmd⟩ `npx vitest run --project demo test/demo/scenes/spring-trace-truth.test.ts` → `1 failed \| 11 passed (12)`; §B.2 `:131` gives the file to `.e` | **Mitigation, verified**: `DAMPING_AXIS.min` **0.2** ≥ `PLOT_DAMPING_FLOOR` **0.2** — the invariant holds; G-KFW11-4's stated GREEN-when clauses pass as cases (1)/(5a); no masking; owner + written cure at KF11-E(j1)/j-R5. **Cure**: land the one-line re-bind (`import { DAMPING_AXIS }`, assert `DAMPING_AXIS.min >= PLOT_DAMPING_FLOOR`) at a keyframes-side seat |
| **MINOR-2** | MINOR (mitigated) | **The COHESION §4a accretion mirror was not written, and the stated reason is inaccurate against this wave's own §Bounds** — `## Close` says *"`COHESION.md` is in no close seat's hard-bound writable set"*, but §B.2 `:141` grants `.j` exactly *"`INBOX.md` + the SS-6 accretion register (COHESION §4a)"* | §B.2 `:141`; ⟨cmd⟩ `grep -n 'W11-P1' docs/tranches/X/COHESION.md` → no hit | **Mitigation**: the eight producer rows are written **whole** into `INBOX.md:280-287`, nothing is lost, an owner is named (j-R2 → orchestrator) and a landed precedent is cited (KF.W0's close). **Cure**: the orchestrator mirrors W11-P1..P8 into COHESION §4a, or `.j`'s successor seat spends the grant it actually held |
| **MINOR-3** | MINOR (mitigated) | **IMPLEMENTED stamped YES while 2 of 10 gates are RED**, against §State `:18` (*"stays NO until the gates green"*) | the verb table at `## Close`; §State `:18` | **Mitigation**: `WAVE_SPEC.md:34/:47` authorises `complete_with_misses` as the Status of a wave that does not fully meet its criterion; the qualifier travels on **every** statement of the verb in the record and the LEDGER; VERIFIED is correctly **NOT** stamped. Nothing is concealed. **Cure**: none owed — the successor close that stamps VERIFIED reads the qualifier |
| **INFO-1** | INFO | The per-unit commit table's **Σ = 69 is arithmetically coincidental**: `git log --grep="X.KF.W11.<u>"` double-counts `da10a50a` (matched by two units' greps) and omits `d8eb43ff` (`chore(lock)`, carrying no unit grammar), so `.r` landed **2** commits, not 1 | ⟨cmd⟩ the ten greps at this seat reproduce **1·9·11·13·9·10·3·5·5·3 = 69** exactly, and ⟨cmd⟩ the duplicate scan → `da10a50a` | none — the published figures reproduce under the same commands; the total **69** and the bounds audit are unaffected |
| **INFO-2** | INFO | **G-KFW11-3's case (b) lives in a sibling packet's file.** §Gates `:293` names four cases; `spring-derby-truth.test.ts` carries (a)/(c)/(d) and nine cases, while (b) *"first/last plotted x = 0/100"* is asserted — and **passes** — at `spring-trace-truth.test.ts (2a)`. Disclosed in full at the `.c` receipt (KF11-E(c1), a lawful escalation: the bytes are `.e`'s per §B.2 `:118`) but not restated in `## Close` | both runs at this seat; ⟨cmd⟩ the derby reporter lists nine case names, none of them (b) | none owed — the assertion exists and passes where its bytes live; a successor reading `## Close` alone should be pointed at the `.c` receipt |
| **INFO-3** | INFO | **`demo/scenes/easing/EasingScene.vue` sits in §Bounds' Do-NOT-touch paragraph yet was written by `.r`** (`2b649a1d`) | ⟨cmd⟩ `git diff dd28da55..HEAD -- …/EasingScene.vue` — one unread alias + its now-dead `computed` import | **lawful** under the ADDENDUM `:414`, whose `.r` grant excludes only *"a W11–W13 §Bounds row"*, and `EasingScene.vue` is a row of **none** (KF.W12 `:114` carves `EasingSidebar.vue` only). The write is **type-level only**, and its replacement docblock is **true at the bytes** (`demo.isPlaying.value` read at `:89` and `:104`). Recorded so no later seat reads it as a bounds breach |

### Artefacts re-run at this seat

⟨cmd⟩ `npx eslint demo/scenes demo/composables demo/components/playback demo/components/instrument/transport/composables`
→ **exit 0, no output** · ⟨cmd⟩ `git diff --check dd28da55..HEAD` → **no output** · ⟨cmd⟩
`git -C ../keyframes.js rev-parse --short origin/master` → **`bf4a9a9c`** = `HEAD`, **all 69 commits
published** (j-R1's discharge confirmed) · ⟨cmd⟩ `git status --porcelain` in both repos → the same four
known-dirty rows, **none inside this seat's writable set**, `scripts/dev/dev.sh` never touched.

### SELF-COUNT

**10** gate verdicts re-run and **10** reproduced (**8** GREEN · **2** RED) · **17** byte clauses
re-measured, **17** identical · **24 · 24** ratchet, **16**-file histogram identical, **0** rows in a
KF.W11 cured surface · **69** commits and **59** paths re-audited, **0** outside §Bounds · **6**
MUST-NOT-SPLIT families re-verified ONE sha each · **4** BLOCKER cores re-read LANDED · **3**
decisions verified to precede their patches by clock · **5** extended witnesses, **0** assertions
removed · **13** value.js paths, **0** immutable paths touched · **79** INBOX rows, **0** UNREAD · **8**
producer rows present · **0** BLOCKER · **0** CRITICAL · **0** HIGH · **3** MINOR · **3** INFO · **0**
keyframes.js bytes and **0** glass-ui bytes written by this seat.

**CHECK 1 VERDICT: CONFORMANT-HONEST-RED.** Every GREEN the close claims reproduces at an independent
seat's own commands; every RED it carries reproduces too, and reproduces as what the close said it was —
a stale witness anchor over an invariant this seat re-verified holding at the bytes, disclosed with its
owner and its cure rather than skipped, widened or narrowed. **Honest-RED set: G-KFW11-4 · G-KFW11-10.**
