...
SERVED MODEL: claude-opus-5-5[1m] (this receipt; the file's line 1 names the file's creating seat). Seat opened 2026-09-23 03:1x EDT at HEAD `e5bb6a25`.

**Act 0 — crash-recovery and preconditions.** ⟨`git status --porcelain`⟩ → `M docs/tranches/V/reformation/CARRY-LEDGER.md` · `M scripts/dev/dev.sh` only: **no path of `.j`'s writable set dirty, nothing inherited**. LEDGER row 37 reads **X-W5 CLOSED 2026-09-17** → `.j` dispatches (§0az · §0ba). Spec read whole (492 L); COHESION §0az/§0ba read; record read at header, the eighth-sitting Open → Unit plan, `.s`/`.f2`/`.v` receipts' tails.

**Act 1 — anchors at the true bytes (D-19), INTENT recorded where they drifted.**
- j1's RED proxy (`component: Stub` ×14) is **X-W5's ruled shape**, not a defect: ⟨`grep -c "component: Stub" demo/color-picker/router/index.ts`⟩ → `14`, and its own comment names the route NAME as the scene driver. The scene contract `.j` adopts is `VIEW_MAP` (`demo/shell/viewSchema.ts`) → `role="region"` slots (`App.vue` `.pane-wrapper--<role>`) → X-W4's `SceneActionSet` (`usePaneRouter.sceneActions`). **o29 reads the contract at the DOM it produces, never at the router table.** Router/App.vue: no write needed.
- j2's `getContext("2d")` sites: ⟨`grep -n getContext useMixingAnimation.ts`⟩ at HEAD → `:88` (per-tick) · `:143` (arm) · `:162` (clearCanvas) — **three** sites; the pane is `<KeepAlive>`-cached (`PaneSlot.vue:288`), so a route-away DEACTIVATES, never unmounts.
- j3's handlers: the demo's own are `useAtmosphere.ts:327/:330`; the blob's are glass's (`useWebGLCanvas`). **Substrates at the bytes**: smoke cell (SwiftShader) → blob **WebGL2**, atmosphere **`"css"`** (glass `resolveRenderMode("auto")`, `useAtmosphere.ts:158`: no GL surface); real GPU → **both WebGPU** (probe below).
- o29 path per the 2026-09-19 ADDENDUM (o27 ordinal = `o27-focus-affordance`).

**Act 2 — j4 MEASURED AT OPEN, before any cure** (banked `W6-evidence/gates/j-2026-09-23/j4-open.txt`). ⟨`VJS_E2E_PORT=9651 npx playwright test e2e/smoke/oracles/o29-scene-contracts.spec.ts --project=smoke -g "short landscape"`⟩ ×2, identical →
- `[j4] blob preview@rest=1.000 last="Reset" inViewport=true hit=true … preview@last=0.000` → **RED**
- `[j4] atmosphere preview@rest=0.164 last="Reset" inViewport=true hit=true … preview@last=0.079` → **GREEN**

Reading: at 720×450 the last control IS reachable on both routes, and each preview IS reachable at rest; but on Blob the preview and the last control are never on screen together. Layout probe: regions `Picker 16,128 688×628` · `Blob 16,771 688×2553`, document `450/3338` — `shell.css` `.pane-container` (`repeat(auto-fit, minmax(min(var(--pane-min),100%),1fr))`, `--pane-min: 25rem`) gives ONE column at 720, so tuning any Blob control scrolls the stage's blob away. o29 asserts the **simultaneous** reading (V·L4 "persistent preview … amended with short-landscape and last-control proof" — a preview that must be scrolled away to reach the control it previews is not persistent). **Not retired** (it did not measure GREEN); cure → **Act 7 ESCALATION**.

**Act 3 — j1 Gradient pilot.** o29 `gradient scene`: regions `["stage:Gradient","inspector:Palettes"]`; the rail + render tile live in the stage and nowhere else; selecting a stop makes the ONE inspector name it (`Stop 2 of 3`); zero controls nested in controls in the stage; the dock row reads `gradient.reset=ready · gradient.copyCSS=ready · gradient.seedFromPalette=ready`, and the dock's Reset returns the stop the rail just minted. **R.2: j1 measured GREEN at the bytes before this seat's edit** (X-W5 + X-W4 + `.a/.b/.c` had landed the substance). The pilot's adoption act is the pane's contract: `GradientPane.vue` exposed `visualizerRef.value?.resetGradient?.()` — a renamed member would leave a seat reading `ready` and doing nothing (the species `readScenePaneTarget` exists to refuse). Now `defineExpose({…} satisfies GradientSceneTarget)`, direct member calls, an unmounted visualizer THROWS into the `failed` state (`usePaneRouter.ts` resolve → `{kind:"failed"}`); `useTemplateRef`.

**Act 4 — j2 Mix canary.** BEFORE (HEAD bytes of the Mix tree parked in, cures held in the scratchpad and restored after; `j2-before.txt` first instrument, `j2-before-final.txt` final instrument):
- nested controls, palettes mode: `button[Palette menu] inside button[Select palette Dawn]` · `… Dusk` → **RED** (the selection `<button>` hosted the browse route's whole `PaletteCard`: its own press machine + click emit, a menu button, rename field, swatch buttons — two owners of one press).
- narration clock after route-away: first instrument (read at the region-label flip) `ops 1 → 9` over 1.5 s → **RED** (8 frames on a detached canvas); final instrument (read after the canvas leaves the document) did NOT discriminate at HEAD (that run's narration ended inside the pane's leave) — recorded, not claimed.
- backing store after route-away: `363120` px held → **RED** (both instruments).
Cure (`useMixingAnimation.ts`): ONE acquisition site (`arm`), the context held only for the narration window, ONE teardown `release()` (loop stopped, stage/context dropped, `width=height=0`) run at window close, phase reset, `onDeactivated`, `onBeforeUnmount`; a narration cut short SETTLES (its result was computed at `startMix`). `MixSourceSelector.vue`: the selection button's face is the palette's static identity (strip + name + count), `PaletteCard` import dropped. `MixPane.vue`: `defineExpose({…} satisfies MixSceneTarget)`. AFTER: all three limbs GREEN, and the phase machine is not stranded (back on Mix, `.mix-plate` stands, no `.mix-plate--ghost`).

**Act 5 — j3 preview survives.** Draws counted per canvas (`drawArrays`/`drawElements` wrapped before any context exists); loss forced through the context's own `WEBGL_lose_context`. **Blob** (WebGL2 on the smoke cell): lose → `isContextLost()===true` → restore → `false`, draws rise past the at-loss count, opacity > 0 — GREEN at first measurement (the contract glass ships, now ASSERTED, not assumed). **Atmosphere** is substrate-total, never skipped: the substrate is read at the canvas (inline `background-image` ⇔ `"css"`) and must equal `isSoftwareGL(renderer)`; on `"css"` no GL surface exists (draws `0` over 1 s) and the CSS render stands complete (`gradient(`, opacity > 0). **Real-GPU cell** (`j3-real-gpu.txt`, config `o29-gpu-headed.config.ts`, headed, no swiftshader): both previews hold a **WebGPU** context (`{"webgpu":"ctx","webgl2":null,…}`); o29's WebGL arm fails there at its first poll with the message `the preview draws through no WebGL context (not a WebGL substrate)`, because glass's `useWebGPUCanvas` treats a `"destroyed"` device loss — the ONLY loss a page can force (`GPUDevice.destroy()`) — as terminal by design and re-arms only on a genuine GPU reset. Residual **J3-WEBGPU** below.

**Act 6 — commit.** ⟨`git commit … -- GradientPane.vue useMixingAnimation.ts MixPane.vue MixSourceSelector.vue o29-scene-contracts.spec.ts`⟩ → **`17dd0df4`** `feat(demo/scene-adoption)` (the one family; `git diff --check` clean). Untouched in the writable set: `MixConfigBar.vue` · `MixAnimationCanvas.vue` · `BlobPane.vue` · `AuroraPane.vue` (no cure needed there at the bytes).

**Gates — BEFORE → AFTER (settled bytes = `17dd0df4`; transcripts `o29-after-run{1,2}.txt`).**

| gate | ⟨cmd⟩ | BEFORE | AFTER ×2 |
|---|---|---|---|
| j1 gradient scene | `npx playwright test e2e/smoke/oracles/o29-scene-contracts.spec.ts --project=smoke -g "gradient scene"` | o29 absent (RED by absence); GREEN at the bytes on authoring (R.2) | **GREEN · GREEN** |
| j2 mix canary | `… -g "mix canary"` | **RED** (nesting ×2 · backing 363120 · clock 1→9) | **GREEN · GREEN** |
| j3 preview survives | `… -g "preview survives"` (blob + atmosphere) | o29 absent; blob GREEN at first measurement | **GREEN · GREEN** (2/2 each run) |
| j4 short landscape | `… -g "short landscape"` | blob **RED** · atmosphere GREEN (at open) | blob **RED · RED** · atmosphere GREEN · GREEN → **ESC-W6j-1** |
| full o29 | `… o29-scene-contracts.spec.ts --project=smoke` | — | `5 passed · 1 failed` · `5 passed · 1 failed` (the 1 = j4 blob) |
| H1 | `node docs/tranches/X/gates/gate-no-chassis.mjs 17dd0df4 b2dd375c a87f8930 3c558956 b0991fd3 9557e6b5 e07bff63 7dff25f6 63713d4c adc312f6 e0e204a9 c222542d f90aeb02` | GREEN (Check 3) | positive control fires first, then `GATE H1 (no copied chassis) — GREEN` over **29 demo/ files / 13 commits**, ×2 byte-identical (`h1.txt`) |
| H2 `.j` leg | `test -f …/motion-quarantine.md` + ⟨`grep -n MOTION-SOURCED o29`⟩ | owed | file tracked; **2** MOTION-SOURCED markers (`:32`, `:266`), **2** cite `motion-quarantine.md`; no motion PROPERTY asserted (only that a clock drew and stopped) — **GREEN** |
| §7 prettier | `npx prettier --check` on touched paths | three `.vue` HEAD-dirty (prettier-delta 9 · 18 · 73 lines) | new/clean files **pass**; HEAD-dirty three at **9 · 18 · 68** (≤ HEAD: no new violation) |
| §7 eslint | `npx eslint <5 touched>` | — | exit **0** |
| §7 vue-tsc | `npx vue-tsc --noEmit -p tsconfig.demo.json` | 0 | exit **0** |
| §7 vitest | `npx vitest run` | 639/641 (R-12) | **639 passed · 2 failed** — C-5 `spectrum-luma` + NG-6 `reka-binding-idiom`, the foreign born-RED canaries already routed (R-12 → X-W8 `.i`, §0z E2); none touches `.j`'s bytes |

**Act 7 — ESCALATION `ESC-W6j-1` (§3a file-bound expansion; j4's Blob limb).** Measured reason: at 720×450 the shell lays the Blob scene out as ONE column (`demo/styles/shell.css` `.pane-container` auto-fit over `--pane-min: 25rem`), the stage (Picker, 628 px tall, the blob at y 83–325) above the inspector (Blob, from y 771), in a 450 px viewport — so no scroll position shows the preview and the inspector's last control together (`preview@last=0.000`, ×3 incl. open). The cure is an ARRANGEMENT: a short-landscape second track or a persistent (sticky) stage seat for scenes whose inspector tunes the stage's live preview — the shell's property (`viewSchema.ts:95` "the SHELL owns the arrangement"; `shell.css:184-185` itself hands "the ratio / ultrawide half" to CC-056 · V·L3 at X-W6 while `shell.css` sits outside W6.md §4). Every in-bounds substitute is refused: a second blob renderer inside `BlobPane.vue` would be a duplicated WebGL/WebGPU surface and a second hero (not the scene's preview), and nothing route-private can move the stage region. **Ask**: grant `demo/styles/shell.css` (the `.pane-container` arrangement at short landscape) to a `.j2` seat under X-W6, or route the limb to X-W8 (shell owner) by id `J4-SHORT-LANDSCAPE-BLOB`; o29's j4 blob arm is the gate either way (Atmosphere's arm is GREEN).

**Residuals (each with an owner).**
- **J3-WEBGPU** — the real-GPU cell's forced-loss arm is unwitnessable from a page (glass: `"destroyed"` terminal by design; a genuine reset re-arms). Producer row → glass BK relay (a test seam for a non-destroyed device loss, or a producer-side loss/recovery oracle cited back); `.j` writes no mail (INBOX is outside its set) — the close seat relays.
- **MX-3** (pre-existing, not `.j`'s to cure) — the colors-mode add slot and palette swatches are WatercolorDot impostor seats (glass 7.0.0 `inheritAttrs:false` drops `tag`/`aria-label`/`@click`); routed MX-CLUSTER + **X.W4.g** (the glass-8 trigger-gated cut, GLASS8-REPIN-CENSUS `:405`/`:408`/`:441`). j2 drives its narration through the palettes mode's real buttons for that reason and says so in the spec.
- The in-pane `Copy CSS` (`GradientVisualizer.vue:299`) beside the dock's `gradient.copyCSS` is the X-W4 design (pane affordance + dock seat on one target command, no second code path) — recorded, not adjudicated a defect; `GradientVisualizer.vue` is outside `.j`'s set.
...
## Check 3 — RESUME 2026-09-23 (L-20 fresh adversarial pass 3, over the eighth sitting's close `11a82954` + Repairs 1–2 `75420aab`/`46b2cb17`/`70a27b79`/`1b4447f8`)

SERVED MODEL: claude-opus-5-5[1m]

VERIFY-ONLY; authored no byte of the wave. Spec read whole once (`W6.md`, 492 lines, five ADDENDA); of this record only the eighth
Open → Unit plan (`:7824-7902`), Check 2 (`:8438-8519`) and Repair 2 (`:8521-8597`). Runs 05:03–05:08 EDT, HEAD `e3a57285`
(⟨`uptime`⟩ → load 10.06 at the open, 18.26 at the last gate). Transcripts in this seat's scratchpad, not committed.

**Crash-recovery.** ⟨`git status --porcelain`⟩ → ` M docs/tranches/V/reformation/CARRY-LEDGER.md` · ` M scripts/dev/dev.sh` only.
Neither is in this seat's set (this record, the LEDGER), so nothing was inherited and nothing was touched.

### Axes 2/3/4/5 — bounds, masking, families, E-3

Since Check 2 the only commits are `1b4447f8` and `e3a57285`. ⟨`git show --stat`⟩ → this record (+78) and the LEDGER (+1). No product,
test or instrument byte moved. ⟨`git diff --stat 3e933c33..HEAD -- scripts/dev/dev.sh docs/tranches/X/waves/W6.md
docs/tranches/V/megatranche/registry/adjudicated/ src api docs/tranches/X/waves/W[0-9]*.md`⟩ → **empty**. E-3 held, and dev.sh is
untouched. Check 2's bounds, masking and family readings over `75420aab`/`46b2cb17` still hold, since no byte has moved since. Repair 2's
temporary diagnostic was restored and never committed: ⟨`git show --stat 1b4447f8`⟩ lists the record only.

### Axes 1/9 — every claimed GREEN re-run at this seat's clock

| gate(s) | ⟨cmd⟩ | this seat |
|---|---|---|
| f1 · f5 · h2 · i2 | `node docs/tranches/X/gates/gate-{catalog-totality,specimen-grammar,blob-pipeline,lband-door}.mjs` | **GREEN**, EXIT 0 ×4, `GATE f1/f5/h2/i2 … — GREEN` |
| c3 | `npx vite-node docs/tranches/X/gates/gate-literal-dialect.mjs` | **GREEN** `GATE c3 (literal dialect) — GREEN` |
| c1 | `node …/wb-gradient-stopeditor/gate-structure.mjs` | **GREEN** `GATE G4 (structure) — GREEN` |
| d2 | `node …/x-w6/gate-easing-readout.mjs` | **GREEN** `DATED ASK` · local restyle `0 line(s)` |
| e2 | `grep -rn requestAnimationFrame demo/workbenches/gradient/ \| wc -l` · `gate-prm-idiom.mjs` | **GREEN** `0` · `GATE e2 (PRM idiom) — GREEN` |
| a1 (vitest leg) · c2 · c4 | `npx vitest run test/gradient-order-invariant.test.ts test/interpolation-subset.test.ts` | **GREEN** `Tests 22 passed (22)` |
| H1 | `gate-no-chassis.mjs 46b2cb17 17dd0df4 … f90aeb02` (16 shas) | **GREEN** positive control fires · `29 demo/ files over 16 commits` · EXIT 0 |
| H2 (file) · H3 | ⟨`git ls-files …/motion-quarantine.md \| wc -l`⟩ · the parser R1 one-liner | **GREEN** `1` · EXIT `0` |
| g2 | `VJS_E2E_PORT=9813 npx playwright test …/companion-pane-track-start.spec.ts -g "companion panes share one track start" --project=smoke` | **GREEN** `1 passed (14.9s)` EXIT 0 |
| j1 · j2 · j3 | `VJS_E2E_PORT=9812 npx playwright test e2e/smoke/oracles/o29-scene-contracts.spec.ts --project=smoke` | **GREEN**, inside `1 failed · 5 passed (41.9s)` |
| **j4** | same run | **RED** `[j4] blob preview@rest=0.880 … preview@last=0.000` (`Expected: > 0 · Received: 0`); atmosphere `preview@last=0.079` |
| a2 · a3 · a4 · b3 · **e1 · a13** | `VJS_E2E_PORT=98{11,14} npx playwright test e2e/smoke/views/gradient.spec.ts e2e/smoke/oracles/o21-gradient-rail.spec.ts --project=smoke` ×2 | **GREEN ×2** `22 passed (1.9m)` · `22 passed (1.8m)` EXIT 0 (load ≈10–18) |

A first o29 attempt run concurrently with the suite died at webServer start (`EADDRINUSE 127.0.0.1:8091`, two Playwright configs
sharing one fixed port). That is an artefact of this seat's concurrency, not a gate reading. o29 was re-run serially, as above.

**24 claimed GREENs reproduced · 0 claimed GREENs failed.** e1 → a13 read GREEN 2 of 2 here. That does **not** cure C2-1. The gate's
own record at the same bytes is RED 2 of 2 at Check 2 (load ≈16) and RED 1 of 8 at Repair 2. Repair 2 measured the species (one
4333-px, 1/255 tile re-raster at x 231–461, reverting next frame), and a flake does not become stable by passing twice more. The
spec's close bar (§6: every born-RED gate GREEN by its command) is a stability claim. The gate is not close-stable at the bytes, and
the escalation that would re-read it (`ESC-W6e1-1`) has no ruling.

### Axes 6/7/8/10 — mail, the four-verb line, the goal, honest-RED adjudication

**Mail.** ⟨`grep -n "^| [IO]-[0-9]" docs/tranches/V/coordination/INBOX.md | grep -i unread | grep -ci x-w6`⟩ → `0`, so there is
**0 UNREAD in scope**. **Four-verb** stays `IMPLEMENTED no`; no seat moved it, which is lawful. **Owner rulings.** ⟨`grep -n
'ESC-W6j-1\|ESC-W6e1-1' docs/tranches/X/COHESION.md docs/tranches/X/waves/W6.md`⟩ → **empty**. The last COHESION addendum is still
`§0ba` (`:2947`, commit `277b8d25`), so neither escalation has a ruling. **Goal (§2a) at the bytes: NOT MET.** CC-057's short-landscape
last-control proof is RED on the Blob limb (j4). H1 holds.
...
| g1 | `W6.md:484` (2026-09-22 ADDENDUM): honest-RED by id → X-W10 (M-23) | R-6 · X-W10 | **HONEST-RED** |
| i3 | `W6.md:492` (fifth ADDENDUM): `I3-SEED-SIZE` honest-RED with relay O-52 | R-5 · glass subpath → `.i3` re-sit | **HONEST-RED** |
| j4 | none. §5/§6 admit j4 only as "cured or retired" with its measurement, and it measures RED (`preview@last=0.000`). `ESC-W6j-1` is open with no ruling | R-j4 · OWNER | **UNRELIEVED — HIGH** |
| e1 (→ a13) | none. No addendum names e1 honest-RED, and `ESC-W6e1-1` is unruled. GREEN 2/2 here, but RED 2/2 at Check 2 and 1/8 at Repair 2, so it is not close-stable | ESC-W6e1-1 · OWNER (E-3 re-reading of e1's measure, or an X-W1 launch flag) | **UNRELIEVED — HIGH (flake-class; pending the ruling)** |

**Successors' "Opens after".** X-W7: at the spec bytes (`W7.md:6`) it opens after X-W3, X-W4 **and X-W6**. X-W3 and X-W4 are CLOSED,
but the X-W6 conjunct is FALSE, so X-W7 is **lawfully BLOCKED**. The LEDGER cell (`X-W1`) under-states the spec, and Check 2's
"X-W7 GREEN, independent of this wave" is corrected here (C3-3). X-W8 (`W5 · W6 · W7`): the W6 conjunct is FALSE, so it is
**lawfully BLOCKED**. X-W10 (`X-W5 · X-W6 · …`): **BLOCKED**. X-W11 (every wave IMPLEMENTED): **BLOCKED**.

### Register

| # | severity | claim | receipt | cure |
|---|---|---|---|---|
| C3-1 | HIGH | j4's Blob limb is RED with no spec relief. `ESC-W6j-1` is still unruled | this seat: `[j4] blob preview@rest=0.880 … preview@last=0.000`, `Received: 0`; COHESION has no `ESC-W6j-1` row | the OWNER rules `ESC-W6j-1` (an X-W5 shell grant, or a dated E-3 re-reading of j4); then a seat cures or re-reads |
| C3-2 | HIGH | e1 → a13 is flake-class RED with no relief. It passed 2/2 here, but the same bytes read RED 2/2 at Check 2 and 1/8 at Repair 2 (compositor 1-LSB tile re-raster). `ESC-W6e1-1` is unruled | this seat `22 passed` ×2; Check 2 `:8470` RED ×2; Repair 2 species table | the OWNER rules `ESC-W6e1-1`: a dated E-3 re-reading of what e1's `moved` measures (e.g. persistence across frames), or an X-W1 launch-flag change; then GREEN ×3 under load |
| C3-3 | INFO | Check 2's successor line "X-W7 … GREEN, independent of this wave" is wrong at the spec bytes. `W7.md:6` names X-W6 in Opens-after, and the LEDGER cell `X-W1` under-states it | ⟨`sed -n 6p docs/tranches/X/waves/W7.md`⟩ | corrected here; the LEDGER's X-W7 cell is the COHESION seat's to align (not this seat's row) |
| C3-4 | INFO | C2-4 (duplicate INBOX id O-52 · the `J3-WEBGPU` relay) is still owed | Repair 2 carries it | the COHESION seat, or the next INBOX-granted seat |

**Honest-RED set (relieved at the spec bytes, owner-named):** b1 (`B1-G3D` → X-W11) · g1 (→ X-W10) · i3 (`I3-SEED-SIZE`, O-52).

### Verdict

**NOT-CONFORMANT.** 24 claimed GREENs reproduced and 0 failed. Landed-wrong 0; no masking; E-3 held; 0 UNREAD in scope; bounds clean.
Two gates stay unrelieved and each is HIGH. j4 is RED at this seat. e1 → a13 is GREEN 2/2 here but not close-stable, and its
...
SERVED MODEL: claude-opus-5-5[1m] · executes COHESION §0bb R-v1 (`:2963`) · W6.md §5 `.a` a2/a3 rows + falsifiers (`:156-181`) · GRADSTOP-A §14/§15.

**Acts, in order**

1. Crash-recovery ⟨`git status --porcelain`⟩ → the two standing rows only (`CARRY-LEDGER.md`, `scripts/dev/dev.sh`); nothing inherited in the `.a` set.
2. Reproduce R-v1 — capture-v.mjs block 1 verbatim (mint 30 % + 62 %, press the 62 % handle at its centre, 14 moves to `bar.x + 0.10·bar.w`, 20 ms apart), in a scratch copy that also logs `got/lostpointercapture` and the dragged stop per move, against a FRESH `npx vite --port 9951 --strictPort` (killed after). Result at HEAD `58e06db7`: moves 1–8 track (58.8 → 31.3); **move 9 crosses the 28.9 % neighbour → 27.4 %, and `lostpointercapture:stop-4-…` fires on that same move**; moves 10–14 (x 338.9 → 270.2) leave it at **27.4 %** → final `[0, 27.4, 28.9, 100]`. The `.v` frame's figure, reproduced exactly. **Verdict: the RAIL stops tracking, not the capturer** (the capturer's pointer path is continuous and on the handle row).
3. Root cause (read at bytes, `GradientStopEditor.vue` HEAD `:347`): `seat.setPointerCapture(e.pointerId)` captures on the handle `<button>`; every write re-sorts the stops (normalise-on-write), so the keyed `v-for` (`:627-628`) MOVES a handle node when it crosses; a node moved in the DOM loses pointer capture; later moves land on the bar, whose handler returned early while `draggingId` was set, and the release ended the drag. Direction-dependent: Vue's keyed diff moves the node that is out of the longest increasing subsequence — on a LEFTWARD crossing that is the dragged node; on a rightward one it is the neighbour (why a2's existing rightward drag stayed GREEN).
4. Falsifier FIRST (born RED at HEAD product bytes): `gradient.spec.ts` a2 test (`:172`) gains — the dragged stop (by `data-stop-id`) must settle within **2 %** of the release x mapped onto the rail's own axis (`--rail-inset` + padding box, the product's inverse read independently), asserted after the existing rightward crossing AND after a new leftward crossing back to ~25 %; the ascending-order assertion stays and is re-read after the leftward leg. ⟨`VJS_E2E_PORT=9952 npx playwright test e2e/smoke/views/gradient.spec.ts -g "neighbour-crossing" --project=smoke`⟩ → **1 failed**: `expect(…).toBeLessThanOrEqual(2)` · `Received: 21.069863013698633` at the leftward leg (rightward leg passed).
5. Cure at the rail (`GradientStopEditor.vue`, the `.a` set): capture on `bar` (`bar.setPointerCapture`), the element whose identity the re-order cannot touch; the rail's `pointermove` / `pointerup` / `pointercancel` own the live drag (`dragTo` / `endDrag`; cancel still only disarms — GRADSTOP-A §12); the handle's `@pointermove` / `@pointerup` / `@pointercancel` bindings and `onHandlePointerCancel` DELETED (no second path; a8 one owner). No clamp on the sole mutator, no spacing rule (§14/§15); model untouched.
6. Re-probe on a fresh server (`:9953`, killed after): moves 9–14 now track 27.4 → 23.5 → 19.6 → 15.6 → 11.7 → **7.8** (release x 270.2), final `[0, 7.8, 28.9, 100]`; `gotpointercapture:DIV` at move 1, `lostpointercapture:DIV` only at release.
7. Hygiene: ⟨`npx eslint <both files>`⟩ → no output · ⟨`npx vue-tsc --noEmit -p tsconfig.demo.json`⟩ → exit 0, 0 lines · ⟨`npx tsc -p tsconfig.e2e.json | grep -c gradient.spec`⟩ → `0` · prettier diff-line count (⟨`npx prettier <f> | diff - <f> | grep -c '^[<>]'`⟩): `GradientStopEditor.vue` 0 → 0; `gradient.spec.ts` 50 (HEAD) → **46** (the new hunk adds none) · ⟨`git diff --check -- demo e2e`⟩ → empty.
8. Commit `b1c5ff34` `fix(demo/gradient-model)` — pathspec `GradientStopEditor.vue` + `e2e/smoke/views/gradient.spec.ts` (2 files, +77 −19), body carries the cause.

**Gates, BEFORE → AFTER** (serial, fresh webServers, line reporter; the settled post-prettier bytes of `b1c5ff34`; transcripts in the seat scratchpad)

| gate | ⟨cmd⟩ | BEFORE | AFTER |
|---|---|---|---|
| a2 · a3 ×3 | `VJS_E2E_PORT=998{1,2,3} npx playwright test e2e/smoke/views/gradient.spec.ts e2e/smoke/oracles/o21-gradient-rail.spec.ts -g "neighbour-crossing\|one axis: every handle" --project=smoke` | a2 **RED** with the new falsifier (21.07 > 2); a3 GREEN (baseline) | **GREEN ×3**: `2 passed (21.2s)` · `2 passed (20.4s)` · `2 passed (20.1s)`, EXIT 0 ×3 (load 14.5 · 14.0 · 11.6) |
| suite (gradient + o21, incl. the new falsifier) ×3 | `VJS_E2E_PORT=999{1,2,3} npx playwright test e2e/smoke/views/gradient.spec.ts e2e/smoke/oracles/o21-gradient-rail.spec.ts --project=smoke` | 22 passed ×1 (baseline; falsifier absent) | **GREEN ×3**: `22 passed (2.1m)` · `22 passed (2.1m)` · `22 passed (2.0m)`, EXIT 0 ×3 (load 14.1 · 13.3 · 15.5). The falsifier lives inside the a2 test, so the count stays 22 |
| a5–a11 guard (the move owner changed) | `GRADIENT_URL=http://localhost:9954/#/gradient node docs/tranches/X/waves/W6-evidence/gradient/gate-a-gesture-paint.mjs` (fresh server, killed after) | GREEN (Check 3) | **GREEN** ×1: `travel@1px=0.00px travel@120px=120.00px` · a7 `before=2 middle=2 right=2 cancel=2` · **a8 `5` writes over 5 pointermoves** · a10 rules intersected 0 · a9 `[]`, `3 → 2` — `GATE X.W6.a (gesture + paint) — GREEN` |
