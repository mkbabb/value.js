...
## Check 3 — RESUME 5, L-20 fresh adversarial pass 3, 2026-09-22 (VERIFY-ONLY)

SERVED MODEL: claude-opus-5-5[1m] · seat: X-W5 CHECK 3 over `## Close — RESUME 5` as `## Repair 2` (RESUME 5) left it · HEAD at open `85469fba` · date of record 2026-09-17 (COHESION §0j).
This seat cured nothing. It wrote this section and one LEDGER event line. Inputs: `W5.md` whole (396 L, both ADDENDA), `## Close — RESUME 5`, `## Check 2` and `## Repair 2` (RESUME 5), and the commits they name.

**Verdict: NOT-CONFORMANT.** Every claimed static GREEN reproduces ×2 (byte-identical runs), and census-parity reproduces GREEN ×2. The two RED legs of the §0ax `.c2` gate, o16 R2 and o12 O-12·3, reproduce ×2 and still have no relief. ⟨cmd⟩ `grep -c 'ESC-W5c2' docs/tranches/X/COHESION.md` → **0**, and ⟨cmd⟩ `git log -1 --oneline -- docs/tranches/X/COHESION.md` → `df34be29` (§0ax), unchanged. The row stays **PARTIAL**, and IMPLEMENTED stays unstamped.

### K3.0 Crash-recovery
⟨cmd⟩ `git status --porcelain` → ` M docs/tranches/V/reformation/CARRY-LEDGER.md` · ` M scripts/dev/dev.sh`. Both are outside this seat's writable set, so there is **no inherited partial**. `dev.sh` was not touched.

### K3.1 Axes 1 and 9: static gates (`scratchpad/k3static.sh` ×2, ⟨cmd⟩ `diff s1 s2` → IDENTICAL)
⟨cmd⟩ `git diff --stat 85469fba..HEAD -- demo e2e src docs/tranches/X/COHESION.md docs/tranches/X/waves/W5.md` → empty. No product, test or ruling byte has moved since Repair 2.
...
| C4 · C8 · D2 exits | 0 · 0 · 0 | 0 · 0 · 0 | YES |
| C3 fork census | 23 | 23 (routed) | YES |
| D3 physical names | 0 (role-name grep hits in `animations.css`: stage 11 · inspector 11 · action 12, every role present) | 0 | YES |
| D4 (Mix · AdminNames `<Transition `) | 1 · 1 | 1 · 1 | YES |
| E1 ×4 · E2 `P122` | 0 0 0 0 · 9 3 18 10 | same | YES |
| landmark `grep -rl "Color tool panes" e2e \| wc -l` | 0 | 0 | YES |
| containment `contain: layout paint;` decl | 1 | 1 | YES |

### K3.2 The live oracles, ×2 (load average 10.4 at start → 16.3 at end)
⟨cmd⟩ `VJS_E2E_PORT=5261|5263 npx playwright test e2e/smoke/oracles/o12-blob-seat.spec.ts e2e/smoke/oracles/o16-computed-cascade.spec.ts e2e/visual/census-parity.spec.ts --reporter=line` was run twice. Both runs gave **10 passed / 7 failed**, EXIT=1.
- census-parity: 0 failures in both runs, so it is **GREEN ×2**.
- `o16:113` W5-census R2 failed in both projects in both runs at `:218`: Expected `"0.4s"`, Received **`"0.44s"`**. ⟨cmd⟩ `git blame -L 218,218 e2e/smoke/oracles/o16-computed-cascade.spec.ts` → `27f54cc36` (2026-07-11), a pre-W5 literal. ⟨cmd⟩ `grep -o -- '--spring-snappy-settle: [^;]*' node_modules/@mkbabb/glass-ui/dist/styles/tokens/scheme-spring.css` → `0.44s` (glass-ui 7.0.0). W5's `.d2` animations bytes consume `var(--spring-snappy-duration)` and write no literal, so the drift is the producer token against a hard-coded oracle literal. The cure is a consumer test byte (re-derive R2/R8 from the resolved token), not an upstream-only fix, and it needs a grant on `o16:218` and `:266`.
- `o12:134` O-12·3 [smoke] failed in both runs: hover response **0.51 /255** (run 1) and **0.11 /255** (run 2) against the 6/255 floor, on software GL.
- **New divergence from Check 2 (INFO, environment):** in both runs, the four `[oracles-safari]` o12 legs (·1+2, ·3, ·4, ·5) failed at `bootWithBlob` (`o12:66`, `[data-testid="goo-blob-canvas"]` not attached within 15 s). Check 2 had only O-12·3 failing in safari. No demo or e2e byte moved between the two checks (K3.1), and the load was 16, so this is attributed to the environment (webkit WebGL boot under load), not to a W5 byte. It is registered, not weighed. It does not change the verdict, because O-12·3 is RED either way.

D1 of record (headed real-GPU) was not re-run. It is banked 2/2 at the close and 2/2 at Check 1, and no byte has moved since.

### K3.3 Axes 2–8
- **(2) Bounds.** The four commits since the close (`7ca62fd0` · `725e26dc` · `7a5d368d` · `85469fba`) touch only `execution/A/X-W5.md` and `execution/LEDGER.md` (⟨cmd⟩ `git show --stat`). The RESUME 5 product shas were bounds-checked at Check 2 K2.3 and have not moved. ⟨cmd⟩ `git log --oneline 0afc25c3^..HEAD -- scripts/dev/dev.sh | wc -l` → **0**.
- **(3) Masking.** None. The o16 literal and the o12 floor are unchanged. There is no skip, no allowlist and no `node_modules` byte.
- **(4) Families.** Unchanged since Check 2: P-1..P-4 + EB-2 are one commit (`52dc0a5b`).
- **(5) E-3.** ⟨cmd⟩ `git diff --stat 5ad17633..HEAD -- docs/tranches/X/waves/W5.md docs/tranches/X/waves/W6.md docs/tranches/X/waves/W7.md docs/tranches/X/waves/W8.md docs/tranches/X/waves/W10.md docs/tranches/V/megatranche/registry/adjudicated/` → empty.
- **(6) Mail.** ⟨cmd⟩ `ls -t ../glass-ui/docs/tranches | head -1` → BK. ⟨cmd⟩ `find <value.js V · V/coordination · BK coordination · keyframes V/coordination> -maxdepth 1 -type f -newer execution/A/X-W5.md` → empty. INBOX has one UNREAD row, **I-40**, which belongs to Track C and is out of scope. **0 UNREAD in scope.**
- **(7) Four-verb.** Unmoved (IMPLEMENTED NO). That is lawful while the addendum gate is RED.
- **(8) Goal criterion.** Met at the product bytes, as Check 1 found and K3.1 re-reads. The remainder is test-oracle debt inside the §0ax gate.

### K3.4 Register
| # | severity | claim | receipt | cure |
|---|---|---|---|---|
| K3-1 | **HIGH** | The §0ax `.c2` gate "o16 GREEN ×2" is RED and unrelieved. The pre-W5 oracle literal `"0.4s"` at `o16:218` (and `:266`) disagrees with the producer token `--spring-snappy-settle: 0.44s`. The fix needs a test byte that no grant covers. | K3.2 ×2 runs × 2 projects · `ESC-W5c2` hits in COHESION: 0 | COHESION rules ESC-W5c2-1 and grants `o16:218`/`:266`. Then an Opus repair seat re-derives R2/R8 from the resolved `--spring-snappy-duration`. Run o16 ×2. |
| K3-2 | **HIGH** | The §0ax `.c2` gate "o12 GREEN ×2" is RED on O-12·3 (0.51 · 0.11 /255 against 6/255). It is unruled, and no W5 addendum names it honest-RED. | K3.2 | COHESION rules ESC-W5c2-2, for example a headed real-GPU reading of record as §0ax does for D1. No floor change and no skip. |
| K3-3 | INFO | The `[oracles-safari]` o12 legs could not boot the blob canvas ×2 at load 16. Check 2 did not see this, and no byte moved in between. | K3.2 | Re-read in a quiesced window at the next oracle run. Attributed to the environment. |
| K3-4 | INFO | D1 headless SwiftShader stays RED beside the record. It is not the reading of record (§0ax). | close CL5.2 | operator item (registered) |

### K3.5 Axis 10: honest-RED adjudication
- **Relieved by the spec's own routing** (owners named in CL5.5): B3 → X-W6 CC-056 · A3/C2 → X-W8 (§0k.3 S-1) · A5-OUTLINE → X-W10 · C3 layout forks → X-W8 `.i` · C7 `DockStatusLamp.vue:70` → X-W8 `.h` (§0aq) · D1 headless → by-instrument (§0ax).
- **Unrelieved:** o16 R2 (K3-1) and o12 O-12·3 (K3-2). Neither is producer-only, because each cure is a consumer test byte or a ruling. Neither is routed to a successor, and no addendum names either one honest-RED. Each has an owner, COHESION via ESC-W5c2-1/-2, but neither has a ruling. **Axis 10 is NOT met.**
...
## Check 1 — RESUME 6, L-20 fresh adversarial pass 1, 2026-09-23 (VERIFY-ONLY)

SERVED MODEL: claude-opus-5-5[1m] · seat: X-W5 CHECK 1 (RESUME 6) · HEAD at open `f46ab34b` · date of record 2026-09-17 (COHESION §0j).
This seat cured nothing. It wrote this section and one LEDGER event line. Inputs: `W5.md` whole (398 L, all three ADDENDA), `## RESUME OPEN 6`, the RESUME 6 `.c3`/`.d3` receipts, `## Close — RESUME 6`.
...
**Verdict: NOT-CONFORMANT.** Every GREEN the close claims reproduces, and every RED it reports reproduces as RED. Two of the third ADDENDUM's gates are RED with no relief in the spec: they are escalated (ESC-W5c3-1, ESC-W5d3-1), and COHESION has not ruled on either (⟨cmd⟩ `grep -c 'ESC-W5c3-1\|ESC-W5d3-1' docs/tranches/X/COHESION.md` → **0**). The record is honest (PARTIAL, IMPLEMENTED not stamped). The row stays PARTIAL.

### K1.1 Axes
- **(1) Claimed GREENs, at this seat's clock:** A1 exit **0** · App grep **0** · A4 `bindPane` **10** · A7 `role="status"` **5** · B4 **0 · 3 · 0** · C4 exit **0** · C8 exit **0** · D2 exit **0** · D3 **0** · D4 Mix `<Transition ` **1** · E1 **0 0 0 0** · E2 P122 **9 3 10 18** · C3 **23** (routed, as stated) · §7 ⟨cmd⟩ `npx vue-tsc -p tsconfig.demo.json --noEmit` → **0**. 14 of 14 static and typecheck readings agree with CL6.2. D1 (headed real GPU) and the live probes are banked, which is lawful: ⟨cmd⟩ `git diff --stat d1bc67c3..HEAD -- demo src` → empty.
- **(2) Bounds:** ⟨cmd⟩ `git show --stat` on `8ddafa23` `881d3792` `b9f63632` `a072eef3` `28da478f` `f46ab34b` `545f675f` → only the o16 spec, the new cold-nav witness, `waves/W5/green/**`, this record, INBOX and LEDGER. All of these are inside §0ay's `.c3`/`.d3` grants or the seat set. `dev.sh`: not in any commit. **Clean.**
- **(3) Masking:** `8ddafa23` swaps the R2/R8 literals for a resolved-token read (no `0.44s` literal, no skip). The witness's `try{…}finally{ctx.close()}` is cleanup and swallows nothing. There is no `test.skip`, allowlist or `node_modules` byte. **Clean.** INFO: R2 and R8 compare against a value read through the same cascade. If the token failed to resolve, both sides would read `0s`, but R3's leave row and the `linear(` timing assertion keep the census from being vacuous.
- **(4) Families:** each commit carries one meaning. P-1..P-4 `52dc0a5b` untouched. **Clean.**
- **(5) E-3:** ⟨cmd⟩ `git diff --stat d1bc67c3..HEAD -- docs/tranches/X/waves/W5.md docs/tranches/V/megatranche/registry/ src scripts/dev/dev.sh` → empty. **Held.**
- **(6) Mail:** the only UNREAD row is I-40 (Track C). CL6.4's sweep has nothing newer. **0 UNREAD in scope.**
- **(7) Four-verb:** unchanged. IMPLEMENTED is NO, which is correct while the ADDENDUM-3 gates are RED. **Lawful.**
- **(8) Goal criterion:** NOT MET at the bytes. "Moving between scenes is an animated … event": on a cold navigation, a pane is left frozen in `vj-enter-enter-from/-active` (the gradient rail sits off-screen). That is a scene that never finishes entering.
- **(9) Figures:** 59 tracked W5 artefacts (CL6.3) were not recounted; no W5 path moved since. The RED figures reproduce (below).

### K1.2 RED gates, re-run ×2 at this seat (load ≈ 4.6–7.2)
| gate | ⟨cmd⟩ | run 1 | run 2 | relief under axis (10) |
|---|---|---|---|---|
| o16 W5-census | `VJS_E2E_PORT=5471/5477 npx playwright test e2e/smoke/oracles/o16-computed-cascade.spec.ts` | 2 passed / 2 failed, `R4 translate duration Expected "0.3s" Received undefined` | same | **NONE.** ADDENDUM 3 names it as `.c3`'s gate ("o16 GREEN ×2"). ESC-W5c3-1 is unruled. No later wave is routed it by the spec. R4's register may be producer-owned (glass 7.0.0 `@utility cartoon-surface` ships no transition), but no spec or ruling byte says so yet |
| cold-nav witness (both arms) | `VJS_E2E_PORT=5475/5479 npx playwright test e2e/smoke/oracles/cold-nav-scene-enter.spec.ts --project=smoke` | 2 failed: `/#/gradient` Received 2 · `/#/` Received 1 | same (1 · 2) | **NONE.** ADDENDUM 3 `.d3` gate ("0 stuck enter states ×2"). ESC-W5d3-1 is unruled, and the cure needs `usePaneRouter.ts`, which is outside the grant |
| W6 a5–a11 · a12 · e1 · g2 (by W6's commands) | not re-run (no product byte; `.d3` read them RED at HEAD) | — | — | **NONE.** ADDENDUM 3 `.d3` gate. Turns only with the ESC-W5d3-1 cure |
| o12 O-12·3 headed real GPU | banked (CL6.2 0.72 · 0.78 /255) | — | — | **RELIEVED:** §0ay ESC-W5c2-2 routes it to X-W8 `.i` as O12-3-HOVER-GPU, and the residual register names that owner |
| B3 · A3 · C3 · C7 `DockStatusLamp:70` · D1 headless | banked | — | — | **RELIEVED:** X-W6 CC-056 (§3) · X-W8 `.i`/`.h` (§0aq, first ADDENDUM) · §0ax (D1 headed = the reading of record) |

### K1.3 Register
| severity | claim | receipt | cure |
|---|---|---|---|
| **HIGH** | The ADDENDUM-3 `.c3` gate "o16 GREEN ×2" is RED with no spec relief: the census now fails at R4, and R5/R11 are latent. | K1.2 row 1 (×2); `c3.4` table | COHESION rules ESC-W5c3-1: grant R4/R5/R11 on the §0ay resolved-token pattern, and dispose of R4's missing producer register (a glass mail row, never a frontend hack). Then an Opus repair seat, with o16 GREEN ×2 |
| **HIGH** | The ADDENDUM-3 `.d3` gate "5 fresh contexts × cold navigation → 0 stuck enter states ×2" is RED on both arms, and with it W6 a5–a12/e1/g2. The §2 goal criterion (animated scene entry) fails at the bytes. | K1.2 row 2 (×2); `d3.2` root at Vue `resolveTransitionHooks` enter guard | COHESION rules ESC-W5d3-1 (option (a) recommended: grant `usePaneRouter.ts` `lazyPane` inside the W5F-07 family, PaneSlot keys on `(pane, resolved)`, retire the `App.vue:134` flip). Then a `.d4` Opus seat; witness ×2 plus the W6 re-reads plus D1 non-regression |
| INFO | R2/R8 read their expectation through the live cascade. This is sound under §0ay, but a token that failed to resolve would give `0s` on both sides. | `8ddafa23` diff | none required; the other census rows guard it |

**Honest-RED set (relieved):** o12 O-12·3 → X-W8 `.i` (O12-3-HOVER-GPU, §0ay) · B3 → X-W6 CC-056 · A3 / C3 → X-W8 `.i` · C7 `DockStatusLamp:70` → X-W8 `.h` · D1 headless → §0ax (not of record).
...
SERVED MODEL: claude-opus-5-5[1m] · unit X.W5.d4 (Opus) · executes W5.md ADDENDUM 4 (`:400`) · COHESION §0az ESC-W5d3-1 RULED (a) (`:2918-2945`) · this record's R7.3–R7.4 · `.d3` receipt (`28da478f`) · HEAD at open `2d18b6da`. **Status: PARTIAL.** The cure landed and its own witness is GREEN ×2. W6 g2 is still RED at W6's own command, and the e1 suite is intermittent, because those instruments read the pane's pre-start enter pose during the software-GL frame stall. That is ESC-W5d4-1, below.

**d4.0 Crash-recovery.** ⟨cmd⟩ `git status --porcelain` → the four standing rows (`CARRY-LEDGER.md` · two W6 catalog PNGs · `scripts/dev/dev.sh`). None is in the writable set. **No inherited partial.** Anchors at true bytes: `PANE_LOAD_DELAY_MS` `usePaneRouter.ts:209`, `function lazyPane` `:211-224` (§0az cites `:189-223`, so the intent was applied at the true bytes). `App.vue:134` = `:transition-name="viewManager.ready.value ? 'vj-enter' : ''"`, which matches.

**d4.1 The literal shape, measured first (uncommitted experiment, reverted with `git checkout -- <own paths>`).** The experiment kept ONE async wrapper as the child in both phases. `lazyPane` flipped a `shallowRef` in the loader's `.then`, and PaneSlot keyed that same wrapper on `` isPaneResolved ? liveKey : `${liveKey}:loading` ``.
⟨cmd⟩ `CASES='[["gradient",800],["gradient",800],["",800],["gradient",0]]' node scratchpad/mountprobe.mjs` (fresh contexts, pane chunks held, MutationObserver insert counts) → on `#/gradient` held ×2, **both regions latched to their ErrorBoundary**: *"Failed to execute 'insertBefore' on 'Node': The node before which the new node is to be inserted is not a child of this node."*. `gradient-stop-bar` was inserted 1 time and was present 0 times.
The reason: the pending-key instance is the same wrapper, so when the loader resolves it grows the pane inside its own leave, and KeepAlive then parks it. `runtime-core` `defineAsyncComponent` setup, `load().then(() => { loaded.value = true; … instance.parent.update() })`, re-renders a deactivated instance. So the occupant under the pending key must be a **plate phase that never resolves to the pane**, and it must not be cached. This is how (a) has to be built at these bytes. It is not a different cure: the key is still `(pane, resolved)`, readiness still comes from the loader promise, and no internal field is read.

**d4.2 The cure: ONE commit `f55e59b7` (pathspec: `usePaneRouter.ts` · `PaneSlot.vue` · `App.vue`; ⟨cmd⟩ `git show --stat f55e59b7` → 3 files, +78 −7).**
- **Readiness publish (`usePaneRouter.ts`, `lazyPane`).** The factory now returns the pane's **plate phase**, a `defineComponent` named `PANE_PLATE_PHASE` that renders one async wrapper. That wrapper carries the whole P-3 contract unchanged: `loadingComponent: PaneLoadingPlate`, `delay: PANE_LOAD_DELAY_MS`, `errorComponent: PaneErrorPlate`, and the rejection re-raised as `PaneChunkError(pane)`. Its loader's fulfilment sets `resolution.value = module.default` and **resolves the wrapper to `PaneLoadingPlate`**, so the plate phase ends on the plate and never on the pane. `resolvedPane(component)` is exported and reads the resolution reactively: an eager pane (Picker, NotFound) is itself, and a lazy pane is `null` until the loader fulfils. The W5F-07 family P-1..P-4 (`52dc0a5b`) stays unsplit. The header comment records the measured reason for the plate phase.
- **(pane, resolved) key (`PaneSlot.vue`).** `livePane = computed(() => resolvedPane(liveComponent))`. When it is resolved, the child is `<component :is="livePane" :key="liveKey" :ref=… v-bind=liveProps>`, cached as before. Otherwise it is `` <component :is="liveComponent" :key="`${liveKey}:plate`"> `` under `<KeepAlive :exclude="PANE_PLATE_PHASE">`, never cached. Out-in then runs plate-leave and after it pane-enter, and `leavingVNodesCache[key]` never equals the entering vnode because the keys differ. Only the resolved pane carries the mount report (`bindPane`, gate A2's state arm) and the pane props.
- **Flip retirement (`App.vue:134`).** `:transition-name="viewManager.ready.value ? 'vj-enter' : ''"` became `transition-name="vj-enter"`. `useViewManager`'s `ready` output is now unread in `demo/`. That file is outside this unit's bounds, so the residual is named in d4.6.
- LOCKS held: ⟨cmd⟩ `grep -rn '__asyncResolved\|__asyncLoader' demo/ | wc -l` → **0** · no frame element (the Transition child is the pane root or the plate root, and `.pane-wrapper > *` is unchanged) · no settle, wait, forced class removal, `test.skip` or CSS byte (⟨cmd⟩ `git diff HEAD~2 -- demo/styles | wc -l` → 0).

**d4.3 Witness plate arm: commit `5a7d3113`** (`e2e/smoke/oracles/cold-nav-scene-enter.spec.ts`). Per context, `goto(…, commit)` runs, then `expect(.pane-plate).first().toBeVisible()`, and only then `networkidle`. This is the `PANE_LOAD_DELAY_MS` plate-still-shows witness: the chunk is held 800 ms against the 200 ms delay.

**d4.4 Probes at the cure bytes (evidence `docs/tranches/X/waves/W5/green/d4-cold-nav-cure-2026-09-23.json`, committed with this record).**
- ⟨cmd⟩ `CASES='[["gradient",800]×2,["",800]×2,["gradient",0],["mix",800]]' node scratchpad/mountprobe.mjs` → stuck **0** in all 6. `gradient-stop-bar` inserts `1,1,0,0,1,0`: no double mount. `.pane-plate` inserts `2,2,1,1,0,1`: the plate shows on every held chunk and never on the unheld one. Boundary errors: 0.
- ⟨cmd⟩ read-only DIAG instrument (`d3-enter-guard-diag.mjs`, a scratch copy that differs only in the port) run on `#/` at hold 0 ×4, 300 ×2 and 100 ×2 → `enter-dropped` **0** and stuck **0** in all 8.

**d4.5 Gates, BEFORE → AFTER (load 6.7–14 across the runs; four tracks share the host).**

| gate | BEFORE (R7.3 / `.d3`) | AFTER, this seat | state |
|---|---|---|---|
| cold-nav witness (2 routes × 5 fresh contexts), ⟨cmd⟩ `VJS_E2E_PORT=8894\|8895 npx playwright test e2e/smoke/oracles/cold-nav-scene-enter.spec.ts --project=smoke` | RED ×2 (`/#/gradient` 2, `/#/` 1) | run 1 **2 passed (39.1s)** · run 2 **2 passed (38.3s)** | **GREEN ×2, both arms** |
| `PANE_LOAD_DELAY_MS` plate on a slow chunk (witness arm) | — | visible in every context of both runs · mountprobe plate inserts ≥ 1 on every held case | **GREEN** |
| W6 a5–a11, ⟨cmd⟩ `GRADIENT_URL=http://localhost:9401/#/gradient node docs/tranches/X/waves/W6-evidence/gradient/gate-a-gesture-paint.mjs` ×2 | RED of record | EXIT 0 ×2 · `GATE X.W6.a (gesture + paint) — GREEN` · *outside of the viewport* 0 ×2 | **GREEN ×2** |
| W6 a12, `WBGSE-D-probe2.mjs` block 5 (a scratchpad copy that differs only at `:6`, `9000`→`9401`; ⟨cmd⟩ `diff` → that one line; PNGs written to the scratchpad, the evidence dir untouched) | RED of record | EXIT 0 ×2 · `"afterOverhangPx": 0` ×2 | **GREEN ×2** |
| W6 e1 (+ its suite), ⟨cmd⟩ `VJS_E2E_PORT=941n npx playwright test e2e/smoke/views/gradient.spec.ts e2e/smoke/oracles/o21-gradient-rail.spec.ts --project=smoke` ×4 | RED 1/2 of record | 20/22 (`o21:60`, `o21:254` 1.22) · **22/22** · 20/22 (`gradient:401` W5-10 reads `.pane-wrapper > *` mid-enter at `matrix(1, 0.00095, …, 15.29, 0)`; `gradient:451` e1 *"ramp moves"*) · 21/22 (`o21:254`) | **e1 ✓ 3 of 4; the suite is not GREEN ×2** |
| W6 g2, ⟨cmd⟩ `VJS_E2E_PORT=942n npx playwright test e2e/smoke/views/companion-pane-track-start.spec.ts -g "companion panes share one track start" --project=smoke` ×6 | RED ×2 (6.50) | RED · ✓ · RED · RED · ✓ · RED, each RED Received **6.50144** | **RED 4 of 6** (ESC-W5d4-1) |
| D1 of record, headed real GPU, ⟨cmd⟩ `PROBE_HEADED=1 PROBE_BASE=http://localhost:8173 node docs/tranches/V/megatranche/workflows/gates/scene-swap-budget.mjs` on a scratch `vite build --mode gh-pages --outDir scratchpad/d4-dist` (BUILD_EXIT=0, at the cure bytes) ×2 | GREEN 14/14 (gradient 1/83 · extract 0/88 · mix 0/88 · generate 0/88, median 10) | run 1 `pass:true` gradient 1/84 · extract 0/88 · mix 0/88 · generate 0/88, median 10 · run 2 `pass:true` 1/84 · 0/87 · 0/88 · 0/88, median 10 | **GREEN ×2, unchanged** |
| D3 physical names | 0 | ⟨cmd⟩ `grep -rc 'pane-wrapper--left\|pane-wrapper--right' demo/` summed → **0** | GREEN |
| D5 PRM | held | held: no motion declaration added (0 `demo/styles` bytes); the enter is the existing `vj-enter` family under the global guard | GREEN (banked) |
| §7 | — | `npm run typecheck` → lib **0** · demo **0** · test **11** (`demo/color-session/space-catalog.ts`, pre-existing) · `tsc -p tsconfig.e2e.json` **5** (`o23-specimen-gamut-honesty.spec.ts`, pre-existing; the witness 0) · `npx eslint <the 4 touched files>` EXIT 0 · `npx eslint . --max-warnings=0` EXIT 1 (`v-apotheosis-workflow.js:198` parse error plus 3 foreign unused-directive warnings, all pre-existing) · `npx vitest run` **639/641** (C-5 `spectrum-luma`, NG-6 `reka-binding-idiom`: the foreign canaries of record) | held |

**The mechanism of the remaining RED (measured).** ⟨cmd⟩ `node scratchpad/d4a12/raf.mjs` (5 fresh contexts on `#/`, one swiftshader browser, a rAF tick log plus the About card's class timeline). Context 0 has a **2760 ms frame gap** (333 → 3093 ms), and About's `vj-enter-enter-to` lands at 3109 ms. Contexts 1–4 have no gap: enter-to lands at ≈270 ms and the classes clear at ≈715 ms.
⟨cmd⟩ `node scratchpad/d4a12/about.mjs`: in the stalled context the About card rests at `matrix(0.999391, 0.0348995, …, 563.2, 0)` with offset **−6.5 px** and `getAnimations()` empty at both 300 ms and 1.8 s. By 4 s the transform is `none` and the offset is 0.
The enter is therefore **not stuck** (the witness proves it, ×2). It is waiting on a starved frame, as the witness header already records for the swiftshader boot. g2's settle arm (`readRow`: own running Animations only, plus two stable samples 250 ms apart) cannot see a Vue enter that has not started yet. It publishes the pre-start pose, and the 6.50144 matches the RED of record. The W5-10 / o21 / e1 misses are the same class: geometry read mid-enter under load 12–14.

**d4.6 Residuals (each has a named owner) and the escalation.**
- **ESC-W5d4-1 (for COHESION).** W6 g2 is RED 4 of 6 and the W6 e1 suite is 1 of 4 fully green at W6's own commands. The stuck enter is cured (witness GREEN ×2, 0 drops on the DIAG instrument). The instruments that still fail read pane geometry while the pane's legitimate `vj-enter` has not yet started. They do this in the first fresh context of a swiftshader browser, where the renderer gives no frame for about 2.8 s.
  The cure belongs to those instruments' settle arms: treat a `vj-enter-enter-*` class on the read element as "moving", or read after the region reports settled. The files are `e2e/smoke/views/companion-pane-track-start.spec.ts` (g2) and the `gradient.spec.ts:401` / `o21-gradient-rail.spec.ts` readers (W6 `.g` / `.a`). Both are outside `.d4`'s bounds.
  No product byte of `.d4` can make a starved frame arrive. A settle or CSS neutralisation here would be the banned masking. **Ruling asked:** grant the g2 / W5-10 / o21 settle-arm correction to the X-W6 seat that owns those specs. `.d4`'s product cure stands.
- `useViewManager.ts`'s `ready` ref (`:46-49`, exported `:92`) has had no reader in `demo/` since the flip retired. The file is outside `.d4`'s bounds, so the retirement is routed to the X-W5 close or to the next `useViewManager` writer.
- E13: no mail was swept at this unit seat (R7.1 covers the sitting), and no INBOX byte was written.

**d4.7 Commits (self-count 3 with this record's).** `f55e59b7` is the cure (one commit: readiness publish, `(pane, resolved)` key and the flip retirement). `5a7d3113` is the witness plate arm. The record commit carries this receipt plus `green/d4-cold-nav-cure-2026-09-23.json`. Each commit carries an exact pathspec, and `scripts/dev/dev.sh` was never staged.

## Close — RESUME 7, 2026-09-23 (VERIFY-ONLY close seat)

SERVED MODEL: claude-opus-5-5[1m] · seat: X-W5 CLOSE (RESUME 7) · HEAD at open `d6d8b38f` · date of record 2026-09-17 (COHESION §0j).
This seat cured nothing. It wrote this section, one INBOX sweep line and the LEDGER row cells. Inputs: `W5.md` whole (400 L, all four ADDENDA, the fourth 2026-09-23 at `:400`),
this record's `## RESUME OPEN 7` (R7.0–R7.4), the RESUME 7 `.c4` and `.d4` receipts, and `## Close — RESUME 6` (the last close section).

### CL7.0 Crash-recovery
⟨cmd⟩ `git status --porcelain` → ` M docs/tranches/V/reformation/CARRY-LEDGER.md` · ` M docs/tranches/X/waves/W6-evidence/catalog/after-catalog-open.png` ·
` M …/after-specimen-dots.png` · ` M scripts/dev/dev.sh`. None is in this seat's writable set (`X-W5.md`, `LEDGER.md`, `INBOX.md`). **No inherited partial.** `dev.sh` untouched.

### CL7.1 Commit roster and bounds (⟨cmd⟩ `git show --stat` per sha)

| unit | sha | paths | in writable set? |
|---|---|---|---|
| `.c4` | `e2efb30a` | `e2e/smoke/oracles/o16-computed-cascade.spec.ts` (+92/−38) | YES (ADDENDUM 4: the o16 spec whole) |
| `.c4` | `2d18b6da` | `execution/A/X-W5.md` · `waves/W5/green/c4-o16-2026-09-23.json` | YES |
| `.d4` | `f55e59b7` | `demo/shell/usePaneRouter.ts` (+59) · `demo/shell/PaneSlot.vue` (+24) · `demo/color-picker/App.vue` (+1/−1, the `:134` flip → `transition-name="vj-enter"` only) | YES — ONE commit, as §0az locks it |
| `.d4` | `5a7d3113` | `e2e/smoke/oracles/cold-nav-scene-enter.spec.ts` (+9/−1) | YES |
| `.d4` | `d6d8b38f` | `execution/A/X-W5.md` · `waves/W5/green/d4-cold-nav-cure-2026-09-23.json` | YES |

`.c4` touched no demo byte, which is lawful: ADDENDUM 4 grants `demo/styles/**` and the demo components, but does not require them, and the receipt measured every exposed rule already reading its token. ⟨cmd⟩ `grep -rn '__asyncResolved\|__asyncLoader' demo/ | wc -l` → 0 (the §0az lock holds). **Landed-wrong: none.**

### CL7.2 Gates, re-run at this seat's clock (static arms: `scratchpad/close.sh` ×2, ⟨cmd⟩ `diff cl7a cl7b` → IDENTICAL; live arms each ×2; load 5.6 → 13.0)
Live surfaces: the playwright oracles on distinct `VJS_E2E_PORT` (5510–5582, tree-true dev servers); W6 a5–a12 against `npx vite --port 9411`; D1 against a fresh
scratch `vite build --mode gh-pages --outDir scratchpad/cl7-dist` (BUILD_EXIT=0; ⟨cmd⟩ `git status --porcelain demo src` → empty, so the bundle is tree-true at `d6d8b38f`) served on `:8183`, `PROBE_HEADED=1`.

| gate | BEFORE (R7.3 / CL6.2) | AFTER (this seat) | state |
|---|---|---|---|
| A1 exit · App grep | 0 · 0 | **0 · 0** | GREEN |
| A4 `bindPane` hits · A7 `role="status"` | 10 · 5 | **10 · 5** | GREEN |
| A2 · A3/C2 · A5 · A6 · B1/B2 · C1 · C5 (live probes) | GREEN / relieved (CL5.2) | banked. The only product byte since CL5.2 is `f55e59b7` (the pane mount phase). Its live arms are re-read below through the cold-nav witness, W6 a5–a12/e1/g2 and D1 | banked |
| B4 `100dvh` · `svh` files · `content-max-h` | 0 · 3 · 0 | **0 · 3 · 0** | GREEN |
| B5 · B6 | recorded | unchanged (no `shell.css` byte) | recorded |
| C3 fork census · C7 viewport `@media` | 23 / 9 files · 3 | **23 / 9 · 3** | routed (X-W8 `.i` / `.h`, §0aq) · at target |
| C4 · C8 exit | 0 · 0 | **0 · 0** | GREEN |
| D2 · D3 physical · role rules | 0 · 0 · 10/10/10 | **0 · 0 · 10/10/10** | GREEN |
| D4 Mix · Admin `<Transition ` | 1 · 1 | **1 · 1** | GREEN |
| D5 PRM | GREEN (banked) | banked: ⟨cmd⟩ `git diff --stat 7ab5a3f1..HEAD -- demo/styles` → empty; no motion declaration added | GREEN (banked) |
| E1 · E2 | 0 0 0 0 · 9 3 18 10 | **0 0 0 0 · 9 3 18 10** | GREEN |
| landmark · fixture · census `not-found` · router `ViewRoute` · containment · plates | 0 · 2 · 3 · 2 · 1 · 2 | **0 · 2 · 3 · 2 · 1 · 2** | GREEN |
| **o16 W5-census** ⟨cmd⟩ `npx playwright test e2e/smoke/oracles/o16-computed-cascade.spec.ts --project=<p>` ×2 per project (`.c4`) | RED ×2 at R4 `Received undefined` | `smoke`: **2 passed** (12.0s) · **2 passed** (8.2s), EXIT 0 ×2 · `oracles-safari`: **2 passed** (6.0s) · **2 passed** (5.9s), EXIT 0 ×2 | **GREEN ×2 both projects** |
| **cold-nav witness** (2 routes × 5 fresh contexts, with the plate arm) ⟨cmd⟩ `npx playwright test e2e/smoke/oracles/cold-nav-scene-enter.spec.ts --project=smoke` ×2 (`.d4`) | RED ×2 (`/#/gradient` 2 · `/#/` 1) | **2 passed** (36.2s) · **2 passed** (36.9s), EXIT 0 ×2 | **GREEN ×2 both arms**, and the `PANE_LOAD_DELAY_MS` plate is visible in every context |
| W6 a5–a11 ⟨cmd⟩ `GRADIENT_URL=http://localhost:9411/#/gradient node docs/tranches/X/waves/W6-evidence/gradient/gate-a-gesture-paint.mjs` ×2 | RED of record | EXIT 0 ×2 · `GATE X.W6.a (gesture + paint) — GREEN` ×2 · *outside of the viewport* 0 ×2 | **GREEN ×2** |
| W6 a12 `WBGSE-D-probe2.mjs` block 5 (scratch copy; ⟨cmd⟩ `diff` vs the evidence file → only `:6` `9000`→`9411`; PNGs land in the scratchpad) ×2 | RED of record | EXIT 0 ×2 · `"afterOverhangPx": 0` ×2 | **GREEN ×2** |
| W6 e1 + suite ⟨cmd⟩ `npx playwright test e2e/smoke/views/gradient.spec.ts e2e/smoke/oracles/o21-gradient-rail.spec.ts --project=smoke` ×2 | RED of record; `.d4` read 20 · 22 · 20 · 21 /22 | **22 passed** (1.9m) · **22 passed** (2.0m), EXIT 0 ×2 | **GREEN ×2 at this seat**. `.d4` saw it intermittent, so it is read as load-sensitive and not stable (ESC-W5d4-1 class) |
| **W6 g2** ⟨cmd⟩ `npx playwright test e2e/smoke/views/companion-pane-track-start.spec.ts -g "companion panes share one track start" --project=smoke` ×2 | RED ×2 (6.50) | **1 failed** ×2, EXIT 1 ×2, each `Received: 6.501441955566406` | **RED ×2** (ESC-W5d4-1) |
| D1 of record, headed real GPU ⟨cmd⟩ `PROBE_HEADED=1 PROBE_BASE=http://localhost:8183 node docs/tranches/V/megatranche/workflows/gates/scene-swap-budget.mjs` ×2 | GREEN (gradient 1/83 · extract 0/88 · mix 0/88 · generate 0/88, median 10) | run 1 `pass:true`: gradient 1/84 · extract 0/89 · mix 0/89 · generate 0/89, median 10 ×4 · run 2 `pass:true`: 1/84 · 0/88 · 0/88 · 0/88, median 10 ×4 | **GREEN ×2, unchanged** |
| §7 ⟨cmd⟩ `npx vue-tsc -p tsconfig.demo.json --noEmit` · `npx tsc -p tsconfig.e2e.json --noEmit` · `npx eslint <the 5 touched files>` | 0 · 5 (o23) · — | **TSC_EXIT=0** · **5** errors, all in `o23-specimen-gamut-honesty.spec.ts` (pre-existing; the two W5 specs 0) · **ESLINT=0** | GREEN / foreign |
| §7 vitest · repo-wide lint | 639/641 foreign canaries · EXIT 1 foreign (`.d4` §7) | banked: ⟨cmd⟩ `git diff --stat 7ab5a3f1..HEAD -- src test` → empty | no W5 regression |

### CL7.3 §8 Verification Artefacts
- ⟨cmd⟩ `git ls-files docs/tranches/X/waves/W5/{born-red,green,triumvirate} | wc -l` → **12 · 28 · 21**; ⟨cmd⟩ `git ls-files docs/tranches/X/waves/W5 | wc -l` → **61**
  (+2 since CL6 = `green/c4-o16-2026-09-23.json`, `green/d4-cold-nav-cure-2026-09-23.json`); ⟨cmd⟩ `git status --porcelain docs/tranches/X/waves/W5` → empty. Every witness is in git.
- D1 after, both instruments: `green/d2-D1-scene-swap-both-instruments-2026-09-22.json`. The headed ×2 reading in CL7.2 reproduces it at the `f55e59b7` bytes. D5: `green/d2-D5-prm-2026-09-22.json`.
- Portal-integrity delta, B5 reading, B6 rationale, π captures and `audit/visual/layout/`: **unchanged since CL5.3**. No `index.html`, `shell.css` or `demo/styles` byte landed this sitting
  (⟨cmd⟩ `git diff --stat 7ab5a3f1..HEAD -- demo` → the 3 `f55e59b7` files only, and none of them is a layout or portal surface), so §5.2 parsimony applies and CL5.3's readings stand.

### CL7.4 E13 — the four-path sweep (this seat's clock)
⟨cmd⟩ `ls -t ../glass-ui/docs/tranches/ | head -2` → **BK**, BJ. ⟨cmd⟩ `find <p> -maxdepth 1 -type f -newermt 2026-09-23T00:26:06-04:00` (the RESUME 7 open commit `12cff396`) → **empty** on value.js `V/` · `V/coordination` · BK `coordination` · keyframes `V/coordination` · atlas `{P,Q,R}/coordination`.
INBOX: the one UNREAD row is still **I-40** (Track C), outside X-W5 scope. **0 UNREAD in scope.** One sweep line appended to INBOX.

### CL7.5 Residuals (named owners) and escalations

| gate | state | owner |
|---|---|---|
| **W6 g2** companion-pane track start (`Received 6.501441955566406` ×2 at this seat; 4 of 6 at `.d4`) + the W6 e1 suite's load sensitivity (22/22 ×2 here; 20 · 22 · 20 · 21 at `.d4`) | RED ×2 / intermittent | **COHESION / owner**, **ESC-W5d4-1** (unruled: ⟨cmd⟩ `grep -c ESC-W5d4-1 docs/tranches/X/COHESION.md` → 0). The ask is to grant the settle-arm correction (a `vj-enter-enter-*` class counts as moving, or the read waits for region-settled) in `companion-pane-track-start.spec.ts`, `gradient.spec.ts:401` and `o21-gradient-rail.spec.ts` to the X-W6 seat that owns them (`.g` / `.a`). No W5 product byte is owed. |
| `useViewManager.ts` `ready` ref (`:34`, `:47-49`, `:92`) has no reader in `demo/` since the `:134` flip retired (⟨cmd⟩ `grep -rn 'viewManager.ready\|\.ready\.value' demo/` → 0) | dead output | the next `useViewManager.ts` writer, or X-W8 subtraction (the file is a §4 W5 path, but no unit of this sitting was granted it) |
| the dead class `btn-interactive` (`ColorInput.vue:69,78`, `CurrentPaletteEditor.vue:75,78,100`) · `cartoon-surface`'s lost 0.3 s transition | residual (no computed effect) | a BK relay row (mail/orchestrator seat) if the motion is wanted back; otherwise X-W8 subtraction |
| o12 O-12·3 hover frame-diff, headed real GPU | RED, routed | **X-W8 `.i` O12-3-HOVER-GPU** (§0ay/§0az); no W5 byte owed |
| D1 headless SwiftShader | RED beside, not of record | operator (quiesced window), §0ax |
| B3 · A3 · A5-OUTLINE · C3 · C7 `DockStatusLamp:70` | relieved / routed, unchanged | X-W6 (CC-056) · X-W8 · X-W10 · X-W8 `.i`/`.h` · X-W8 `.h` |
| `not-found` visual goldens | owed | visual-baseline owner (X-W1 / CI-capable seat) |
| Vue upstream: dev-root-fragment × out-in; `defineAsyncComponent` re-rendering a deactivated instance (`.d4` d4.1) | letter (note, not a gate) | owner-report letter; never a `node_modules` patch |
| typecheck 11 `space-catalog.ts` · e2e-tsc o23 ×5 · vitest NG-6/C-5 · repo lint `v-apotheosis-workflow.js:198` | foreign | CL.7 row 11 · o23's owner · X-W8 `.i` · docs/V owner |
| L-18 quartet | not opened | follows a GREEN hard gate |

**Escalations carried:** ESC-W5c3-1 and ESC-W5d3-1 are RULED at §0az and DISCHARGED by `.c4` and `.d4` (o16 GREEN ×2 in both projects; the cold-nav witness GREEN ×2 on both arms).
**New and open: ESC-W5d4-1** (raised by `.d4`, reproduced here: g2 RED ×2).

**Verdict: PARTIAL.** ADDENDUM 4's `.d4` gate set includes *W6 a5–a12/e1/g2 GREEN ×2*. At this seat a5–a11, a12 and the e1 suite read GREEN ×2, and g2 reads RED ×2 at W6's own command.
...
## Check 3 — RESUME 7, L-20 fresh adversarial pass 3, 2026-09-23 (VERIFY-ONLY)

SERVED MODEL: claude-opus-5-5[1m] · seat: X-W5 CHECK 3 (RESUME 7) · HEAD at open `9d67b2bd` · date of record 2026-09-17 (COHESION §0j).
This seat cured nothing. It wrote only this section and one LEDGER event line. Inputs: `W5.md` read whole (400 L, all four ADDENDA), `## RESUME OPEN 7`, `## Close — RESUME 7`, `## Check 2 — RESUME 7` and `## Repair 2 — RESUME 7`, plus every commit in `12cff396..HEAD`.
Crash-recovery: ⟨cmd⟩ `git status --porcelain` → the four standing rows (`CARRY-LEDGER.md` · two W6 catalog PNGs · `scripts/dev/dev.sh`). None of them is in this seat's writable set, so there is no inherited partial.

**Verdict: NOT-CONFORMANT.** Nothing has moved since Check 2. ⟨cmd⟩ `git log --oneline 9d67b2bd..HEAD` → empty. ⟨cmd⟩ `grep -c ESC-W5d4-1 docs/tranches/X/COHESION.md` → **0**, and the last section is still §0az. W6 g2 reads RED ×2 at this seat's own clock and has no relief under the spec.

### K3.1 Axes
- **(1) GREENs reproduce.** Static arms at this seat:
  - A1 exit **0** · App grep **0**
  - A4 `bindPane` **10** · A7 `role="status"` **5**
  - B4 **0 / 3 / 0**
  - C4 **0** · C8 **0**
  - D2 **0** · D3 physical **0** · D4 Mix `<Transition ` **1**
  - E1 **0 0 0 0**
  - the §0az lock `__asyncResolved|__asyncLoader` **0**

  Live arms (⟨cmd⟩ `bash scratchpad/k3.sh`, ports 5901–5911, load at end **9.82**):
  - cold-nav witness `--project=smoke` → **2 passed** (36.3s)
  - o16 `--project=smoke` → **2 passed** (8.7s)

  **14/14 reproduce.** D1 headed, W6 a5–a12/e1 and o16 `oracles-safari` are banked: ⟨cmd⟩ `git diff --stat 508cd041..HEAD -- demo src e2e` → empty.
- **(2) Bounds.** ⟨cmd⟩ `git diff --stat 12cff396..HEAD`: every path it lists falls under ADDENDUM 4's grants, or the §4 `modify` row `useViewManager.ts`, or the record, LEDGER, INBOX or `W5/green`. `dev.sh` is in no commit.
- **(3) Masking.** None. ⟨cmd⟩ `git show f55e59b7 e2efb30a 5a7d3113 16852e03 | grep -E '^\+.*(try \{|catch|\.skip|allowlist|setTimeout|waitForTimeout|fixme)'` → 0 lines. The o16 assertion removals in `e2efb30a` are the retirement of R4 and the switch to the resolved-token pattern that §0az ruled; neither is a silent narrowing.
- **(4) Commit families.** ⟨cmd⟩ `git show --stat f55e59b7` → `.d4`'s readiness publish, the `(pane, resolved)` key and the `:134` flip retirement all land in ONE commit, as §0az requires. No commit carries more than one meaning.
- **(5) E-3.** ⟨cmd⟩ `git diff --stat 12cff396..HEAD -- W5.md registry/adjudicated/ CONFORMANCE-2026-08-03.md scripts/dev/dev.sh` → empty.
- **(6) Mail.** Of the INBOX UNREAD rows, only I-40 (Track C) is outside this wave's scope. **0 UNREAD in scope.**
- **(7) Four-verb.** AUDITED YES · SPECIFIED YES · IMPLEMENTED NO · VERIFIED NO. The line was not moved, which is lawful because the ADDENDUM-4 gate set is not whole-GREEN.
- **(8) Goal criterion.** Not met at the bytes. On `/#/`, g2 measures the companion pane 6.50 px off its shared track start.
- **(9) Published figures.** g2 `Received 6.501441955566406` reproduces exactly ×2. The cold-nav and o16 pass counts also reproduce.
- **(10) Honest-RED.** **W6 g2** ⟨cmd⟩ `npx playwright test e2e/smoke/views/companion-pane-track-start.spec.ts -g "companion panes share one track start" --project=smoke` → **1 failed**, EXIT 1, ×2 (`Expected <= 1 · Received 6.501441955566406`).
  - **No relief.** ADDENDUM 4 (`W5.md:400`) names g2 as a `.d4` gate. It does not route g2 to a later wave, does not call it producer-owned, and does not name it as an honest-RED id.
  - ESC-W5d4-1 is unruled (0 hits in COHESION), so the owner cell names an open escalation, not a relief.
- **Successors.** X-W6's conjunct *X-W5 CLOSED* is **not GREEN**. §0az lawfully lets `.a2`/`.i2` run, but `.j` stays omitted. X-W7 (via X-W6), X-W8 and X-W10 remain lawfully BLOCKED-ON the X-W5 close. X-W5's own opens-after (X-W0 · X-W2 · X-W4) are all CLOSED, so they are GREEN.

### K3.2 Register
| severity | claim | receipt | cure |
|---|---|---|---|
| **HIGH** | The ADDENDUM-4 `.d4` gate *W6 g2 GREEN ×2* is RED and the spec gives it no relief. ESC-W5d4-1 is unruled. | g2 `1 failed` ×2, `Received 6.501441955566406` · ⟨cmd⟩ `grep -c ESC-W5d4-1 COHESION.md` → 0 | COHESION rules ESC-W5d4-1. The ruling either grants the instrument settle arm to X-W6 `.g`/`.a`, or orders a product cure for the pre-start `vj-enter` pose. A resume unit then lands the cure and re-reads g2 ×2. |
| MINOR | The W6 e1 suite is load-sensitive (`.d4` read 20 · 22 · 20 · 21 of 22). | CL7.2 | Rides ESC-W5d4-1. |
