...
- **G-b3** GREEN on the baselined measure. BEFORE `No such file or directory` → ⟨`ls src/css/grammar/*.bbnf | wc -l`⟩ `4`; the BBNF path carries no grammar in TS (act 2's grep). Not yet the ONLY CSS reader in value.js — see R-b-1.
