...
Seat `claude-opus-5-5`, 2026-09-25, Track D. Governing text: W7.md (260 lines, read whole; ADDENDUM (e) at 255–260 under ADDENDUM (b) 216–231 and (c) 232–248) · COHESION §0dq (`:3570`–`:3577`, the file end) · the RESUME 5 Unit plan above (Locks L-4, L-6..L-9, L-11).

**Crash-recovery.** ⟨`git status --porcelain -- src/css bench test/css <record>`⟩ → empty; ⟨`git -C ../bbnf-lang-x-p-w7-typescript status --porcelain`⟩ → empty at `89fae4826` (= origin). **No inherited partial work.** Tooling as `.l`/`.l2`: `node_modules/@mkbabb/bbnf-lang` (0.1.4) moved aside as `.bbnf-lang-0.1.4-aside-l3`, the worktree's `typescript/` linked in its place (built with `npm run build`) for the unit's life, restored at the end.

**1. The profile: what a refusal spends that the retired parser does not** (`.l`'s instrument, compiled out: ⟨`node bench/paired/instrument.mjs`⟩ → `{"plainEqualsShipped":true,…,"shippedInstrumentTokens":[],"instrumentedTokens":6}`; then ⟨`node bench/paired/profile.mjs l3 node,firefox,chromium,webkit parseStylesheet:rej 3`⟩ → `bench/records/2026-09-24-x-p-w7-profile-l3.json`, load 34.7 → 42.3; counts identical on every engine; times rank-only).

| per rejected pass (27,441 sources) | calls | fails | action calls | **discarded values** | `preludeRun` calls | `textGroup` scans |
|---|---|---|---|---|---|---|
| before (`a2a50158`/`eeed0116` bytes) | 518,848 | 160,863 | 185,657 | **54,565** (`ruleBlock` 25,143 + `openBlock` 24,319 = 90.6%) | 59,256 (2.16 / source, value mode: a substring + action each) | 47,788 |

- **(i) Speculative node build on a failing alternative.** `ruleBlock = (preludeRun , (semiTail | blockTail)) - openComment` built the prelude's text value (a `substring` + the `preludeRun` action) and then failed at the missing `;`/`{`; `openBlock = preludeRun , "{" , restText` did it again. A rejected sheet (no block) discards both: 49,462 values per pass.
- **(ii) A re-scan of text already classified.** `openBlock` re-runs `preludeRun` at the very offset `ruleBlock`'s last attempt scanned it (the `*` loop's failed iteration and the trailing `?` start at one offset): every prelude scanned twice, `textGroup` 47,788 scans.
- **(iii) A failure record built dear.** The value grammar's refused-component record (`badTerm`: `Object.freeze({ ...refused(…), span: Object.freeze({…}) })`, three freezes and a spread; the colour grammar's `invalid`, one freeze) is internal — read once by its entry into the published `failure` — yet frozen. ⟨`node bench/paired/profile.mjs l3-val node,firefox parseCssValue:rej,parseStylesheet:large 3`⟩: on rejected `parseCssValue` `badTerm` is Firefox's top action (32,067 calls, 11.67 ms vs node 4.11: ×2.8, the engine's `Object.freeze`).
- **Not the cause (measured, each named by ADDENDUM (e)):** no `Error`/exception anywhere (the emitter answers `FAIL`, a symbol); no error/label/expected arrays (the parser keeps no failure state); no memo fills (memoization off). The published `failure` (`src/css/result.ts`) is **shared by both arms byte for byte** (the retired `grammar.ts` carries the identical constructor): ⟨`node bench/paired/bisect.mjs l3-rej-1 node,firefox rej 11 …`⟩ → Firefox `failureFault` 15 ms of `retiredFault` 42.5 — it weighs both arms alike, so it is not moved (L-9: `result.ts` untouched). The sticky-regex entry on short runs is not it either: probe `sloop` (prelude and `textBody` runs as code-unit loops) read Firefox rej 1.015/1.048 against the product's 1.0/1.022 in the same page (`…-browser-l3-loop1.json`).

**2. Bisection by arms under `_build/`** (new instrument `bench/paired/arm.mjs`: HEAD's `/css` surface with `./generated/grammar` resolved to an emitted-module variant; hand-cut probe variants banked in `bench/records/2026-09-25-x-p-w7-l3-probes/cut{,2,3,4}.mjs`, probes only, never shipped; every arm checked deep-equal to the product on 29,944 sources + 4 large sheets before timing, 0 diffs). Firefox rej `parseStylesheet`, paired median vs retired, clean reps (load 18–34, recorded in each record):

| arm (what it removes) | rej | record |
|---|---|---|
| product (the open bytes) | 1.065 · 1.052 / 1.066 · 1.101 | `…-browser-l3-probe1.json` / `…-probe3.json` |
| `l3defer` — (i): the prelude recognized, its value built at commit | 1.021 · 1.106 | probe1 |
| `l3memo` — (i) + (ii): the retry reads the kept scan | **.944 · 1.0** / 1.013 · 1.057 | probe1 / probe3 |
| `l3gap` / `l3rule` — the `ruleGap` array / `openRule`'s substring | 1.0 · .975 / 1.043 · 1.024 | probe3 (within noise: not admitted) |
| `sempty` / `sall` — empty-loop arrays / all three allocations | 1.053 · 1.014 / .992 · 1.0 (product 1.014 · 1.021) | `…-alloc1.json` (within noise: not admitted) |
| `sbad` — (iii): the refused record plain, built once | rej `parseCssValue` **.708 · .724** (product .929 · .91) · rej `parseStylesheet` .984 · .917 (1.009 · .973) | `…-bad1.json` |

   (i) + (ii) move every engine the same way (`…-probe2.json`, Chromium rej .539 → .488, WebKit .745 → .723; large flat). (iii) is the largest single lever on SpiderMonkey.

**3. The cure, structural, one module every engine (L-9, L-11).**
- **Emitter — bbnf-lang `f0059db14`** (branch `x-p-w7-typescript`, pushed: ⟨`git rev-parse HEAD origin/x-p-w7-typescript`⟩ → both `f0059db1…`). (a) *A refusal builds nothing it discards:* in a value-mode sequence (`,` and `a << b`), an element that references a `text` rule is recognized and its value — the action over its span, a function of the span alone — is built only once every later element has matched (`emit.ts` `deferredText`). (b) *A refusal re-scans nothing it has classified:* `analysis/reentry.ts` `reenteredRules(ast)` — a text run (`bulk.ts`'s notion: an unbounded repetition over class runs) in the LEAD of both sides of a RETRY PAIR (two arms of a choice; a `?`/`*`/`+` element and the element after it; `a - b`'s two sides), from the grammar alone, no input, no engine — keeps its last recognize answer keyed by input and offset; every entry reaching it clears it (a parse starts with none); a run that trips the depth fault stores nothing; the audit build re-runs every kept answer and checks it. On value.js's grammar it names exactly `preludeRun`. (c) The emitter identity (`gen.ts` `EMITTER_SHA256`) now hashes `analysis/bulk.ts` and `analysis/reentry.ts` (adjacent: `.l2` left `bulk.ts` out, so an analysis edit did not move the drift header). README "Actions" states the contract (actions of one sequence may run in any order; they are pure). Tests `test/reentry.test.ts` (7): the classification on a sheet grammar (`["pre"]`, `grp` covered) and on value.js's (`["preludeRun"]`); the prelude's action runs 0 times on a refused sheet and 2 on `a;b{c}`; the positional values of 9 sheets; `grp` scanned twice (not four times) and `pre` called twice, scanned once, on `a (b) (c) {x`; a fresh parse keeps nothing; an audit build over every sheet of ≤ 5 units from `a ␠ ( ) { } ;` (19,608 parses) checks every kept answer — `violations 0`.
- **Re-emission — value.js `31121b23`** through ⟨`node scripts/gen-grammar.mjs`⟩ on the linked emitter (sha256 header `0b33024a…`). ⟨`node bench/paired/profile.mjs l3-after node,firefox parseStylesheet:rej 3`⟩ → per rejected pass: calls 518,848 → **475,739**, action calls 185,657 → **136,195**, discarded values 54,565 → **5,103** (the rest are map values of colour alternatives that fail late, `rgbFn` 906 · `call` 7,777 on rej `parseCssValue`: map values cannot wait, their parents read them), `textGroup` scans 47,788 → **26,304**.
- **Actions — value.js `f7b087a4`** (`src/css/bbnf/value.ts`, `color.ts`; the `.k2` grant): `Refused` and the colour grammar's `invalid` are the grammars' FAILURE RECORDS — internal, read once by their entry (`index.ts` `refusal`/`colorResult`) into the published frozen `failure`, never published — so they are built once, plain (no freeze, no spread); the two shared refusal constants are frozen once at module load. Published results stay built frozen (`test/css/bbnf-frozen.test.ts` unchanged, GREEN). Test `test/css/bbnf-refusal.test.ts` (11): three refusals name exactly their component (`1px @ 2px` → 4..5 …) and are deeply frozen; seven refusals across every entry deeply frozen; the record itself `{kind,code,expected,span}` unfrozen and never the published diagnostic.
- **Actions — value.js `1e04369b`** (`src/css/bbnf/stylesheet.ts`): the stylesheet readers build each list in ONE array, in one pass (`list`/`column`/`nonEmpty` replace `[first, ...rest.map(…)]` and `.filter(Boolean)` copies in `commaItems`, `semiItems`, `spaceItems`, `timelineArgs` and the rule list's `blocks`, which ran on every parse, every refused sheet included). Admitted on its own paired numbers, same page as the product: Firefox large `parseStylesheet` **5/5 reps below the product** (`…-browser-l3-list4.json`: .965 · 1.009 · .985 · .991 · .991 vs 1.054 · 1.048 · 1.052 · 1.066 · 1.0; list2 .943 · .946 · .936 vs .957 · .918 · 1.0), Chromium/WebKit rej and large flat (`…-list5.json`: .48/.466 vs .469/.488 · .318/.329 vs .332/.333 · .694/.694 vs .686/.714 · .6/.6 vs .6/.6), node flat (`2026-09-23-x-p-w7-l3-list-node.json`: rej .527 vs .523 · large .366 vs .385 · whole .548 vs .556). Pinned in `test/css/bbnf-refusal.test.ts` (2 more: the three list readers' answers, and a rule list's blocks on an accepted and a refused sheet).
- **Measured, not admitted:** `ruleGap`'s value array, `openRule`'s substring, empty-loop arrays (`…-l3-probe3.json`, `…-alloc1.json`: within noise, and removing a positional value an action might read is the grammar's call, not the emitter's); keeping a token rule's answer (`ws`, `ws1`, `number`, `percentage`, `angle`, `mathFn`, `openComment` — the first `reentry` classification): `…-cand1.json` (tokens kept) Firefox rej .955 · .991, large .98 · 1.0 (product 1.023 · 1.053 / .973 · .978); `…-cand2.json` (text runs only) rej 1.039 · .957, large 1.026 · 1.024 (product 1.092 · 1.04 / 1.017 · .991) — not separable at load 15–22; the analysis keeps text runs by construction (a kept answer costs a store on every call, a token rule re-reads one first-unit-guarded token).

**4. Resume seat (2026-09-25, `claude-opus-5-5`; the predecessor seat was killed after its fifth final read, acts 1–3 above are its text, verified here).** ⟨`git status --porcelain -- src/css bench test/css <record>`⟩ → only this record (the 37-line receipt above, inherited, kept) and `bench/paired/arm.mjs` + the `l3` records untracked (inherited, committed below); bbnf-lang ⟨`git -C ../bbnf-lang-x-p-w7-typescript status --porcelain | wc -l`⟩ → `0`, ⟨`git rev-parse HEAD origin/x-p-w7-typescript`⟩ → both `f0059db14…`. The commits the receipt names exist: ⟨`git log --oneline | grep -E "^(31121b23|f7b087a4|1e04369b)"`⟩ → all three.

**5. Gates on the final bytes** (bbnf-lang `f0059db14`; value.js src at `1e04369b`, ⟨`node bench/paired/build.mjs`⟩ → `srcDirty ""`, `bankedManifestOk 79/79`).
- **L-G2** ⟨`node bench/paired/equiv.mjs product <scratch>`⟩ ×2 → `compared 1376531/1376531 rows · mismatches 92 (ASCII-only sources 1 · CP-CASE 4)` both = 88 F-b-4 + 4 CP-CASE, the RESUME 4 Check 3 reading exactly. **GREEN ×2.**
...
- **E-4** ⟨`node scripts/gen-grammar.mjs --check`⟩ ×2 (emitter linked) → `grammar.js is current (sha256 0b33024a…)` both; ⟨`git status --porcelain src/css/bbnf/generated | wc -l`⟩ → `0`. **GREEN.**
- **E-2** ⟨`node bench/paired/audit.mjs`⟩ → `rules 160 · checks 200137551 · violations 0 · modeCalls 4791680 · modeDiffs 0` (the audit build re-checks every kept `preludeRun` answer). **GREEN.**
- **E-1/E-3/E-5** ⟨`npx vitest run`⟩ ×2 in the worktree → `Test Files 21 passed (21) · Tests 299 passed (299)` both (was 20/292: + `reentry.test.ts` 7); ⟨`npx tsc --noEmit -p .`⟩ → `0`. **GREEN.**
- **E-6** ⟨`npx esbuild --minify src/css/bbnf/generated/grammar.js | wc -c`⟩ → `92127` (was 89,097); `| gzip | wc -c` → `12902` (was 12,617); ceilings 125,646 / 14,517. **GREEN.**
- **`.cp` pins** ⟨`npx vitest run test/css/custom-property-case.test.ts`⟩ ×2 → `Tests 4 passed (4)` both. **GREEN ×2.**
- ⟨`npm run -s test:css-equivalence`⟩ → `Tests 19 passed (19)`, every `MIRROR-DEFECTS` line `0`. ⟨`npx vitest run test/css`⟩ → `Test Files 7 passed (7) · Tests 75 passed (75)` (was 5/58 at the RESUME 4 close; + `bbnf-refusal.test.ts` 13 and the `.cp` pins). **GREEN.**
- ⟨`npx vue-tsc -p tsconfig.{lib,demo,test}.json --noEmit`⟩ → `0 · 0 · 0`. **GREEN.**
- ⟨`npx vitest run`⟩ → `Tests 2 failed | 958 passed (960)`: `spectrum-luma` C-5 BORN-RED and `reka-binding-idiom` NG-6, the foreign pair. **GREEN** (0 CSS failures).
- **L-6** ⟨`git diff HEAD --stat -- package.json package-lock.json`⟩ → empty. **Held.**

**6. L-G1 on the final bytes, load recorded** (records committed at `1a9501c8`; Firefox 150.0.2 · WebKit 26.4 · Chromium 148.0.7778.96; set-aside cells counted, never claimed). The 1-minute load never fell below ~15 this sitting (sibling tracks' Playwright fleets); every read below started at the lowest load the host reached.
- **node** ⟨`node --expose-gc bench/paired/isolated.mjs l3-final-node-r{3,4} product 3 11 whole,acc,rej,large`⟩ (`r3` 40.2 → 35.9 · `r4` 20.2 → 26.8, both at `1e04369b` src bytes): `whole-7/7`, `accepted-7/7`, `rejected-7/7` **GREEN** both; worst per-cell medians r4: whole color .681 · scalar .587 · value .406 · values .413 · keyframe .701 · timing .611 · stylesheet .579 (ceilings .90/.81/.57/.58/.78/.87/.85: **inside**); rej stylesheet .574; large .396. **GREEN ×2.**
- **browsers, read 1** ⟨`node bench/paired/browser.mjs l3-close-r6 firefox,webkit,chromium 2 whole,acc,rej,large 11 product`⟩ (18.9 → 30.3): **65/66 cells < 1 in both reps.** Chromium rej / large `parseStylesheet` .469 · .479 / .336 · .322; WebKit .683 · .694 / .56 · .60; **Firefox rej .921 · .928**; **Firefox large 1.05 · 1.047 (RED)**.
- **browsers, read 2** ⟨same, tag `l3-close-r7`⟩ (30.3 → 20.2): **65/66.** Chromium .481 · .473 / .33 · .334; WebKit .709 · .708 / .577 · .593; **Firefox rej .936 · .951**; **Firefox large .95 · 1.033 (+1 SA 1.083) (RED)**.
- **Firefox re-reads** ⟨`node bench/paired/browser.mjs l3-close-ff-r{8,9} firefox 2 whole,acc,rej,large 11 product`⟩ (15.1 → 21.3 · 21.3 → 24.4): r8 21/22 (rej .915 · .984; **large .964 · 1.049**); r9 20/22 (rej 1.0 clean, its other rep set aside ×4 at .882–1.045; **large 1.018 · 1.074**).
- **Earlier final-bytes reads by the predecessor** (`…-browser-l3-final-r4/r5.json`, at `1e04369b`; 27.6 → 41.0 · 31.9 → 124.5): 65/66 each; Firefox rej .965 · .959 / .958 · .981; Firefox large .951 · 1.05 / 1.059 · 1.0.
- **Firefox rej `parseStylesheet` (the cell §0dq ruled `.l3` onto): x1.038 at open → 10 of 11 clean reps in the six full reads on the final bytes below 1 (.915–.984), one at 1.0 (r9, its sibling rep set aside); both reps below 1 in r6, r7, r8, final-r4, final-r5 (5 of 6 reads); the probe reads' product arm adds .972 · .936 · .962 · .989 and .942 · .958 · 1.025 · .952.** Every node / Chromium / WebKit cell GREEN in every read.
- **Firefox large `parseStylesheet`: RED.** Clean reps on the final bytes: .951 · 1.05 · 1.059 · 1.0 · 1.05 · 1.047 · .95 · 1.033 · .964 · 1.049 · 1.018 · 1.074 — median ≈ 1.04; not one read with both reps below 1.

**7. Firefox large, probed after the cure (not a refusal cost).** Per-rule profile (`…-profile-l3-val.json`, `parseStylesheet:large`, counts identical on every engine): per pass 84,363 calls, 18,171 fails, 32,018 action calls, **57 discarded values** (the refusal work is gone from this cell); Firefox ranks `ruleList`/`ruleBlock` (14.3 / 14.0 ms inclusive of 41 profiled) · `blockTail` 9.7 · `blockBody` (recognize) 9.0 · the value lists 6.7–7.7 · glue (the stylesheet layer, `src/css/stylesheet.ts`/`rules.ts`) 7.3 against node's 3.5 — the accepted sheet's own scans, value parses and assembly, each ×1.3–2.1 of V8's share. Two further structural candidates, each deep-equal to the product (⟨`node bench/paired/equiv.mjs l3athead`⟩ → `mismatches 92`, the product's rows), both flat at noise, not admitted:
  - `l3athead` (cut5: `atPrelude`'s choice answers FAIL at its head on a non-`@` code unit; today its 8 alternatives each re-test their leaf guard — 1,006 `atScrollTimeline` calls per pass, all failing): ⟨`node bench/paired/browser.mjs l3-athead1 firefox 4 large,rej 11 product,l3athead parseStylesheet`⟩ (26.9 → 29.0) large product .949 · 1.014 · 1.028 · .961 vs arm .942 · 1.062 · 1.0 · 1.065.
  - `l3noenv` (probe: `success`'s envelope left unfrozen — 13,250 `parseCssValue` envelopes per pass the stylesheet layer reads once): ⟨`… l3-noenv1 … product,l3noenv …`⟩ (22.9 → 16.9) large product .962 · .955 · 1.041 · .921 vs arm .987 · .949 · .946 · .971; rej product .942 · .958 · 1.025 · .952 vs arm .92 · .935 · .902 · .934. Within noise on large; `result.ts` stays (L-9: shared by both arms).

**8. Tooling restored (L-4).** ⟨`rm node_modules/@mkbabb/bbnf-lang && mv node_modules/@mkbabb/.bbnf-lang-0.1.4-aside-l3 node_modules/@mkbabb/bbnf-lang`⟩; ⟨`grep '"version"' node_modules/@mkbabb/bbnf-lang/package.json`⟩ → `0.1.4`. (On 0.1.4 `gen-grammar.mjs --check` cannot run — 0.1.4 carries no `gen`; E-4 was read ×2 on the linked emitter above, as at the RESUME 4 close.)

**Commits.** bbnf-lang `f0059db14` (emitter: deferred text values + re-entered text runs kept; pushed, branch only) · value.js `31121b23` (its re-emission through `gen-grammar.mjs`) · `f7b087a4` (action: the refused record plain, built once; `test/css/bbnf-refusal.test.ts`) · `1e04369b` (action: the stylesheet readers' lists in one pass; pinned in the same test) · `1a9501c8` (records) · this receipt.

**Gate readings BEFORE → AFTER.**

| Gate | BEFORE (RESUME 5 Baseline) | AFTER (final bytes) | Verdict |
|---|---|---|---|
| L-G1 Firefox rej `parseStylesheet` | x1.038 (born-RED) | .915–.984 in 10 of 11 clean reps; 5 of 6 reads both reps < 1 | **GREEN** (one rep at 1.0, r9) |
| L-G1 Firefox large `parseStylesheet` | VOID (load 37) | .95–1.074, median ≈ 1.04, 12 clean reps; 0 of 6 reads both reps < 1 | **RED** → ESC-W7l3-1 |
| L-G1 Firefox, the other 20 cells | GREEN (`.l2`/RESUME 4 close) | < 1 in every read | GREEN ×2 |
| L-G1 Chromium · WebKit, 22 cells | GREEN | < 1 in r6 and r7 (rej .469–.481 / .683–.709; large .322–.336 / .56–.60) | GREEN ×2 |
| L-G1 node, 22 cells + whole-7/7 ceilings | GREEN | r3 · r4 all three 7/7 GREEN, inside ceilings | GREEN ×2 |
| L-G2 V-1 | 92 = 88 + 4 | 92 = 88 + 4, ×2 | GREEN ×2 |
| L-G3 | 0 | 0 | GREEN |
| `.cp` pins | 4/4 | 4/4 ×2 | GREEN ×2 |
| css-equivalence | 19/19, MIRROR-DEFECTS 0 | 19/19, MIRROR-DEFECTS 0 | GREEN |
| vitest `test/css` | 5 / 58 (RESUME 4 close) | 7 / 75 | GREEN |
| `npm test` | foreign pair only | 958/960, foreign pair only | GREEN |
| vue-tsc lib/demo/test | 0/0/0 | 0/0/0 | GREEN |
| E-2 audit | 0 violations · 0 modeDiffs | 0 · 0 (200,137,551 checks) | GREEN |
| E-4 `--check` | current | current ×2 (`0b33024a…`) | GREEN |
| E-6 size | 89,097 / 12,617 | 92,127 / 12,902 (≤ 125,646 / 14,517) | GREEN |
| bbnf-lang suite + tsc | 20/292 ×2 · 0 | 21/299 ×2 · 0 | GREEN ×2 |
| K2-b deepFreeze | 0 | 0 | GREEN |

**Adjacent edits (§0bt).** bbnf-lang `typescript/src/gen.ts` `EMITTER_SHA256` now hashes `analysis/bulk.ts` and `analysis/reentry.ts` (inside the writable set; named because `.l2` left `bulk.ts` out). None in value.js outside the set.

**Residuals.** Firefox large `parseStylesheet` (ESC-W7l3-1 below). `gen-grammar.mjs --check` in CI waits for X.P.W7P's 0.2.0 pin (§0cp). The probe cuts under `bench/records/2026-09-25-x-p-w7-l3-probes/` are probes only, never shipped.

**ESCALATION ESC-W7l3-1 (to the owner, via the orchestrator; ADDENDUM (b): "a cause it cannot cure … returns ESCALATED with the per-rule profile").** `.l3`'s charge is cured: the refusal cost is found (speculative prelude values, the prelude re-scan, and the refused record's freezes), cured once for every engine (emitter `f0059db14` + actions `f7b087a4`, `1e04369b`), and Firefox rej `parseStylesheet` moved from x1.038 to .915–.984. The cell that stays RED is **Firefox large `parseStylesheet`** (median ≈ 1.04 over 12 clean reps; it read .932–.995 at the RESUME 4 close and 1.033–1.127 at `.l2`: it sits within about ±6% of the retired parser). It is **not a refusal cost** — 57 discarded values per pass. The profile puts SpiderMonkey's extra share in the accepted sheet's own work: the rule-list scans, the value lists, and the stylesheet layer's assembly (glue 7.3 vs 3.5 ms on V8). The profile is `bench/records/2026-09-24-x-p-w7-profile-l3-val.json`, the phase split is `…-bisect-l3-large-1.json`, and the reads are listed above. Six levers were measured and none separates from noise on this cell with the others held: the four refused at `.g`, (b)'s uniform scan (shipped), the list readers (shipped, 5/5 reps below the product in `list4`), the atPrelude head guard, and the unfrozen envelope. The owner decides: (i) a further unit for SpiderMonkey's accepted-sheet cost, scoped to the stylesheet layer (`src/css/stylesheet.ts`, `rules.ts`) and the value lists, with a Firefox-only profiler; or (ii) read Firefox large at a stated noise band. Relief is not taken here (§0di, §0dq).

## RESUME 5 Close (2026-09-25, close seat `claude-opus-5-5`, Track D; verify-only, cures nothing)

**Open.** Spec read whole (W7.md, 260 lines; ADDENDUM (e) at `:255` binds). From this record: header, `## RESUME 5 — Open` through `## RESUME 5 Unit plan`, and the `.l3` receipt (the last section). Every earlier section stands (E-3). **Crash-recovery:** ⟨`git status --porcelain -- src bench test scripts/gen-grammar.mjs docs/tranches/X/execution/D docs/tranches/X/execution/LEDGER.md package.json package-lock.json`⟩ → empty; ⟨`git -C ../bbnf-lang-x-p-w7-typescript status --porcelain | wc -l`⟩ → `0`; ⟨`git rev-parse HEAD origin/x-p-w7-typescript`⟩ → `f0059db14…` both. parse-that master's dirty `rust/**`, `README.md`, `.cargo/config.toml` rows are outside this wave's sets (untouched). **No inherited partial work.**

### Act 1: commit roster, and whether `.l3` stayed in its writable set
⟨`git show --stat --format= <c>`⟩ per commit:

| Commit | Paths touched | In set? |
|---|---|---|
| bbnf-lang `f0059db14` (deferred text values + re-entered text runs kept; = origin) | `typescript/{README.md, src/analysis/{index,reentry}.ts, src/emit.ts, src/gen.ts, test/reentry.test.ts}` | yes (`typescript/**`; `gen.ts` the named §0bt adjacent edit, inside the set) |
| value.js `31121b23` (re-emission) | `src/css/bbnf/generated/grammar.{js,d.ts}` | yes (via `gen-grammar.mjs`; `--check` current, below) |
| value.js `f7b087a4` (refused record plain) | `src/css/bbnf/{color,value}.ts`, `test/css/bbnf-refusal.test.ts` | yes (`.k2` grant + `test/css/**`) |
| value.js `1e04369b` (readers' lists in one pass) | `src/css/bbnf/stylesheet.ts`, `test/css/bbnf-refusal.test.ts` | yes |
| value.js `1a9501c8` (records) | 45 paths; ⟨`git show --name-only --format= 1a9501c8 \| grep -vc '^bench/'`⟩ → `0` | yes (`bench/**`) |
| value.js `e393a7a8` (receipt) | this record | yes |

**Landed wrong: none.** No `package.json`/lockfile, `ci.yml`, `src/css/result.ts`, bbnf-lang master, PR #1 or `scripts/dev/dev.sh` path in any commit. ⟨`git log --oneline e393a7a8..HEAD`⟩ at open → empty.

### Act 2: the gates re-read by this seat (RESUME rule: the gates `.l3` turns plus stay-GREEN and `.cp`'s ADDENDUM (d) gates; older unit gates cited from `## Check 3 of the RESUME 4 Close`)

**Correctness, gates and size** (final bytes: bbnf-lang `f0059db14`, value.js src at `1e04369b`, HEAD `e393a7a8`).
- ⟨`node bench/paired/build.mjs`⟩ → `valuejsHead e393a7a8…`, `srcDirty ""`, `bankedManifestOk 79/79`, `retiredAt 2155142b…`.
- **L-G2 / V-1** ⟨`node bench/paired/equiv.mjs product <scratch>`⟩ ×2 (load 26.69 → 30.49) → `compared 1376531/1376531 rows · mismatches 92 (ASCII-only sources 1 · CP-CASE 4)` both = 88 F-b-4 + 4 CP-CASE. **GREEN ×2.**
- **L-G3** ⟨`grep -rln "instrument\|__prof\|PC\[\|NOW()" src/css/bbnf/generated | wc -l`⟩ → `0`; ⟨`node bench/paired/instrument.mjs`⟩ (emitter linked) → `"plainEqualsShipped":true`, `"shippedInstrumentTokens":[]`. **GREEN.**
- **K2-b** ⟨`grep -rn deepFreeze src | wc -l`⟩ → `0`; **V-8** ⟨`grep -rnE 'GRAMMAR_MODULES|new ParserState|reset\(\)' src/css/bbnf --exclude-dir=generated | wc -l`⟩ → `0`. **GREEN.**
- **E-4** ⟨`node scripts/gen-grammar.mjs --check`⟩ ×2, `node_modules/@mkbabb/bbnf-lang` linked to the worktree (`npm run -s build` exit 0; worktree `status --porcelain | wc -l` → `0` after) → `grammar.js is current (sha256 0b33024a890599da…)` both; ⟨`git status --porcelain src/css/bbnf/generated | wc -l`⟩ → `0`. Link removed, 0.1.4 restored (⟨`grep '"version"' node_modules/@mkbabb/bbnf-lang/package.json`⟩ → `0.1.4`). **GREEN ×2.**
- **E-2** ⟨`node bench/paired/audit.mjs`⟩ (linked emitter; load 42.65 → 42.48) → `rules 160 · sources 29944 · sheets 4 · checks 200137551 · violations 0 · modeCalls 4791680 · modeDiffs 0`. **GREEN.**
...
- **E-6** ⟨`npx esbuild --minify src/css/bbnf/generated/grammar.js | wc -c`⟩ → `92127`; `| gzip | wc -c` → `12902` (ceilings 125,646 / 14,517). **GREEN.**
- **`.cp` pins (ADDENDUM (d))** ⟨`npx vitest run test/css/custom-property-case.test.ts`⟩ ×2 → `Tests 4 passed (4)` both. **GREEN ×2.**
- ⟨`npm run -s test:css-equivalence`⟩ → `Tests 19 passed (19)`; ⟨`… | grep -oE 'MIRROR-DEFECTS [0-9]+' | sort | uniq -c`⟩ → `28 MIRROR-DEFECTS 0` (no non-zero line). ⟨`npx vitest run test/css`⟩ → `Test Files 7 passed (7) · Tests 75 passed (75)`. **GREEN.**
- ⟨`npx vue-tsc -p tsconfig.{lib,demo,test}.json --noEmit | grep -c 'error TS'`⟩ → `0 · 0 · 0`. **GREEN.**
- ⟨`npx vitest run`⟩ → `Test Files 2 failed | 70 passed (72) · Tests 2 failed | 958 passed (960)`: `test/spectrum-luma.test.ts` C-5 BORN-RED and `demo/test/shell/reka-binding-idiom.test.ts` NG-6, the foreign pair. **GREEN** (0 CSS failures).
- **L-6** ⟨`git diff HEAD --stat -- package.json package-lock.json | wc -l`⟩ → `0`. **Held.**

**Timing** (load recorded in every log; the host carried the other three tracks' fleets throughout, 1-minute load 19.6–43.2; no quiet window reached). Records moved to scratchpad `w7l3close/` (`l3close-r{1,2}`, `l3close-ff-r3`, `l3close-node-r{1,2}` + logs); ⟨`git status --porcelain bench | wc -l`⟩ → `0`. Firefox 150.0.2 · WebKit 26.4 · Chromium 148.0.7778.96 (the `.l2`/`.l3` pins). Set-aside cells are counted and never claimed.
- **browsers, read 1** ⟨`node bench/paired/browser.mjs l3close-r1 firefox,webkit,chromium 2 whole,acc,rej,large 11 product`⟩ (load 42.58 → 32.06; 8 set-aside reps, every row with ≥ 1 clean rep): ⟨`grep -E '^product' | grep -c '<1 '`⟩ → **66/66**. Firefox **rej parseStylesheet .896 · .939**, **large parseStylesheet .938 · .967**; WebKit rej .709 · .691, large .60 · .571; Chromium rej .477 · .486, large .338 · .341.
- **browsers, read 2** ⟨same, tag `l3close-r2`⟩ (load 34.53 → 19.58, peak 33 in between; **57 set-aside reps**, every row with ≥ 1 clean rep, several Firefox rows with exactly one): **65/66**. Firefox **rej .933 · .94 (+1 SA .956)**; **large .975 · 1.007 → RED**; WebKit rej .702 · .692, large .556 · .577; Chromium rej .458 · .473, large .327 · .316.
- **Firefox re-read** ⟨`node bench/paired/browser.mjs l3close-ff-r3 firefox 4 large,rej 11 product parseStylesheet`⟩ (load 27.98 → 37.80): rej **.989 · .959 · .966 · .952** clean (2 SA .944, .906); large **1.04 · 1.176 · 1.013 · 1.013** clean (4 SA 1.028 · 1.142 · .997 · 1.075) → **RED**.
- **node, read 1** ⟨`node --expose-gc bench/paired/isolated.mjs l3close-node-r1 product 3 11 whole,acc,rej,large`⟩ (load 19.58 → 30.46; 21 set-aside reps): `whole-7/7`, `accepted-7/7`, `rejected-7/7` all `GREEN`; whole medians color .656 · scalar .569 · value .378 · values .385 · keyframe .703 · timing .64 · stylesheet .56 (ceilings .90/.81/.57/.58/.78/.87/.85: **inside**); rej stylesheet .566; large .393.
- **node, read 2** ⟨same, tag `l3close-node-r2`⟩ (load 30.46 → 28.76; 42 set-aside reps): all three 7/7 `GREEN`; whole .677 · .608 · .395 · .405 · .682 · .628 · .591 (inside); acc keyframe .626; rej stylesheet .546; large .605 (clean .366 · .844).
- (The instrument's `O-2` line reads `DOES NOT HOLD` in both node reads: it is `.o`'s stock-reproduction check, meaningful only on the stock product; recorded, not a W7 regression.)
- **Reading.** `.l3`'s charge holds on this seat's reads: **Firefox rej parseStylesheet** is below the retired parser in all 8 clean reps (.896–.989). **Firefox large parseStylesheet** stays at noise distance and reads RED: .938 · .967 (r1), .975 · 1.007 (r2), 1.013–1.176 (ff-r3); 8 clean reps, median ≈ 1.01. Every node, Chromium and WebKit cell is below 1 in both reads.

### Acts 3 and 4: verification artefacts and E13
- The spec names no Verification Artefacts clause beyond its gates. This seat's paired records are in scratchpad `w7l3close/`; `.l3`'s banked records (`1a9501c8`) stand as the records of record.
- E13 ⟨`find <p> -maxdepth 1 -type f -newer INBOX.md`⟩ over value.js `V/` + `V/coordination/`, glass `BK/coordination/` + `BL/` (⟨`ls -td glass-ui/docs/tranches/B*/ | head -3`⟩ → `BL BK BJ`), keyframes.js `V/coordination/`, atlas `P/coordination/` → `0 · 0 · 0 · 1 · 0 · 0`. The one file is `glass-ui/docs/tranches/BL/FORMATION-PROGRESS.md` (11:10): glass's own formation cursor, not mail and not addressed to Track D or W7's scope. ⟨`grep -cE '\| *UNREAD *\|' INBOX.md`⟩ → `0`. **0 UNREAD in scope.**

### Gate table, BEFORE → AFTER (BEFORE = RESUME 5 Baseline; AFTER = this seat's reads)

| Gate | BEFORE | AFTER (this seat) | Verdict |
|---|---|---|---|
| L-G1 Firefox rej parseStylesheet | x1.038 (born-RED) | .896 · .939 · .933 · .94 · .989 · .959 · .966 · .952 (8/8 clean reps < 1) | **GREEN ×2** |
| L-G1 Firefox large parseStylesheet | VOID (load 37) | .938 · .967 (r1) · .975 · 1.007 (r2) · 1.04 · 1.176 · 1.013 · 1.013 (ff-r3) | **RED** (noise distance) → ESC-W7l3-1 |
| L-G1 Firefox, other 20 rows | GREEN | < 1 in both reads (r2 on thin clean samples, load-reported) | **GREEN ×2** |
| L-G1 WebKit 22 rows | GREEN | 22/22 < 1, both reads (rej .69–.71, large .556–.60) | **GREEN ×2** |
| L-G1 Chromium 22 rows | GREEN | 22/22 < 1, both reads (rej .458–.486, large .316–.341) | **GREEN ×2** |
| L-G1 node whole/acc/rej/large | GREEN | 7/7 · 7/7 · 7/7, both reads; large .393 / .605 | **GREEN ×2** |
| whole-7/7 inside ceilings | GREEN | keyframe .703 / .682 (.78) · stylesheet .56 / .591 (.85) · value .378 / .395 (.57) | **GREEN ×2** |
| L-G2 V-1 | 92 = 88 + 4 | 92 = 88 F-b-4 + 4 CP-CASE ×2 | **GREEN ×2** |
| L-G3 | 0 | 0; `plainEqualsShipped true` | **GREEN** |
| `.cp` pins (ADDENDUM (d)) | 4/4 | 4/4 ×2 | **GREEN ×2** |
| stay-GREEN: css-equiv · test/css · npm test · vue-tsc · `--check` · K2-b · V-8 | GREEN | 19/19, 28× MIRROR-DEFECTS 0 · 7/75 · 958/960 (foreign pair) · 0/0/0 · current ×2 (`0b33024a…`, linked) · 0 · 0 | **GREEN** |
| E-1/E-3/E-5 · E-2 · E-6 | GREEN | 21/299 ×2, tsc 0 · 200,137,551 checks, 0 violations, 0 modeDiffs · 92,127 / 12,902 B | **GREEN** |
| P-6 · E-8 · V-9 + CI step · Z-1 · Z-3 publish half | relieved | relieved by citation to X.P.W7P (§0cp 1), GATE-KEYED on the owner's OTP | **relieved → X.P.W7P** |

### Residuals (named owners)
- **R-l3-1: Firefox (SpiderMonkey) large parseStylesheet** stays at noise distance above the retired parser: 8 clean reps here, .938–1.176, median ≈ 1.01; the receipt's 12 clean reps median ≈ 1.04. It is not a refusal cost (57 discarded values per pass). The per-rule profile (`2026-09-24-x-p-w7-profile-l3-val.json`, `bisect-l3-large-1.json`) places the gap in the accepted sheet's own scans, value lists and the stylesheet-layer glue. Owner: **the owner, via ESC-W7l3-1**.
- **The release chain:** P-6, E-8, V-9 and its CI step (`gen-grammar.mjs --check` on the installed pin), Z-1, Z-3's publish half, and the bbnf-lang PR #1 merge. Owner: **X.P.W7P**, GATE-KEYED on the owner's npm one-time password.
- **Load.** No quiet window was reached this sitting (19.6–43.2). Read 2 has 57 set-aside browser reps and node read 2 has 42; the verdicts rest on clean reps only. Owner: whichever seat executes ESC-W7l3-1's ruling, which re-reads Firefox large on a quiet host.
- **Foreign test rows** (not W7's): `spectrum-luma` C-5 BORN-RED, `reka-binding-idiom` NG-6. Owners: their Track A/B units.

### Escalations
- **ESC-W7l3-1** (the `.l3` receipt) is carried open, and this seat's reads confirm it: the refusal cost is cured on every engine, and Firefox large parseStylesheet stays RED at noise distance. The owner chooses: (i) a further unit on SpiderMonkey's accepted-sheet cost, scoped to `src/css/stylesheet.ts`, `rules.ts` and the value lists; or (ii) a stated noise band for this cell. ⟨`grep -n ESC-W7l3-1 docs/tranches/X/COHESION.md | wc -l`⟩ → `0` (unruled). No relief is taken here.
- **ESC-W7p-1 / e-1 / v-1 / z-1** stay with X.P.W7P.
- This seat raises no new escalation. **Adjacent edits: none. Out-of-set writes: none.** `scripts/dev/dev.sh` untouched.

### Four-verb line
W7.md has no §State clause and designates no seat to stamp a verb. ADDENDA (b), (c) and (e) bind L-G1 on every engine ×2, and Firefox large parseStylesheet reads RED in 2 of this seat's 3 Firefox reads. X.P.W7 is therefore **not IMPLEMENTED**. It stays **PARTIAL**, and VERIFIED is not stamped. It moves to IMPLEMENTED once ESC-W7l3-1 is ruled and executed.

**Verdict: PARTIAL.** `.l3` found the refusal cost and cured it in set: speculative prelude values, the prelude re-scan, and the refused record's freezes. Firefox rej parseStylesheet went from x1.038 to .896–.989. Node, Chromium and WebKit are below the retired parser on every cell, twice. Firefox is below it on 21 of 22 rows. Firefox large parseStylesheet stays RED at noise distance, and ESC-W7l3-1 holds it for the owner.

**Commits (this seat):** this RESUME 5 Close, plus the LEDGER status cell.

## Check 1 of the RESUME 5 Close (2026-09-25, L-20 fresh adversarial pass 1, `claude-opus-5-5`, Track D; verify-only, cures nothing)

**Open.** Spec read whole (W7.md, 260 lines; ADDENDA (b), (c), (e) bind L-G1 on every engine ×2, "no relief, no engine split"). From this record: header, `## RESUME 5 — Open` through `## RESUME 5 Unit plan`, `## RESUME 5 Close` (the last section). Crash-recovery: ⟨`git status --porcelain -- docs/tranches/X/execution/D/X-P-W7.md docs/tranches/X/execution/LEDGER.md src bench test`⟩ → empty; bbnf-lang worktree ⟨`status --porcelain | wc -l`⟩ → `0`, ⟨`git rev-parse HEAD origin/x-p-w7-typescript`⟩ → `f0059db14…` both. Host load 31.9–35.2 throughout (the other tracks' fleets); recorded, not a quiet window.

### Axes
- **(1) Claimed GREENs reproduce** (this seat's runs, HEAD `20b3e355`, src at `1e04369b`):
  - ⟨`node bench/paired/build.mjs`⟩ → `srcDirty ""`, `bankedManifestOk 79/79`, `retiredAt 2155142b…`.
  - **L-G2** ⟨`node bench/paired/equiv.mjs product <scratch>`⟩ ×2 → `compared 1376531/1376531 rows · mismatches 92 (ASCII-only sources 1 · CP-CASE 4)` both. **GREEN ×2.**
  - **L-G3** ⟨`grep -rln "instrument\|__prof\|PC\[\|NOW()" src/css/bbnf/generated | wc -l`⟩ → `0`. **K2-b** ⟨`grep -rn deepFreeze src | wc -l`⟩ → `0`. **V-8** ⟨`grep -rnE 'GRAMMAR_MODULES|new ParserState|reset\(\)' src/css/bbnf --exclude-dir=generated | wc -l`⟩ → `0`. **E-6** ⟨`npx esbuild --minify …grammar.js | wc -c`⟩ → `92127`, `| gzip | wc -c` → `12902`. **GREEN.**
  - ⟨`npx vitest run test/css`⟩ → `7 passed (7) · 75 passed (75)`; `.cp` pins ⟨`npx vitest run test/css/custom-property-case.test.ts`⟩ → `4 passed (4)`. ⟨`npm run -s test:css-equivalence`⟩ → `Tests 19 passed (19)`, ⟨`… | grep -oE 'MIRROR-DEFECTS [0-9]+' | sort | uniq -c`⟩ → `26 MIRROR-DEFECTS 0` (no non-zero line; the close printed 28 lines — a line-count difference of the reporter, not a defect count). ⟨`npx vue-tsc -p tsconfig.{lib,demo,test}.json --noEmit | grep -c 'error TS'`⟩ → `0 · 0 · 0`. bbnf-lang ⟨`npx vitest run`⟩ → `21 passed (21) · 299 passed (299)`, ⟨`npx tsc --noEmit -p . | grep -c 'error TS'`⟩ → `0`. **GREEN.**
  - **Immutability contract after `.l3` un-froze the refusal records** (the cure's risk: an unfrozen `Refused` leaking into a published result): scratch probe ⟨`npx vite-node frozen-probe.ts`⟩ walks every accepted result of the 7 entries over the 29,944 sources plus 3 real sheets → `{"ok":40109,"walked":255039,"unfrozen":0,"refusedLeak":0}`. **Held.**
  - Cited, not re-run (they need the emitter linked into `node_modules`, a mutation this verify seat declines): E-2 audit (200,137,551 checks, 0 violations), `gen-grammar --check` ×2. The close's commands and outputs are complete and consistent with the unchanged bytes.
- **(2) File bounds.** ⟨`git show --stat` per commit⟩: bbnf-lang `f0059db14` → `typescript/{README.md, src/analysis/{index,reentry}.ts, src/emit.ts, src/gen.ts, test/reentry.test.ts}`; value.js `31121b23` → `src/css/bbnf/generated/grammar.{js,d.ts}`; `f7b087a4` → `src/css/bbnf/{color,value}.ts`, `test/css/bbnf-refusal.test.ts`; `1e04369b` → `src/css/bbnf/stylesheet.ts`, `test/css/bbnf-refusal.test.ts`; `1a9501c8` → `bench/**` only; `e393a7a8` → this record; `20b3e355` → this record + LEDGER (1 line). All inside `.l3`'s grant. `scripts/dev/dev.sh` in no commit. **Held.**
- **(3) Masking.** ⟨`git diff 059ec6af..HEAD -- src test bench/paired/*.mjs | grep -nE '^\+.*(try *\{|catch|\.skip|\.only|allowlist)'`⟩ → nothing; bbnf-lang `f0059db14` diff ⟨same grep + `navigator|userAgent`⟩ → nothing; its `.length > 0` hits are emitter-internal work lists, not input-length tests (L-9). The un-freezing of `Refused`/`invalid` is not a narrowed contract: both are internal records consumed by `refusal()`/`colorResult()` in `src/css/bbnf/index.ts:25–45`, and the probe above shows 0 unfrozen published nodes. **None.**
- **(4) Families.** Emitter cure (`f0059db14`) + its re-emission (`31121b23`) · two action cures each with its pins · records · receipt · close: one commit per meaning, no family split. **Held.**
- **(5) E-3.** ⟨`git diff --stat 059ec6af..HEAD -- docs/tranches/X/parse-that/waves docs/tranches/V/megatranche/registry/adjudicated scripts/dev/dev.sh`⟩ → empty. **Held.**
- **(6) Mail.** ⟨`grep -cE '\| *UNREAD *\|' docs/tranches/V/coordination/INBOX.md`⟩ → `0`. **Clean.**
- **(7) Four-verb line.** The close left X.P.W7 at PARTIAL, stamped nothing, and the LEDGER cell (`:105`) says `PARTIAL — ESC-W7l3-1 unruled`. Lawful: an unmet binding gate forbids IMPLEMENTED. **Held.**
- **(8) Goal at the bytes.** The goal (W7.md title, R-3, ADDENDA (b)/(e)): the emitted parser is faster than the retired parser on every entry, on every engine. Node, Chromium and WebKit meet it on every cell. Firefox meets it on 21 of 22 rows. Firefox large `parseStylesheet` does not. **NOT MET** on that one cell (axis 10).
- **(9) Published figures.** V-1 92 = 88 + 4, E-6 92,127 / 12,902, test/css 7/75, pins 4/4, css-equiv 19/19 reproduce exactly (above).
- **(10) Honest-RED adjudication.**
  - **Firefox large `parseStylesheet` (L-G1): RED, NOT relieved by the spec.** The close's reads: .938 · .967 · .975 · 1.007 · 1.04 · 1.176 · 1.013 · 1.013 (8 clean reps, median ≈ 1.01). This seat's re-read ⟨`node bench/paired/browser.mjs w7chk1-ff firefox 3 large,rej 11 product parseStylesheet`⟩ (load 33.87 → **216.89**: a sibling fleet spiked the host mid-read) set aside **all 24 reps** (spread 1.9–30.6), so it is VOID. It is recorded, not claimed either way; record moved to scratchpad, ⟨`git status --porcelain bench src test | wc -l`⟩ → `0`. The relief test at the spec bytes: ADDENDUM (e) says *"the gate stays (below the retired parser on every engine ×2; no relief, no engine split)"*. ADDENDUM (c) refuses *"recording Firefox's cells instead of gating them"*. ADDENDUM (b) routes an uncured cause to the owner *"as a measured decision, never as a quiet relaxation"*. The cell is not producer-owned (bbnf-lang's emitter and value.js's actions are both in W7's grant). No later wave owns it: W7P is the release chain only. No spec line names it as an honest-RED. ESC-W7l3-1 is the lawful escalation, and the register names the owner for it (the owner). But ⟨`grep -n ESC-W7l3-1 docs/tranches/X/COHESION.md | wc -l`⟩ → `0`: it is **unruled**. An open escalation is not relief. **Real defect, HIGH** (the wave's goal criterion, which the owner states and refuses to relax).
  - **P-6 · E-8 · V-9 + CI step · Z-1 · Z-3 publish half: RED, relieved.** ADDENDUM 2026-09-24 §1: *"X.P.W7's close relieves P-6, E-8, V-9 plus the CI step, Z-1 and Z-3's publish half by citing X.P.W7P"*. The register names the owner: X.P.W7P, OTP-keyed. **Honest-RED.**
  - Foreign test rows (`spectrum-luma` C-5, `reka-binding-idiom` NG-6) are not W7 gates (G-suite: 0 CSS failures). They are owner-named to Tracks A/B.
- **Successors.** X.P.W7P opens when: (i) the owner supplies an npm OTP or publishes step 1; **and** (ii) *"X.P.W7 has CLOSED its engineering gates"* (W7P.md:3). (i) is not met (LEDGER:106 `GATE-KEYED`). (ii) is **RED** (Firefox large). W7P is **lawfully blocked** on both conjuncts.

### Register
| Severity | Claim | Receipt | Cure |
|---|---|---|---|
| HIGH | L-G1 Firefox large `parseStylesheet` is not below the retired parser ×2. The spec grants no relief, and ESC-W7l3-1 is unruled. | Close's 8 clean reps .938–1.176 (median ≈ 1.01), 2 of 3 reads RED. This seat's read is VOID (load 216.89, 24/24 SA). ADDENDA (b)/(c)/(e) refuse relief. COHESION has 0 rows for ESC-W7l3-1. | The owner rules ESC-W7l3-1 (option (i), a SpiderMonkey accepted-sheet unit scoped to `src/css/stylesheet.ts`/`rules.ts`/the value lists; or option (ii), a stated noise band as a COHESION ruling and W7.md addendum). Then execute it and re-read the cell ×2 on a quiet host. |
| INFO | `test:css-equivalence` printed 26 `MIRROR-DEFECTS 0` lines here, against 28 at the close. There are no non-zero lines in either run. | ⟨`… \| grep -oE 'MIRROR-DEFECTS [0-9]+' \| sort \| uniq -c`⟩ → `26 MIRROR-DEFECTS 0` | None. The count is reporter output lines; 19/19 pass. |
...
## Check 2 of the RESUME 5 Close (2026-09-25, L-20 fresh adversarial pass 2, `claude-opus-5-5`, Track D; verify-only, cures nothing)

**Open.** Spec read whole (W7.md, 260 lines; ADDENDA (b), (c), (e) bind L-G1 on every engine ×2, "no relief, no engine split"). From this record: header, `## RESUME 5 — Open` through `## RESUME 5 Unit plan`, `## RESUME 5 Close`, `## Check 1 of the RESUME 5 Close`, `## Repair 1 of the RESUME 5 Close` (the last section). Crash-recovery: ⟨`git status --porcelain src bench test | wc -l`⟩ → `0`; this record and LEDGER clean at open; bbnf-lang worktree ⟨`status --porcelain | wc -l`⟩ → `0`, ⟨`git rev-parse --short HEAD` · `origin/x-p-w7-typescript`⟩ → `f0059db14` both. parse-that master's dirty `rust/**`, `README.md`, `.cargo/config.toml` rows lie outside W7's sets (not touched). **What changed since Check 1:** ⟨`git log --oneline 352aa91a..HEAD -- docs/tranches/X/parse-that docs/tranches/X/COHESION.md src bench`⟩ → empty; ⟨`grep -n ESC-W7l3-1 docs/tranches/X/COHESION.md`⟩ → nothing. Repair 1 cured 0 and escalated 1; no product byte moved. Host load ⟨`uptime`⟩ 79.82 / 119.84 / 80.66 at open, 67.12 / 109.99 / 79.23 later (the other tracks' redeploy).

### Axes
- **(1) Claimed GREENs reproduce** (this seat's runs; HEAD `352aa91a`, src at `1e04369b`):
  - ⟨`node bench/paired/build.mjs`⟩ → `srcDirty ""`, `bankedManifestOk 79/79`, `retiredAt 2155142b…`.
  - **L-G2** ⟨`node bench/paired/equiv.mjs product <scratch>`⟩ ×2 → `compared 1376531/1376531 rows · mismatches 92 (ASCII-only sources 1 · CP-CASE 4)` both. **GREEN ×2.**
  - **L-G3** ⟨`grep -rln "instrument\|__prof\|PC\[\|NOW()" src/css/bbnf/generated | wc -l`⟩ → `0` · **K2-b** ⟨`grep -rn deepFreeze src | wc -l`⟩ → `0` · **V-8** ⟨`grep -rnE 'GRAMMAR_MODULES|new ParserState|reset\(\)' src/css/bbnf --exclude-dir=generated | wc -l`⟩ → `0` · **E-6** ⟨`npx esbuild --minify …grammar.js | wc -c`⟩ → `92127`, `| gzip | wc -c` → `12902`. **GREEN.**
  - ⟨`npx vitest run test/css`⟩ → `7 passed (7) · 75 passed (75)`; `.cp` pins ⟨`npx vitest run test/css/custom-property-case.test.ts`⟩ → `4 passed (4)`; ⟨`npm run -s test:css-equivalence`⟩ → `Tests 19 passed (19)`, ⟨`grep -oE 'MIRROR-DEFECTS [0-9]+' | sort | uniq -c`⟩ → `28 MIRROR-DEFECTS 0` (the close's count; Check 1's 26 was reporter lines, as ruled INFO). ⟨`npx vue-tsc -p tsconfig.lib.json --noEmit | grep -c 'error TS'`⟩ → `0`. bbnf-lang ⟨`npx vitest run`⟩ → `21 passed (21) · 299 passed (299)`, ⟨`npx tsc --noEmit -p . | grep -c 'error TS'`⟩ → `0`. **GREEN** — 13 gate readings reproduced.
  - Cited, not re-run: E-2 audit and `gen-grammar --check` ×2 (both need the emitter linked into `node_modules`, a mutation a verify seat declines; bytes unchanged since the close); vue-tsc demo/test and the full `npm test` (Check 1 and the close read them; no src byte moved).
- **(2) File bounds.** The wave's `.l3` commits were rostered at `git show --stat` by the close and Check 1 (bbnf-lang `f0059db14` → `typescript/**`; value.js `31121b23`, `f7b087a4`, `1e04369b` → `src/css/bbnf/**` + `test/css/bbnf-refusal.test.ts`; `1a9501c8` → `bench/**` only). Since: `20b3e355` (record + LEDGER), `1a4d514d` (record + LEDGER, 1 line), `352aa91a` (record only) — ⟨`git show --stat --format= 1a4d514d 352aa91a`⟩ confirms. `scripts/dev/dev.sh` in no commit. **Held.**
- **(3) Masking.** No product byte moved since Check 1's grep (no try/catch, `.skip`/`.only`, allowlist, UA sniff or input-length test added). ⟨`git status --porcelain src bench test | wc -l`⟩ → `0`. **None.**
- **(4) Families.** Unchanged since Check 1: one commit per meaning, no family split. **Held.**
- **(5) E-3.** ⟨`git diff --stat 059ec6af..HEAD -- docs/tranches/X/parse-that/waves docs/tranches/V/megatranche/registry/adjudicated scripts/dev/dev.sh | wc -l`⟩ → `0`. **Held.**
- **(6) Mail.** ⟨`grep -cE '\| *UNREAD *\|' docs/tranches/V/coordination/INBOX.md`⟩ → `0`. **Clean.**
- **(7) Four-verb line.** The LEDGER cell (`:105`) reads `PARTIAL — ESC-W7l3-1 unruled`; nothing stamped IMPLEMENTED/VERIFIED. Lawful while a binding gate is RED. **Held.**
- **(8) Goal at the bytes.** Faster than the retired parser on every entry and every engine: node, Chromium, WebKit on every cell ×2; Firefox 21/22. Firefox large `parseStylesheet` is not. **NOT MET** on that one cell.
- **(9) Published figures.** V-1 92 = 88 + 4, E-6 92,127 / 12,902, test/css 7/75, pins 4/4, css-equiv 19/19 with 28 `MIRROR-DEFECTS 0`, bbnf-lang 21/299: reproduce exactly.
- **(10) Honest-RED adjudication.**
  - **Firefox large `parseStylesheet` (L-G1): RED, NOT relieved.** The close's 8 clean reps are .938–1.176 (median ≈ 1.01), and 2 of its 3 reads are RED. This seat does not re-read the cell: host load was 67–120 (1-minute), well past the ≥1.6× spread voids that Check 1 hit at 216 and the close at 43. A read at this load is VOID by the instrument's own rule, and a VOID read cannot move the verdict either way. The spec bytes grant no relief. ADDENDUM (e): *"the gate stays … no relief, no engine split"*. ADDENDUM (c) refuses *"recording Firefox's cells instead of gating them"*. ADDENDUM (b) says an uncured cause goes to the owner *"as a measured decision, never as a quiet relaxation"*. The cell is not producer-owned: the cure scope named is value.js `src/css/**` or the bbnf-lang emitter, both W7's own. No later wave is assigned it: X.P.W7P holds only the release acts. It is not an honest-RED named by id. The register names its owner as **the owner, via ESC-W7l3-1**, which is still unruled (COHESION 0 rows). **This is a real HIGH.**
  - **P-6 · E-8 · V-9 + CI step · Z-1 · Z-3 publish half: RED, relieved.** ADDENDUM 2026-09-24 §1: *"X.P.W7's close relieves P-6, E-8, V-9 plus the CI step, Z-1 and Z-3's publish half by citing X.P.W7P"*. Owner-named to X.P.W7P, which is OTP-keyed. **Honest-RED.**
  - Foreign rows (`spectrum-luma` C-5, `reka-binding-idiom` NG-6) are not W7 gates (G-suite: 0 CSS failures). They are owner-named to Tracks A/B.
- **Successors.** X.P.W7P opens after (i) the owner's npm OTP, which is **not met** (LEDGER:106 `GATE-KEYED`), **and** (ii) *"X.P.W7 has CLOSED its engineering gates"*, which is **RED** (Firefox large). W7P is **lawfully blocked** on both conjuncts.

### Register
| Severity | Claim | Receipt | Cure |
|---|---|---|---|
| HIGH | L-G1 Firefox large `parseStylesheet` is not below the retired parser ×2. The spec grants no relief, and ESC-W7l3-1 is unruled. | The close's 8 clean reps .938–1.176 (median ≈ 1.01), 2 of 3 reads RED. Repair 1 cured 0. ⟨`git log 352aa91a..HEAD -- src bench COHESION.md parse-that`⟩ → empty. ⟨`grep -c ESC-W7l3-1 COHESION.md`⟩ → 0. Load 67–120 this sitting, so a re-read would be VOID. | The owner rules ESC-W7l3-1: (i) a SpiderMonkey accepted-sheet unit (`src/css/stylesheet.ts`/`rules.ts`/value lists), or (ii) a stated noise band as a COHESION ruling plus a W7.md addendum. Then execute it and re-read the cell ×2 on a quiet host. |
...
## Check 3 of the RESUME 5 Close (2026-09-25, L-20 fresh adversarial pass 3, `claude-opus-5-5`, Track D; verify-only, cures nothing)

**Open.** Spec read whole (W7.md, 260 lines; ADDENDA (b), (c), (e) bind L-G1 on every engine ×2, "no relief, no engine split"). From this record: `## RESUME 5 — Open` through `## RESUME 5 Unit plan`, the `.l3` receipt, `## RESUME 5 Close`, `## Check 2 of the RESUME 5 Close` and `## Repair 2 of the RESUME 5 Close` (the last section). Crash-recovery: ⟨`git status --porcelain docs/tranches/X/execution/D docs/tranches/X/execution/LEDGER.md src bench test`⟩ → empty; bbnf-lang worktree ⟨`status --porcelain | wc -l`⟩ → `0`, ⟨`git rev-parse --short HEAD` · `origin/x-p-w7-typescript`⟩ → `f0059db14` both. parse-that master's dirty `rust/**`, `README.md`, `.cargo/config.toml` rows lie outside W7's sets (not touched). **What changed since Repair 2:** ⟨`git log --oneline 2b810276..HEAD`⟩ → empty; ⟨`git log --oneline 352aa91a..HEAD -- src bench docs/tranches/X/COHESION.md docs/tranches/X/parse-that`⟩ → empty; ⟨`grep -c ESC-W7l3-1 docs/tranches/X/COHESION.md`⟩ → `0` (unruled).

### Axes
- **(1) Claimed GREENs reproduce** (this seat's runs; HEAD `2b810276`, src at `1e04369b`):
  - ⟨`node bench/paired/build.mjs`⟩ → `srcDirty ""`, `bankedManifestOk 79/79`, `retiredAt 2155142b…`.
  - **L-G2** ⟨`node bench/paired/equiv.mjs product <scratch>`⟩ ×2 → `compared 1376531/1376531 rows · mismatches 92 (ASCII-only sources 1 · CP-CASE 4)` both. **GREEN ×2.**
  - **L-G3** ⟨`grep -rln "instrument\|__prof\|PC\[\|NOW()" src/css/bbnf/generated | wc -l`⟩ → `0` · **K2-b** ⟨`grep -rn deepFreeze src | wc -l`⟩ → `0` · **V-8** ⟨`grep -rnE 'GRAMMAR_MODULES|new ParserState|reset\(\)' src/css/bbnf --exclude-dir=generated | wc -l`⟩ → `0` · **E-6** ⟨`npx esbuild --minify src/css/bbnf/generated/grammar.js | wc -c`⟩ → `92127`, `| gzip | wc -c` → `12902` (≤ 125,646 / 14,517). **GREEN.**
  - ⟨`npx vitest run test/css`⟩ → `Test Files 7 passed (7) · Tests 75 passed (75)`; `.cp` pins ⟨`npx vitest run test/css/custom-property-case.test.ts`⟩ → `Tests 4 passed (4)`; ⟨`npm run -s test:css-equivalence | grep -oE 'MIRROR-DEFECTS [0-9]+|Tests +[0-9]+ passed[^)]*\)' | sort | uniq -c`⟩ → `28 MIRROR-DEFECTS 0` · `1 Tests 19 passed (19)`; ⟨`npx vue-tsc -p tsconfig.lib.json --noEmit | grep -c 'error TS'`⟩ → `0`; bbnf-lang ⟨`npx vitest run`⟩ → `21 passed (21) · 299 passed (299)`, ⟨`npx tsc --noEmit -p . | grep -c 'error TS'`⟩ → `0`. **GREEN** — 12 distinct gates reproduced (build/manifest · L-G2 (×2) · L-G3 · K2-b · V-8 · E-6 · test/css · pins · css-equiv · vue-tsc lib · bbnf-lang suite · tsc).
  - Cited, not re-run (bytes unchanged since the close): E-2 audit and `gen-grammar --check` ×2 (both need the emitter linked into `node_modules`, a mutation a verify seat declines); vue-tsc demo/test and the full `npm test`.
- **(2) File bounds.** The `.l3` commits were rostered at `git show --stat` by the close (bbnf-lang `f0059db14` → `typescript/**`; value.js `31121b23`, `f7b087a4`, `1e04369b` → `src/css/bbnf/**` + `test/css/bbnf-refusal.test.ts`; `1a9501c8` → `bench/**` only). Since: record/LEDGER-only commits (`20b3e355`, `1a4d514d`, `352aa91a`, `8bcdced1`, `2b810276`); ⟨`git log --oneline 2b810276..HEAD`⟩ → empty. `scripts/dev/dev.sh` in no commit. **Held.**
- **(3) Masking.** No product byte moved since the close (⟨`git log 352aa91a..HEAD -- src bench`⟩ → empty; ⟨`git status --porcelain src bench test | wc -l`⟩ → `0`). No try/catch, `.skip`, allowlist, UA sniff or input-length test. **None.**
- **(4) Families.** Unchanged: one commit per meaning, no family split. **Held.**
- **(5) E-3.** ⟨`git diff --stat 059ec6af..HEAD -- docs/tranches/X/parse-that/waves docs/tranches/V/megatranche/registry/adjudicated scripts/dev/dev.sh | wc -l`⟩ → `0`. **Held.**
- **(6) Mail.** ⟨`grep -cE '\| *UNREAD *\|' docs/tranches/V/coordination/INBOX.md`⟩ → `0`. **Clean.**
- **(7) Four-verb line.** LEDGER `:105` reads `PARTIAL — ESC-W7l3-1 unruled`; nothing stamped IMPLEMENTED/VERIFIED. Lawful while a binding gate is RED. **Held.**
- **(8) Goal at the bytes.** Faster than the retired parser on every entry, every engine: node, Chromium, WebKit on every cell ×2 (the close's reads); Firefox 21/22. Firefox large `parseStylesheet` is not. **NOT MET** on that one cell.
- **(9) Published figures.** V-1 92 = 88 + 4, E-6 92,127 / 12,902, test/css 7/75, pins 4/4, css-equiv 19/19 with 28 `MIRROR-DEFECTS 0`, bbnf-lang 21/299: reproduce exactly.
- **(10) Honest-RED adjudication.**
  - **Firefox large `parseStylesheet` (L-G1): RED, NOT relieved.** This seat re-read it: ⟨`node bench/paired/browser.mjs c3p3-ff firefox 4 large,rej 11 product parseStylesheet`⟩ (load 72.67 → 94.59; record moved to scratchpad `w7c3/`, ⟨`git status --porcelain bench src test | wc -l`⟩ → `0`): large **1 clean rep x1.03** (12 set-aside, .878–1.313); rej clean x1.041 · .982 · .891 (6 set-aside). At this load the read is VOID for a verdict (18 of 22 reps set aside); its one clean large rep is above 1, consistent with the close's RED (8 clean reps .938–1.176, median ≈ 1.01) and never a GREEN claim. The rej cell's one clean 1.041 is load-borne (spread 1.51, near the 1.6 bar) against the close's 8/8 clean reps below 1; it is recorded, not re-verdicted. The spec grants the large cell no relief: ADDENDUM (e) *"the gate stays … no relief, no engine split"*; ADDENDUM (c) refuses *"recording Firefox's cells instead of gating them"*; ADDENDUM (b) sends an uncured cause to the owner *"as a measured decision, never as a quiet relaxation"*. It is not producer-owned (the cure scope is value.js `src/css/**` or the bbnf-lang emitter, both W7's own) and no later wave is routed it (X.P.W7P holds only the release acts). **Not honest-RED: a real HIGH.** Its owner is named (the owner, via ESC-W7l3-1), which does not relieve it.
  - **P-6 · E-8 · V-9 + CI step · Z-1 · Z-3 publish half: RED, relieved.** ADDENDUM 2026-09-24 §1: *"X.P.W7's close relieves P-6, E-8, V-9 plus the CI step, Z-1 and Z-3's publish half by citing X.P.W7P"*. Owner-named to X.P.W7P (OTP-keyed). **Honest-RED.**
  - Foreign rows (`spectrum-luma` C-5, `reka-binding-idiom` NG-6) are not W7 gates (G-suite: 0 CSS failures); owner-named to Tracks A/B.
- **Successors.** X.P.W7P (W7P.md:3) opens when (i) the owner supplies an npm OTP — **not met** (LEDGER:106 `GATE-KEYED`) — **and** (ii) *"X.P.W7 has CLOSED its engineering gates"* — **RED** (Firefox large). **Lawfully blocked** on both conjuncts.

### Register
| Severity | Claim | Receipt | Cure |
|---|---|---|---|
| HIGH | L-G1 Firefox large `parseStylesheet` is not below the retired parser ×2; the spec grants no relief; ESC-W7l3-1 is unruled. | The close's 8 clean reps .938–1.176 (median ≈ 1.01), 2 of 3 reads RED; this seat's read (load 72–95): 1 clean rep x1.03, 12 set-aside. ⟨`git log 352aa91a..HEAD -- src bench COHESION.md parse-that`⟩ → empty; ⟨`grep -c ESC-W7l3-1 COHESION.md`⟩ → `0`. | The owner rules ESC-W7l3-1 ((i) a SpiderMonkey accepted-sheet unit on `src/css/stylesheet.ts`/`rules.ts`/value lists, or (ii) a stated noise band as a COHESION ruling + W7.md addendum); execute it; re-read the cell ×2 on a quiet host. |
| INFO | Firefox rej `parseStylesheet` read one clean rep at x1.041 in this seat's load-72–95 read. | ⟨`… c3p3-ff …`⟩ rej clean 1.041 (spread 1.51) · .982 · .891. | None: load-borne; the close's 8/8 clean reps < 1 stand. The quiet-host re-read owed by the HIGH re-reads it too. |

**Honest-RED set:** P-6 · E-8 · V-9 + CI step · Z-1 · Z-3 (publish half), relieved to X.P.W7P (§0cp 1).

**Verdict: NOT-CONFORMANT.** The row stays `PARTIAL`. 12 claimed GREENs reproduce; bounds, masking, families, E-3 and mail held. The one binding RED (Firefox large `parseStylesheet`) waits on the owner's ruling of ESC-W7l3-1; no repair round can cure it without that ruling. LEDGER not moved (no CLOSED stamp). **Adjacent edits: none.** `scripts/dev/dev.sh` untouched.

## RESUME 6 — Open (2026-09-25, seat 0, `claude-opus-5-5`, Track D; COHESION §0dt + W7.md ADDENDUM (f))

- **Mode:** RESUME 6. ⟨`grep -n "X.P.W7" LEDGER.md`⟩ → `:105` status `PARTIAL — ESC-W7l3-1 unruled (RESUME 5 Close …)` (not CLOSED); this record exists (2379 lines before this section). Governing text: W7.md read whole (268 lines; ADDENDUM (f) at `:262` binds — ESC-W7l3-1 RULED (i): unit `.l4` profiles and cures the accepted-sheet success path; (ii) a noise band REFUSED; the final L-G1 reads only at 1-min load < 8, bounded wait ≤ 90 min, load per rep, never a claimed unread cell; then `.cp` verified) and COHESION §0dt (`:3596`, the file end). Read from this record: `## RESUME 5 — Open` through `## RESUME 5 Unit plan` and `## Check 3 of the RESUME 5 Close` (the last section).
- **alreadyDone: `.o .p .t .e .v .k .k2 .g .l .l2 .l3 .cp`**, verified at the bytes: ⟨`git log --oneline --all | grep -E "^(eeed0116|628d23b6|a1eddca8|31121b23|f7b087a4|1e04369b|1a9501c8|e393a7a8)"`⟩ → all eight present (`.l3` = `31121b23` re-emission · `f7b087a4` · `1e04369b` · `1a9501c8` records · `e393a7a8` receipt; `.cp` = `eeed0116` · `628d23b6` · `a1eddca8`); bbnf-lang ⟨`git -C ../bbnf-lang-x-p-w7-typescript log --oneline -1`⟩ → `f0059db14` (`.l3`) = `origin/x-p-w7-typescript`, worktree clean.
- **`.cp` is NOT re-dispatched** although §0dt / ADDENDUM (f) 4 sequence "then `.cp` is verified": both ADDENDUM (d) halves are landed — the case cure `eeed0116` with pins ⟨`npx vitest run test/css/custom-property-case.test.ts`⟩ → `Tests 4 passed (4)`, and the easing half RULED V-A64 and cited ⟨`grep -n "V-A64" CHANGELOG.md`⟩ → `:11` (`timingFunctionDescriptions` and the `bounce-*` family, consumer migration note). ADDENDUM (f) 4: "verify rather than redo" — `.cp`'s gates ride as stay-GREEN in `.l4` and are re-read by the close. `.z` re-opens only in X.P.W7P.
- **Crash-recovery:** ⟨`git status --porcelain -- src bench test CHANGELOG.md package.json package-lock.json docs/tranches/X/execution/D docs/tranches/X/execution/LEDGER.md docs/tranches/X/parse-that docs/tranches/V/coordination scripts/gen-grammar.mjs`⟩ → ` M test/ink.test.ts` · `?? test/ink-real-composite.test.ts` — both OUTSIDE W7's writable set (`test/css/**`; they are Track A X-W7L's ink rows) — not touched; ⟨`git -C ../bbnf-lang-x-p-w7-typescript status --porcelain`⟩ → empty. HEAD `82d5c74d` (§0dt). **No inherited partial work.**
- **Preconditions:** `.l4` opens on §0dt's ruling (not a predecessor wave): ⟨`grep -n "^## §0dt" COHESION.md`⟩ → `3596`; W7.md:262 ADDENDUM (f) present; `.l3` commits exist (above); the `.l` instrument present (`bench/paired/instrument.mjs`, `bench/paired/profile.mjs`); ⟨`node bench/paired/build.mjs`⟩ → `srcDirty ""`, `bankedManifestOk 79/79`, `retiredAt 2155142b`; bbnf-lang in node_modules `0.1.4` (restored). Release chain stays X.P.W7P (LEDGER:106 `GATE-KEYED … one-time password`). **MET.** No npm act in this wave.

### E13 Step-0 mail sweep (2026-09-25)
- ⟨`ls -td glass-ui/docs/tranches/*/ | head -3`⟩ → `BL BK BJ` (BL newest, no `coordination/`; BK stays the mail path, BL root swept).
- ⟨`find <path> -maxdepth 1 -type f -newer INBOX.md`⟩ over value.js `V/` + `V/coordination/` · glass `BK/coordination/` + `BL/` · keyframes.js `V/coordination/` · atlas `P/coordination/` → 0 on every path except glass `BL/FORMATION-PROGRESS.md` (glass's internal resume cursor, not a letter). ⟨`grep -cE '\| *UNREAD *\|' INBOX.md`⟩ → `0`.
- **Result: 0 unrowed · 0 UNREAD in scope.** A dated sweep line was appended to INBOX.md.

## RESUME 6 Baseline (BEFORE, read-only, 2026-09-25; product bytes = `.l3`'s final bytes `1e04369b` on emission `31121b23`, unchanged)

- ⟨`node bench/paired/build.mjs`⟩ → `valuejsHead 82d5c74d`, `srcDirty ""`, `bankedManifestOk 79/79`, `retiredAt 2155142b`.
- **RESUME rule:** `.l4` turns L-G1 Firefox large `parseStylesheet` (and every cell it could move); every other gate is cited from `## Check 3 of the RESUME 5 Close` (V-1 92 = 88 F-b-4 + 4 CP-CASE ×2 · L-G3 0 · K2-b 0 · V-8 0 · E-6 92,127/12,902 · test/css 7/75 · css-equiv 19/19 MIRROR-DEFECTS 0 · vue-tsc lib 0 · bbnf-lang 21/299 + tsc 0).
- **Load:** the shared host is busy (sibling fleets): 1-min load 25.04 → 43.69 across the read (5-min ≈ 60). This BEFORE read is not a quiet-host read (L-13 binds only `.l4`'s final gate reads); recorded as-is. Record moved to the scratchpad; ⟨`git status --porcelain bench src | wc -l`⟩ → `0`.

| Gate | Command | BEFORE (2 reps, paired median; every rep) | Reading |
|---|---|---|---|
| L-G1 Firefox large parseStylesheet | ⟨`node bench/paired/browser.mjs w7r6open firefox,webkit,chromium 2 large,rej 11 product parseStylesheet`⟩ (load 25.04 → 43.69) | **x1.056** clean (+4 SA .851/1.052/1.094/1.005, spread 2.5–6.7) | **born-RED** (§0dt: median ≈ 1.04) |
| L-G1 Firefox rej parseStylesheet | same | .957 / .957 | GREEN (stay-GREEN; `.l3`) |
| L-G1 WebKit rej / large parseStylesheet | same | .727 / .704 · .586 / .586 | GREEN (stay-GREEN) |
| L-G1 Chromium rej / large parseStylesheet | same | .481 / .467 · .334 / .334 | GREEN (stay-GREEN) |
| `.cp` pins (ADDENDUM (d)) | ⟨`npx vitest run test/css/custom-property-case.test.ts`⟩ | `Tests 4 passed (4)` | GREEN (stay-GREEN; `eeed0116`) |
| `.cp` easing half | ⟨`grep -n "V-A64" CHANGELOG.md`⟩ | `:11` migration note (ruled removal) | GREEN (stay-GREEN; `628d23b6`) |
| K2-b post-hoc walk | ⟨`grep -rn deepFreeze src \| wc -l`⟩ | 0 | stay-GREEN |
| L-G3 instrument bytes in shipped `generated/` | ⟨`grep -rln "instrument\|__prof\|PC\[\|NOW()" src/css/bbnf/generated \| wc -l`⟩ | 0 | stay-GREEN |

**greenBeforeCure: none.**

## RESUME 6 Unit plan (2026-09-25)

**alreadyDone:** `.o .p .t .e .v .k .k2 .g .l .l2 .l3 .cp` (never re-dispatched; `.cp`'s ADDENDUM (d) gates ride as stay-GREEN). **`.z`:** re-opens only in X.P.W7P. **Owed:** `[X.P.W7.l4]` alone, then close. One concurrent. Every seat Opus 5.5 (owner's Opus-only order, 2026-09-23; ADDENDUM (f) "Opus, effort high"). An ESCALATED `.l4` does not halt the wave: the close runs on it.

### Rulings cited (never re-opened)
§0dt / W7.md ADDENDUM (f) (ESC-W7l3-1 RULED (i): `.l4` success-path unit; (ii) noise band REFUSED; quiet-host protocol: final L-G1 reads only at 1-min load < 8, bounded wait ≤ 90 min, load per rep, never a claimed unread cell) · §0dq / ADDENDUM (e) (no relief, no engine split) · §0di / ADDENDUM (c) ((c) record-instead-of-gate REFUSED) · §0cx / ADDENDUM (b) (root cause; one cure every engine; never a post-hoc freeze walk; impasse → ESCALATED with the per-rule profile) · §0dj / ADDENDUM (d) (`.cp`; easing removal = V-A64) · §0cp (release chain → X.P.W7P, OTP-keyed; LEDGER hunk hygiene) · §0ck 1–7 (F-b-4 accepted; positional semantics; bbnf-lang choreography) · §0bt ADJACENT-LINE RULE.

### Locks
- **L-4 (bbnf-lang):** writes only in `../bbnf-lang-x-p-w7-typescript` on `x-p-w7-typescript`; push the branch only; never local master; PR #1 stays open. Every re-emission of `src/css/bbnf/generated/` goes through `node scripts/gen-grammar.mjs` only (emitter linked from the worktree as `.l`–`.l3` did; restore 0.1.4 after, ⟨`grep '"version"' node_modules/@mkbabb/bbnf-lang/package.json`⟩ → `0.1.4`); `--check` passes after.
- **L-6 (no npm):** no `npm publish`, no version bump, no dependency move (X.P.W7P); ⟨`git diff HEAD -- package.json package-lock.json`⟩ empty at the end.
- **L-7 (LEDGER, §0cp 4):** read `git diff` on LEDGER.md before any commit touching it; stage only this seat's hunk.
- **L-8 (instrument compiled out):** L-G3 stays 0 in shipped `generated/`.
- **L-9 (one module, every engine):** no engine sniffing, no input-length test, no per-engine branch in emitted or action code; K2-b stays 0 (no post-hoc freeze walk); no try/catch around a defect; a file both arms load (`src/css/result.ts`, any shared helper) moves only if the profile names it, and then for both arms identically (stated in the receipt).
- **L-12 (success-path discipline, ADDENDUM (f) 2):** one shape per node kind (no optional-field shape forks), spans instead of substring copies, no intermediate lists (no map-then-spread, no filter copy), a freeze only where the immutability contract needs it (results still built frozen; `Object.isFrozen` deep pin stays GREEN) — each cure named with a test or an audit-build assertion.
- **L-13 (quiet-host protocol, ADDENDUM (f) 3):** the final L-G1 ×2 reads start only when `uptime`'s 1-min load < 8; bounded wait loop (Monitor until-loop / background poll, ≤ 90 min), load recorded per rep; if the host never quiets, the reads are recorded with their load and the cell is stated UNREAD with the reason — never claimed.
- **Families:** an emitter cure = one bbnf-lang commit (`typescript/src/**` + tests), pushed, + its value.js re-emission commit; an action cure = one value.js `src/css/**` commit with its tests; records = one value.js `bench/**` commit; the receipt = one record commit.

### `[X.P.W7.l4]` — Firefox's accepted large-sheet cost, found and cured (W7.md 262–268: ADDENDUM (f); COHESION §0dt)
- **Writable:** bbnf-lang `typescript/**` on `x-p-w7-typescript` · value.js `src/css/bbnf/**` (`generated/` through `node scripts/gen-grammar.mjs` only), `src/css/stylesheet.ts`, `src/css/rules.ts`, the value-list builders and the freeze helper and its callers under `src/css/**` (the `.k2` grant + ADDENDUM (f) 1's named files) · `bench/**` (probes beside `bench/paired/`, records) · `test/css/**` (pins) · this record. Never `scripts/dev/dev.sh`, never `test/ink*` (Track A).
- **Acts:** (1) profile: ⟨`node bench/paired/profile.mjs l4 node,firefox,chromium,webkit parseStylesheet:large`⟩ with the `.l` instrument (per-rule counts + time, compiled out) on the ACCEPTED large sheets, V8 the control; name what SpiderMonkey spends on the success path that the retired parser does not (per-declaration allocation, intermediate arrays, `Object.freeze` on hot paths, per-node closures, substring vs span, megamorphic shapes) across `src/css/stylesheet.ts`, `rules.ts`, the value lists and the emitted success actions; bisect with arms under `_build/` (`bench/paired/arm.mjs`); (2) cure structurally once for every engine per L-12; (3) re-emit if the emitter moved; (4) final gate reads per L-13; (5) impasse → ESCALATED with the per-rule profile.
- **Gates:** L-G1 every entry × {acc, rej} + large sheets < 1.00 paired on node, Chromium, WebKit, Firefox, ×2, final reads at load < 8 (`bench/paired/isolated.mjs`, `bench/paired/browser.mjs`) · L-G2 V-1 = 88 F-b-4 + 4 CP-CASE exactly ×2 (`bench/paired/equiv.mjs`) · L-G3 0 · stay-GREEN: `.cp` pins 4/4 ×2 + CHANGELOG V-A64 note present, `test:css-equivalence` 19/19 MIRROR-DEFECTS 0, `npx vitest run test/css` (incl. `bbnf-refusal.test.ts` and the deep-frozen pin), `npm test` (foreign failures only), `vue-tsc` 0/0/0, `gen-grammar --check` 0, K2-b 0, E-1..E-7 on the final emitter (E-6 ≤ 125,646 / 14,517), bbnf-lang TS suite ×2 + `tsc --noEmit` 0, whole-7/7 inside ceilings.
- **Locks:** L-4, L-6, L-7, L-8, L-9, L-12, L-13.

### Close (after `.l4`)
Engineering gates binding (accepted + rejected halves and large sheets under the retired parser on every engine, ×2, read per L-13) plus `.cp`'s ADDENDUM (d) gates re-read. Release gates P-6 · E-8 · V-9 + the CI step · Z-1 · Z-3 (publish half) relieved by citing X.P.W7P (§0cp; OTP-keyed). No `npm publish` in this wave.

## RESUME 6 Unit receipts

### X.P.W7.l4

Seat `claude-opus-5-5`, 2026-09-25, Track D. Governing text: W7.md (268 lines, read whole; ADDENDUM (f) at 262–268 under (b) 216–230, (c) 232–247, (e) 255–260, stay-GREEN (d) 249–253) · COHESION §0dt (`:3596`, the file end) · the RESUME 6 Unit plan above (Locks L-4, L-6..L-9, L-12, L-13).

**Crash-recovery.** ⟨`git status --porcelain -- src/css bench test/css docs/tranches/X/execution/D/X-P-W7.md`⟩ → empty; ⟨`git -C ../bbnf-lang-x-p-w7-typescript status --porcelain`⟩ → empty at `f0059db14` (= origin). **No inherited partial work.** Tooling as `.l`–`.l3`: `node_modules/@mkbabb/bbnf-lang` (0.1.4) moved aside as `.bbnf-lang-0.1.4-aside-l4`, the worktree's `typescript/` (`npm run build`) linked in its place for the unit's life, restored at the end. ⟨`node bench/paired/instrument.mjs`⟩ → `{"plainEqualsShipped":true,…,"shippedInstrumentTokens":[],"instrumentedTokens":6}`.

**1. The profile, and what it names** (the `.l` instrument, compiled out; then two new uninstrumented probes, because the instrumented timer's cost swamps a 4% gap).
- ⟨`node bench/paired/profile.mjs l4 node,firefox,chromium,webkit parseStylesheet:large 5`⟩ → `bench/records/2026-09-24-x-p-w7-profile-l4.json` (load 32.3 → 31.5). Counts identical on every engine (84,363 calls · 18,171 fails · 32,018 action calls · **57 discarded values** per pass: the refusal cures of `.l3` hold; the success path builds almost nothing it throws away). Per pass: node wall 14.96 ms (entries 13.49 · glue 1.46 · actions 2.43), Firefox 35.6 (30.8 · **4.8** · 7.8), Chromium 39.74 (38.32 · 1.42 · 5.36), WebKit 23 (20.6 · 2.4 · 4.8). Firefox's glue (the stylesheet layer's own JS between entries) is 3.3× node's; the entries 2.3×. Ranking inside Firefox: `ruleList` 11.4 · `valueTop` 10.2 · `declaration` 3.0 · `commaItems` 2.2 · `atPrelude` 2.0 · `semiItems` 1.8 ms (timer-inflated, rank only).
- **The large sheets are not accepted sheets.** ⟨`node -e '…parseStylesheet on each sheets/MANIFEST.json sheet, both arms…'`⟩ → all four are REFUSED by both arms: `value-js-index.css` at the rule list's `closing brace` fault (199,077 of 523,495), `keyframes-js-index.css` at declaration 3 (`url(…)`), `keyframes-js-vendor-monaco.css` at declaration 1,016, `wpt-bulma-0.7.5.css` — **at different points per arm**: ⟨`node -e '…the declarations the product's path reads, the first each arm's parseCssValue refuses…'`⟩ → `wpt-bulma-0.7.5.css decls 949 P# 946 calc(50% - (1em / 2)) | R# 32 rgba(10, 10, 10, 0.2)`. The retired parser refuses the legacy comma `rgba(10, 10, 10, 0.2)` (⟨`R.hand.parseCssValue("rgba(10, 10, 10, 0.2)")`⟩ → `ok false`, `expected ["scalar"]`; the product accepts it, css-color-4 §legacy), so on bulma the retired arm stops at its 32nd declaration and the product reads 946: **the product does ≈ 29.6× the retired's declaration work on that sheet.** The other three sheets refuse at the same point in both arms.
- **Where the time goes, per sheet and per reader, uninstrumented** (new instruments: `bench/paired/phases.mjs` — `parseStylesheet` / `parseCssValue` over the sheets' declaration values / the rule-list scan alone, each arm, paired in one page; `bench/paired/readers.mjs` — every reader call `parseStylesheet` makes over the large sheets, logged by `build.mjs`'s recorder arm and replayed per reader, with the retired arm's counterparts over the same arguments (`retired-split.mjs`: the retired arm re-bundled with its module-private `blocks` exported, bytes unchanged) and each sheet timed alone). ⟨`node bench/paired/readers.mjs t3 node,firefox 11`⟩ (load 31.8 → 29.4) → `…-readers-t3.json`, Firefox ms/pass:

| Firefox (SpiderMonkey) | whole | `value-js` | `kf-index` | `monaco` | **`bulma`** | rule-list scan | parseCssValue (1,972 values) |
|---|---|---|---|---|---|---|---|
| product (open bytes `1e04369b`/`31121b23`) | 10.0 | 1.25 | 0.875 | 3.0 | **4.25** | 3.25 | 2.625 |
| retired | 9.75 | 1.25 | 1.75 | 5.125 | **1.375** | 4.625 (`blocks`) | 5.0 |

   Every OPERATION is below the retired parser in Firefox — the rule-list scan (.70; `phases` t1/t3: .717 of the retired `blocks`), the value parser (.53; `phases` t1 whole-value set .427), and every sheet the two arms read equally (`value-js` 1.0 at the timer's grain, `kf-index` .50, `monaco` .59). **The cell is RED on one sheet, `bulma`, where the arms do unequal work (3.1×)**; node hides it because the retired `blocks` scanner is 4.6× slower on V8 than on SpiderMonkey (node `R:ruleList` 14.3–21.5 ms vs Firefox 4.5–4.6; the retired whole: node 17.8 vs Firefox 9.75). The residue the cure can reach is the product's per-node cost on the accepted prefix it reads: its allocation per pass (V8's `total_allocated_bytes`, 256 KB grain) was ≈ 6.2 MB against the retired's ≈ 5.9 MB, with 2.3 MB in `parseCssValue` over 1,972 values and 1.8 MB in the rule-list scan.

**2. Bisection by arms under `_build/`** (the base bytes saved as arm `l4base` = `_build/product.mjs` at the open; each candidate built as `product` from the working tree by ⟨`node bench/paired/build.mjs`⟩, or cut by hand and bundled by ⟨`node bench/paired/arm.mjs <name> <probe>`⟩; hand-cut probes banked in `bench/records/2026-09-25-x-p-w7-l4-probes/{p1,p3,p13,lazy}.mjs`, probes only, never shipped; every arm checked deep-equal to `l4base` before timing: 7 entries (and every sheet reader + `splitTopLevel` ×3) × 29,944 sources + 4 sheets, **0 diffs, 0 throws**). Firefox large `parseStylesheet`, paired median vs retired, same page; clean reps, then set-aside (SA):

| arm (what it removes) | Firefox large | record |
|---|---|---|
| `l4p1` / `l4p3` / `l4p13` — hand cuts: the rule list's gap values; the block body's second copy; both (vs product 1.101 · 1.033) | 1.029 · 1.007 / 1.014 · .981 / 1.032 · .971; 5-rep re-read: not separable at load 35–60 (`p13b`: 11 of 12 SA) | `…-browser-l4-p13a.json` / `…-p13b.json` |
| E1 `l4e1` — the grammar discards separators and gaps (`sep >> item`, `ruleGap >> ( ( ruleBlock << ruleGap ) * , fault ? )`), a block body read as text once (vs `l4base` .944 · 1.117 · 1.077 · .959) | .948 · 1.075 · 1.158 · .939 (rej .955 · .9 · .874 vs .966 · .982 · .862) | `…-browser-l4-e1.json` |
| E3 = E1 + `styleRule` built once + sheet readers bound + `listOf` one pass (vs `l4base` 1.024 · 1.025 · 1.146 · 1.016; SA 1.026 · 1.162 · 1.047 · 1.081) | **1.0 · 1.038 · .955 · .879** (SA .934 · .964 · 1.041 · 1.025) | `…-browser-l4-e3.json` |
| E5 = E3 + the `numeric` groups leaf (vs E3 `l4e4` .87 · .975 · .981 · .857) | **.889 · .947 · .797 · .962** | `…-browser-l4-e5.json` |
| `l4lazy` — E5 + every repetition array allocated on its first item (an emitter lever; hand cut) | .821 · .909 · .888 · .845 | `…-browser-l4-e5.json` |

   E3 against its own base in one page: 7 of 8 reps below it; E5 reads < 1 on every clean rep (load 26.9–33.8). `l4lazy` is not separable from E5 at this load and is an emitter lever `.l3` already measured within noise (`sempty`): **not admitted** (no bbnf-lang change in this unit).
- Allocation, V8 (⟨`node --expose-gc bench/paired/alloc.mjs l4base product`⟩, 256 KB grain): whole pass 6,245 → **4,967 KB**; `parseCssValue` over the 1,972 values 2,309 → **1,815**; rule-list scan 1,796 → **768**; `bulma` 3,634 → **2,592**.

**3. The cure — value.js `5d46e376`, one module, every engine (L-9, L-12).** Each accepted node is built once, in the shape it is published in:
- **`listOf` (`src/css/bbnf/value.ts`, the value-list builder).** Every value ran it three times (comma, slash, space), each building `[first, ...rest]` (a spread copy through the iterator) and a `find` closure call per item, although almost every level holds one item. Now a one-item level is its item (no allocation), a refused first item refuses at once, and a real list is built in one array, in one pass, the first refused item still refusing in order. (L-12: no intermediate lists.)
- **The `numeric` leaf hands its groups** (`value.bbnf` `numeric` gains two capture groups; `numeric: { kind: "groups", fn: numericScalar }`): the number and unit arrive as the leaf's own match, and the second regex (`NUMERIC.exec` over the token) and its match array are gone — the `.k2` cure for `tokenQuantity`, carried to the value-list numeric. (L-12: spans, not copies.)
- **The rule list and the top-level lists discard separators and gaps in the grammar** (`stylesheet.bbnf`: `commaItems`/`semiItems` `sep >> run`, `spaceItems = ws >> ( spaceRun << ws ) *`, `ruleList = ruleGap >> ( ( ruleBlock << ruleGap ) * , fault ? )`): the emitter recognizes the discarded side instead of building it, so per block the gap's value array and substrings, the `[block, gap]` pair and the `column` copy are gone, and the blocks arrive as the rule list's own list; per list item the `[sep, item]` pair is gone. `ruleList`'s grouping keeps the `*`/`?` retry pair visible to `analysis/reentry.ts`, so `preludeRun`'s kept answer (`.l3`) stays: ⟨`grep -o "M0[a-z]*" src/css/bbnf/generated/grammar.js | sort | uniq -c`⟩ → `5 M0i · 3 M0o · 3 M0s` (as before; an ungrouped first spelling lost it and was re-grouped).
- **A block body is read as text once:** `blockBody: { kind: "text", fn: same }` and `blockTail` loses its action (`"{" >> blockBody << "}"` keeps the body's text), so the `inner` second copy is gone.
- **A style rule is built once, frozen as built** (`src/css/stylesheet.ts` `styleRule` replaces `parseStyleBody`): no intermediate frozen body record, no `success` wrapper around it, no spread of it into the rule; `children` only when the body nests rules (the published shape, unchanged). The body is still read before the selector list (the refusal order holds).
- **Each sheet reader calls its own entry** (`src/css/bbnf/sheet.ts`): `E.atPrelude(prelude)` … instead of one `read(name, …)` doing `parser.entries[name](source)` for every reader — no keyed lookup, and no call site shared by fifteen entries (the `.k` binding, carried to the readers).
- Re-emitted through ⟨`node scripts/gen-grammar.mjs`⟩ on the linked emitter (sha256 header `c1c91dad…`); the emitter is unchanged (bbnf-lang stays `f0059db14`). Pins: `test/css/bbnf-success-path.test.ts` (11): a one-item level is the item; a real list in order, deeply frozen; the first refused item refuses (`a, 1px @ 2px, # b` → 7..8); the numeric groups (`1.5e3px`, `-.5%`, `+2`, `10E-1Q`); the rule list across gaps, stray `;` and comments with the body read once, and its fault; the lists without separators; a style rule with no `children` key on a plain body and with `children` on a nested one, deeply frozen.
- **Adjacent edits (COHESION §0bt):** `src/css/grammar/stylesheet.bbnf:24,26,28,51` (the three list rules and `ruleList`: the separators and gaps discarded where they stand — the builders in `src/css/bbnf/stylesheet.ts` destructure exactly these shapes, so the "no intermediate lists" cure requires the four lines) and `src/css/grammar/value.bbnf:39–40` (the `numeric` leaf's two groups + a comment line: the `groups` action requires them). Same repository, same concern; the frozen oracle holds (L-G2 below).
- `result.ts` (shared by both arms) untouched; no engine sniff, no input-length test, no try/catch, no post-hoc freeze walk (K2-b 0).

**4. Gates on the final bytes (`5d46e376`; emitter `f0059db14`, linked).**
- **L-G2 / V-1** ⟨`node bench/paired/equiv.mjs product <scratch>/eq1`⟩ → `compared 1376531/1376531 rows · mismatches 92 (ASCII-only sources 1 · CP-CASE 4)` = 88 F-b-4 + 4 CP-CASE (read 2 below).
- **L-G3** ⟨`grep -rln "instrument\|__prof\|PC\[\|NOW()" src/css/bbnf/generated | wc -l`⟩ → `0`; ⟨`node bench/paired/instrument.mjs`⟩ → `"plainEqualsShipped":true`, `"shippedInstrumentTokens":[]`. **GREEN.**
- **K2-b** ⟨`grep -rn deepFreeze src | wc -l`⟩ → `0`; **V-8** ⟨`grep -rnE 'GRAMMAR_MODULES|new ParserState|reset\(\)' src/css/bbnf --exclude-dir=generated | wc -l`⟩ → `0`. **GREEN.**
- **E-4** ⟨`node scripts/gen-grammar.mjs --check`⟩ ×2 → `grammar.js is current (sha256 c1c91dad543dd45a…)` both. **GREEN ×2.**
- **E-2** ⟨`node bench/paired/audit.mjs`⟩ (reads the grammar from disk through the linked emitter's audit build) → `rules 160 · sources 29944 · sheets 4 · checks 200137551 · violations 0 · modeCalls 4791680 · modeDiffs 0`. **GREEN.**
- **E-1/E-3/E-5** ⟨`npx vitest run`⟩ in the bbnf-lang worktree `typescript/` → run 1 (concurrent with value.js's full `npx vitest run`, load ≈ 30): `1 failed | 298 passed (299)`; runs 2, 3, 4: `Test Files 21 passed (21) · Tests 299 passed (299)`. The emitter is untouched by this unit (worktree clean at `f0059db14`), so the one failure is that suite's own flake under contention, not a regression; recorded, not hidden. ⟨`npx tsc --noEmit -p . | grep -c 'error TS'`⟩ → `0`. **GREEN ×2** (consecutive).
- **E-6** ⟨`npx esbuild --minify src/css/bbnf/generated/grammar.js | wc -c`⟩ → `92300`; `| gzip | wc -c` → `13002` (ceilings 125,646 / 14,517; was 92,127 / 12,902: +173 B for the new named groups and `>>` sequences). **GREEN.**
- ⟨`npx vitest run test/css`⟩ → `Test Files 7 passed (7) · Tests 75 passed (75)` before the pin file; with it, see read 2. ⟨`npm run -s test:css-equivalence`⟩ → `Tests 19 passed (19)`, `28 MIRROR-DEFECTS 0`. **GREEN.**
- ⟨`npx vue-tsc -p tsconfig.{lib,demo,test}.json --noEmit | grep -c 'error TS'`⟩ → `0 · 0 · 0` (a first `lib`/`test` read of 1 each — `rest[i]` possibly `undefined` under `noUncheckedIndexedAccess` in the new `listOf` loop — cured before the commit with `for (const item of rest)`). **GREEN.**
- ⟨`npx vitest run`⟩ → `Test Files 4 failed | 70 passed (74) · Tests 9 failed | 978 passed (987)`: `test/ink.test.ts` ×5 (Track A's dirty working-tree edit, not W7's), `test/spectrum-luma.test.ts` C-5 BORN-RED, `demo/test/generate/generate-rail.test.ts` EC-10 ×2, `demo/test/shell/reka-binding-idiom.test.ts` NG-6 — **all foreign; 0 CSS failures.**
- **L-6** ⟨`git diff HEAD --stat -- package.json package-lock.json | wc -l`⟩ → `0`. **Held.**
- **Read 2:** L-G2 ⟨same⟩ → `mismatches 92 (ASCII-only sources 1 · CP-CASE 4)` = 88 + 4 → **GREEN ×2**. `.cp` pins ⟨`npx vitest run test/css/custom-property-case.test.ts`⟩ ×2 → `Tests 4 passed (4)` both; ⟨`grep -n "V-A64" CHANGELOG.md`⟩ → `:11` → **GREEN ×2**. ⟨`npx vitest run test/css`⟩ (with the pin file) → `Test Files 8 passed (8) · Tests 86 passed (86)` (incl. `bbnf-refusal.test.ts` and the deep-frozen pin `bbnf-frozen.test.ts`) → **GREEN**.

**5. The final L-G1 reads — L-13, the quiet-host protocol (this seat, the predecessor's resumption; 2026-09-25).**
- **Resumption.** The predecessor seat committed `5d46e376` (cure) and `daa8e85a` (bench records) and wrote this receipt through act 4's read 2, then stopped before the final reads (killed). This seat: ⟨`git status --porcelain -- src bench test/css`⟩ → empty; the only dirty path in the writable set was this record (the receipt above, 56 lines, judged against the spec and kept as written); ⟨`git diff --stat 5d46e376 HEAD -- src/css`⟩ → empty (the product bytes are the cure's); ⟨`node bench/paired/build.mjs`⟩ → `srcDirty ""`, `bankedManifestOk 79/79`, `retiredAt 2155142b`.
- **Bounded wait.** A detached waiter (scratchpad `l4final/wait.sh` then `wait2.sh`; the first background waiter was killed by the host at 16:45 with exit 144 and re-armed at once) polled `sysctl -n vm.loadavg` every 20–30 s from **16:29:49 to 18:00:04** (90 min) for a 1-minute load < 8. ⟨`cat l4final/status`⟩ → `NEVER-QUIET 18:00:04`. The load log (240 samples) reads a minimum 1-minute load of **9.15** at the open (16:29, before the wait) and **26.85** inside the window, a maximum of **256.15** (17:19–17:33 peaks 194–256; sibling fleets). **The host never quieted.**
- **The reads recorded with their load, not claimed.** ⟨`node bench/paired/browser.mjs l4final-ff firefox 4 large,rej 11 product parseStylesheet`⟩ (load 42.74 → 35.86; record moved to scratchpad `l4final/`, ⟨`git status --porcelain bench src | wc -l`⟩ → `0`): Firefox **large parseStylesheet** clean reps **1.023 · .88**, set-aside 10 (.966 · .954 · 1.02 · 1.026 · .944 · 1.0 · 1.091 · .891 · 1.0 · .932); Firefox **rej parseStylesheet** clean **1.024 · .946 · .892**, set-aside 9 (.78–1.099). At this load the rejected cell — GREEN in all 8 clean reps of the RESUME 5 Close — itself reads a 1.024 clean rep: the host, not the product, sets the reading. **L-G1 Firefox large parseStylesheet: UNREAD** (reason: 1-minute load never below 8 in the bounded 90-min window; minimum 26.85). The full L-G1 matrix (node · Chromium · WebKit · Firefox × {acc, rej} + large, ×2) was not re-run on the final bytes at load 27–256: a read there would not count under L-13, and it would load the three sibling tracks further.
- **What stands on the final bytes.** The bisection's paired arms (act 2, same page, load 26.9–33.8) read E5 = the shipped cure below 1 on every clean Firefox large rep (.889 · .947 · .797 · .962) and below its own base in 7 of 8 reps; V8 allocation per pass 6,245 → 4,967 KB. These are unit evidence, not the gate.
- **Tooling restored (L-4).** ⟨`rm node_modules/@mkbabb/bbnf-lang && mv node_modules/@mkbabb/.bbnf-lang-0.1.4-aside-l4 node_modules/@mkbabb/bbnf-lang && grep '"version"' …/package.json`⟩ → `"version": "0.1.4"`; bbnf-lang worktree ⟨`status --porcelain | wc -l`⟩ → `0` at `f0059db14` (= origin; no emitter change in this unit).

**Gate table, BEFORE → AFTER** (BEFORE = RESUME 6 Baseline; AFTER = the final bytes `5d46e376` on emission `c1c91dad…`, emitter `f0059db14`).

| Gate | BEFORE | AFTER | Verdict |
|---|---|---|---|
| L-G1 Firefox large parseStylesheet | x1.056 (born-RED, load 25–44) | bisection E5 .889 · .947 · .797 · .962 (unit evidence, load 27–34); final read at load 43→36: clean 1.023 · .88, 10 SA | **UNREAD** (L-13: never quiet in 90 min, min load 26.85) |
| L-G1 every other cell (node · Chromium · WebKit · Firefox, acc/rej/large) | GREEN (RESUME 5 Close) | not re-read on the final bytes (L-13) | **UNREAD on the final bytes** |
| L-G2 V-1 | 92 = 88 + 4 | 92 = 88 F-b-4 + 4 CP-CASE ×2 | **GREEN ×2** |
| L-G3 | 0 | 0; `plainEqualsShipped true` | **GREEN** |
| `.cp` pins + V-A64 | 4/4 · `:11` | 4/4 ×2 · `:11` | **GREEN ×2** |
| css-equiv · test/css · npm test · vue-tsc · `--check` · K2-b · V-8 | GREEN | 19/19, 28× MIRROR-DEFECTS 0 · 8/86 (+11 pins) · 978/987 (9 foreign) · 0/0/0 · current ×2 (`c1c91dad…`) · 0 · 0 | **GREEN** |
| E-1/E-3/E-5 · E-2 · E-6 | GREEN | 21/299 ×2 consecutive (1 contention flake in run 1, emitter untouched), tsc 0 · 200,137,551 checks, 0 violations, 0 modeDiffs · 92,300 / 13,002 B | **GREEN** |
| whole-7/7 inside ceilings | GREEN | not re-read (timing; L-13) | **UNREAD on the final bytes** |
| L-6 no npm/dep move | held | `package*.json` diff 0 | **held** |

**Commits (this unit):** value.js `5d46e376` (action + grammar + re-emission + pins; the predecessor seat) · `daa8e85a` (bench records + instruments; the predecessor seat) · this receipt's commit. bbnf-lang: none.

**Residuals.**
- **R-l4-1 (the quiet-host read).** L-G1 on the final bytes, all four engines ×2, is owed at a 1-minute load < 8. Owner: the close seat or the next Track-D sitting, under L-13; the host carried the three sibling tracks through the whole window.
...
## Check 1 of the RESUME 6 Close (2026-09-25, L-20 fresh adversarial pass 1, `claude-opus-5-5`, Track D; verify-only, cures nothing)

**Read:** W7.md whole (268 lines; ADDENDUM (f) `:262` binds); this record's `## RESUME 6 — Open` through `## RESUME 6 Unit plan`, the `.l4` receipt, and `## RESUME 6 Close`. **Crash-recovery:** ⟨`git status --porcelain -- src bench test/css package.json package-lock.json docs/tranches/X/execution/D docs/tranches/X/execution/LEDGER.md`⟩ → empty; bbnf-lang worktree ⟨`git log --oneline -1`; `status --porcelain | wc -l`⟩ → `f0059db14` (= `origin/x-p-w7-typescript`), `0`. Host load at this check: ⟨`uptime`⟩ → 1-min 51.92 (sibling fleets).

### Axes
1. **Claimed GREENs reproduce (13 re-run, 0 fail).** L-G2 ⟨`node bench/paired/equiv.mjs product <scratch>/eq{1,2}.json`⟩ ×2 → `compared 1376531/1376531 · mismatches 92 (ASCII-only sources 1 · CP-CASE 4)`, missingRows 0, both · L-G3 ⟨`grep -rln "instrument\|__prof\|PC\[\|NOW()" src/css/bbnf/generated | wc -l`⟩ → `0` · K2-b ⟨`grep -rn deepFreeze src | wc -l`⟩ → `0` · V-8 → `0` · E-4 ⟨`node ../bbnf-lang-x-p-w7-typescript/typescript/dist/cli.js gen src/css/grammar/css.bbnf --actions <esbuilt actions.ts> --out src/css/bbnf/generated/grammar.js --entries <gen-grammar.mjs's 24> --check`⟩ ×2 → `current (sha256 c1c91dad543dd45a…)` exit 0 both (the worktree CLI invoked directly; node_modules untouched, still `0.1.4`) · E-6 ⟨`npx esbuild --minify … | wc -c`; `| gzip | wc -c`⟩ → `92300` / `13002` · `.cp` pins ⟨`npx vitest run test/css/custom-property-case.test.ts`⟩ ×2 → `4 passed (4)` both; ⟨`grep -n V-A64 CHANGELOG.md`⟩ → `:11` · ⟨`npx vitest run test/css`⟩ → `8 passed (8) · 86 passed (86)` · ⟨`npm run -s test:css-equivalence`⟩ → `Tests 19 passed (19)`, ⟨`grep -oE 'MIRROR-DEFECTS [0-9]+' | sort | uniq -c`⟩ → `34 MIRROR-DEFECTS 0` (only value) · vue-tsc lib/demo/test → `0 · 0 · 0` · bbnf-lang ⟨`npx vitest run`⟩ ×2 → `21 passed (21) · 299 passed (299)` both, ⟨`npx tsc --noEmit -p .`⟩ → `0` · L-6 ⟨`git diff HEAD --stat -- package.json package-lock.json | wc -l`⟩ → `0`. Not re-run (cited): E-2 routing audit (200M checks, linked emitter) and full `npm test` (foreign failures only).
2. **Writes in bounds.** ⟨`git show --stat 5d46e376 daa8e85a ef91a5a3 2015419a 214e1fc1`⟩ → `src/css/bbnf/{sheet,stylesheet,value}.ts`, `generated/grammar.{js,d.ts}`, `src/css/stylesheet.ts`, `test/css/bbnf-success-path.test.ts`, `src/css/grammar/{stylesheet,value}.bbnf` (declared §0bt adjacent edits: the builders destructure exactly those `>>`/`<<`/group shapes; same repo, same concern; equivalence unchanged at 92) · 20 `bench/**` paths · the record · LEDGER. `scripts/dev/dev.sh`, `package*.json`, `ci.yml`, `src/css/result.ts`: absent from every commit. **0 landed-wrong.**
3. **No masking.** ⟨`git show 5d46e376 -- src test/css | grep -nE '^\+.*(try|catch|\.skip|allow|userAgent|navigator)'`⟩ → 0 hits; the one `.length >` hit is `nested.length > 0` (a structural empty-children test, not an input-length heuristic). No engine split (L-9 held).
4. **Families:** cure + re-emission + pins one commit; records one; receipt one; close one; LEDGER one. Not split.
5. **E-3:** ⟨`git diff 82d5c74d..HEAD --stat -- docs/tranches/X/parse-that/waves docs/tranches/V/megatranche/registry/adjudicated`⟩ → empty. (COHESION moved 38 lines in the window, by no commit of this wave.)
6. **Mail:** ⟨`grep -cE '\| *UNREAD *\|' docs/tranches/V/coordination/INBOX.md`⟩ → `0`.
7. **Four-verb line:** the close stamped PARTIAL, not IMPLEMENTED/VERIFIED — lawful for an unread binding gate.
8. **Goal criterion at the bytes:** NOT MET as read. The goal (faster than the retired parser on every entry, every engine, ADDENDA (b)/(c)/(e)/(f)) has no GREEN read of L-G1 on the final bytes `5d46e376` on any engine: every cell was last read GREEN on the `.l3` bytes, and the Firefox large cell was born-RED at open (×1.056).
9. **Published figures reproduce:** 1,376,531 rows / 92 = 88 + 4 · 92,300 / 13,002 · 8/86 · 19/19 · 21/299 · sha256 `c1c91dad…` — all match.
10. **Honest-RED adjudication.** L-G1 (Firefox large accepted `parseStylesheet` born-RED ×1.056; every other cell UNREAD on the final bytes): **no spec relief.** It is not producer-owned (the emitter and actions are this wave's), no later wave is routed it (W7P takes only the release acts, §0cp), and ADDENDUM (f) refuses a noise band as "relief by another name"; (f) 3 permits recording an unread cell, not closing on it. ESC-W7l4-1 is unruled (⟨`grep -n ESC-W7l4-1 docs/tranches/X/COHESION.md`⟩ → 0 hits). The release gates P-6 · E-8 · V-9 + CI step · Z-1 · Z-3 publish half ARE relieved (ADDENDUM 2026-09-24 §1, X.P.W7P).

### Register
| # | Severity | Claim | Receipt | Cure |
|---|---|---|---|---|
| C1-1 | HIGH | L-G1 — the wave's binding speed gate — is unread on the final bytes on every engine, and its last Firefox-large read is RED (×1.056 BEFORE; loaded clean reps 1.023 · .88 · 1.012); no spec relief. The close states this honestly (PARTIAL); it is a gate failure, not a close defect. | RESUME 6 Close Act 2 / Act 5; `.l4` receipt §5; this check's load 51.92 | The quiet-host L-G1 ×2 read on all four engines (R-l4-1) in a Track-D sitting with the sibling fleets paused, or the owner's ruling on ESC-W7l4-1 / R-l4-2 (equal-work large cell). A repair seat cannot cure it by product bytes. |
| C1-2 | INFO | The one "ASCII-only" mismatch in V-1 sits inside the 4 CP-CASE rows (92 = 88 F-b-4 + 4 CP-CASE), as prior checks read. | eq1/eq2 JSON | none |
...
## .gap2 receipt (single seat, ADDENDUM (i))

Seat `claude-opus-5-5`, 2026-09-25, Track D, single seat. Governing text: W7.md ADDENDUM (i) (the unit) under ADDENDUM (h)'s law; this record's `.gap` receipt (the method). Owner law: CSS grammar only in BBNF.

**Emitter pin.** As `.gap`: `node_modules/@mkbabb/bbnf-lang` (0.1.4) moved aside, the bbnf-lang worktree's `typescript/` (`f0059db14`, worktree clean before and after, its `dist/` as built at `.gap`; nothing written there) linked for the unit's life, restored at the end (⟨`grep '"version"' …`⟩ → `0.1.4`). ⟨`node scripts/gen-grammar.mjs --check`⟩ before any change → `current (sha256 6eba8334…)`: the pinned emitter reproduces HEAD's emission.

**The grammar changes (`src/css/grammar/value.bbnf`; actions in `src/css/bbnf/value.ts`).**
1. **`lineNames`** `= /\[\s*(?:(?:-?[a-zA-Z_]|--)[\w-]*(?:\s+(?:-?[a-zA-Z_]|--)[\w-]*)*)?\s*\]/`, a `valueTerm` before `operator`: css-grid-2 §7.2 `<line-names> = '[' <custom-ident>* ']'`, identifiers alone (the empty `[]` included, per `*`). Action `lineNamesValue`: one keyword spelled canonically (`[  a   b ]` → `[a b]`), so it serializes to text that reads back equal. Any other `[]` block stays refused: `[1px]`, `[a, b]`, `["a"]`, `[[a]]`, `[a/b]`, `[a;b]`, `[a()]`, `[a`, `a]`, `[a]b`, `[a](b)`.
2. **`urlCall`** body optional, `/\(\s*/ >> ( urlBody ? ) << close`, and **`urlHead`** `/url\((?!\s*["'])/i` (no longer excluding `)`), so `url()` / `url( )` are the empty `<url-token>` (css-syntax-3 §4.3.6), never a `call`. Action `urlValue`: no body → the `url` call with no arguments (`serializeCssValue` spells `url()`). A bad url stays refused.
3. **`mathSpace`** separated by **`mathSep = termSep | /\s*(?=\*)|(?<=\*)/`**: inside a math function a `*` stands directly against its neighbours (css-values-4 §10.1: only `+`/`-` need whitespace). `calc(1px*2)` ≡ `calc(1px * 2)`. `/` needed no change (`calc(4px/2)` already read; it is the slash list's separator). Guards: `calc(1px+2px)`, `calc(1px+ 2px)`, `foo(1*2)`, `1px*2` stay refused.
- Re-emitted ⟨`node scripts/gen-grammar.mjs`⟩ → `sha256 96c3fa6303735a4943068565fa8c82e72835271c601e1811ed1d8b022e80b191`. The emitter is unchanged; no producer row.

**Falsifiers** — `test/css/bbnf-gap.test.ts` +11 tests (25 total; sections 4 line names, 5 empty url, 6 unspaced `*`; each form round-trips `parse → serializeCssValue → parse` deep-equal, the corpus declarations `grid-template-columns:[rail] var(--rail-track) [stage] minmax(0, 1fr)` and `margin-inline: calc(2*var(--spacing)*-1)` parse inside `parseStylesheet`).
- **RED before:** the file against HEAD `3d06dc97`'s `src/` (a `git archive` copy with `test/`, `tsconfig*.json`, `vitest.config.ts`) → `Tests 8 failed | 17 passed (25)`: all 8 new acceptance tests fail; the 3 new stay-refused guards and the 14 `.gap` tests pass.
- **GREEN after ×2:** ⟨`npx vitest run test/css`⟩ → `Test Files 9 passed (9) · Tests 111 passed (111)` ×2.

**Gates (final bytes, value.js `08331dfa`).**
- **L-G2** ⟨`node bench/paired/equiv.mjs product`⟩ ×2 (after ⟨`node bench/paired/build.mjs`⟩, `bankedManifestOk 79/79`) → `compared 1376531/1376531 rows · mismatches 396 (ASCII-only sources 300 · CP-CASE 4)` both reads, rows identical (JSON equal). A HEAD baseline read on the prior bytes (`src` files swapped to `git show HEAD:` for one build, then restored) → 255. Row for row against it: **254 of 255 identical** (the standing 92 all identical); **1 moved**: `collect:parseStylesheet(sheet)` on the keyframes-js-index sheet (a `.gap` GAP-URL row) now refuses at `[top]auto[stage]1fr[bottom]auto` (0–31) instead of `[rail]` (0–6), since the sheet now reads past `[rail]`. **396 = 88 F-b-4 + 4 CP-CASE + 163 §18 + 141 §18-A.**
- **Prefix equivalence** → `sheets 4 · rows 528 · declarations 1405 · refused 0 · mismatches 0` ×2. **GREEN.** Corpus not regenerated (E-3).
- **L-G3** ⟨`grep -rln "instrument\|__prof\|PC\[\|NOW()" src/css/bbnf/generated | wc -l`⟩ → `0`; ⟨`node bench/paired/instrument.mjs`⟩ → `"plainEqualsShipped":true`, `"shippedInstrumentTokens":[]`. **GREEN.**
- **E-4** `--check` ×2 → `current (sha256 96c3fa63…)` both. **E-2** ⟨`node bench/paired/audit.mjs`⟩ → `rules 171 · sources 29944 · sheets 4 · checks 325136977 · violations 0 · modeDiffs 0`. **E-6** minified 98,922 B / gzip 13,786 B (ceilings 125,646 / 14,517; was 97,876 / 13,595). **K2-b** 0 · **V-8** 0. **GREEN.**
- ⟨`npm run -s test:css-equivalence`⟩ → first read `4 failed | 15 passed`: MIS_ACCEPT assay 5 (`url()` `url( )` `url(  )` `url(\t)` `url(\n)`) and real 3 (the three `[…]` line-name values). `bench/css-equivalence/w6-classes.ts` gains two mechanism classes (repair, never match): **GAP-URL0** (`url()` spelled `url("")`) and **GAP-LN** (each line-names block spelled the ident `x`). Re-read ×2 → `Tests 19 passed (19)`, every entry MIRROR-DEFECTS 0 (assay GAP-URL0 5, real GAP-LN 3); stylesheet `STYLESHEET DEFECTS 0`. **GREEN.**
- ⟨`npx vue-tsc -p tsconfig.{lib,test}.json --noEmit | grep -c 'error TS'`⟩ → `0 · 0`; ⟨`npx eslint src/css test/css bench/css-equivalence/w6-classes.ts`⟩ → exit 0; ⟨`npm run build`⟩ → `built`, exit 0. **GREEN.**
- **L-6** `package*.json` diff 0. **No timing claim**: no bench was timed; L-G1 ×2 on the final bytes is owed in the orchestrator's quiet window (R-l4-1).

**The named equivalence delta (DIVERGENCE-LEDGER §18-A, beside §18).** 141 new rows, none non-ASCII:
- **GAP-URL0 65** (5 sources `url()`, `url( )`, `url(  )`, `url(\t)`, `url(\n)` × 13): 10 accepted (the two value entries), 55 moved to `syntax_mismatch` (`coerceToSyntax` ×11).
- **GAP-LN 63**: 3 real sources × 13 (6 accepted, 33 `syntax_mismatch`) + 12 sheet texts handed to `parseCssValue`/`parseCssValues` (24 rows) whose leading `[data-x]` now reads as line names, the refusal moving from `[data-x]` to the `{` after it.
- **GAP-MUL 13** (`color-mix(in hsl, red calc(var(--v)*1%), blue)`): 2 accepted (a generic call in the value entries), 11 `syntax_mismatch`. No differential cell exists, so no w6 class.
- Sections: `entry:parseCssValue(s)` 21+21 · `public:coerceToSyntax(<…>)` 9 × 11.

**Commits:** value.js `08331dfa` (grammar + actions + re-emission + pins + value-differential classes + CHANGELOG `[Unreleased]` + DIVERGENCE-LEDGER §18-A) · this receipt. bbnf-lang, parse-that: none.

**Open (named owners; listed, not cured).**
- **Line names juxtaposed with no whitespace** (`[top]auto[stage]1fr[bottom]auto`, minified; css-syntax-3 tokenizes `]` then the next token, no whitespace needed): the keyframes-js-index sheet now stops there (the one moved §18 row). Owner: the orchestrator (a later BBNF unit).
