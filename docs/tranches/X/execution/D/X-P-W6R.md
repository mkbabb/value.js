...
SERVED MODEL: claude-opus-5-5 — REPAIR SEAT (round 1), 2026-09-23 ~20:15–20:30 EDT. Register: Check 1 (D-1 HIGH · D-2 INFO · D-3 INFO).

- **Crash-recovery.** ⟨git status --porcelain -- bench .github package.json⟩ → empty: no inherited partial work. `package.json` is now clean (X-W7R's pin has been committed), and this repair does not touch it (lock L-1 is moot). parse-that is not written. The dirty rows `scripts/dev/dev.sh`, `docs/tranches/V/reformation/CARRY-LEDGER.md` and the sibling evidence dirs belong to other seats and are untouched.

### D-1 (HIGH) → cure → commit `73d344a2`
- **Cause 1: depth-1 checkout.** `bench/retired.ts:36-37` resolves `RETIRED_AT:src/css/grammar.ts` from git history, and `stylesheet.measure.test.ts` `sheetsAt(REPO, real.provenance.valueJs, "demo")` reads the demo sheets at a pinned commit. The producer job's checkout used depth 1, the default. **Cure:** ci.yml producer job `actions/checkout@v4` now sets `with: fetch-depth: 0` (ci.yml:47-49), with a comment naming both readers (ci.yml:43-46). This follows the same idiom as the existing job at ci.yml:320.
- **Cause 2: untracked sibling.** `stylesheet.measure.test.ts:67` read keyframes.js sheets from `../keyframes.js`. **Cure (the register's first option):** keyframes.js's sheets at `real.provenance.keyframesJs` (`9bdcdad5`) are frozen into the tracked file `bench/css-equivalence/keyframes-sheets.json`, which has 65 sheets plus `provenance` and `sheetsSha256`. The file is generated by `bench/css-equivalence/freeze-keyframes-sheets.mjs`. The generator uses the test's own `sheetsAt` logic: `git ls-tree` at the sha, `.css` + `<style>` blocks, and blank sheets dropped. It supports `--check`. The test now reads the frozen file. It **throws** if the file's `provenance.keyframesJs` ≠ `real-corpus.json`'s, so the two corpora cannot drift apart. This is a hard failure, not a fallback.
- **Adjacent edits (§0bt, same concern: `.c`'s instrument runs in CI):**
  - `bench/css-equivalence/stylesheet.measure.test.ts:8-9` (header) and `:65-71` (the `whole` sheet list now reads the frozen file, with the provenance guard). Reason: the register's cure names this edit as adjacent-line.
  - `bench/css-equivalence/freeze-keyframes-sheets.mjs` (new, 45 lines) and `bench/css-equivalence/keyframes-sheets.json` (new, generated). Reason: they are the tracked corpus file the cure prescribes.
- No skip, no `continue-on-error` (⟨git diff 73d344a2~1 73d344a2 -- ci.yml \| grep -c continue-on⟩ → `0`), no allowlist.

### Gate re-readings (write-then-measure, double-run)
| Gate | Command | Reading |
|---|---|---|
| D-1 gate (CI recipe, no sibling) | ⟨`git clone --depth 1 --branch tranche-u file://…/value.js vj` (actions/checkout's default) → `git fetch --unshallow` (fetch-depth 0) → symlink node_modules → `test -e ../keyframes.js` → `NO-SIBLING`; fresh `TMPDIR` each run → `npm run test:css-equivalence`⟩ x2 | run 1 `exit 0 · Test Files 2 passed (2) · Tests 19 passed (19)`; run 2 the same. The fresh TMPDIR ends holding `value-js-retired-2155142bad8b`, so the retired parser was materialized from the clone's own history |
| negative control (the fetch-depth half is load-bearing) | ⟨the same depth-1 clone, no unshallow⟩ | `exit 1` · `fatal: path 'src/css/grammar.ts' does not exist in '2155142b…'` |
| C-1 | ⟨`grep -n "css-equivalence\|fetch-depth" ci.yml`⟩ · package.json | `ci.yml:76 - run: npm run test:css-equivalence` · `ci.yml:49 fetch-depth: 0` · `package.json:69` |
| C-2 (host) | ⟨`npm run test:css-equivalence`⟩ x2 (before and after commit) | `Test Files 2 passed (2) · Tests 19 passed (19)` both; the sheet census still reads `32021 sheets: STYLESHEET DEFECTS 0`, which is the same count as before the cure, so the frozen set equals the sibling read |
| corpus freshness | ⟨`node bench/css-equivalence/freeze-keyframes-sheets.mjs --check`⟩ x2 | `keyframes-sheets.json matches (65 sheets @ 9bdcdad5c1dbc50ebab033930388f583360c45a4)` both |

Gates `.l` (L-1..3) and `.p` (P-1..6) touch no path this cure moved, so they stand as banked in Check 1.

### D-2 / D-3 (INFO)
- D-2: no cure (immutable evidence, E-3).
- D-3: no cure inside W6R's bounds (W7.md is a dated spec, E-3). It is routed to the W7 opener or the orchestrator, as the register says. It is not escalated, because it is INFO.
