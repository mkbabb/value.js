...
- **R-c-3: out of scope and named in the test.** `calc()` in channels and percentages (18 WPT mix cases, 35 legacy) needs css-values-4 §10 in the colour grammar. `color(display-p3-linear …)` inputs (76 cases) are outside the frozen `CssColorSpace`; `in display-p3-linear` IS admitted. `color(xyz-d50 … none …)` (22 cases) is covered by the standing "concrete xyz-d50" guard. Relative colour syntax (`from`) is not a production.
