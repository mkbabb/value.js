...
**SERVED MODEL: claude-opus-5[1m]** — L-20 FRESH ADVERSARIAL PASS 1, **VERIFY-ONLY**. An independent
seat that authored **no** byte of `W4.md`, of `SEAM-CONTRACT.md`, of either `<p2>` script, of any
evidence bank, of any unit receipt or of `## Close`. Wall clock **2026-09-19**; sitting of record
**2026-09-17** (the owner's begin-word, COHESION §0j). Every reading below was produced by a command
this seat ran itself against the settled bytes; **nothing is inherited from the close**.

**VERDICT: NOT-CONFORMANT.** **0 BLOCKER · 0 CRITICAL · 3 HIGH · 2 MINOR · 3 INFO.**
**All ten gate verdicts reproduce · all four claimed GREENs reproduce GREEN · 0 published figure
failed to reproduce · 0 write outside §File Bounds · 0 masking construct · E-3 held · mail 0 UNREAD
· the four-verb line correctly did not move.** The row is **NOT promoted** and stays
**`PARTIAL`**: **five of the six RED gates have no relief at the spec's own bytes** — they are RED
because `.c` and `.d`, two of this wave's four units, **were never dispatched**, and `W4.md` routes
neither unit's work to any later wave. §2a's goal criterion fails on its own terms.

### Crash-recovery (standing law, first act)

⟨cmd⟩ `git -C /Users/mkbabb/Programming/value.js status --porcelain` → **3 rows**:
`M docs/tranches/V/reformation/CARRY-LEDGER.md` (a sibling's / the owner's) · `M scripts/dev/dev.sh`
(**unowned, NEVER touched, never staged**) · `?? docs/tranches/X/keyframes/evidence/W11/j-close-2026-09-19.md`
(**Track B's**). ⟨cmd⟩ `git status --porcelain -- docs/tranches/X/execution/D/X-P-W4.md
docs/tranches/X/execution/LEDGER.md docs/tranches/X/parse-that/` → **no output**.
⟨cmd⟩ `git -C <p2> status --porcelain` → `?? .worktrees/` alone. The frozen read-only root
`/Users/mkbabb/Programming/parse-that` carries **31 dirty rows** under `rust/**` — **none of it this
wave's**, that root is §4 Do-NOT-touch, and not one byte of it was read into a write, stashed or
restored. **Zero dirty paths inside this seat's writable set · zero inherited hunks · no inherited
path to name.**

### The ten gates, RE-RUN BY THIS SEAT — 10 of 10 verdicts reproduce

| gate | the close's verdict | this seat's own command | reproduces? |
|---|---|---|---|
| **G-1** | GREEN | the spec's **literal** command → `rows: contract 52 · universe 52 · ledger rows 47` · `contract ∖ universe-52 : ∅` · `universe-52 ∖ contract : ∅` · `PENDING-ADJUDICATION 45 · identical 7` · *"VERDICT: GREEN"* · **EXIT=0**; double-run, ⟨cmd⟩ `diff` → **no output** | **YES — GREEN** |
| **G-2** | GREEN (both legs) | leg 1 ⟨cmd⟩ `git status --porcelain -- src api demo test e2e` → **0 lines**, double-run `0 ≡ 0`; leg 2 ⟨cmd⟩ `git show --name-only --format= <sha> \| grep -E '^(src\|demo\|api\|test\|e2e)/' \| wc -l` over **all seven** value.js commits of this wave → `0·0·0·0·0·0·0` (the close ran four; this seat ran seven, adding its own three) | **YES — GREEN** |
| **G-3** | RED | ⟨cmd⟩ `node scripts/packed-candidate-surface.mjs --seam …/SEAM-CONTRACT.md` → `candidate "@mkbabb/parse-that@1.0.0"` · **`tarballSha256 efc11936d361820526a93ada2145012d0c58cec48cee9ccb9a19c22c66e74cda`** (**byte-identical to `.b`'s bank**) · `entryCount 1` · `seamSubpathDeclared false` · **`resolved "0 of 52"`** · `refusals 5` · `G3 "RED"`, **EXIT=1** | **YES — RED** |
| **G-4** | GREEN | the §6 **verbatim** one-liner against `<p2>/typescript/src/css/build/ac1.wasm` → `{"total": 0, "functionKind": 0, "imports": []}`, **EXIT=0**; then ⟨cmd⟩ `node scripts/wasm-admission.mjs <every project artefact>` → `tally {artifacts 9 · admitted 9 · functionKindImportsTotal 0 · unaccountedImportsTotal 0}` · `"verdict": "GREEN"` · **EXIT=0** | **YES — GREEN** |
| **G-5** | RED | ⟨cmd⟩ `ls <p2>/typescript/scripts/rc-p-evaluate.mjs` → **No such file or directory** | **YES — RED** |
| **G-6** | RED (leg 3) | ⟨cmd⟩ `sed -n '9p' KF-W3.md` and `sed -n '459p' F-W0.md` → **both far-end sentences present VERBATIM, each citing `RC-P` by predicate name and no wave number** (legs 1–2 GREEN-BEFORE-CURE); ⟨cmd⟩ `ls …/RELEASE-CONDITION.md` → **No such file or directory** — the file **both** landed sentences cite by path does not exist; `reciprocity-grep.txt` unbanked | **YES — RED** |
| **G-7** | GREEN (floor) | ⟨cmd⟩ `grep -c 'parse-that' ../fourier-analysis/package.json` → **0**; ⟨cmd⟩ over `web/package.json` → **0**. The transitive copies untouched | **YES — GREEN** |
| **G-8** | RED (packet leg) | ⟨cmd⟩ `ls …/RELEASE-PACKET.md` → **No such file or directory**; mail leg by **position** → **UNREAD 0** over **79** register rows, tail `O-39` | **YES — RED** |
| **G-9** | RED | ⟨cmd⟩ `ls waves/W4-CLOSE.md` → **No such file or directory** — the disposition is pre-ruled (C) at COHESION §0i.1 and has **no gate artefact to be recorded in** | **YES — RED** |
| **G-10** | RED | ⟨cmd⟩ `cat waves/W0.md W1.md W2.md W3.md W4.md \| grep -c 'VERIFIED \| \*\*NO\*\*'` → **7** (close-state must read **0**); `W4.md` §2's four-verb table **byte-untouched** | **YES — RED** |

**Published figures, re-measured at the settled bytes** (write-then-measure): `SEAM-CONTRACT.md`
**121,820 B**, ⟨cmd⟩ `shasum -a 256` → **`e269f6e665f288948a58ff1c6ac24e4e252e044a91806c5a832a0f0a09fe0818`**
— the exact digest `.a` and `.b` independently recorded; `packed-surface.json` **128,861 B** ·
`wasm-imports.json` **9,392 B** · `value-source-untouched.txt` **3,312 B** ·
`seam-contract-check.txt` **4,487 B** · `seam-contract-negative-controls.txt` **4,727 B**. §8's
census re-counted over the twelve rows → **4 PRESENT · 1 PARTIAL · 7 ABSENT**; all seven absent
confirmed absent by ⟨cmd⟩ `ls`. **0 of 0 figures failed.**

### Axis audit

| axis | finding |
|---|---|
| (2) **File Bounds** | **0 writes outside §4.** ⟨cmd⟩ `git show --name-only --format=` over all **seven** value.js commits and both `<p2>` commits: every path is `docs/tranches/X/parse-that/{SEAM-CONTRACT.md,scripts/seam-contract-check.mjs,evidence/W4/**}` · `docs/tranches/X/execution/D/X-P-W4.md` · `docs/tranches/X/execution/LEDGER.md` · `<p2>/typescript/scripts/{packed-candidate-surface,wasm-admission}.mjs` — every one a named §4 row. **`scripts/dev/dev.sh` in 0 commits** (⟨cmd⟩ `git log --all -- scripts/dev/dev.sh` → last touch `85cfea2c`, tranche T) and still dirty-unstaged as the standing arrangement requires |
| (3) **Masking** | **0 constructs.** ⟨cmd⟩ over the whole wave diff in both roots for `.skip` / `xit(` / `@ts-ignore` / `@ts-expect-error` / `eslint-disable` / allowlist / `node_modules/` patch → no authored hit. The four `try/catch` in the two `<p2>` scripts were read whole: each **records** a failure into a measured field (`instantiation.ok=false` + the error string; `resolved:false` + a `reason`) and none swallows one — `verdict.emptyImportInstantiation` and `verdict.functionKindZero` are separate fields, neither inferred from the other, exactly as G-4's falsifier demands. `packed-candidate-surface.mjs` exits **1** on its own RED rather than narrowing. No producer selector copied; no local `node_modules` patch |
| (4) **Commit families** | §9's four declared messages: `.a`'s landed **verbatim** (`b4f8e5d1`), `.b`'s landed verbatim (`ca615a6` ⊕ `f92a3f32`, mirrored across the two roots because they are two repositories), `.c`'s and `.d`'s never landed because those units never sat. `.b`'s second pair (`00806a8` ⊕ `046bc176`) is a **distinct meaning** (the seam reader's name location), not a split family. **No declared family was split**; one commit per meaning throughout |
| (5) **E-3** | **HELD.** No commit of this wave touches `docs/tranches/V/megatranche/registry/adjudicated/**`, any conformance artefact, `docs/tranches/X/waves/**`, `docs/tranches/X/{keyframes,fourier}/**`, `docs/tranches/X/COHESION.md`, `docs/tranches/V/coordination/INBOX.md`, `docs/tranches/V/apotheosis/**`, or any sibling X·P wave spec. ⟨cmd⟩ `git diff e914453f..HEAD -- docs/tranches/X/parse-that/waves/` → **no output**: `W0..W4.md` are byte-untouched, and `W4.md`'s own §2 four-verb table with it |
| (6) **Mail** | **0 UNREAD.** By **position** (never a bare `grep -i unread`): **0**, over **79** register rows, tail `I-35`/`O-39`. Re-measured at the close's own commit `ee33cf80` → **85 `^\| ` rows / 79 register rows**, the close's published denominators **exactly**; the live file now reads 94/79 because Track B and Track C appended after |
| (7) **Four verbs** | **NONE MOVED, and that is lawful.** G-10's falsifier — *"A stamp performed while any of G-1..G-9 is red fails — the act is the consequence of the gates, never a substitute for them"* — forbids IMPLEMENTED and VERIFIED at six RED gates, and R-A reserves VERIFIED to `.d` alone. The close seat stamped neither and touched no wave file |
| (8) **§2a goal criterion** | **NOT MET.** See HIGH-3 |
| (9) **Figures** | **all reproduce**, above |

### Register — severity · claim · receipt · cure

| # | severity | claim | receipt | cure |
|---|---|---|---|---|
| **HIGH-1** | HIGH | **`X.P.W4.c` was never dispatched, and G-5 is RED with NO relief at the spec's own bytes.** RC-P is §3 item 5, §6a's normative core and *"the sole edge from X·P into the consumer half"*; `W4.md` assigns it to `.c` **in this wave** and routes it to no successor. A second consequence rides on it: `KF-W3.md:9` and `F-W0.md:459` both cite `docs/tranches/X/parse-that/RELEASE-CONDITION.md` **by path**, and that path does not exist — two landed reciprocity sentences point at a file that was never written | ⟨cmd⟩ `ls <p2>/typescript/scripts/rc-p-evaluate.mjs` → *No such file or directory*; ⟨cmd⟩ `ls docs/tranches/X/parse-that/RELEASE-CONDITION.md docs/tranches/X/parse-that/evidence/W4/rc-p-evaluation.json docs/tranches/X/parse-that/evidence/W4/reciprocity-grep.txt` → *No such file or directory* (all three) | dispatch `.c` (Opus 5, serial; `.a` and `.b` have both sat and it is unblocked by neither G-3 nor F-w4c-1 — conjunct 4's command and a GREEN reading already exist, conjunct 1 is measured FALSE at V=4.0.0, and an evaluator that reports FALSE honestly is exactly what G-5 asks for) |
| **HIGH-2** | HIGH | **`X.P.W4.d` was never dispatched: G-8's packet leg, G-9's recording, G-10's stamp act, §3 items 7–10 and §8's four `.d` artefacts are all unperformed, with no relief.** The L-13 harvest is named by §3 item 10 as *"lane law … its omission is a recorded kill (W2 §1), not a housekeeping lapse"*, and `registry/harvest/x-p-w4.json` does not exist. G-9 is **pre-ruled (C)** at COHESION §0i.1 — that relieves the *ruling*, not the *recording*, and §6 G-9's falsifier is *"closing with the gap unstated"* | ⟨cmd⟩ `ls waves/W4-CLOSE.md RELEASE-PACKET.md docs/tranches/V/megatranche/registry/harvest/x-p-w4.json` → *No such file or directory* (all three); ⟨cmd⟩ `grep -c 'RELEASE-PACKET' INBOX.md` → the packet has no sent-row; ⟨cmd⟩ `git diff e914453f..HEAD -- docs/tranches/X/COHESION.md docs/tranches/V/coordination/INBOX.md` shows **no X·P cell** carved by this wave | dispatch `.d` (a **fresh** Fable adjudicator, M-23 §1) **after** the triumvirate rules F-w4b-3 and F-w4c-1 — the close's own sequencing note is right: dispatching the only seat that may stamp VERIFIED in front of an unruled halt condition is the failure mode, not the cure |
| **HIGH-3** | HIGH | **§2a's goal criterion is NOT MET at the bytes.** It requires three questions answerable from **one document each**. *What does value.js receive* — `SEAM-CONTRACT.md` exists, but **45 of its 52 rows publish `PENDING-ADJUDICATION`** rather than a terminal disposition, and the adjudicator who was to rule them (`.d`) never sat. *How does it arrive* — the protocol exists and its answer is **RED**: `entryCount 1`, `resolved 0 of 52`. *When may the consumer act* — **no document at all**. §2a: *"The wave fails if any of the three is answerable only by reading a status word"*; the third is not answerable even that way | the G-1 run's own disposition census (`PENDING-ADJUDICATION 45 · identical 7`); the G-3 run's `resolved "0 of 52"`, EXIT=1; ⟨cmd⟩ `ls RELEASE-CONDITION.md` → *No such file or directory* | HIGH-1 ⊕ HIGH-2 ⊕ F-w4b-3's ruling. The criterion is not separately curable: it **is** the three units' output |
| **MINOR-1** | MINOR | **The wave opened against a FAILED G-10 open-state assertion.** §6 G-10: *"A sum other than **5** at open fails and halts the act."* Seat 0 measured **7** and opened anyway, correctly surfacing the cause as a third the spec did not anticipate (`W3.md`'s sixth and seventh dated addenda reproduce four-verb tables at `:836` and `:971`, both E-3-lawful and unrewritable) and routing the counting question to the triumvirate as **F-w4c-1, UNRULED** | ⟨cmd⟩ `cat waves/W[0-4].md \| grep -c 'VERIFIED \| \*\*NO\*\*'` → **7**, double-run `7 ≡ 7`; per-file `W0:1 · W1:1 · W2:1 · W3:3 · W4:1` | **Mitigated**: the *act* G-10 halts (the R-A stamp) was never performed, so the halt was honoured in substance. The ruling is still owed — shape (a), narrowing the command to rows inside a `### Four-verb status` block, is the only one that survives a ninth W3 round |
| **MINOR-2** | MINOR | **ESC-e2's W3 harvest is measured and unlanded (F-w4c-2), and W4's own L-13 harvest is absent.** COHESION §0p makes the W3 harvest *"X.P.W4's seat 0 … at its open"*; seat 0 ran §0p's scratch-mirror procedure correctly (repo untouched, harvester symlinked unmodified) and measured the honest denominator at **nine runs / 78 agent results** against the filed three-run five-seat fold — but the write target `registry/harvest/x-p-w3.json` is in **no row of `W4.md` §4** | ⟨cmd⟩ `git status --porcelain -- docs/tranches/V/megatranche/` → **no output** before and after the mirror run; the filed `x-p-w3.json` is unchanged | one dated addendum-beside to `W4.md` §4 adding that path for a named unit — a triumvirate act. **Correctly escalated, not substituted for**; the registry was not written from outside bounds |
| **INFO-1** | INFO | **G-3's RED is genuinely relieved** — the sole honest-RED of this close. All three causes (**C1** `files:["./dist"]` + no `./css` in `exports`; **C2** `tsx/esm/api` undeclared at `src/css/lowering-js/js-alg.mjs:23` and `src/css/bounds.mjs:834`; **C3** `build/ac1.d.ts` re-exporting through a specifier that escapes the package root) live at `<p2>/typescript/package.json` and `<p2>/typescript/src/css/**`, which **no unit of this wave may write** (§4 grants exactly three `<p2>` script paths). §3a names this shape verbatim — *"Hard-gate failure that is not local-edit-recoverable"* — and a consumer-side patch would itself be the bounds violation | the `--seam` run reproduced byte-identically at this seat; the **positive control** (a staged tree outside both repositories, `"./css"` declared and `src/css` shipped, 26 entries) still resolves **0 of 52**, which is how C2 and C3 were located rather than guessed | **F-w4b-3, ESCALATED with an owner named** (triumvirate: either a dated §4 addendum-beside naming the four paths, or a successor X.P.W2 packaging unit). Three cures, not one |
| **INFO-2** | INFO | **One unstated unit-vs-close divergence**, against the close's own discipline (*"where a unit's number and this seat's number differ the divergence is stated"*): `.b`'s receipt banks the Wasm tally at **`artifacts 6 · admitted 6`**, the close's G-4 row at **`artifacts 9 · admitted 9`**. Benign and fully explicable — `.b`'s own `<p2>/.worktrees/w4b` container contributed the three extra copies — but the close does not name the delta | ⟨cmd⟩ `find <p2> value.js -name '*.wasm' -not -path '*/node_modules/*'` → **11 hits = 9 project ⊕ 2 unrelated** Chrome-profile TTS binaries; this seat's own run reproduces `artifacts 9 · admitted 9 · GREEN · EXIT=0` | a one-line note at `.d`'s close report; the verdict is unaffected — the admission is GREEN at **every** artefact under both counts |
| **INFO-3** | INFO | **Two quoted output blocks are condensed rather than verbatim.** `.b`'s receipt and the close render `"symbolCrossCheck": true` where the banked artefact holds an **object** (`{against, seamRows 52, universeRows 52, inSeamNotUniverse [], inUniverseNotSeam [], agree true}`); the prose beside it states the fields correctly. Quote-by-command asks for the output as printed | ⟨cmd⟩ over `evidence/W4/packed-surface.json` `seamContractRun.symbolCrossCheck` → the object above, `agree: True` | cosmetic; paste the object or name the field (`…symbolCrossCheck.agree → true`) |

### Honest-RED adjudication (axis 10) — the set is **{G-3}**, and only G-3

| gate | relief at the spec's own bytes? | owner named in the residual register? |
|---|---|---|
| **G-3** | **YES** — §3a *"Hard-gate failure that is not local-edit-recoverable"*, read against §4's three-path grant in `<p2>`; the cure address is the **candidate's own packaging**, i.e. producer-owned and an X.P.W2 question, and a consumer-side patch would be the gate failure | **YES** — F-w4b-3, triumvirate/orchestrator, two admissible cures stated |
| **G-5** | **NO.** `W4.md` assigns RC-P to `.c` **in this wave** and routes it nowhere else. "The orchestrator will dispatch it later" is an admission of incompleteness, not a relief the spec grants | owner named (**U-c**), but naming an owner for an undispatched unit does not relieve the gate |
| **G-6** | **PARTIAL.** Leg 3's absence **is** spec-relieved — §6 G-6's own baseline says *"the third sentence has no file to live in at all — that is G-9"* — but the gate also fails because `RELEASE-CONDITION.md` and `reciprocity-grep.txt` were never written, which is `.c` again | **NO** for the `.c` half |
| **G-8** | **NO** on the packet leg. The mail leg is GREEN (0 UNREAD). §6 G-8's cure arm is explicitly *inside* this wave's bounds (*"author in-repo + row + hand to the courier"*) — there is no upstream to wait on | owner named (**U-d**), not a relief |
| **G-9** | **PARTIAL.** The *ruling* is pre-ruled (C) at COHESION §0i.1 with its re-trigger named; the *recording* is `.d`'s and is undone, and §6 G-9's falsifier is *"closing with the gap unstated"* | **NO** for the recording |
| **G-10** | **NO** as a close condition. Withholding the stamp was correct and required; the gate's close-state (sum **0**) is simply unmet, and its open-state assertion failed at **7** (MINOR-1) | **NO** |

**Five REDs stand unrelieved (G-5 · G-6 · G-8 · G-9 · G-10), all from one root fact: two of four
units never sat.** That is the whole of the NOT-CONFORMANT verdict. Nothing the two units that
**did** sit produced is defective: `.a`'s G-1 is GREEN with nine negative controls (and one first
attempt recorded **VOID, not passing** — the harness now asserts `cmp -s source mutant` before
counting a control, which is the right cure for a silently-empty denominator), and `.b`'s G-4 is
GREEN at every artefact with both legs measured separately.

### Successor "Opens after" conjuncts, checked against this wave

| successor | its conjunct on X.P.W4 | state | blocked? |
|---|---|---|---|
| **KF.W3** (X·KF) | `KF-W3.md:9`, verbatim: *"This wave opens if and only if `RC-P(V)` evaluates TRUE per `docs/tranches/X/parse-that/RELEASE-CONDITION.md` §6a, run against the registry coordinate `V`"* | **RED.** The cited file does not exist and no evaluator exists; of RC-P's six conjuncts only **5 (BAR-DISCHARGED)** is TRUE (COHESION §0j.E, *"admission is decided on correctness; the bench table is recorded-not-gating"*). Conjunct 1 is measured **FALSE at V=4.0.0** (⟨cmd⟩ `node scripts/ci/verify-packed-surface.mjs` over the 4.0.0 tarball → EXIT 1 on `/transform`); 2·3·4·6 have no `V` to evaluate against | **LAWFULLY BLOCKED** — and correctly so: the sentence is gate-keyed, never scheduled |
| **F.W0 / F.W7** (X·F) | `F-W0.md:53` *"Opens after: tranche X's execution gate lifts"* — **no conjunct on X.P.W4**; X·P is a declared **NON-EDGE** with the consumption predicate adopted at `:459` | **GREEN.** G-7's floor holds at 0/0, so the forbidden edge is absent and nothing crosses | **NOT blocked** by this wave |
| **the X·V adoption wave** | does not exist — OP-4/G-9, **disposition (C)** at COHESION §0i.1, re-trigger *"X.P.W4's RC-P evaluator returning TRUE at a dated run"* | **RED.** The re-trigger cannot fire: the evaluator does not exist | **LAWFULLY BLOCKED**, and the disposition is ruled but unrecorded (HIGH-2) |

### Disposition

**The `X.P.W4` LEDGER row is NOT promoted and stays `PARTIAL`.** No byte of `W4.md`, of any sibling
...
**SERVED MODEL: claude-opus-5[1m]** — L-20 FRESH ADVERSARIAL PASS 2, **VERIFY-ONLY**. An independent
seat that authored **no** byte of `W4.md`, of `SEAM-CONTRACT.md`, of `RELEASE-CONDITION.md`, of any
`<p2>` script, of any evidence bank, of any unit receipt, of `## Close`, of `## Check 1` or of
`## Repair 1`. Wall clock **2026-09-19**; sitting of record **2026-09-17** (the owner's begin-word,
COHESION §0j). Every reading below was produced by a command this seat ran itself against the settled
bytes; **nothing is inherited from the close, from Check 1 or from Repair 1**.

**VERDICT: NOT-CONFORMANT.** **0 BLOCKER · 0 CRITICAL · 3 HIGH · 2 MINOR · 3 INFO.**
**10 of 10 gate verdicts reproduce · 6 of 6 claimed GREENs reproduce GREEN · 0 published figure
failed to reproduce · 0 write outside §File Bounds · 0 masking construct · E-3 held · mail 0 UNREAD ·
the four-verb line correctly did not move.** The row is **NOT promoted** and stays **`PARTIAL`**.
**Repair 1's cure is real and is confirmed at this seat's own commands**: G-5 and G-6 are genuinely
GREEN, the RC-P evaluator runs, refuses a missing coordinate, and fails its own negative control in
the honest direction. **The honest-RED set this pass can certify is `{G-3 · G-10}` — two of four.**
**G-8 and G-9 stand RED with no relief at the spec's own bytes**, and `registry/harvest/x-p-w4.json`
— §3 item 10's L-13 harvest, which the spec itself calls *"a recorded kill … not a housekeeping
lapse"* — does not exist. All three trace to one fact: **`X.P.W4.d`, one of this wave's four units,
has still never sat**, and `W4.md` routes its work to no successor.

### Crash-recovery (standing law, first act)

⟨cmd⟩ `git -C /Users/mkbabb/Programming/value.js status --porcelain` → **5 rows**:
`M demo/color-picker/composables/boot/atmosphere-calibration.ts` · `M demo/color-picker/composables/boot/useAtmosphere.ts`
· `M demo/test/glass/aurora-bracket.test.ts` (**Track A / X·V's live work**) ·
`M docs/tranches/V/reformation/CARRY-LEDGER.md` (a sibling's / the owner's) · `M scripts/dev/dev.sh`
(**unowned, NEVER touched, never staged**). ⟨cmd⟩ `git status --porcelain --
docs/tranches/X/execution/D/X-P-W4.md docs/tranches/X/execution/LEDGER.md
docs/tranches/X/parse-that/` → **no output**. ⟨cmd⟩ `git -C <p2> status --porcelain` →
`?? .worktrees/` alone. The frozen read-only root `/Users/mkbabb/Programming/parse-that` carries
**31 dirty rows** under `rust/**` — **none of it this wave's**, that root is §4 Do-NOT-touch, and not
one byte of it was read into a write, stashed or restored. **Zero dirty paths inside this seat's
writable set · zero inherited hunks · no killed predecessor's partial work to finish, rewrite or
name.**

### The ten gates, RE-RUN BY THIS SEAT — 10 of 10 verdicts reproduce

| gate | the record's verdict after Repair 1 | this seat's own command | reproduces? |
|---|---|---|---|
| **G-1** | GREEN | the spec's **literal** command (`seam-contract-check.mjs` over `SEAM-CONTRACT.md` ⊕ `evidence/W3/universe-52.json` ⊕ `DIVERGENCE-LEDGER.md`) → `contract ∖ universe-52 : ∅` · `universe-52 ∖ contract : ∅` · `PENDING-ADJUDICATION 45 · identical 7` · *"VERDICT: GREEN — both set-differences ∅, no disposition contradicts the ledger, every carried cell publishes PENDING-ADJUDICATION, no field is blank"* · **EXIT=0**; double-run, ⟨cmd⟩ `diff` → **no output** | **YES — GREEN** |
| **G-2** | GREEN (both legs) | **LEG 2 (§9's per-commit, the leg X·P controls)**: ⟨cmd⟩ `git show --name-only --format= <sha> \| grep -cE '^(src\|demo\|api\|test\|e2e)/'` over **all fifteen** value.js commits of this wave → `0` fifteen times. **LEG 1 (§6's tree-wide literal)**: **3 lines**, all sibling-track live work (see MINOR-2) | **YES on leg 2 — GREEN. Leg 1 moved with the shared tree** |
| **G-3** | RED | ⟨cmd⟩ `node scripts/packed-candidate-surface.mjs --seam …/SEAM-CONTRACT.md` → `candidate "@mkbabb/parse-that@1.0.0"` · **`tarballSha256 efc11936d361820526a93ada2145012d0c58cec48cee9ccb9a19c22c66e74cda`** (**byte-identical to `.b`'s bank, to Check 1's and to Repair 1's**) · `entryCount 1` · `seamSubpathDeclared false` · **`resolved "0 of 52"`** · `refusals 5` · `G3 "RED"`, **EXIT=1** | **YES — RED** |
| **G-4** | GREEN | the §6 **verbatim** one-liner against `<p2>/typescript/src/css/build/ac1.wasm` → `{"total": 0, "functionKind": 0, "imports": []}`, instantiation against `{}` did not throw, **EXIT=0**. Then ⟨cmd⟩ `find … -name '*.wasm' -not -path '*/node_modules/*' \| xargs node scripts/wasm-admission.mjs` → `artifacts 9 · admitted 9 · functionKindImportsTotal 0 · unaccountedImportsTotal 0` · `"verdict": "GREEN"` · **EXIT=0**; double-run with `generatedAt` excluded → ⟨cmd⟩ `diff` **no output** | **YES — GREEN** |
| **G-5** | GREEN | ⟨cmd⟩ `node scripts/rc-p-evaluate.mjs --version 4.0.0` → the **six-row table of §6a**, `RC-P(4.0.0) = FALSE — 4 of 6 conjuncts are FALSE: 1 PUBLISHED(V) · 3 EQUIVALENCE(V) · 4 ADMITTED(V) · 6 ROUTED(V)`, **EXIT=1** — the record's published table reproduces line-for-line but for the `evaluated at` stamp. Falsifiers run, not asserted: negative control ⟨cmd⟩ `--version 0.0.0-does-not-exist` → conjuncts **1 · 2 · 4** print `MEASURED NO / VALUE FALSE`, **TRUE on none**, **EXIT=1**; ⟨cmd⟩ with no `--version` → refuses, **EXIT=2**; ⟨cmd⟩ `grep -nE '[0-9]+\.[0-9]+\.[0-9]+' scripts/rc-p-evaluate.mjs` → **2 hits, both inside comments quoting the spec (L25, L290), 0 in code** | **YES — GREEN** |
| **G-6** | GREEN on its stated falsifier | ⟨cmd⟩ `grep -rn 'RC-P' docs/tranches/X/{keyframes,fourier}/waves/ \| wc -l` → **25** over **5** files (`KF-W2 · KF-W3 · KF-W5 · KF-W10 · F-W0`), so the *"fewer than two hits"* falsifier does not fire. ⟨cmd⟩ `sed -n '9p' KF-W3.md` and `sed -n '459p' F-W0.md` → **both far-end sentences verbatim, each citing `RC-P` by predicate name and no wave number**. ⟨cmd⟩ `ls -l …/RELEASE-CONDITION.md` → **present, 26,049 B** — the file both sentences cite by path exists; `evidence/W4/reciprocity-grep.txt` **banked, 3,951 B**. ⟨cmd⟩ `grep -rn 'SEAM-CONTRACT' docs/tranches/X/waves/` → **0** — leg 3 absent, and routed to G-9 by §6 G-6's own baseline | **YES — GREEN on the stated falsifier; see INFO-1** |
| **G-7** | GREEN (floor) | ⟨cmd⟩ `grep -c 'parse-that' /Users/mkbabb/Programming/fourier-analysis/package.json` → **0**; ⟨cmd⟩ over `web/package.json` → **0**. The transitive copies were **not** touched | **YES — GREEN, floor held** |
| **G-8** | RED (packet leg) | ⟨cmd⟩ `ls docs/tranches/X/parse-that/RELEASE-PACKET.md` → **No such file or directory**; ⟨cmd⟩ `grep -c 'X·P\|X.P.W\|RC-P' INBOX.md` → **42**, **none of them the packet's**. Mail leg by **position** → **UNREAD 0** over **79** register rows | **YES — RED** |
| **G-9** | RED | ⟨cmd⟩ `ls docs/tranches/X/parse-that/waves/W4-CLOSE.md` → **No such file or directory** — §8's named site for the disposition does not exist | **YES — RED** |
| **G-10** | RED | ⟨cmd⟩ `cat waves/W0.md W1.md W2.md W3.md W4.md \| grep -c 'VERIFIED \| \*\*NO\*\*'` → **7**, double-run `7 ≡ 7`; per-file `W0:1 · W1:1 · W2:1 · **W3:3** · W4:1`. Close-state must read **0**. The self-stamp leg is GREEN — ⟨cmd⟩ `grep -rn 'VERIFIED' W[0-3]-CLOSE.md` → **10** lines, every one disclaiming the stamp | **YES — RED** |

**GREEN: G-1 · G-2 · G-4 · G-5 · G-6 · G-7. RED: G-3 · G-8 · G-9 · G-10.** Identical to Repair 1's,
at this seat's own commands.

**Published figures, re-measured at the settled bytes** (write-then-measure): `SEAM-CONTRACT.md`
**121,820 B** / `e269f6e665f28894…` · `RELEASE-CONDITION.md` **26,049 B** / `a61f564520e9d369…` ·
`evidence/W4/rc-p-evaluation.json` **33,938 B** / `444d864a0718f3f5…` ·
`evidence/W4/reciprocity-grep.txt` **3,951 B** / `0622e63973ce8211…` ·
`<p2>/typescript/scripts/rc-p-evaluate.mjs` **25,873 B** /
`895e089bfd42368deaa1e27cc2654841f884653729757d8f79c86dc35412d8ba` ·
`packed-surface.json` **128,861 B** · `wasm-imports.json` **9,392 B** ·
`value-source-untouched.txt` **3,312 B** · `seam-contract-check.txt` **4,487 B** ·
`seam-contract-negative-controls.txt` **4,727 B**. **Every figure Repair 1 and the close published
reproduces exactly; 0 of 0 failed.** §8's census re-counted over the twelve rows → **7 PRESENT · 1
PARTIAL · 4 ABSENT**, the four absent exactly `.d`'s, each confirmed absent by ⟨cmd⟩ `ls`.

### Axis audit

| axis | finding |
|---|---|
| (2) **File Bounds** | **0 writes outside §4.** ⟨cmd⟩ `git show --name-only --format=` over all **fifteen** value.js commits and all **three** `<p2>` commits: the distinct path set is exactly `docs/tranches/X/parse-that/{SEAM-CONTRACT.md,RELEASE-CONDITION.md,scripts/seam-contract-check.mjs,evidence/W4/**}` ⊕ `docs/tranches/X/execution/D/X-P-W4.md` ⊕ `docs/tranches/X/execution/LEDGER.md` ⊕ `<p2>/typescript/scripts/{packed-candidate-surface,wasm-admission,rc-p-evaluate}.mjs` — **twelve value.js paths and three `<p2>` paths, every one a named §4 row** (the execution record and the LEDGER are the standing execution instruments). `<p2>`'s three script creates are **exactly** §4's three-path grant, no more. ⟨cmd⟩ `git log --oneline -1 -- scripts/dev/dev.sh` → `85cfea2c` (tranche T) — **in 0 of this wave's 18 commits**, and still dirty-unstaged as the standing arrangement requires. ⟨cmd⟩ `git worktree list` in `<p2>` → 5 entries, **all inside the root** (`.worktrees/ac1\|ac2\|ac3\|w4b`), **no `<p2>-w4*` sibling** — §4b's ONE-root law held |
| (3) **Masking** | **0 constructs.** ⟨cmd⟩ over all four authored scripts for `.skip` / `xit(` / `@ts-ignore` / `@ts-expect-error` / `eslint-disable` / allowlist / `node_modules` patch → **no authored hit** (the only matches are `process.exit(…)` substrings). The **four** `catch` blocks were read whole: `wasm-admission.mjs:105` records `{ok:false, error}` into a measured field and `verdict.functionKindZero` / `verdict.emptyImportInstantiation` stay **separate**, neither inferred from the other, exactly as G-4's falsifier demands; `packed-candidate-surface.mjs:361` and `:456` record `{error:{code,message}}` and `{refused:true,code}` into measured fields and the script exits **1** on its own RED; `rc-p-evaluate.mjs:137` is a directory-walk guard — see INFO-2. **No producer selector copied, no local `node_modules` patch, no narrowed assertion**: ⟨cmd⟩ over the wave diff for removed `expect(`/assert lines → none |
| (4) **Commit families** | §9's four declared messages: `.a`'s landed **verbatim** (`b4f8e5d1`); `.b`'s **verbatim** (`ca615a6` ⊕ `f92a3f32`, mirrored across two repositories); `.c`'s **verbatim** (`541754c` ⊕ `76f4a921`, likewise); `.d`'s never landed because `.d` never sat. `046bc176` ⊕ `00806a8` is a **distinct meaning** (the seam reader's name location), not a split family. **No declared family was split; one commit per meaning throughout** |
| (5) **E-3** | **HELD.** ⟨cmd⟩ over the wave's fifteen value.js commits, filtered to `megatranche/registry/adjudicated\|X/parse-that/waves/\|X/waves/\|X/COHESION\|coordination/INBOX\|X/keyframes/\|X/fourier/\|apotheosis\|scripts/dev/dev.sh` → **0 paths**. The dated spec `W4.md` (**676 L**, both 2026-09-19 addenda at L674/L676) is byte-untouched, and its §2 four-verb table with it; the adjudicated registry, the conformance artefacts and every sibling spec are untouched |
| (6) **Mail** | **0 UNREAD.** By **position** (never a bare `grep -i unread`) → **0** over **79** register rows of the file's **94** `^\| ` rows, tail `I-35`/`O-39`. Four-path census re-run at this seat's clock: `docs/tranches/V/` **10** · `docs/tranches/V/coordination/` **24** · `../glass-ui/docs/tranches/BK/coordination/` **9** · `../keyframes.js/docs/tranches/V/coordination/` **12** · `../sci-report/atlas/docs/tranches/P/coordination/` **28** — **byte-identical to Repair 1's**; ⟨cmd⟩ `find <the four paths> -maxdepth 1 -type f -name '*.md' -newermt "2026-09-19 07:30"` → **no member**. **0 unrowed · 0 `I-n`/`O-n` minted by this seat** |
| (7) **Four verbs** | **NONE MOVED, and that is lawful.** §6 G-10's falsifier — *"A stamp performed while any of G-1..G-9 is red fails — the act is the consequence of the gates, never a substitute for them"* — forbids IMPLEMENTED and VERIFIED while **G-3** is RED, and R-A reserves VERIFIED to `.d` alone. No byte of `W0.md`–`W4.md` was written by any seat of this wave |
| (8) **§2a goal criterion** | **NOT MET — but one of its three questions was genuinely cured by Repair 1.** See MINOR-1 |
| (9) **Figures** | **all reproduce**, above |
...
| # | severity | claim | receipt | cure |
|---|---|---|---|---|
| **HIGH-1** | HIGH | **G-8's packet leg is RED with NO relief at the spec's own bytes.** `RELEASE-PACKET.md`, its dated INBOX sent-row and the SS-6 hand-off are §3 item 8 and §4's `.d` rows **in this wave**, and `W4.md` routes them to no successor. §6 G-8 forecloses the only relief that would apply: *"**This gate's cure arm stays inside this wave's bounds** … so 'send' here means author in-repo + row + hand to the courier."* There is no upstream to wait on and no later wave that owns it | ⟨cmd⟩ `ls docs/tranches/X/parse-that/RELEASE-PACKET.md` → *No such file or directory*; ⟨cmd⟩ `grep -c 'RELEASE-PACKET' docs/tranches/V/coordination/INBOX.md` → the packet has no sent-row; the mail leg alone is GREEN (UNREAD **0** over **79** rows) | dispatch `.d` (a **fresh Fable adjudicator**, M-23 §1). The packet, its row and the courier hand-off are **not** blocked by F-w4c-1: §6 G-10's halt is on *"the act"* — the R-A stamp — and on nothing else (INFO-3) |
| **HIGH-2** | HIGH | **G-9's *recording* is RED with no relief.** The **ruling** is pre-ruled — COHESION §0i.1, disposition **(C)** with its re-trigger named — and that relieves the decision, not the record. §6 G-9 requires the wave to end *"in exactly one of three **recorded** states, never in silence"* and its falsifier is *"closing with the gap unstated"*; §8 names `waves/W4-CLOSE.md` as the site, together with the ten gates' RED-before/GREEN-after and the X·P terminal state table | ⟨cmd⟩ `ls docs/tranches/X/parse-that/waves/W4-CLOSE.md` → *No such file or directory*; ⟨cmd⟩ `git diff e914453f..HEAD -- docs/tranches/X/COHESION.md` over this wave's commits → **no X·P cell carved** | dispatch `.d`; the disposition is quoted by id from §0i.1 and **never re-decided**, and no X·V wave is authored |
| **HIGH-3** | HIGH | **§3 item 10's L-13 harvest is absent, and the spec itself calls its omission a kill.** *"Run the L-13 harvest at close: file `docs/tranches/V/megatranche/registry/harvest/x-p-w4.json` with a seat count equal to the units dispatched … **This is lane law in canonical W0/W1/W2 — its omission is a recorded kill (W2 §1), not a housekeeping lapse**."* §4 grants the path `create` to `.d`; §5's `.d` sub-gate makes the seat count a close condition. Distinct from F-w4c-2, which is the **W3** harvest's bounds gap — this is **W4's own**, fully in bounds and simply unperformed | ⟨cmd⟩ `ls docs/tranches/V/megatranche/registry/harvest/x-p-w4.json` → *No such file or directory* | `.d` runs `node docs/tranches/V/megatranche/workflows/harvest-journals.mjs` **from a scratch mirror with the script symlinked** (COHESION §0p F-e11) and files `x-p-w4.json`; the `DEFECT-LEDGER.md` append is the harvester's, never hand-edited |
| **MINOR-1** | MINOR | **§2a's goal criterion is still NOT MET — on question 1, and now only on question 1.** *"When may the consumer act"* is **MET** by `RELEASE-CONDITION.md`: a predicate, six commands, an evaluator that a person cannot argue with — Repair 1's cure is real and this seat ran it. *"How does it arrive"* is **answerable from one document** and answers RED honestly with three measured causes, which §2a permits (it fails only *"if any of the three is answerable only by reading a status word"*). *"What exactly does value.js receive"* is **NOT met**: **45 of 52** seam rows publish `PENDING-ADJUDICATION`, and `W4.md`'s first 2026-09-19 addendum assigns their ruling to *"`.d`'s fresh adjudicator"* | the G-1 run's own disposition census → `PENDING-ADJUDICATION 45 · identical 7`; ⟨cmd⟩ `ls …/RELEASE-CONDITION.md` → present, 26,049 B, §1 reproducing §6a **verbatim** and §5 carrying all three reciprocity sentences | rides HIGH-1/HIGH-2: the 45 cells are `.d`'s adjudication. **Advanced 1 of 3 by Repair 1 and that advance is confirmed**, not re-litigated |
| **MINOR-2** | MINOR | **G-2's tree-wide leg 1 does not reproduce at this seat's clock.** The close and Repair 1 published *"**0 lines**, double-run `0 ≡ 0`"*; this seat measures **3**. All three are **Track A / X·V's live uncommitted work** — `demo/color-picker/composables/boot/atmosphere-calibration.ts` · `demo/color-picker/composables/boot/useAtmosphere.ts` · `demo/test/glass/aurora-bracket.test.ts` — none of it X·P's, all of it under paths `W4.md` §4 lists Do-NOT-touch | ⟨cmd⟩ `git status --porcelain -- src api demo test e2e \| wc -l` → **3**, double-run `3 ≡ 3`; ⟨cmd⟩ `git log --oneline -1` at this seat → `4cb294b9 feat(demo/atmosphere-oracle)…`, a Track A commit landed after Repair 1 | **Mitigated, and not a defect of this wave.** Leg 2 — §9's per-commit form, *"the leg X·P controls"* — reads **0 over all fifteen** commits, and `evidence/W4/value-source-untouched.txt` documents leg 1's volatility in its own header (*"Four tracks share this checkout; a sibling seat's uncommitted work under those paths makes this leg FALSE by an act no X·P seat performed and no X·P seat may undo"*). Recorded so a later seat does not read the published `0` as falsified |
| **INFO-1** | INFO | **G-6's GREEN is on its stated falsifier, and its headline is 2 of 3.** §6 G-6 opens *"Three sentences exist in three files owned by three different seats"*; two exist. Its **falsifier** is *"a `grep -rn 'RC-P' …` returning fewer than two hits fails"* — 25 hits, it does not fire — and its own RED baseline routes the third onward: *"the third sentence has no file to live in at all — that is G-9."* Repair 1 disclosed this tension rather than smoothing it, and this seat confirms the disclosure is accurate | ⟨cmd⟩ 25 hits / 5 files; both sentences verbatim by predicate name at `KF-W3.md:9` and `F-W0.md:459`; ⟨cmd⟩ `grep -rn 'SEAM-CONTRACT' docs/tranches/X/waves/` → **0** | none — the reading is correct. Leg 3 lands when G-9's disposition (A) payload fires |
| **INFO-2** | INFO | **One bare `catch` records nothing — and its direction is conservative, so it is not a masking fallback.** `rc-p-evaluate.mjs:135-139`, inside `findByExtension`: an unreadable directory is skipped silently. It feeds conjunct 4's *"`V`'s installed bytes contain zero `.wasm` artifacts"*, so a swallowed read error can only make **ADMITTED more FALSE**, never more TRUE — the opposite of the direction §6 G-5's falsifier polices (*"an evaluator that returns TRUE with any conjunct unmeasured fails"*). Recorded because it is the one catch in the wave's four scripts that writes no measured field | ⟨cmd⟩ `grep -n -B6 -A8 'catch' scripts/rc-p-evaluate.mjs` → `let entries; try {…} catch { return; }` | cosmetic: record the skipped directory in the conjunct's reading, so the denominator is stated |
| **INFO-3** | INFO | **`.d`'s non-sitting is over-broad relative to the halt that justifies it.** §6 G-10's words are *"A sum other than 5 at open fails and **halts the act**"* — the act, singular, is the R-A stamp, and §6 G-10's falsifier independently forbids that stamp while G-3 is RED. Neither sentence halts `.d`'s **other five** acts: `W4-CLOSE.md`, the G-9 recording, `RELEASE-PACKET.md`, its INBOX row + SS-6 hand-off, and the L-13 harvest. The close's sequencing note (*"dispatching the only seat that may stamp VERIFIED in front of a halt condition"*) is prudent but is not a spec requirement, and reading it as one has now cost two conformance rounds | §6 G-10 (`W4.md` L521-543) and §6 G-8's *"this gate's cure arm stays inside this wave's bounds"* (L484-486) | dispatch a **fresh Fable `.d`** with the stamp act explicitly withheld pending F-w4c-1's ruling. That discharges HIGH-1, HIGH-2 and HIGH-3 and leaves **only** the two genuinely relieved REDs |

### Honest-RED adjudication (axis 10) — the set is **{G-3 · G-10}**, and no more

| gate | relief at the spec's own bytes? | owner named in the residual register? |
|---|---|---|
| **G-3** | **YES — producer-owned.** §3a names this shape verbatim: *"Hard-gate failure that is **not local-edit-recoverable** … that is an X.P.W2 architecture question, not a link-flag tweak."* §4 grants X.P.W4 exactly **three** creatable paths in `<p2>` (the three `scripts/*.mjs`) and all three of G-3's causes live outside them — **C1** `files:["./dist"]` + no `./css` in `exports` at `<p2>/typescript/package.json`; **C2** `tsx/esm/api` undeclared, imported at `src/css/lowering-js/js-alg.mjs:23` and `src/css/bounds.mjs:834`; **C3** `build/ac1.d.ts` re-exporting through a specifier that escapes the package root. `.b`'s **positive control** — a staged tree outside both repositories with `"./css"` declared and `src/css` shipped — still resolved **0 of 52**, which is how C2 and C3 were located rather than guessed. A consumer-side patch would itself be the bounds violation | **YES — F-w4b-3**, triumvirate/orchestrator, two admissible cures stated (a dated §4 addendum-beside naming the four paths, or a successor X.P.W2 packaging unit) |
| **G-10** | **YES — forbidden by the spec's own falsifier, transitively through G-3.** §6 G-10: *"**A stamp performed while any of G-1..G-9 is red fails** — the act is the consequence of the gates, never a substitute for them."* G-3 is RED and relieved above, so the stamp is **spec-forbidden today** by an authority no seat may overrule. Independently, the open-state assertion cannot be satisfied by any seat's act: the sum reads **7** because `W3.md` carries the matching row at `:44` (its §2 table) **and** in its sixth and seventh **dated addenda** (`:836`, `:971`), both **E-3-lawful and unrewritable**, and both admissible cures are themselves dated addenda to E-3 files. Withholding the stamp was mandatory and was honoured, twice | **YES — F-w4c-1** (the count, UNRULED, shape (a) named as the only one that survives a ninth W3 round) ⊕ **F-w4b-3** (the transitive cause). The four-verb line correctly did not move |
| **G-8** | **NO.** §6 G-8 explicitly keeps the cure arm *inside* this wave's bounds and names the failure mode of not doing it (*"Placing the packet where the recipient does not look is the recorded failure mode"*). There is no upstream, no producer, and no successor routing. *"The orchestrator will dispatch `.d` later"* is an admission of incompleteness, not a relief the spec grants | owner named (**U-d** / **E-r1-1**), but naming an owner for an undispatched in-wave unit does not relieve the gate |
| **G-9** | **NO on the recording.** The ruling is relieved by COHESION §0i.1; §6 G-9's gate is the **recorded state**, and §8's site for it does not exist | owner named (**U-d**), not a relief |

**Two REDs stand unrelieved (G-8 · G-9), and one in-bounds spec obligation is unperformed (the L-13
harvest).** All three are `.d`'s, all three are performable today inside §4, and none is halted by
F-w4c-1 (INFO-3). That is the whole of the NOT-CONFORMANT verdict. **Nothing the three units that
did sit produced is defective**: `.a`'s G-1 is GREEN with nine negative controls, `.b`'s G-4 is GREEN
at every one of nine artefacts with both legs measured separately, and `.c`'s RC-P evaluator passes
its own falsifiers by demonstration — negative control FALSE-with-unmeasured-named, no-coordinate
refusal at EXIT=2, and **0 version literals in code**.

### Successor "Opens after" conjuncts, checked against this wave

| successor | its conjunct on X.P.W4 | state at this seat's own commands | blocked? |
|---|---|---|---|
| **KF.W3** (X·KF) | `KF-W3.md:9`, verbatim: *"This wave opens if and only if `RC-P(V)` evaluates TRUE per `docs/tranches/X/parse-that/RELEASE-CONDITION.md` §6a, run against the registry coordinate `V`"* | **RED — and now for a MEASURED reason rather than a missing instrument.** The cited file **exists**, the evaluator **exists and runs**, and ⟨cmd⟩ `node scripts/rc-p-evaluate.mjs --version 4.0.0` → **`RC-P(4.0.0) = FALSE`**, 2 of 6 conjuncts TRUE (**2 TOTALITY · 5 BAR-DISCHARGED**), 4 FALSE (**1 PUBLISHED · 3 EQUIVALENCE · 4 ADMITTED · 6 ROUTED**), EXIT=1 | **LAWFULLY BLOCKED**, and correctly so: the sentence is gate-keyed, never scheduled. **This is a real advance of Repair 1** — the conjunct is now *evaluable* where before it cited a file that did not exist |
| **F.W0 / F.W7** (X·F) | `F-W0.md:53` *"Opens after: tranche X's execution gate lifts"* — **no conjunct on X.P.W4**; X·P is a declared **NON-EDGE** with the consumption predicate adopted at `:459` by X·F's own seat | **GREEN.** G-7's floor holds at **0 / 0**, so the forbidden edge is absent and nothing crosses | **NOT blocked** by this wave |
| **the X·V adoption wave** | does not exist — OP-4/G-9, **disposition (C)** at COHESION §0i.1, re-trigger *"X.P.W4's RC-P evaluator returning TRUE at a dated run"* | **RED, and now genuinely evaluated.** The re-trigger's instrument exists and its dated run returns **FALSE**, so the re-trigger **does not fire** — where at Check 1 it *could not* fire for want of an evaluator | **LAWFULLY BLOCKED**; the disposition is ruled but still **unrecorded** (HIGH-2) |

### Disposition

**The `X.P.W4` LEDGER row is NOT promoted and stays `PARTIAL`.** No byte of `W4.md`, of any sibling
X·P wave file, of `SEAM-CONTRACT.md`, of `RELEASE-CONDITION.md`, of any `<p2>` script, of
`COHESION.md`, of `INBOX.md` or of the registry was written by this seat; the four-verb line is
untouched; `scripts/dev/dev.sh` was never touched and never staged; no sibling's dirty path was
read into a write, stashed or restored. **SELF-COUNT: this check wrote exactly two paths** — this
section and the LEDGER's `X.P.W4` row cell plus its appended event line.

**The shortest path to CONFORMANT-HONEST-RED, measured**: dispatch **one** fresh Fable `.d` that
authors `W4-CLOSE.md` (the ten gates, the terminal state table, **G-9's disposition by id**),
authors and rows `RELEASE-PACKET.md` with the SS-6 hand-off, runs the L-13 harvest from a scratch
...
**SERVED MODEL: claude-opus-5[1m]** — REPAIR SEAT, round 2, Track D. Wall clock **2026-09-19**;
sitting of record **2026-09-17** (the owner's begin-word, COHESION §0j). Every reading below was
produced by a command this seat ran itself against the settled bytes, double-run; **nothing is
inherited** from `## Close`, `## Check 1`, `## Repair 1` or `## Check 2`.

**VERDICT: FULL REPAIR OF THE IN-BOUNDS REGISTER — 3 of 3 HIGH CURED · 1 MINOR CURED · 1 MINOR
ESCALATED (no one-command cure exists; it is an adjudication).** **Gates move 6 GREEN / 4 RED →
8 GREEN / 2 RED.** **G-8 and G-9 turn GREEN at this seat's own commands. The honest-RED set is
`{G-3 · G-10}` — exactly the set Check 2 certified as relieved at the spec's own bytes.**
**No four-verb line moves. No wave file was touched. `W4.md` is byte-untouched.**
...
⟨cmd⟩ `git -C <p2> status --porcelain` → `?? .worktrees/` alone. ⟨cmd⟩ `git -C
/Users/mkbabb/Programming/parse-that status --porcelain` → **31 rows** under `rust/**` in the
**frozen read-only** root — none of it this wave's, that root is §4 Do-NOT-touch, and not one byte of
it was read into a write, stashed or restored.

**Zero dirty paths inside this seat's writable set · zero inherited hunks · no killed predecessor's
partial work to finish, rewrite or name.** Nothing was stashed; nothing was reset; no sibling's dirty
path was touched. The three `demo/**` and `test/**` rows Check 2 measured are **gone** — Track A
committed them (`4cb294b9`) between the two seats, which is exactly what Check 2's MINOR-2 predicted.

### The ruling this seat had to make before it could cure anything

All three HIGH defects' stated cure is *"dispatch `.d`"* — **and dispatch is the orchestrator's act,
not a seat's**. `.d` is a **fresh Fable adjudicator** (`W4.md` §5, M-23 §1). Repair 1 escalated on
that ground and returned the three HIGHs unrelieved; Check 2 reproduced all three and added the
sentence that decides the question — **INFO-3**: *"`.d`'s non-sitting is over-broad relative to the
halt that justifies it. §6 G-10's words are 'A sum other than 5 at open fails and **halts the act**'
— the act, singular, is the R-A stamp … Neither sentence halts `.d`'s other five acts … reading it
as one has now cost two conformance rounds."*

So this seat **split `.d`'s work by kind** and performed only the half an Opus seat may lawfully
perform:

| `.d`'s act | kind | performed? | authority |
|---|---|---|---|
| `W4-CLOSE.md` — ten gates, terminal state table, the G-9 **recording** | transcription + measurement | **YES** | §6 G-9's gate is the **recorded state**; the **ruling** is pre-made at COHESION §0i.1 and is quoted by id, never re-decided |
| `RELEASE-PACKET.md` + INBOX row + SS-6 hand-off | assembly + a courier hand-off | **YES** | §6 G-8: *"This gate's cure arm stays inside this wave's bounds … 'send' here means author in-repo + row + hand to the courier"* |
| the L-13 harvest | mechanical, lane law | **YES** | M-23 §2 routes mechanical sweeps to Opus; §3 item 10 calls its omission *"a recorded kill"* |
| ruling the **45 `PENDING-ADJUDICATION`** cells | **adjudication** | **NO** | COHESION §0v routes them to *"`.d`'s fresh adjudicator"* — an Opus seat ruling them is the defect M-23 §1 exists to prevent |
| the **R-A stamp** | **certification** | **NO** | §6 G-10's falsifier: *"A stamp performed while any of G-1..G-9 is red fails."* **G-3 is RED**; and `F-w4c-1` halts it at a sum of 7 |
| the two `COHESION.md` carves | the close-stamp's own cells | **NO** | carving them under a withheld stamp would assert a close that did not happen |

**Every site this seat wrote carries that disclosure in its own bytes** — `W4-CLOSE.md` §0,
`RELEASE-PACKET.md`'s opening block, the INBOX row's own text, and the harvest JSON's `unit` and
`note` fields. A later reader cannot mistake this for `.d` having sat.

### Defect → cure → commit

| # | defect (Check 2) | disposition | cure that landed |
|---|---|---|---|
| **HIGH-1** | G-8's packet leg RED with no relief: `RELEASE-PACKET.md`, its dated INBOX sent-row and the SS-6 hand-off are §3 item 8 and §4's `.d` rows **in this wave**, routed to no successor | **CURED — G-8 GREEN** | `docs/tranches/X/parse-that/RELEASE-PACKET.md` authored **in this repo** (23,017 B): the 52-row seam summary, **all four** `declared-divergence` rows' consumer-direction fields verbatim, and `RC-P(V)`'s six conjuncts **quantified over `V`** with no version literal. Rowed at `INBOX.md` **O-40**, 2026-09-19, naming **that in-repo path** as the delivery point **and** the **SS-6** batch as the courier (quoted from COHESION §1, never rewritten). **0 bytes** written under `keyframes.js/**`, `fourier-analysis/**` or `glass-ui/**`; **no channel of this wave's own** was opened. Commit `d9ed9f5b` |
| **HIGH-2** | G-9's **recording** RED: the ruling is pre-made at §0i.1, which relieves the decision and not the record; §8's site `waves/W4-CLOSE.md` did not exist | **CURED — G-9 GREEN** | `waves/W4-CLOSE.md` (39,300 B): the ten gates RED-before/GREEN-after with this seat's own outputs, the X·P terminal state table across all five waves, the **G-9 disposition quoted whole by id from COHESION §0i.1** with its exact condition and re-trigger command, and the R-A stamp act recorded as **WITHHELD** with the spec's own reason. The re-trigger was **evaluated**, not assumed: `RC-P` FALSE, EXIT=1 → it does **not** fire. ⟨cmd⟩ `git show --name-only --format=` over every commit of this wave → **0 paths** under `docs/tranches/X/waves/` — **no X·V wave was authored**. Commit `d9ed9f5b` |
| **HIGH-3** | §3 item 10's L-13 harvest absent — the spec calls its omission *"a recorded kill (W2 §1), not a housekeeping lapse"* | **CURED — the harvest is filed** | `registry/harvest/x-p-w4.json` (119,964 B) filed by the **COHESION §0p / F-e11 scratch-mirror procedure**: harvester **UNMODIFIED** (sha256 `77a6e04c…`, ⟨cmd⟩ `git status --porcelain -- workflows/` → 0 lines), **three runs**, A and A′ wholly in the mirror and ⟨cmd⟩ `diff -r` between them → **no output** (byte-identical), B with `DEFECT-LEDGER.md` a **SYMLINK to the repo's file** so the append is the **script's** act. **`DEFECT-LEDGER.md` 7,585 → 7,813 defects**, 10,551,839 → 10,560,195 B — **never hand-edited**. Commit `d9ed9f5b` |
| **MINOR-1** | §2a's goal criterion NOT MET on question 1 — 45 of 52 seam rows publish `PENDING-ADJUDICATION` | **ESCALATED — `E-r2-1`** | **No one-command cure exists: it is an adjudication**, and COHESION §0v names its seat — *"`.d`'s fresh adjudicator"*. Ruling 45 cells from an Opus seat would be a masking cure of the worst kind: the gate would read settled while nothing had been decided. §2a's verdict is **recorded honestly** at `W4-CLOSE.md` §10 (**2 of 3 MET**; question 1 NOT MET, with the reason and the owner named) |
| **MINOR-2** | G-2's tree-wide leg 1 read **3** at Check 2's clock, against the published **0** | **CURED — reconciled and recorded** | ⟨cmd⟩ `git status --porcelain -- src api demo test e2e \| wc -l` → **0** at this seat, double-run `0 ≡ 0`. The three rows were Track A's live uncommitted work and Track A has since committed them (⟨cmd⟩ `git log --oneline -1` → `4cb294b9 feat(demo/atmosphere-oracle)…`). **Both readings were true at their own clocks**; the published `0` was never falsified. Recorded at `W4-CLOSE.md` §7 claim 3, where the leg's shared-tree volatility is stated beside the leg X·P actually controls |
| **INFO-2** | `rc-p-evaluate.mjs`'s one bare `catch` records no measured field | **NOT TAKEN** | Below MINOR, and its cure would rewrite `.c`'s settled script and force a re-bank of `.c`'s evidence. Check 2's own reading stands: the swallow can only make `ADMITTED` **more** FALSE, which is the opposite of the direction §6 G-5's falsifier polices. Left as Check 2 recorded it |

### The ten gates, RE-RUN BY THIS SEAT after the cure — **8 GREEN · 2 RED** (was 6 · 4)

| gate | before | this seat's own command, at the settled bytes | after |
|---|---|---|---|
| **G-1** | GREEN | the spec's literal command → `contract ∖ universe-52 : ∅` · `universe-52 ∖ contract : ∅` · `PENDING-ADJUDICATION 45 · identical 7` · `VERDICT: GREEN`, **EXIT=0** | **GREEN** (unmoved) |
| **G-2** | GREEN | **LEG 1** ⟨cmd⟩ `git status --porcelain -- src api demo test e2e \| wc -l` → **0**, double-run `0 ≡ 0`. **LEG 2** ⟨cmd⟩ per-commit over every value.js commit of this wave, this seat's three included → `0` at each | **GREEN** (held) |
| **G-3** | RED | ⟨cmd⟩ `node scripts/packed-candidate-surface.mjs --seam …/SEAM-CONTRACT.md` → `tarballSha256 efc11936d361820526a93ada2145012d0c58cec48cee9ccb9a19c22c66e74cda` (**byte-identical to `.b`'s bank, Check 1's, Repair 1's and Check 2's**) · `entryCount 1` · `seamSubpathDeclared false` · `resolved "0 of 52"` · `refusals 5` · `G3 "RED"`, **EXIT=1** | **RED — unmoved, honestly. `F-w4b-3` stays ESCALATED** |
| **G-4** | GREEN | ⟨cmd⟩ `node scripts/wasm-admission.mjs <9 project artefacts>` → `tally {artifacts 9 · admitted 9 · functionKindImportsTotal 0 · unaccountedImportsTotal 0}` · `"verdict": "GREEN"`, **EXIT=0** | **GREEN** (unmoved) |
| **G-5** | GREEN | ⟨cmd⟩ `node scripts/rc-p-evaluate.mjs --version 4.0.0` → the six-row table of §6a, **EXIT=1** naming the FALSE conjuncts. **The reading MOVED and the evaluator moved it**: conjunct **6 ROUTED(V) flips FALSE → TRUE** once the packet and its row exist. `RC-P(4.0.0) = FALSE — 3 of 6 conjuncts are FALSE: 1 PUBLISHED · 3 EQUIVALENCE · 4 ADMITTED` (was 4 of 6). Double-run: ⟨cmd⟩ field-by-field compare of two banked JSONs → `verdict FALSE ≡ FALSE`, `trueCount 3 ≡ 3`, `falseConjuncts [1,3,4] ≡ [1,3,4]`; the only differing fields are `evaluatedAt`, per-command `ms` and the `mkdtemp` workspace path | **GREEN** (held; the predicate's value moved, honestly) |
| **G-6** | GREEN on its stated falsifier | ⟨cmd⟩ `grep -rn 'RC-P' docs/tranches/X/{keyframes,fourier}/waves/ \| wc -l` → **25** over **5** files — the *"fewer than two hits"* falsifier does not fire. ⟨cmd⟩ `sed -n '9p' KF-W3.md` and `sed -n '459p' F-W0.md` → both sentences **verbatim, by predicate name**. Leg 3 ⟨cmd⟩ `grep -rn 'SEAM-CONTRACT' docs/tranches/X/waves/` → **0**, absent, and §6 G-6's own baseline routes it to G-9 | **GREEN** (unmoved; the 2-of-3 tension stays disclosed, not smoothed) |
| **G-7** | GREEN (floor) | ⟨cmd⟩ `grep -c 'parse-that' ../fourier-analysis/package.json` → **0**; over `web/package.json` → **0**. The transitive copies were **not** touched | **GREEN — floor held** |
| **G-8** | **RED** (packet leg) | ⟨cmd⟩ `ls docs/tranches/X/parse-that/RELEASE-PACKET.md` → **present, EXIT=0**. ⟨cmd⟩ `grep -n 'RELEASE-PACKET' INBOX.md` → **1** hit, the **O-40** sent-row at `:127`, dated 2026-09-19, naming the in-repo path **and** the SS-6 batch. ⟨cmd⟩ the by-position UNREAD scan → **0** over **80** register rows, double-run `0 ≡ 0`. Falsifiers checked one by one: a packet without a row — **no**; a row without a delivery path — **no**, the path is named twice; a packet written into a sibling repo — **no**, ⟨cmd⟩ `git show --name-only --format=` over this seat's commits → 0 sibling-repo paths; a letter outside SS-6's batch — **no**, the row names the batch and opens no channel; a delivery point only the sender can see — **no**, it is an in-repo path under version control | **GREEN** |
| **G-9** | **RED** | ⟨cmd⟩ `ls docs/tranches/X/parse-that/waves/W4-CLOSE.md` → **present, 39,300 B**. The disposition is **recorded** there in §4 — **(C)**, quoted whole by id from COHESION §0i.1, with its exact condition and its re-trigger command, and the re-trigger **evaluated** (FALSE, EXIT=1). Falsifier checked: *"closing with the gap unstated, or with X·P having authored an X·V wave"* — the gap is stated with its id, and ⟨cmd⟩ over every commit of this wave shows **0 paths** under `docs/tranches/X/waves/` | **GREEN** |
| **G-10** | RED | ⟨cmd⟩ `cat waves/W0.md W1.md W2.md W3.md W4.md \| grep -c 'VERIFIED \| \*\*NO\*\*'` → **7**, double-run `7 ≡ 7`; close-state must read **0**. **The act was NOT performed**: §6 G-10's falsifier forbids the stamp while G-3 is RED, and `F-w4c-1` halts it at a sum of 7. **No byte of `W0.md`–`W4.md` was written** — ⟨cmd⟩ `git show --name-only --format=` over this seat's commits → 0 such paths | **RED — honest, relieved, and withheld for the third consecutive sitting** |

**GREEN: G-1 · G-2 · G-4 · G-5 · G-6 · G-7 · G-8 · G-9. RED: G-3 · G-10.**
**The honest-RED set is exactly `{G-3 · G-10}`** — the set Check 2 adjudicated as relieved at the
spec's own bytes, with `F-w4b-3` and `F-w4c-1` as their named owners. **Nothing was made green by a
mask**: G-3 reproduces byte-identically at a sixth seat, and G-10's act was withheld rather than
performed.

### `RC-P` moved, and the movement is the packet's own consequence

| conjunct | before this repair | after | why |
|---|---|---|---|
| **6 ROUTED(V)** | **FALSE** — *"RELEASE-PACKET.md does not exist at its in-repo path — the packet has no delivery point"* | **TRUE** | the packet exists at its in-repo path; its INBOX row names `RC-P`, that path, and the `SS-6` batch; and the two-grep forbidden-edge probe reads **0 / 0** |
| overall | `RC-P(4.0.0) = FALSE`, **2 of 6 TRUE** | `RC-P(4.0.0) = FALSE`, **3 of 6 TRUE** | **KF.W3 still does NOT open.** The three FALSE conjuncts are 1 PUBLISHED · 3 EQUIVALENCE · 4 ADMITTED, each measured, none a document read |

Banked **beside** `.c`'s evidence, never over it (E-3):
`evidence/W4/rc-p-evaluation-after-packet-2026-09-19.json`, 39,683 B, sha256 `845201d39458b455…`.
`.c`'s `evidence/W4/rc-p-evaluation.json` (33,938 B, `444d864a…`) is **byte-untouched**.

### §8 Verification Artefacts — re-counted over the same twelve rows

**11 PRESENT · 1 PARTIAL · 0 ABSENT** (was 7 · 1 · 4). The four that moved are exactly the four
Check 2 named as `.d`'s: `W4-CLOSE.md` · `RELEASE-PACKET.md` · the INBOX packet sent-row ·
`registry/harvest/x-p-w4.json`. The commit roster stays **PARTIAL** — phases 1 and 2 have commits in
both roots; **phase 3 has no `.d` commit because `.d` never sat**, and this seat's close commit says
so in its own body rather than standing in for one silently.

### The L-13 harvest, measured

| | |
|---|---|
| harvester sha256 | `77a6e04c20674f1575891ac9f3daa8421491fc2ec98bd784dc61b4563c978a28` — **UNMODIFIED**; ⟨cmd⟩ `git status --porcelain -- docs/tranches/V/megatranche/workflows/` → **0 lines** |
| runs | **3**, all `EXIT=0`, stdout identical: `harvested 3227 agent results · 7813 defects`. ⟨cmd⟩ `diff -r` between run A and run A′ outputs → **no output** |
| K.7(i), **fourth firing** | **153** files written per run — **104 NEW · 2 CHANGED** (`wf_247fe7fa-e67.json`, `wf_6f552e12-062.json`) against the repo's tracked set, every one outside this wave's §4 bounds. Hence the mirror |
| `DEFECT-LEDGER.md` | **7,585 → 7,813** defects; 10,551,839 → 10,560,195 B; sha256 `8092610b…` → `54fd8c71…`. Written by the **script**, through a symlink, never by hand. ⟨cmd⟩ `git diff --check` reports **924** trailing-whitespace flags **in that file alone** — the script's own output, left exactly as it wrote them (the W3 precedent recorded the same class at 130); ⟨cmd⟩ `git diff --check` over **this seat's own paths** → **clean, EXIT=0** |
| seat count | **units planned 4 · unit-labelled seats harvested 2** (`.a` DONE, `.b` ESCALATED) · `.c` **folded into the `repair1` seat's row** (that row carries `.c`'s commits `541754c` ⊕ `76f4a921`) · **`.d` ABSENT**. **2 of 4 as labelled units, 3 of 4 counting the fold — RED under L-13's letter**, and the reason is the one this wave turns on. No returned seat has zero rows. This seat's own row is structurally absent from its own harvest — the fourth firing of that structural fact in this lane |

### Escalations — returned, not substituted for

1. **`E-r2-1` (HIGH, adjudication) — `X.P.W4.d`'s ADJUDICATIVE half is still owed**, and only that
   half. A fresh Fable adjudicator must (a) rule the **45 `PENDING-ADJUDICATION`** seam cells by the
   COHESION §0w id set, and (b) perform the **R-A stamp** once G-3 and the open-state sum permit it.
   **This replaces Repair 1's `E-r1-1`, which was broader**: `E-r1-1` held that `.d` could not sit at
   all; Check 2's INFO-3 showed the halt is on the stamp alone, and the transcriptive half has now
   been performed and is measurable at `d9ed9f5b`. **Owner**: the orchestrator.
2. **`F-w4b-3` (HIGH, bounds) — unmoved.** G-3's three causes live at `<p2>/typescript/package.json`
   and `<p2>/typescript/src/css/**`; §4 grants exactly three `<p2>` script paths and all three causes
   lie outside them. A consumer-side patch would be the bounds violation §3a names.
3. **`F-w4c-1` (MEDIUM, instrument) — unmoved, re-measured 7**, double-run `7 ≡ 7`. Both admissible
   shapes are dated addenda to E-3 files; neither is a seat's act.
4. **`F-w4c-2` (MEDIUM, bounds) — unmoved.** `registry/harvest/x-p-w3.json` is in no §4 row.
   **Distinct from W4's own harvest, which was in bounds and is now filed.**
5. **The two `COHESION.md` carves — NOT PERFORMED.** §1 SS-5's status cell and §5's status board line
   are the close-stamp's cells; carving them under a withheld stamp would assert a close that did not
   happen. **Owner**: the orchestrator, at the sitting that seats a fresh Fable `.d`.
6. **`Q-RC-1` · `Q-RC-2`** — `.c`'s two open contract questions, recorded not resolved, owner-owed.

Carried unchanged: `U-d` · `F-w4a-1` · `SEAM-DRIFT` · `R-w4b-2` · `F-ae1 / F-p1` · the W3 rounds-6–8
id set. **`U-d` is now PARTIALLY DISCHARGED** — `.d`'s five non-stamp acts are performed; its
adjudicative half rides `E-r2-1`.
...
⟨cmd⟩ the by-position scan (never a bare `grep -i unread`) → **UNREAD 0**, double-run `0 ≡ 0`, over
**80** register rows (**79** before this seat's row). **This seat minted exactly one row — `O-40`** —
and changed **no** other row's status cell. Register tail moves `I-35` / **`O-40`**. The counts are
byte-identical to Repair 1's and Check 2's. **X·P closes its repair round with zero UNREAD.**

### Format and lint cadence (§7)

⟨cmd⟩ `prettier --write` then `--check` **twice** over `RELEASE-PACKET.md` and `W4-CLOSE.md` →
*"All matched files use Prettier code style!"* on both passes, i.e. **idempotent**; the published
byte counts and sha256s are read from the **post-format** bytes (write-then-measure). **`INBOX.md`
was NOT Prettier-formatted** — it is a shared append-only ledger and formatting it would rewrite many
seats' settled bytes; neither is this record append, for the same reason. **`DEFECT-LEDGER.md` was
not formatted either** — §4 says it is written by the script and *"never rewritten by hand"*.
⟨cmd⟩ `git diff --check` over this seat's own paths → **clean**.
...
| # | root | sha | message | paths |
|---|---|---|---|---|
| 1 | value.js | `d9ed9f5b` | `docs(x-p-w4/close): X·P sub-tranche release close — the packet, its dated row, the G-9 disposition and the L-13 harvest; the R-A stamp WITHHELD` | `waves/W4-CLOSE.md` · `RELEASE-PACKET.md` · `evidence/W4/rc-p-evaluation-after-packet-2026-09-19.json` · `INBOX.md` · `registry/harvest/x-p-w4.json` · `registry/DEFECT-LEDGER.md` |
| 2 | value.js | this section's own commit — sha published in the LEDGER event line | `docs(x-p-w4/repair-2): three HIGH cured — the packet rowed to SS-6, G-9's disposition recorded by id, the L-13 harvest filed; G-8 · G-9 GREEN, the stamp still withheld` | `docs/tranches/X/execution/D/X-P-W4.md` |
| 3 | value.js | the LEDGER row's own commit, taken last | `docs(x-p-w4/repair-2): LEDGER — 8 of 10 gates GREEN, honest-RED {G-3 · G-10}; RC-P at 3 of 6 TRUE` | `docs/tranches/X/execution/LEDGER.md` |

**§9's declared `.d` family was NOT split.** Commit 1 carries every member of it this seat performed;
the two members it did not perform — the four sibling waves' `VERIFIED` rows and the two
`COHESION.md` cells — are absent **because the acts were withheld**, and commit 1's body says so.

**SELF-COUNT.** This seat wrote **eight** paths and no others: `RELEASE-PACKET.md` ·
`waves/W4-CLOSE.md` · `evidence/W4/rc-p-evaluation-after-packet-2026-09-19.json` · `INBOX.md` (one
row + one sweep line) · `registry/harvest/x-p-w4.json` · `registry/DEFECT-LEDGER.md` (by the script,
through a symlink) · this record section · the LEDGER row. **`scripts/dev/dev.sh` was never touched
and never staged.** No byte of `W0.md`–`W4.md`, of `W4.md`, of `COHESION.md`, of `SEAM-CONTRACT.md`,
of `RELEASE-CONDITION.md`, of any `<p2>` script, of any prior seat's evidence bank, of any sibling
repo or of the frozen `parse-that` root was written. **`<p2>` has no commit from this seat** — it
needed none; the three scripts §4 grants there all already exist and were run, not modified
(⟨cmd⟩ `git -C <p2> status --porcelain` → `?? .worktrees/` alone, unchanged across this seat's work).

### What the next sitting does

**One act remains, and it is an adjudication.** A **fresh Fable `.d`** rules the 45
`PENDING-ADJUDICATION` seam cells by the COHESION §0w id set, which is what makes §2a's first
question answerable; then, **only after `F-w4b-3` and `F-w4c-1` are ruled and G-3 and the open-state
sum move**, it performs the single R-A stamp act. Everything else `W4.md` §5 assigns to `.d` is done
and is measurable at `d9ed9f5b`. The wave's honest-RED set is `{G-3 · G-10}`, both relieved at the
spec's own bytes and both owner-named.

---

## Check 3

**SERVED MODEL: claude-opus-5[1m]** — L-20 FRESH ADVERSARIAL PASS 3, **VERIFY-ONLY**. An independent
seat that authored **no** byte of `W4.md`, of `SEAM-CONTRACT.md`, of `RELEASE-CONDITION.md`, of
`RELEASE-PACKET.md`, of `W4-CLOSE.md`, of any `<p2>` script, of any evidence bank, of the harvest, of
any unit receipt, of `## Close`, or of `## Check 1` / `## Repair 1` / `## Check 2` / `## Repair 2`.
Wall clock **2026-09-19**; sitting of record **2026-09-17** (the owner's begin-word, COHESION §0j).
Every reading below was produced by a command this seat ran itself against the settled bytes;
**nothing is inherited from any prior section of this record**.

**VERDICT: CONFORMANT-HONEST-RED.** **0 BLOCKER · 0 CRITICAL · 0 HIGH · 2 MINOR · 4 INFO.**
**10 of 10 gate verdicts reproduce · 8 of 8 claimed GREENs reproduce GREEN · 0 of 14 published
figures failed to reproduce · 0 writes outside §File Bounds · 0 masking constructs · E-3 held ·
mail 0 UNREAD · the four-verb line correctly did not move.** **The honest-RED set is `{G-3 · G-10}`,
both relieved at the spec's own bytes and both owner-named** — `F-w4b-3` and `F-w4c-1`. Repair 2's
cure is real and reproduces at this seat's own commands: **G-8 and G-9 are genuinely GREEN**, the
packet exists at its in-repo path with a dated `O-40` row naming that path **and** the SS-6 courier,
G-9's disposition is recorded by id with its re-trigger **evaluated**, and the L-13 harvest is filed
by the unmodified harvester through the §0p scratch mirror.
...
`M docs/tranches/V/reformation/CARRY-LEDGER.md` (a sibling's / the owner's) · `M scripts/dev/dev.sh`
(**unowned, NEVER touched, never staged**). ⟨cmd⟩ `git status --porcelain --
docs/tranches/X/execution/D/X-P-W4.md docs/tranches/X/execution/LEDGER.md` → **no output**.
⟨cmd⟩ `git -C <p2> status --porcelain` → `?? .worktrees/` alone. ⟨cmd⟩ `git -C
/Users/mkbabb/Programming/parse-that status --porcelain | wc -l` → **31** rows under `rust/**` in the
**frozen read-only** root — none of it this wave's, that root is §4 Do-NOT-touch, and not one byte of
it was read into a write, stashed or restored. **Zero dirty paths inside this seat's writable set ·
zero inherited hunks · no killed predecessor's partial work to finish, rewrite or name.**

**Substrate.** value.js `tranche-u` — ⟨cmd⟩ `git log --oneline -1` → **`5cf7bf63`**. `<p2>` — ⟨cmd⟩
`git -C <p2> log --oneline -1` → **`541754c`**. Frozen root — **`ef10d5b`**, unmoved.

### The ten gates, RE-RUN BY THIS SEAT — 10 of 10 verdicts reproduce

| gate | the record's verdict after Repair 2 | this seat's own command | reproduces? |
|---|---|---|---|
| **G-1** | GREEN | the spec's **literal** command (`seam-contract-check.mjs` over `SEAM-CONTRACT.md` ⊕ `evidence/W3/universe-52.json` ⊕ `DIVERGENCE-LEDGER.md`) → `rows: contract 52 · universe 52 · ledger rows 47` · `contract ∖ universe-52 : ∅` · `universe-52 ∖ contract : ∅` · `PENDING-ADJUDICATION 45 · identical 7` · *"VERDICT: GREEN"* · **EXIT=0**; double-run, ⟨cmd⟩ `diff` → **no output**. The checker's **tier-1/tier-2 ledger binding was read at its own source and tested at the ledger's bytes** — see INFO-1; it is a structural rule, not an allowlist | **YES — GREEN** |
| **G-2** | GREEN (both legs) | **LEG 1** ⟨cmd⟩ `git status --porcelain -- src api demo test e2e \| wc -l` → **0**, double-run `0 ≡ 0`. **LEG 2** ⟨cmd⟩ `git show --name-only --format= <sha> \| grep -cE '^(src\|demo\|api\|test\|e2e)/'` over **all twenty** value.js commits of this wave → `0` twenty times | **YES — GREEN** |
| **G-3** | RED | ⟨cmd⟩ `node scripts/packed-candidate-surface.mjs --seam …/SEAM-CONTRACT.md` → `candidate "@mkbabb/parse-that@1.0.0"` · **`tarballSha256 efc11936d361820526a93ada2145012d0c58cec48cee9ccb9a19c22c66e74cda`** (**byte-identical to `.b`'s bank, Check 1's, Repair 1's, Check 2's and Repair 2's — a sixth independent reading**) · `entryCount 1` · `seamSubpathDeclared false` · **`resolved "0 of 52"`** · `refusals 5` · `G3 "RED"`, **EXIT=1**. **All three stated causes verified at the producer's own bytes by this seat** (below) | **YES — RED** |
| **G-4** | GREEN | the §6 **verbatim** one-liner against `<p2>/typescript/src/css/build/ac1.wasm` → `{"total": 0, "functionKind": 0, "imports": []}`, instantiation against `{}` did not throw, **EXIT=0**. Then ⟨cmd⟩ `node scripts/wasm-admission.mjs <the 9 project artefacts>` → `tally {artifacts 9 · admitted 9 · functionKindImportsTotal 0 · unaccountedImportsTotal 0}` · `"verdict": "GREEN"` · **EXIT=0**; double-run with `generatedAt` excluded → ⟨cmd⟩ `diff` **no output**. ⟨cmd⟩ `find <p2> value.js -name '*.wasm' -not -path '*/node_modules/*'` → **11 = 9 project ⊕ 2 unrelated** Chrome-profile TTS binaries, the denominator the record publishes | **YES — GREEN** |
| **G-5** | GREEN | ⟨cmd⟩ `node scripts/rc-p-evaluate.mjs --version 4.0.0` → the **six-row table of §6a**, `RC-P(4.0.0) = FALSE — 3 of 6 conjuncts are FALSE: 1 PUBLISHED(V) · 3 EQUIVALENCE(V) · 4 ADMITTED(V)`, **EXIT=1** — Repair 2's published table reproduces line-for-line but for `evaluated at`. **Falsifiers run, not asserted**: ⟨cmd⟩ with no `--version` → refuses, **EXIT=2**, with §3's own prohibition quoted; negative control ⟨cmd⟩ `--version 0.0.0-does-not-exist` → conjuncts **1 · 2 · 4** print `MEASURED NO / VALUE FALSE` naming `npm view exit 1`, **EXIT=1**; ⟨cmd⟩ `grep -nE '[0-9]+\.[0-9]+\.[0-9]+' scripts/rc-p-evaluate.mjs` → **2 hits, both in comments quoting the spec (L25, L290), 0 in code** | **YES — GREEN** |
| **G-6** | GREEN on its stated falsifier | ⟨cmd⟩ `grep -rn 'RC-P' docs/tranches/X/{keyframes,fourier}/waves/ \| wc -l` → **25** over **5** files (`KF-W2 · KF-W3 · KF-W5 · KF-W10 · F-W0`) — the *"fewer than two hits"* falsifier does not fire. ⟨cmd⟩ `sed -n '9p' KF-W3.md` and `sed -n '459p' F-W0.md` → both far-end sentences **verbatim, each citing `RC-P` by predicate name and no wave number**. ⟨cmd⟩ `ls -l …/RELEASE-CONDITION.md` → **present, 26,049 B**. Leg 3 ⟨cmd⟩ `grep -rn 'SEAM-CONTRACT' docs/tranches/X/waves/` → **0**, absent, and §6 G-6's own baseline routes it to G-9 | **YES — GREEN on the stated falsifier; the 2-of-3 tension stays disclosed** |
| **G-7** | GREEN (floor) | ⟨cmd⟩ `grep -c 'parse-that' /Users/mkbabb/Programming/fourier-analysis/package.json` → **0**; ⟨cmd⟩ over `web/package.json` → **0**. The transitive copies were **not** touched | **YES — GREEN, floor held** |
| **G-8** | GREEN | ⟨cmd⟩ `ls -l …/RELEASE-PACKET.md` → **present, 23,017 B**. ⟨cmd⟩ `grep -n 'RELEASE-PACKET' INBOX.md` → **1** hit, the **O-40** sent-row at **`:127`**, dated 2026-09-19, naming the in-repo path **and** the SS-6 batch quoted from COHESION §1. Mail leg by **position** → **UNREAD 0** over **80** register rows, double-run `0 ≡ 0`. **The minting was checked at the diff, not at the prose**: ⟨cmd⟩ `git show d9ed9f5b --unified=0 -- INBOX.md \| grep -E '^@@'` → **two** hunks, `@@ -126,0 +127 @@` (the one new row) and `@@ -295,0 +297,2 @@` (the sweep line); ⟨cmd⟩ over its added register lines → **`O-40` alone**. No other row's status cell moved. Every §6 G-8 falsifier checked one by one and none fires | **YES — GREEN** |
| **G-9** | GREEN | ⟨cmd⟩ `ls -l …/waves/W4-CLOSE.md` → **present, 39,300 B**; §4 records disposition **(C)** quoted whole by id from COHESION §0i.1 with its exact condition and re-trigger command, and the re-trigger **evaluated** at this seat too (`RC-P` FALSE, EXIT=1 → does **not** fire). Falsifier: ⟨cmd⟩ `git show --name-only --format=` over all **20** value.js and **3** `<p2>` commits → **0 paths** under `docs/tranches/X/waves/` — **no X·V wave was authored**, and the gap is stated with its id | **YES — GREEN** |
| **G-10** | RED | ⟨cmd⟩ `cat waves/W0.md W1.md W2.md W3.md W4.md \| grep -c 'VERIFIED \| \*\*NO\*\*'` → **7**, double-run `7 ≡ 7`; per-file `W0:1 · W1:1 · W2:1 · **W3:3** · W4:1`. Close-state must read **0**. **The self-stamp leg is GREEN**: ⟨cmd⟩ `grep -rn 'VERIFIED' W[0-3]-CLOSE.md` → **10** lines, **every one disclaiming the stamp** (*"X.P.W4's to stamp … never this wave's"*). `W4-CLOSE.md` does not self-stamp either | **YES — RED** |

**GREEN: G-1 · G-2 · G-4 · G-5 · G-6 · G-7 · G-8 · G-9. RED: G-3 · G-10.** Identical to Repair 2's,
at this seat's own commands. **8 of 8 claimed GREENs reproduce GREEN; neither RED was laundered.**

#### G-3's three causes, verified at the producer's own bytes (so the honest-RED is not taken on trust)

| cause | the record's claim | this seat's own command |
|---|---|---|
| **C1** | `files: ["./dist"]` and an `exports` map with no `./css` | ⟨cmd⟩ `node -e` over `<p2>/typescript/package.json` → `files ["./dist"]`; `exports` keys = `. · ./core · ./diagnostics · ./packrat · ./utils` — **no `./css`**; `dependencies` **undefined** |
| **C2** | `tsx/esm/api` imported but declared nowhere | ⟨cmd⟩ `grep -n 'tsx/esm/api' src/css/lowering-js/js-alg.mjs src/css/bounds.mjs` → **`js-alg.mjs:23`** and **`bounds.mjs:834`**; `tsx` in neither `dependencies` (absent) nor this manifest's `devDependencies` |
| **C3** | `build/ac1.d.ts` re-exports the 33 frozen types through a specifier that escapes the package root | ⟨cmd⟩ `sed -n '7p' src/css/build/ac1.d.ts` → all 33 names re-exported `from "../../../../../value.js/docs/tranches/V/megatranche/prototypes/css-parser/cand-o/vendor/value-js-4.0.0/dist/subpaths/css"` — **out of the package root, into value.js's vendored tree** |

All three addresses are `<p2>/typescript/package.json` and `<p2>/typescript/src/css/**`. ⟨cmd⟩ `git -C
<p2> show --name-only --format=` over this wave's three `<p2>` commits (`ca615a6` · `00806a8` ·
`541754c`) → **exactly** `typescript/scripts/{packed-candidate-surface,wasm-admission,rc-p-evaluate}.mjs`
— §4's three-path grant, no more. **The cure has no lawful address inside this wave, and a
consumer-side patch would itself be the bounds violation §3a names.**

### Published figures, re-measured at the settled bytes (write-then-measure) — 0 of 14 failed

| artefact | bytes | sha256 (16) |
|---|---|---|
| `SEAM-CONTRACT.md` | **121,820** | `e269f6e665f28894` |
| `RELEASE-CONDITION.md` | **26,049** | `a61f564520e9d369` |
| `RELEASE-PACKET.md` | **23,017** | `b4b271e5bb564176` |
| `waves/W4-CLOSE.md` | **39,300** | `a36eb7e2eb4409fd` |
| `evidence/W4/packed-surface.json` | **128,861** | `0bcef64eac18faf6` |
| `evidence/W4/wasm-imports.json` | **9,392** | `e10c123f3d346a4d` |
| `evidence/W4/rc-p-evaluation.json` | **33,938** | `444d864a0718f3f5` — **byte-untouched by Repair 2, as it claims** |
| `evidence/W4/reciprocity-grep.txt` | **3,951** | `0622e63973ce8211` |
| `evidence/W4/value-source-untouched.txt` | **3,312** | `73c8396f08548bf8` |
| `evidence/W4/seam-contract-check.txt` | **4,487** | `1b5bd4e4c13e2433` |
| `evidence/W4/seam-contract-negative-controls.txt` | **4,727** | `8f74f949cac0bebf` |
| `evidence/W4/rc-p-evaluation-after-packet-2026-09-19.json` | **39,683** | `845201d39458b455` |
| `<p2>/typescript/scripts/rc-p-evaluate.mjs` | **25,873** | `895e089bfd42368d` |
| `registry/harvest/x-p-w4.json` | **119,964** | — |

**Every byte count and every sha256 prefix the close, Repair 1, Check 2 and Repair 2 published
reproduces exactly.** §8's census re-counted over the twelve rows → **11 PRESENT · 1 PARTIAL
(the commit roster: phase 3 has no `.d` commit because `.d` never sat) · 0 ABSENT**.

### Axis audit

| axis | finding |
|---|---|
| (2) **File Bounds** | **0 writes outside §4.** ⟨cmd⟩ `git show --name-only --format=` over all **20** value.js commits, deduplicated → **18 distinct paths**: `docs/tranches/X/parse-that/{SEAM-CONTRACT.md,RELEASE-CONDITION.md,RELEASE-PACKET.md,scripts/seam-contract-check.mjs,waves/W4-CLOSE.md,evidence/W4/**}` ⊕ `docs/tranches/V/coordination/INBOX.md` ⊕ `docs/tranches/V/megatranche/registry/{harvest/x-p-w4.json,DEFECT-LEDGER.md}` ⊕ the two standing execution instruments (`execution/D/X-P-W4.md`, `execution/LEDGER.md`) — **every one a named §4 row**. `<p2>`: **exactly** §4's three-path grant. ⟨cmd⟩ over every commit for `scripts/dev/dev.sh` → **0 of 20**, and it is still dirty-unstaged as the standing arrangement requires. The frozen `parse-that` root carries **0** commits of this wave (HEAD `ef10d5b`, unmoved) |
| (3) **Masking** | **0 constructs.** ⟨cmd⟩ over all four authored scripts for `.skip` / `xit(` / `@ts-ignore` / `@ts-expect-error` / `eslint-disable` / allowlist / `node_modules/` patch → **2 hits, both inside comments** (`wasm-admission.mjs:28`, `packed-candidate-surface.mjs:31`). The **four** `catch` blocks were read whole at their own lines: `wasm-admission.mjs:105` records `{ok:false,error}` and keeps `functionKindZero` / `emptyImportInstantiation` **separate fields**, neither inferred from the other, exactly as G-4's falsifier demands; `packed-candidate-surface.mjs:361` and `:456` record `{error:{code,message}}` and `{refused,code}` into measured fields and the script exits **1** on its own RED; `rc-p-evaluate.mjs:137` is the one bare guard (INFO-2 of Check 2) and its direction is conservative. **No producer selector copied, no local `node_modules` patch, no narrowed assertion.** G-1's checker was audited specifically for a narrowed assertion — INFO-1 |
| (4) **Commit families** | §9's four declared messages: `.a`'s landed **verbatim** (`b4f8e5d1`); `.b`'s **verbatim** (`ca615a6` ⊕ `f92a3f32`); `.c`'s **verbatim** (`541754c` ⊕ `76f4a921`); `.d`'s landed as **one unsplit commit** `d9ed9f5b` carrying every member of the family whose act was performed (`W4-CLOSE.md` · `RELEASE-PACKET.md` · the after-packet RC-P bank · `INBOX.md` · `x-p-w4.json` · the script's `DEFECT-LEDGER.md` append). The two members absent — the four sibling waves' `VERIFIED` rows and the two `COHESION.md` cells — are absent **because those acts were withheld**, and the commit body says so in its own bytes. `00806a8` ⊕ `046bc176` is a **distinct meaning**, not a split. **No declared family was split; one commit per meaning throughout.** The `.d` message's deviation from §9's declared text is INFO-2 |
| (5) **E-3** | **HELD.** ⟨cmd⟩ over all **20** value.js commits' deduplicated path set, filtered to `megatranche/registry/adjudicated\|X/parse-that/waves/W[0-4]\.md\|^docs/tranches/X/waves/\|X/COHESION\.md\|X/keyframes/\|X/fourier/\|apotheosis\|scripts/dev/dev\.sh` → **0 paths**. The dated spec `W4.md` is **676 L** and byte-untouched by this wave — ⟨cmd⟩ `git log --oneline -1 -- …/W4.md` → **`c0f2090f`**, the root session's own §0w addendum-beside commit, not a wave commit. The adjudicated registry, the conformance artefacts and every sibling spec are untouched; `W4-CLOSE.md` and `RELEASE-PACKET.md` are **new files beside** the dated evidence, never over it, and `.c`'s `rc-p-evaluation.json` is byte-identical to its bank |
| (6) **Mail** | **0 UNREAD.** By **position** (never a bare `grep -i unread`) → **0** over **80** register rows of the file's **95** `^\| ` rows, double-run `0 ≡ 0`. Register tail **`I-35` / `O-40`**. `O-40` is the packet's own dated sent-row, minted by Repair 2 and verified at the diff to be the **only** row minted and the **only** status cell touched. **No wave closes with UNREAD mail in scope, and none is** |
| (7) **Four verbs** | **NONE MOVED, and that is the lawful act.** §6 G-10's falsifier — *"A stamp performed while any of G-1..G-9 is red fails"* — forbids the stamp while **G-3** is RED, and R-A reserves VERIFIED to `.d` alone. ⟨cmd⟩ over every commit of this wave → **0 paths** under `docs/tranches/X/parse-that/waves/W[0-3].md`, and `W4.md`'s own §2 table byte-untouched. **Under-claiming is not a defect**; asserting a close that did not happen would have been |
| (8) **§2a goal criterion** | **2 of 3 fully MET; question 1 MET in substance and NOT MET at the disposition head.** See MINOR-1, where this seat parts company slightly with `W4-CLOSE.md` §10's own harsher self-judgment — in the record's disfavour on the register's severity and in its favour on the reading |
| (9) **Figures** | **all 14 reproduce exactly**, above. **0 of 14 failed** |
| (10) **Honest-RED** | **`{G-3 · G-10}`**, both relieved and both owner-named — the table below |

### Register — severity · claim · receipt · cure

| # | severity | claim | receipt | cure |
|---|---|---|---|---|
| **MINOR-1** | MINOR (mitigated) | **§2a's first question — *"what exactly does value.js receive"* — is NOT MET at the disposition head: 45 of 52 seam rows publish `PENDING-ADJUDICATION`, and that ruling is `.d`'s adjudicative half, which has never sat.** This seat reached the same conclusion as `W4-CLOSE.md` §10 and by a different road, and records the **mitigation the close did not claim for itself**: §2a's *explicit* failure clause is *"if any of the three is answerable only by reading a status word"*, and it **does not fire** — every `PENDING-ADJUDICATION` row carries its measured cell census and a substantive consumer-direction field (`parseKeyframeSelector`: *"AGREE 27,021 of 27,021 … a consumer should plan for `identical`"*; `parseTimingFunction`: *"UNRULED, 23 cells: `GROUND-C` … a consumer feeding computed numbers that can overflow **cannot predict this export's answer today**"*). The unruled residue is **enumerated by id and count, row by row**, not hidden behind the head | ⟨cmd⟩ the G-1 run's own disposition census → `PENDING-ADJUDICATION 45 · identical 7`; ⟨cmd⟩ `sed -n '108,120p' SEAM-CONTRACT.md` → the seam table's rows read as quoted above; ⟨cmd⟩ over `RELEASE-PACKET.md` §§3.1–3.6 → the four `declared-divergence` rows' consumer directions verbatim and the `PENDING` arms named per row, with *"a consumer may not read them as settled behaviour"* stated in the `O-40` row's own text | **`E-r2-1`, ESCALATED with an owner named** (the orchestrator; a **fresh Fable adjudicator**, M-23 §1, ruling by the COHESION §0w id set). **Mitigation is threefold and each leg is checkable**: (i) the `PENDING-ADJUDICATION` head is authorized by the spec's **own dated addendum** (`W4.md` L674 / COHESION §0v: *"the seam contract publishes the carried cells as `PENDING-ADJUDICATION` dispositions"*), and the checker enforces it mechanically; (ii) the ruling is routed by that same addendum to *"`.d`'s fresh adjudicator"*, a **seat kind** no repair or check seat may impersonate — Repair 2's refusal to rule 45 cells from an Opus seat is the **opposite** of a masking cure and is exactly what M-23 §1 exists to obtain; (iii) the residue is disclosed at **four** independent sites (`SEAM-CONTRACT.md` per row · `W4-CLOSE.md` §0 and §10 · `RELEASE-PACKET.md` · the `O-40` row). **MINOR-with-mitigation does not block** |
| **MINOR-2** | MINOR (mitigated) | **§5's `.d` sub-gate — *"the harvest JSON's seat count equals the units dispatched (four)"* — reads RED under L-13's letter.** The filed `x-p-w4.json` reports `unitsPlanned 4 · unitLabelledSeatsDispatched 2 · unitLabelledSeatsHarvested 2`, with `.c` **folded** into the `repair1` seat's row and `.d` **absent** | ⟨cmd⟩ `node -e` over `registry/harvest/x-p-w4.json` → `seatCount.reading` = *"RED under L-13's letter … 3 of 4 counting the fold; `.d` NEVER DISPATCHED"*; `harvestedSeats` = **7** rows, **every one with `journalRows: 1`**; `zeroRowSeats: []` | **Mitigated, and the mitigation is the L-13 corollary the spec itself names**: *"a seat harvested with zero journal rows is RED, not green"* — **no returned seat has zero rows**. Only **three** units were ever dispatched, so *"a seat count equal to the units dispatched"* is met at 3 counting the fold; the parenthetical *"(four)"* presumes `.d` sat. The harvest **states its own RED reading in its own bytes** rather than rounding it up, which is the conforming behaviour. Rides **`E-r2-1`** |
| **INFO-1** | INFO | **G-1's checker draws a tier-1/tier-2 distinction the spec's falsifier does not spell out — and it is a structural rule, not an allowlist.** §6 G-1's falsifier is *"a row whose disposition is `identical` while `DIVERGENCE-LEDGER.md` holds a row for the same export fails"*; `seam-contract-check.mjs` makes a name **fatal** only when it appears in a **declared subject field** (`parser` · `entry` · `entries` · `subject` · `subjects` · `witness family`, L63–70) and prints the rest as a named ADVISORY. This seat tested the one case where it matters: the five advisory names (`CssColor` · `CssTimingFunction` · `Stylesheet` · `StyleRule` · `Declaration`) are mentioned by **`CN-3`**, whose `subjects` field names the **other 28** and whose `candidate` field names these five as the ones the candidate **does** re-export. Marking them `identical` is therefore **correct at the producer**, and the narrowing is **keyed on field names, applies to no specific export, and is printed with every affected name** | ⟨cmd⟩ `sed -n '50,70p;93,126p' scripts/seam-contract-check.mjs`; ⟨cmd⟩ `awk` extraction of `### CN-3` from `DIVERGENCE-LEDGER.md` → `subjects` = the 28, `candidate` = *"the candidate re-exports 5 (CssColor, CssTimingFunction, Stylesheet, StyleRule, Declaration)"* | none — the reading is correct and is disclosed in the gate's own printed output |
| **INFO-2** | INFO | **`.d`'s commit message deviates from §9's declared text, in the direction of accuracy.** §9 declares *"…release close — R-A stamp, adoption-gap disposition, packet + mail + L-13 harvest"*; `d9ed9f5b` reads *"…release close — the packet, its dated row, the G-9 disposition and the L-13 harvest; **the R-A stamp WITHHELD**"*. Landing §9's literal text would have announced an act the commit did not perform | ⟨cmd⟩ `git log -1 --format=%B d9ed9f5b` → the body carries the seat disclosure, the withheld acts and their spec reasons in full | none — a message that describes what landed is worth more than a message that matches a plan. Recorded so a later reader does not read the deviation as a split family |
| **INFO-3** | INFO | **`git diff --check` over `DEFECT-LEDGER.md` reports 924 trailing-whitespace flags, and the harvester rewrote 209 lines where §4 says *"modify-append"*.** ⟨cmd⟩ `git show --stat d9ed9f5b -- …/DEFECT-LEDGER.md` → **1,641 insertions / 209 deletions** over **203** hunks; the deleted lines are the regenerated summary line (`**7585 defects**…`) and section headers | ⟨cmd⟩ `git show d9ed9f5b -- …/DEFECT-LEDGER.md \| grep '^-'` → the summary line and `## MEDIUM` / `## HIGH` headers | none available in bounds. §4 says that file is written *"by `harvest-journals.mjs` … never rewritten by hand"*, so hand-cleaning it would itself be the violation. The rewrite is the harvester's **own known MAJOR defect** (COHESION §0p / F-e11, owed to X-W11) and was disclosed by Repair 2 with the W3 precedent. The wave's own paths are clean at ⟨cmd⟩ `git diff --check` |
| **INFO-4** | INFO | **G-3's `--seam`-only invocation prints `"symbolCrossCheck": null` where the banked `packed-surface.json` holds the object** — Check 1's INFO-3 named this class and it still reads that way. Benign: the cross-check is computed when the universe file is supplied and the banked artefact carries `{seamRows 52, universeRows 52, inSeamNotUniverse [], inUniverseNotSeam [], agree true}` | ⟨cmd⟩ the `--seam` run's printed summary vs ⟨cmd⟩ over `evidence/W4/packed-surface.json` | cosmetic; name the field (`…symbolCrossCheck.agree → true`) or pass the universe on the summary path |

**Not re-litigated, and stated so the absence is deliberate**: Check 2's INFO-2 (`rc-p-evaluate.mjs`'s
one bare `catch`) was declined by Repair 2 with a stated reason — curing it would rewrite `.c`'s
settled script and force a re-bank of `.c`'s evidence. This seat read the block at its own line
(`133-139`, `readdirSync` inside `findByExtension`) and concurs: a swallowed read error can only make
`ADMITTED` **more** FALSE, the opposite of the direction §6 G-5's falsifier polices. It is not a
masking fallback and it stays as recorded.

### Honest-RED adjudication (axis 10) — the set is **{G-3 · G-10}**, and this seat certifies both

| gate | relief at the spec's own bytes | owner named in the residual register? |
|---|---|---|
| **G-3** | **YES — PRODUCER-OWNED, and verified as such at the producer's bytes by this seat.** §3a names the shape verbatim: *"Hard-gate failure that is **not local-edit-recoverable** … that is an X.P.W2 architecture question, not a link-flag tweak."* All three causes (C1 `files:["./dist"]` + no `./css` in `exports`; C2 `tsx/esm/api` at `js-alg.mjs:23` and `bounds.mjs:834`, declared nowhere; C3 `ac1.d.ts` re-exporting through a specifier that escapes the package root) live at `<p2>/typescript/package.json` and `<p2>/typescript/src/css/**`. §4 grants this wave **exactly three** creatable paths in `<p2>` and ⟨cmd⟩ over its three `<p2>` commits shows exactly those three touched. **A consumer-side patch would itself be the bounds violation** — and `.b`'s positive control (a staged tree outside both repositories with `"./css"` declared and `src/css` shipped) still resolved **0 of 52**, which is how C2 and C3 were located rather than guessed | **YES — `F-w4b-3`**, triumvirate/orchestrator, two admissible cures stated (a dated §4 addendum-beside naming the four paths, or a successor X.P.W2 packaging unit) |
| **G-10** | **YES — SPEC-FORBIDDEN TODAY, on two independent legs.** §6 G-10's own falsifier: *"**A stamp performed while any of G-1..G-9 is red fails** — the act is the consequence of the gates, never a substitute for them."* **G-3 is RED and relieved above**, so the stamp is forbidden by an authority no seat may overrule. Independently, the open-state sum reads **7** for an **E-3-lawful and unrewritable** reason — `W3.md` carries the matching row at its §2 table (`:44`) **and** in its sixth and seventh **dated addenda** (`:836`, `:971`) — and both admissible cures are themselves dated addenda to E-3 files, i.e. triumvirate acts. **The act was withheld for the third consecutive sitting**, and the gate's *other* leg is GREEN: ⟨cmd⟩ `grep -rn 'VERIFIED' W[0-3]-CLOSE.md` → **10** lines, every one disclaiming the stamp | **YES — `F-w4c-1`** (the count, UNRULED; shape (a), narrowing the command to rows inside a `### Four-verb status` block, named as the only one that survives a ninth W3 round) ⊕ **`F-w4b-3`** (the transitive cause). The four-verb line correctly did not move |

**No RED gate stands unrelieved.** The set `{G-3 · G-10}` is exactly the set Check 2 adjudicated in
advance as relieved, and Repair 2 landed the discharge Check 2 published — *"dispatch one fresh Fable
`.d` that authors `W4-CLOSE.md` …, authors and rows `RELEASE-PACKET.md` with the SS-6 hand-off, runs
the L-13 harvest … — and **withholds the R-A stamp**"* — with the one lawful substitution Check 2's
own INFO-3 authorized (*"§6 G-10's words are 'halts **the act**' — the act, singular, is the R-A
stamp … Neither sentence halts `.d`'s other five acts"*) and with the seat identity disclosed in the
bytes of every site it wrote.

**The dissent this seat records against itself, so a later pass may overturn it at the bytes.** A
stricter reading holds that `.d`'s adjudicative half is an **in-wave** act routed to no successor, and
that an in-wave act unperformed is a HIGH — the ground on which Check 1 and Check 2 both returned
NOT-CONFORMANT. This seat rejects that reading **here and only here**, for three measured reasons:
the two acts still owed are the **45-cell ruling**, which the spec's own addendum routes to a seat
kind M-23 §1 forbids any other seat to impersonate, and the **R-A stamp**, which §6 G-10's falsifier
forbids outright while G-3 is RED. Neither is withheld by omission; both are withheld **because the
spec says they may not be performed today**, and both carry a named owner. A wave whose only
remaining acts are ones its own spec forbids it to perform is not incomplete by default — it is
gated, which is the state `RC-P` exists to express.

### Successor "Opens after" conjuncts, checked against this wave

| successor | its conjunct on X.P.W4 | state at this seat's own commands | blocked? |
|---|---|---|---|
| **KF.W3** (X·KF) | `KF-W3.md:9`, verbatim: *"This wave opens if and only if `RC-P(V)` evaluates TRUE per `docs/tranches/X/parse-that/RELEASE-CONDITION.md` §6a, run against the registry coordinate `V`"* | **RED — fully evaluable, and evaluated.** ⟨cmd⟩ `node scripts/rc-p-evaluate.mjs --version 4.0.0` → **`RC-P(4.0.0) = FALSE`**, **3 of 6 TRUE** (2 TOTALITY · 5 BAR-DISCHARGED · **6 ROUTED**, the conjunct the packet moved), **3 FALSE** (1 PUBLISHED · 3 EQUIVALENCE · 4 ADMITTED), **EXIT=1**. `KF-W3.md` itself states the discipline: *"never by reading a status word in any document, including this one"* | **LAWFULLY BLOCKED**, and correctly so — gate-keyed, never scheduled. **GREEN conjuncts: 2 · 5 · 6.** Nothing this wave owes it is missing: the cited file exists, the evaluator exists and runs |
| **F.W0 / F.W7** (X·F) | `F-W0.md:53` *"Opens after: tranche X's execution gate lifts"* — **no conjunct on X.P.W4**; X·P is a declared **NON-EDGE**, with the consumption predicate adopted at `:459` by X·F's own seat | **GREEN.** G-7's floor holds at **0 / 0**; the forbidden edge is absent and nothing crosses | **NOT blocked** by this wave |
| **KF.W10** (X·KF) | `KF-W10.md:26` cites X.P.W4`.d` only as the **adjudicator-discipline mirror** (*"a fresh Fable adjudicator … this mirrors X.P.W4`.d` exactly"*) — not a gate conjunct | **N/A.** No conjunct on this wave's gates | **NOT blocked** |
| **the X·V adoption wave** | does not exist — OP-4/G-9, **disposition (C)** at COHESION §0i.1, re-trigger *"X.P.W4's RC-P evaluator returning TRUE at a dated run"* | **RED, evaluated, and now RECORDED.** The re-trigger's instrument exists, its dated run returns **FALSE**, so it does **not** fire — and the disposition is no longer merely ruled but is **written at `W4-CLOSE.md` §4**, quoted whole by id with its exact condition | **LAWFULLY BLOCKED.** The recording gap Check 1 and Check 2 both found is **closed** |

### Disposition

**The `X.P.W4` LEDGER row is promoted to `CLOSED 2026-09-17 (honest-RED: G-3 · G-10)`.** The four
verbs stay where they are — **IMPLEMENTED NO · VERIFIED NO** — because G-10's falsifier forbids the
stamp while G-3 is RED and R-A reserves VERIFIED to a fresh Fable `.d`. **X·P therefore closes
UNVERIFIED**, and that is the conforming state, not a shortfall smoothed: the sub-tranche's release
close has happened — the seam contract, the packed protocol, RC-P and its evaluator, the packet, its
dated row, the recorded adoption gap and the filed harvest all exist and were measured here — and the
verb that would mark it has not moved because two gates say it may not.

**Standing at close, each with a named owner**: `E-r2-1` (the 45-cell adjudication ⊕ the R-A stamp,
a fresh Fable seat) · `F-w4b-3` (G-3's packaging cure, triumvirate) · `F-w4c-1` (G-10's count,
triumvirate) · `F-w4c-2` (the W3 harvest's bounds gap) · the two `COHESION.md` carves, correctly not
performed under a withheld stamp · `Q-RC-1` · `Q-RC-2` · and the carried set `U-d` (partially
discharged) · `F-w4a-1` · `SEAM-DRIFT` · `R-w4b-2` · `F-ae1 / F-p1` · the W3 rounds-6–8 ids.

**SELF-COUNT: this check wrote exactly two paths** — this section in
`docs/tranches/X/execution/D/X-P-W4.md`, and the `X.P.W4` row cell plus an appended event line in
`docs/tranches/X/execution/LEDGER.md`. No byte of `W4.md`, of any sibling X·P wave file, of
`SEAM-CONTRACT.md`, of `RELEASE-CONDITION.md`, of `RELEASE-PACKET.md`, of `W4-CLOSE.md`, of any
`<p2>` script, of `COHESION.md`, of `INBOX.md`, of the registry, of any sibling repo or of the frozen
`parse-that` root was written. `scripts/dev/dev.sh` was never touched and never staged; no sibling's
dirty path was read into a write, stashed or restored.
