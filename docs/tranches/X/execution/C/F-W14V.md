...
SERVED MODEL: claude-opus-5-5 · 2026-09-25 · addendum (h) item 3(i) (C3-3 = ESC-u1-1, F-81 i: one easing catalogue). Spec read whole (87 lines); record read header → RESUME 1 plan + `.r1` receipt; COHESION §0eb + §0ec (file end) read.

**Acts, in order**
1. Crash-recovery ⟨`git -C fourier-analysis status --porcelain`⟩ → only `src/fourier_analysis/contours/*` (F.CT) + `?? .worktrees/`; nothing inside `api/**`, `web/src/**`, `web/e2e/**` → no inherited work.
2. Anchors at true bytes: `api/models/shared.py:69` `easing: str = "sine"` (matches). Two client catalogues in `web/src/lib/easings.ts`: `EASING_LABELS`/`EASING_FNS` (22 names, `ease-in-out-sine`…) and `ANIMATION_EASINGS` (six short keys `linear sine quad cubic circ expo`, `:202`). Server readers of `animation_settings`: `visualizations` (the `Visualization` model), `visualization_versions` (only inserted, never parsed back), request bodies. Local persistence holding the easing: the IndexedDB draft store `fourier-drafts` v2 (`draftStorage.ts`, `WorkspaceDraft.animationSettings`); `useViewState`'s localStorage key carries no easing. Dev db ⟨`$group animation_settings.easing` on :27018⟩ → `visualizations {sine: 789}`, `visualization_versions {sine: 789}`, 0 `atom_diff` ops on `animation_settings`.
3. `set_hash` measured before choosing the migration's shape: `atomdiff.set_hash` does digest `animation_settings`, but `PUT` already edits `animation_settings` in place without re-keying (`_head_set_hash` returns the stored key), so `set_hash` is the version key stamped at create/remix, not a live digest. The rewrite is the same kind of in-place edit and does not re-key; `visualization_versions` (content-addressed history, `_id = slug:set_hash`) are left as recorded. Both are written into the migration's docstring.
4. RED first: `api/tests/test_migrate_animation_easing.py` (8 cases): 8 seeded rows (six legacy keys, one already `ease-in-out-cubic`, one off-catalogue `bounce`) → `run_migration` → each read back through `GET /api/visualizations/{slug}` (real ASGI stack) holds its catalogue name, `bounce` holds the default; idempotency (dry 5/1, first 5/1, second 0/0 with 8 on catalogue); `AnimationSettings(easing=<legacy>)` raises (no alias) ×5; the default is `ease-in-out-sine`. ⟨`MONGO_TEST_URI=mongodb://127.0.0.1:27018 .venv/bin/python -m pytest api/tests/test_migrate_animation_easing.py -q`⟩ ×2 → `8 failed in 0.66s` · `8 failed in 0.45s`. Committed as fourier `8022412`.
5. Cure (one meaning, fourier `71e0c6d`, 11 files +243/−39):
   - **Server.** `api/models/shared.py`: `AnimationEasing = Literal["linear", "ease-in-out-sine", "ease-in-out-quad", "ease-in-out-cubic", "ease-in-out-circ", "ease-in-out-expo"]`, `DEFAULT_ANIMATION_EASING = "ease-in-out-sine"`, `easing: AnimationEasing = DEFAULT_ANIMATION_EASING`. `api/scripts/migrate_animation_easing.py` (new; the `migrate_flags_field` shape: `run_migration(db, dry_run)`, `Report`, a post-condition that no stored easing is off the catalogue, `--dry-run`): one `update_many` per legacy value → catalogue name; an off-every-catalogue value → the default (discarded, counted apart). The legacy map lives only in this one-time script. Registered in `run_pending_migrations.MIGRATIONS` after `migrate_visualization` and before `migrate_visualization_forks` (which builds `VisualizationVersion` from stored atoms, so it would refuse a legacy key).
   - **Client.** `easings.ts`: `AnimationEasingName` = the six catalogue names; `ANIMATION_EASINGS` is built from the catalogue's own `EASING_FNS` (labels and descriptions unchanged, so the picker's copy is unchanged); `DEFAULT_ANIMATION_EASING` exported and read by `defaults.ts`, the store's initial ref and `coerceAnimationEasingName`. `types.ts` `AnimationSettings.easing: AnimationEasingName` (was `string`), so `useWorkspaceLoader.ts`'s `as EasingName` cast is gone and the store's `type EasingName` alias (its only reader) is deleted. `draftStorage.ts` `DB_VERSION` 2 → 3: the v3 upgrade walks the store and deletes every pre-v3 `animationSettings.easing`, so the restore seam lands the default. Nothing maps an old key to a new one on the client.
   - **Adjacent edits (§0bt):** `web/e2e/f-w14u-shell.spec.ts:250` and `web/e2e/f-w14u-gallery.spec.ts:66` open `fourier-drafts` at `3` (was `2`). These seeders mirror the app's DB version, and opening at 2 after the app's v3 open is a `VersionError`. No assertion changed. (Both are also inside the unit's writable set.)
6. Dev db migration (:27018): ⟨`MONGO_URI=mongodb://localhost:27018/fourier .venv/bin/python -m api.scripts.migrate_animation_easing --dry-run`⟩ → `scanned 789 · rewritten 789 · reset_off_catalogue 0`; live run 1 → `rewritten 789`; live run 2 → `rewritten 0 · already_on_catalogue 789`; run 3 (re-read) → `rewritten 0`. ⟨`$group`⟩ → `[{'_id': 'ease-in-out-sine', 'n': 789}]`.

**Commits**: fourier `8022412` (the falsifier) and `71e0c6d` (the cure plus the two adjacent seeder lines), pushed on `m/w1-bump-migration`. Pathspec commits only; the F.CT `src/fourier_analysis/contours/*` edits were not touched.

**Gates BEFORE → AFTER**
- api legacy-doc test (`test_migrate_animation_easing.py`): RED `8 failed` ×2 → GREEN `8 passed in 6.78s` · `8 passed in 7.83s`. At the committed bytes, with `test_owner_required`: `9 passed in 1.79s` · `9 passed in 2.02s`.
- Migration idempotent on :27018: `789 → 0` rewritten (runs 2 and 3 both 0). The test's idempotency case also shows this (second run `(0, 0)`).
- api suite, including `test_owner_required`: ⟨`MONGO_TEST_URI=mongodb://127.0.0.1:27018 .venv/bin/python -m pytest api/tests -q`⟩ → `294 passed (101.8s)` · `294 passed (67.5s)` GREEN ×2. That is the banked 286 plus 8 new.
- vitest ⟨`npx vitest run`⟩ → `20 files · 116 passed` ×2 GREEN (banked 116, unchanged).
- vue-tsc ⟨`npx vue-tsc --noEmit; echo $?`⟩ → `0` ×2 (before and after commit). eslint on the six changed `src` files → clean.
- Neighbour e2e (the draft store and the easing picker; `:3100` → `:8000`, chromium, `--workers=1`): ⟨`f-w14u-gallery f-w14u-shell settings-persistence`⟩ → `47 passed (2.0m)`; ⟨`f-w14u-vdock`⟩ → `10 passed (42.0s)`.
- **G-u:** F-81 (i) is CURED. With `.r1`'s F-79/v88 cure (`e97a959`), G-u reads **255/256 dispositioned plus one ADOPT-AT-LANDING** (F-81 ii / F-9 transport = O-86 TIMELINE-TRANSPORT, 11.0.0).

**Residuals**
- F-81 (ii): the Timeline transport is O-86, ADOPT-AT-LANDING. No consumer transport was built.
- The /morph easing pickers stay ADOPT-AT-LANDING on O-74a E-3 (`.au3`) and were not touched.
- `visualization_versions` keep their recorded `animation_settings` (789 rows, `sine`). This is content-addressed history, never parsed through `AnimationSettings` and not read by the web (`/diff` ops carry no easing on dev). Recorded as a by-design exclusion in the migration's docstring.
...
SERVED MODEL: claude-opus-5-5 · 2026-09-26 · REPAIR SEAT (round 1 of RESUME 1), Track C. Spec `F-W14V.md` read whole (87 lines, addenda (a)–(h) + the §0ec amendment). Record read: `## Check 1 (RESUME 1)` whole, the last `## Close` by `sed` range, the F-W14U record's R-1 ruling (`F-W14U.md:1945-1956`) by range. fourier HEAD at open = `2c4dcda` (F.CT only past `545bbdc`; web/api = `545bbdc`).

**Crash-recovery.** ⟨`git -C fourier-analysis status --porcelain`⟩ at open → `?? .worktrees/` only (a sibling's; untouched). No inherited edit in `web/src/**` or `web/e2e/**`. value.js: no dirty path under `execution/C/F-W14V.md` or `LEDGER.md`. Mid-seat, F.CT's `src/fourier_analysis/contours/*` went dirty (a sibling's; untouched, never staged).

**Instrument.** api `:8020` from the fourier tree at `2c4dcda` (⟨`.venv/bin/uvicorn api.main:app --port 8020`⟩ with `:8000`'s `MONGO_URI`/`ADMIN_TOKEN`/`BLOB_DIR`/rate-limit env; started while `src/` was clean and never reloaded, so F.CT's later dirty `src/` is not imported: R-3 does not recur). vite `:3120` = the working tree (`VITE_PROXY_API=http://localhost:8020`); vite `:3121` = a clean ⟨`git archive 2c4dcda web`⟩ export (the before bytes). `:8000` (pid 38613) is still the stale pre-`71e0c6d` server (C1R1-2) and was not used or touched.

### Defect → cure → commit → gate re-reading
- **C1R1-1 HIGH (UIA-F-17 `:162` RED outside the named 21) → CURED, two failure modes, four fourier commits.**
  - **Mode 2, the pause override (F.W14U R-1 / C2R-1, homed here) — a product cure.** Falsifier first, fourier `f024d98`: `web/e2e/f-w14v-hold.spec.ts` h1 holds the `/compute/bases` response back 3 s by a routed delay (the ruling's "routed delay on the second write" shape), pauses by keyboard once the epicycles have started the clock, and after the bases land asserts the transport still reads Play at the same `aria-valuenow`. ⟨`BASE_URL=http://localhost:3121 npx playwright test e2e/f-w14v-hold.spec.ts --project=chromium --workers=1 --repeat-each=2`⟩ at the before bytes → **`2 failed`**, both `the late bases write left the reader's pause standing` (the Pause control was back). Cure, fourier `2cc409b` (`web/src/stores/animation.ts`, `web/src/components/visualization/composables/useWorkspaceLoader.ts`): the store records the reader's hold on its one channel (`toggle`; `playing === false` cannot tell a reader's pause from `reset()` or the reduced-motion park), and `autoPlay()` (the app starting motion on its own) honours it. The loader's data watcher goes through `autoPlay` on the first arrival and on any later one, and does not rewind a held clock; a new drawing releases the hold (`releaseHold()` before each route load, and when the data is cleared for an upload). ⟨same, `:3120`⟩ → `2 passed (15.0s)` · `2 passed (15.6s)`.
  - **Mode 1, the Export click timeout (`64a1865` `.u1`'s path) — a test-path cure, no product defect found.** Read at the bytes with scratch probes (not committed): the canvas dock is anchored right (`.controls-dock-anchor { right: .5rem }`) and expands on pointer **entry** (glass `onMouseEnter`: collapsed → 60 ms → hover; a leave inside the 60 ms cancels it). The old path hovered the dock's *centre*, measured while the previous export's collapse was still morphing (at dialog-gone the class is already `collapsed` but the box is still the expanded 481–819 px and Export frame still paints), so the pointer landed on the part that shrank away toward the Edit control. Second, the export dialog hands focus back to Export frame, the dock holds itself open for that focus, and the hold's release schedules a collapse that only a fresh entry cancels (read from glass 10.1.0 `dist/dock.js:323-326`, `release()` → `min(collapseDelay, 800)` → collapse, not isolated by a probe; the failing frame shows the dock collapsed with the pointer on Edit and the Export frame tooltip still open, `status: Export frame`). A resting pointer on a dock that is not releasing a hold keeps it expanded: probe, 4 × 5 s at 150 ms sampling → `E` throughout. So the case's own path was racing the dock's settle, and the product holds.
    - fourier `8a9a0ac` (first form): leave the dock, re-enter on the persistent `Edit contour` control (the suite's `expandCanvasDock` idiom, `f-w14v-u1.spec.ts:41`), assert `Export frame` visible; each export first asserts `Play animation` (R-1 homed). ⟨`:162 --repeat-each=10`⟩ → `10 passed` once, then `9 passed · 1 failed` and `8 passed · 2 failed` (click timeout: Export frame visible, then collapsed within the 800 ms settle). **Not sufficient**, so not the cure.
    - fourier `05f99a6` (final form): leave the dock and wait for it to **rest collapsed** (`class ~ collapsed`, no `data-morphing`), re-enter on `Edit contour`, wait for the **expanded class** and a **settled morph** (glass marks the root `data-morphing` for ~550 ms while `.dock-layers` intercepts pointer events: probe → `Em:root` 97–603 ms, then `E`), then assert `Export frame` visible and click. The fixed `waitForTimeout(800)` is retired, replaced by state waits. No timeout raised, no skip, no retry, and no assertion removed: the four exports, the switch settings and both floors are unchanged. A §0bt restatement of `.u1`'s own path, named in both commits.
  - **Instrument correction (read before the gate lines).** The first full run was started against `:3120`, the working-tree dev server that had hot-reloaded through every edit of this seat. It was stopped at 45/547 and **discarded**: it showed the F.W14 admin families RED (`f-w14-admin-idiom` tabs rendering "No visualizations yet"), while the same spec is `12 passed` against a fresh export of the same final bytes (⟨`git archive 05f99a6 web`⟩ → vite `:3122`) and `12 passed` at the before bytes `:3121`. A dev server carried through many HMR cycles is not the served page. A second start on `:3122` was also discarded at 101/547: that export held `web/` only, and vite's paper plugin reads `../paper/fourier_paper.tex`, so every paper case failed. Every gate reading below is on `:3122` re-served from ⟨`git archive 05f99a6 web paper`⟩ (sanity: `f-w13-radius` + `f-w14-residuals` → `8 passed`), a clean export of the final bytes, except the ×2 hold and ×10 `:162` readings named `:3120`, which are re-read on `:3122` in the combined line.
  - **Gate, `:162` at `--repeat-each=10`, load ≥ 20.** ⟨`BASE_URL=http://localhost:3120 npx playwright test e2e/f-w14-uia.spec.ts:162 --project=chromium --workers=1 --repeat-each=10`⟩, two runs at once (each the other's load), load 23.7–32.1 → **`10 passed (2.6m)` · `10 passed (2.5m)`**. On the clean export: ⟨`BASE_URL=http://localhost:3122 … e2e/f-w14-uia.spec.ts:162 e2e/f-w14v-hold.spec.ts --repeat-each=10`⟩, load 23.9 → 17.1 → **`20 passed (3.7m)`** (`:162` ×10 + h1 ×10). Before, same host and window: the before bytes `:3121` → `9 passed · 1 failed` (click timeout on Export frame); Check 1 read `7ee9b65` 11/11, `97325fb` 7/10, `545bbdc` 9/13.
- **C1R1-2 INFO (R-1 grown: the stale `:8000` writes legacy easings) → not a seat cure; owner unchanged.** Below MINOR, and its one cure (restart `:8000` at HEAD, re-run `python -m api.scripts.migrate_animation_easing`) belongs to the host/instrument owner: `:8000` is a shared server this seat did not start. This seat neither used it nor wrote to the db outside the e2e seed's own records. `run_pending_migrations` heals it on deploy.
- **C1R1-3 INFO ((h)(1) "confirm path") → none needed**, as the Check read it (Delete is an undoable `deleteSelected()`).
- **Adjacent edits (§0bt):** none outside the unit. `web/e2e/f-w14-uia.spec.ts` is the case the register names; `f-w14v-hold.spec.ts` is new.
- **Observation for the root (no relay written):** glass 10.1.0's dock collapses after a hold's `release()` unless a fresh pointer *entry* follows, so a pointer already resting on the dock can end on a collapsed dock (read from `dist/dock.js:323-326`; the consumer path is cured without relying on it). If a later seat isolates it with a probe, it is a glass hover-model row (O-series), not a consumer shim.

### Gates a cure could move (re-read at the settled bytes)
fourier web/api = `05f99a6` (HEAD moved to `603ae37` during the runs by F.CT commits only: ⟨`git diff --stat 05f99a6 HEAD -- web api`⟩ → empty).

| Gate | Reading | Verdict |
|---|---|---|
| vue-tsc | ⟨`npx vue-tsc --noEmit; echo $?`⟩ → `0` · `0` | GREEN ×2 |
| vitest | ⟨`npx vitest run`⟩ → `Test Files 20 passed (20)` · `Tests 116 passed (116)` ×2 | GREEN ×2 |
| h1 (R-1 falsifier) | before `:3121` → `2 failed`; after → `2 passed` · `2 passed`; ×10 on `:3122` inside the combined `20 passed` | RED → GREEN ×2 |
| `:162` ×10, load ≥ 20 | `10 passed` · `10 passed` (load 23.7–32.1) and ×10 on `:3122` inside `20 passed` (load 23.9 → 17.1) | GREEN |
| full e2e `--workers=1` ×2 (§2) | ⟨`FW14_PHASE=w14v-repair1-r<n> BASE_URL=http://localhost:3122 MONGO_URI=mongodb://127.0.0.1:27018/fourier npx playwright test --workers=1 --reporter=line`⟩ (546 cases): r1 (load 17.7 → 32.1) **`23 failed · 3 skipped · 520 passed (28.4m)`** · r2 (load 32.1 → 17.0) **`24 failed · 3 skipped · 519 passed (27.6m)`** | see below |

**The full-e2e reading.** ⟨`comm -12` of the two runs' sorted failure lists⟩ → **23 stable**; UIA-F-17 `:162` and h1 are GREEN in both runs (⟨`grep -c 'f-w14-uia.spec.ts:162\|f-w14v-hold' f1.txt f2.txt`⟩ → `0` · `0`).
- **The named honest-RED set, 21, exactly:** contrast-floor `:82` ×2 + `:128` · gallery-admin-a11y ×4 · visual-checkpoint `:81 :102 :123` · au3 L1-12 `:119` · c3 `:128 :138` · p `:130` ×2 · `.pd` `:104` ×6.
- **f-w14u-d d2 `:136` (1440 light in both; 1440 dark in r2 only)** — the Close's r2-only load intermittent, now read in both runs at load 17–32. Not attributable to this seat by bytes: alone at the before bytes (`:3121`, `2c4dcda`) ⟨`… e2e/f-w14u-d.spec.ts -g d2 --repeat-each=2`⟩ → **`2 failed · 4 passed`**; at the after bytes → `6 passed`, and alone on a fresh tree server ×2 → GREEN ×2 (load 44.6). Its failure is `traceOff`'s wait: after the View options menu closes by an outside click, the canvas dock stays `expanded` for 30 s (the same glass hold/release model as C1R1-1's second limb). **Residual R-1 below.**
- **paper-performance `:235`** — **instrument, not bytes.** Both runs log 32 × `Failed to load resource: … 403 (Forbidden)` from the served page; the `:8020` api log holds no 403, so the 403s are the scratch export's vite (its `node_modules` is a symlink into the repo). Alone on a fresh vite over the working tree (same bytes) ⟨`BASE_URL=http://localhost:3120 … e2e/paper-performance.spec.ts:235 e2e/f-w14u-d.spec.ts:136`⟩ → `4 passed (20.4s)` · `4 passed (18.6s)`.

**Self-count:** 5 gate rows (vue-tsc, vitest, h1, `:162`, full e2e), each read ×2 from the settled bytes; 23 stable = 21 named + d2 + `:235`, each figure read twice (⟨`wc -l f1.txt f2.txt`; `comm -12 … | wc -l`⟩ → `23 24 23` ×2).

### Residuals (named owners)
- **R-1 · f-w14u-d d2 `:136`, a pre-existing load intermittent (RED alone at the before bytes 2/6).** It is outside this register (C1R1-1 names `:162`) and read GREEN alone at the after bytes. Its cause is the canvas dock not collapsing after the View options menu closes by an outside click, i.e. a glass hold that is not released. **Owner:** the next Check, to weigh it against §2; if it is ruled in scope, the cure is a probe-isolated glass hover-model relay (with C1R1-1's release observation) plus d2's setup waiting on state, in `web/e2e/**`.
- **R-2 · instrument:** full runs from a `git archive` export need `web paper` (not `web`), and a real `node_modules`, or vite answers 403 on the paper's assets. This seat's two discarded starts are recorded above.
- **R-3 · C1R1-2 stands** with its owner (the stale `:8000`).
