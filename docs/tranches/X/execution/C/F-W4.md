...
#### Act 10 — two disclosures the receipt above would otherwise round off

1. **A one-line `git checkout --` on `LEDGER.md`, on this seat's OWN uncommitted error, disclosed rather than absorbed.** The first attempt at the F.W4 row split the line on `" | "` and re-joined it; because the row's empty commit cell rendered as `** | | gated`, the split produced **four** fields where the row has **five**, and the assignment intended for the commit cell **overwrote the notes cell** — the cell holding `.i`/`.g`/`.a`/`.c`'s landed summaries. Caught by a cell count taken immediately after the write (⟨cmd⟩ `/usr/bin/awk 'NR==67 {n=gsub(/ \| /,""); print n+1}'` → **4**, against a 5-column table). ⟨cmd⟩ `git diff --stat docs/tranches/X/execution/LEDGER.md` → **1 file changed, 1 insertion(+), 1 deletion(-)** — **the single changed line was this seat's own, and no other seat's work was in the working tree** — and only then `git checkout -- docs/tranches/X/execution/LEDGER.md`. The row was then set by **exact substring replacement** of `| F.W4 | F.W1 | **OPEN 2026-09-17** | | ` (asserted unique, count == 1), and the cell count re-read **5**. ⊘ **The standing law forbids a blanket restore, and this was not one**: one path, one line, this seat's own, verified before the act and re-verified after. Stated because a receipt that omits the near-miss is a receipt about a tidier session than the one that happened.
...
**SERVED MODEL: claude-opus-5[1m]** · FRESH ADVERSARIAL CHECK (L-20, pass 1), VERIFY-ONLY — an independent seat that authored **no byte** of this wave's cures, of any unit receipt, of `## Close`, of `F-W4-CLOSURE.md` or of any addendum. Run 2026-09-18 at fourier `0a16b83` (dirty **0** before and after) and value.js `8a0c3c63`. **This seat wrote zero fourier bytes and zero `web/**` bytes**; every reading below is its own command, double-run where the close double-ran.

**VERDICT: CONFORMANT-HONEST-RED** — 0 BLOCKER/CRITICAL/HIGH · 1 MEDIUM · 5 MINOR · 2 INFO. **All 18 gate verdicts reproduce** at this seat's own commands, including all **10** claimed GREEN and all **8** RED. Honest-RED set of **8**, each owner-named; two of the eight carry an owner but no relief this seat will call spec-given, and both are booked below rather than laundered.

### §A — CRASH-RECOVERY sweep (standing law), before any other act

⟨cmd⟩ `git -C /Users/mkbabb/Programming/fourier-analysis status --porcelain | wc -l` → **0** at open and **0** at close. ⟨cmd⟩ `git -C /Users/mkbabb/Programming/value.js status --porcelain` → **15 paths, not one inside this seat's writable set** (`docs/tranches/X/execution/C/F-W4.md` ⊕ `LEDGER.md`): ten `demo/**` SFCs · `docs/tranches/V/reformation/CARRY-LEDGER.md` · `scripts/dev/dev.sh` (unowned, **never touched, never staged**) · two untracked `e2e/smoke/**a11y-control-targets.spec.ts` · `docs/tranches/X/waves/evidence/W4/`. **Zero inherited paths on this unit.**

### §B — Gate reproduction, 18 of 18, at this seat's own commands

| # | gate | close's verdict | THIS SEAT'S COMMAND AND READING | reproduces |
|---|---|---|---|---|
| 1 | `G-F4-DECISIONS` | GREEN | ⟨cmd⟩ `grep -c '\*\*VERDICT' DECISIONS-F.W4.md` → **11** over **10** decision heads (D2 splits (a)/(b)); ⟨cmd⟩ `ls …/gallery/GalleryMarquee.vue` → *No such file*, `grep -rl GalleryMarquee src` → **0** | **YES** |
| 2 | `G-F4-VITEST` | GREEN | ⟨cmd⟩ `npx vitest run` ×2 → **7 files / 50 tests passed**, exit 0, both runs: `curve-transition` 5 · `contour-bounds` 7 · `paper-figure` 5 · `paper-search` 8 ⊕ `paper-search-scoring` 9 ⊕ `contrast` 8 ⊕ `derive-loops` 8 — **all four ruled subjects present** | **YES** |
| 3 | `G-F4-NO-UNUSED` | **RED** (mechanism green) | ⟨cmd⟩ `npx eslint src` ×2 → **exit 1, 2 errors**, byte-identical: `BasisCanvas.vue 15:1` and `31:1` *"'./lib/canvas-drawing' import is duplicated"*. Mechanism verified at the bytes: `tsconfig.json:10-11` `noUnusedLocals`+`noUnusedParameters`; `eslint.config.js` errors on both named rules over `src/**`, **no `ignores` carve-out, no `--max-warnings`**, and its header **declines** `allowSeparateTypeImports` in writing (the option would have hidden nine of the original ten findings — a seat refusing to narrow its own gate); `ci.yml:127-147` runs `npm run lint` as a **blocking** `web-build` step | **YES** |
| 4 | `G-F4-A11Y-ROUTE` | GREEN | ⟨cmd⟩ `npx playwright test e2e/visualization-ux.spec.ts -g "keystone: (/equation\|/paper\|/morph\|/demo/shape-extractor)"` → **4 passed**. The `checkA11y` helper asserts `withTags(wcag2a,wcag2aa,wcag21a,wcag21aa)` then `expect(blocking).toEqual([])` over `impact === serious\|critical` — **not narrowed** | **YES** |
| 5 | `G-F4-ADMIN-AXE` | RED, producer-caused | ⟨cmd⟩ `npx playwright test e2e/gallery-admin-a11y.spec.ts` ×2 → **4 failed**; ⟨cmd⟩ the same run piped through `grep -oE '\[(serious\|critical)\] [a-z-]+' \| sort \| uniq -c` → **`4 [serious] aria-hidden-focus`** and **nothing else**; ⟨cmd⟩ `grep -c color-contrast` over the run → **0**. The offending node, read from the run's own `error-context.md`: `<span aria-hidden="true" tabindex="0" style="position: fixed; … clip-path: inset(50%) …">` at `div[role="region"] > span:nth-child(1)`/`(3)` — a FocusScope sentinel inside a glass-ui collapsed layer. ⟨cmd⟩ `grep -rn 'clip-path: inset(50%)' src` → **∅** and `grep -rln 'aria-hidden="true" tabindex="0"' src` → **∅**: **no app byte emits it** | **YES** |
| 6 | `G-F4-OCCLUSION` | GREEN | ⟨cmd⟩ `VISUAL_OUT=<scratch> npx playwright test e2e/visual-baseline.spec.ts` → **21 passed**, fourier dirty **0** after. Read at the spec's bytes: the gate now evaluates `main.scrollWidth − main.clientWidth` **and** `shell.scrollHeight − shell.clientHeight` with a missing `<main>` asserted as a **failure**, not a pass. ⟨cmd⟩ `git diff f7fa1e3..HEAD -- web/e2e/visual-baseline.spec.ts` shows the `documentElement` read **deleted** (`-`) and the `<main>` read added | **YES** |
| 7 | `G-F4-CONTRAST-FLOOR` | RED | ⟨cmd⟩ `npx playwright test e2e/contrast-floor.spec.ts` → **3 of 3 failed**; the run's own error text: *"light arm … **19 of 38 pairs**"* · *"dark arm … **10 of 38 pairs**"* · the third test names **6** pairs with no re-derivable expression (`FR-CP-D1` · `ECD D-5` · `ECD D-6` · `EV D·D-B3` · `PS D-B2` · `PV D/M-8`). **Every close figure exact** | **YES** |
| 8 | `G-F4-PRM-CLOCK` | GREEN | this seat's own rAF-instrumented probe (`addInitScript` wrapper, `reducedMotion: "reduce"` context), counts taken at **t=1.5s / 4.5s / 7.5s** so a frozen clock is distinguishable from a slow one: **unreduced** `/morph` 27→65→106 · `/equation` 31→69→111 · `/` 47→83→121 (ticking); **reduced** `/morph` **1→1→1** · `/equation` **3→3→3** · `/` **21→21→21** — **delta 0 over two consecutive 3-second windows on all three routes**, **0 page errors** in either arm. Non-blankness checked rather than assumed: the reduced frame renders **30** (`/morph`) and **17** (`/equation`) SVG paths. **M-D1 holds — the reduced arm parks on a painted frame, never a blank one** | **YES** (verdict; see D-2 on the numerals) |
| 9 | `G-F4-DERIVER` | GREEN | ⟨cmd⟩ `npm run derive:loops` ×2 into two files, ⟨cmd⟩ `diff` → **no output (byte-identical)**. All eight fields + the method-limits block: `nativeLoopsByDirective` **31** (15 native / 16 callsite, **unkeyed 0**) · `loopBounds` 1 closed / 30 runtime · `loopMultiplicity` · `isCandidateSets` **5 sites / 3 resolved** · `producerInternalLoops` **7** first-party packages from the installed dist · `urlRefEdges` **7** · `disclosureState` · `loopSourceProvenance`. The limits block states the static-reader caveat and **refuses to mint a cardinality** (FR-NP-2) | **YES** |
| 10 | `G-F4-KATEX-QUIET` | GREEN as banked | this seat's console-instrumented probe: `/paper` → **2 `.katex` nodes, warns=0, consoleErrors=0, no 4xx/5xx**; `/equation` → **0 `.katex` nodes, warns=0, 1 console error, `500 /api/equations/compute`**. **The close's reading and its own disclaimer both reproduce exactly** — the `/equation` leg is unmeasurable with the compute API down, and the close said so instead of crediting it | **YES** |
| 11 | `G-F4-DEAD-DEP` | GREEN | `F-W4-SCRUB-LEDGER.md` §1 deletions-with-proof · §2 measured-dead-not-deleted · §3 `VV-R2-B` retained by lock · §4 the two dead deps · §5 the bundle diff **both directions** · §6 the `url(#id)` exclusions · §7 the regrowth gate. Verified at the bytes: `class-variance-authority`/`clsx` **still in `web/package.json:25-26`** (declared, not deleted out of bounds) with ⟨cmd⟩ `grep -rl 'class-variance-authority\|from "clsx"' web/src` → **∅**; `components/paper/search/index.ts` → *No such file* | **YES** |
| 12 | `G-F4-CONV-STUDY` | GREEN | `F-W4-CONV-STUDY.md` §0 lock discharged · §1 what is on each side at the adopted pin · §2 equivalence by measurement · §3 the cost on the table · **§4 which renderer owns which regime** · §5 the gBCR cost baseline · §6 what would falsify it. The §5.1(3) order held: `.b`'s `b3b9a35` (`L-B3` memoised **and measured**) precedes `.f`'s `0a16b83` | **YES** |
| 13 | `G-F4-CARRY-CLOSURE` | leg (a) GREEN · leg (b) RED | leg (a): ⟨cmd⟩ `grep -c '^- \*\*' carry/F-W4-CARRY.md` → **438**, matching the transcript's 438/438. leg (b): the operand reproduces at the canonical (**1007** rows / 54 records), band-expanded **1009**, and §3 enumerates **18** dispositions id-for-id = 1 BOOKED + 11 + 5 + 1 = **17 escapes** | **YES** |
| 14 | `G-F4-ANCHORS` | RED | ⟨cmd⟩ read of `fourier/docs/tranches/F/SUBSTRATE-LEDGER.md` §2.1 (`### 2.1 G-11 — THE CORRECTED ANCHOR TABLE`): the register's last row is **`| 11 | BASELINE D-1 …`**. **F.W4's rows 12–18 are not appended**, exactly as the close states | **YES** |
| 15 | `G-F4-CENSUS-CELLS` | RED, 3 of 6 | cell 1 `raw-findings.json:1393` → `ADDENDA-a` ✓ · cell 3 `lane-frontend.md:617` → `ADDENDA-e` ✓ (landed at `.e`'s closing docs commit `980785e3`, not its cure `1a971c0` — the clause miss reproduces) · cell 4 `:558-559` → `ADDENDA-{b,f}` ✓ · cell 5 re-read at `CENSUS-2026-08-03.md:362` — a **condition** row, not a claim, so the discharge posture is right · **cell 2 `:2968` and cell 6 `HLG-8` appear in `ADDENDA-z` ONLY, and there as `NOT CORRECTED` with owners `.c` and `.a`** | **YES** (see D-6) |
| 16 | `G-F4-NEG-ROSTER` | GREEN (HELD) | at the bytes: Tooltip shim alive at `ui/tooltip/Tooltip.vue` with ⟨cmd⟩ `grep -rl 'ui/tooltip' src \| wc -l` → **11** · `CanvasOverlayButton` **0 files** and not re-created · `InfoCard` **0** and not re-created · ⟨cmd⟩ `grep -rc 'surface="cartoon"' src` → **0 files** (CP KILL-6 never executed) · ⟨cmd⟩ `git log --grep=x-f-w4 --name-only \| grep -c moon.json` → **0** · `GalleryMarquee` deleted, so `GM-19` cannot be certified | **YES** |
| 17 | `G-F4-VUE-TSC-CLEAN` | RED, 18 → 1 | ⟨cmd⟩ `npx vue-tsc -b --force` ×2 → **byte-identical, one diagnostic**: `src/components/visualization/ContourEditorCanvas.vue(42,9): error TS6133: 'dragging' is declared but its value is never read.` | **YES** |
| 18 | `G-F4-ZERO-CONSOLE` | HONEST-RED, MEASURED | ⟨cmd⟩ `nc -z localhost 27017` → **DOWN** · `nc -z localhost 8000` → **DOWN** at this seat too. ⟨cmd⟩ `npx playwright test e2e/paper-performance.spec.ts` → **5 passed**, including the test carrying `.a`'s DMT N-2 locator rider. The three mongo-dependent legs are unreadable here for the same reason | **YES** |

**18 of 18 reproduce. 10 GREEN · 8 RED — the close's own tally, at this seat's own commands.**

### §C — Bounds, masking, families, E-3, mail, the four-verb line

- **Bounds (axis 2).** ⟨cmd⟩ `git log --grep=x-f-w4 --format=%h \| while read c; do git show --format= --name-only $c; done \| sort -u` → **111 paths, every one under `web/` or `.github/`**; ⟨cmd⟩ the same list filtered `grep -vE '^(web/\|\.github/)'` → **∅**. `scripts/dev/dev.sh` in **0** paths of either repo. ⟨cmd⟩ `grep -E 'BasisCanvas\|ContourEditorCanvas'` over that list → **∅**: **the EVALUATE-ONLY pair is in zero commits of this wave**, which is the load-bearing fact under two of the REDs. value.js side: the 22 wave commits ⊕ the two close commits touch **exactly 17 paths**, re-counted here, and **not one** is a `demo/**`, `api/**`, `src/**`, `e2e/**` or `docs/tranches/V/reformation/` path — no sibling seat's staged bytes swept in. ⟨cmd⟩ `git log --grep=x-f-w4 --format='%h %b' \| grep -c Claude-Session` → **72**; all 24 value.js commits carry the trailer.
- **Masking (axis 3).** ⟨cmd⟩ `git diff f7fa1e3..HEAD -- web/ .github/ \| grep '^+' \| grep -Ei 'test\.skip\|\.skip\(\|test\.fixme\|continue-on-error\|allowlist\|eslint-disable\|@ts-ignore\|@ts-expect-error\|xit(\|--max-warnings\|\.only('` → **10 hits, every one prose** — `ci.yml` comments *forbidding* the constructs, `eslint.config.js`'s own decline register, and three CSS focus-ring comments using the English word "allowlist". ⟨cmd⟩ the narrower probe `grep -E '^[+-].*(test\.fixme|test\.skip|\.skip\(|describe\.skip)'` over `web/e2e/` + `web/scripts/` → **∅ in both directions**: **no skip was added and none was removed to hide a signal**. ⟨cmd⟩ `git diff --stat f7fa1e3..HEAD -- '*node_modules*' 'patches/'` → **empty**. The one surviving `test.fixme` in `visualization-ux.spec.ts` predates this wave (F.W0-era, booked glass-ui-a11y baseline) and is untouched by the diff. The admin spec stubs the **HTTP boundary only** and states why in its header; the assertion bar is the same zero-serious/zero-critical the `/visualize` keystones carry. The occlusion gate's `.catch(() => {})` on `page.goto` is **pre-existing** — ⟨cmd⟩ the file's own diff shows it on no `+` line. **Zero masking constructs introduced.**
- **Commit families (axis 4).** One commit per meaning across 72, and every §5.1(3) same-commit family this seat could falsify holds at the bytes: `DMT N-2`'s label **with** the e2e locator (`5081b37`, `paper-performance.spec.ts` **1 insertion / 1 deletion**, the locator line and nothing else) · the `ECD-B-2 ⊕ CCD-M-2` INVERSION LOCK as one edit (`714b758`) · `CP-ROW-40` as one extraction (`9c7a1b6`) · the `GCM-48 → GCM-30 → GCM-28` cycle as one deletion (`e1b4d88`) · `PSM-1` with its three riders (`a3030dd`) · `FR-CP-LM1`'s `old[1]` fix with its `deep` drop (`8c74054`) · `AA-8` strictly before `AA-1` (`048f03c` precedes `326926c`). **`MPC-31`'s ONE CUT was not split — it was not landed at all**, deferred whole and returned as the `A-a-5 ⊕ A-c-3` seam with the dispatcher named. **No family split.**
- **E-3 (axis 5).** The wave's 17 value.js paths contain **no** `waves/`, `carry/`, `conformance/`, `registry/adjudicated/`, `formation/`, `COHESION.md` or `EXECUTION-RUNBOOK.md` entry. `F-W4.md`, `F-W4-CARRY.md`, `CENSUS-CANONICAL.md`, the 66 adjudicated `fr-*.md` and every sibling `F-W*.md` are **byte-untouched by every commit of this wave**; corrections travel as the eight dated `F-W4-ADDENDA-*-2026-09-18.md`. Both `LEDGER.md` edits are minimal in-place (`784e112e` **1+/1−**; `2b65e85c` **3+/1−**). **E-3 HELD.**
- **Mail (axis 6).** ⟨cmd⟩ `grep -nE '\| *UNREAD' docs/tranches/V/coordination/INBOX.md` → **no output (exit 1)**. Six-path sweep at this seat's own clock, delta `-newermt "2026-09-18 22:00"`: the four sibling paths ⊕ the atlas **P**-lane ⊕ the fourier **F** surface → **∅**; the value.js path returns `INBOX.md` and five `*-inbox-2026-09-18-value-4.1-*` letters authored by a concurrent sibling seat, **each rowed 1× (O-34..O-38) and each OUTBOUND**, so none is inbound mail in this wave's scope. This wave's own **O-32** (glass-ui BH/BK) and **O-33** (latex-paper) are present. **0 unrowed · 0 UNREAD in scope.**
- **Four-verb line (axis 7).** **IMPLEMENTED — PARTIAL 2026-09-18**, and **VERIFIED deliberately not stamped**, on the spec's own posture table (*"stamped only at X·F's sub-tranche release close; no wave stamps VERIFIED at its own close"*). **Lawful.**
- **Goal criterion (axis 8).** MET at the bytes, not merely at the gates. The spec's scope is the frontend defect mass over `web/src` across eight axes ⊕ the convergence study ⊕ reduced-motion clock gating ⊕ the dead-dep scrub ⊕ the vitest-floor **decision**. All five arms are in the tree and measurable by this seat: **72 commits over all seven implementing units**; PRM gating measured frozen-and-painted in the reduced arm on three routes; a measured convergence memo naming which renderer owns which regime; a scrub ledger with per-deletion zero-consumer proofs and a two-direction bundle diff; D6 ruled and a real runner executing 50 tests; `vue-tsc` **18 → 1**; the axe keystone set widened from three routes to seven with the four new ones clean; the occlusion gate's green-by-construction **killed** and re-pointed at `<main>` with a vertical-clip arm; the first automated reading of the admin surface; the first executable both-arm contrast harness in this repo's history; and a deriver that publishes all four blind spots **with its own method limits**. ⊘ **Three gate chassis are the wave's product, not its scaffolding**, and each is honest about what it now shows RED.

### §D — Register

| # | severity | claim | receipt | cure |
|---|---|---|---|---|
| **D-1** | **MEDIUM** | **Leg (b)'s five homonym-collision escapes have no relief the close will cite, and this seat will not invent one.** `fr-ConvergenceLegend C-11` · `fr-ConvergenceTimeline M-5` · `fr-EquationPanel D-L1` · `fr-FullscreenViewer FM-4` · `fr-VisualizationView L-23` are canonical F.W4 rows whose only bytes in `F-W4.md` belong to another record's row of the same spelling. ⊘ **Weighed the other way in the same breath**: `F-W4.md` is IMMUTABLE under E-3, so booking them into the spec is the one act this wave was forbidden; the only lawful act was to name them, which `.z` did — id-for-id, with each impersonating byte and a `grep` receipt beside it. **They are loud drops, and the gate exists to kill silent ones.** | `F-W4-CLOSURE.md` §3.3, re-read at the bytes; each row's *"no relief that this wave can cite"* is the closure's own sentence | F.W4's repair docket books the five at a successor wave, or a dated addendum-beside homes them; **named owner already stands** |
| **D-2** | MINOR | **`G-F4-PRM-CLOCK`'s published callback numerals do not reproduce, and three seats published three different sets for one measurement.** Close: *"unreduced **33** callbacks on each of `/morph`, `/equation`, `/`; reduced **1 · 3 · 3**"*. `.z`/`F-W4-CLOSURE.md` §9: *"0 callbacks in the reduced arm vs 15"*. This seat, windowed: unreduced **27 / 31 / 47** at t=1.5s and still climbing; reduced **1 / 3 / 21**, frozen. The `/` reduced figure is **21, not 3**. ⊘ The **verdict** is unaffected and reproduces cleanly — a raw rAF count over an unstated settle window is not a falsifiable receipt shape, which is the finding | this seat's three-window probe, §B row 8 | publish the **window** with the count, or publish the delta-over-window (which is what decides the gate) instead of a cumulative total |
| **D-3** | MINOR | **Act 3 undercounts the artefacts it verifies, against Act 1 of the same document.** Act 3: *"The **seven** dated addenda-beside (`-a-` through `-z-`) … **Twelve** artefacts, all present."* Act 1 of the same close says *"the **eight** dated `F-W4-ADDENDA-{a,b,c,d,e,f,g,z}`"*, and the tree holds **eight**; with `DECISIONS`, `CONV-STUDY`, `SCRUB-LEDGER` and the two relay letters the total is **thirteen**, not twelve | ⟨cmd⟩ `ls docs/tranches/X/fourier/ \| grep -E 'F-W4-ADDENDA'` → **8 files**, `{a,b,c,d,e,f,g,z}` | a dated correction beside; the SELF-COUNT law wants the enumeration, not the numeral |
| **D-4** | MINOR | **`.a` wrote `web/e2e/contrast-pairs.ts` (`f32e912`) outside its declared writable set and did not escalate it** — while four sibling units **declined the identical invitation in writing**. ⊘ Mitigated three ways and so not graded higher: the path is inside the **wave's** §1 Bounds (*Shared*: `web/e2e/*`, gate wiring only), the content is `.a`'s own cured rows rather than a foreign seat's, and **the close found it, named it LANDED-WRONG 1, refused to fix it, and assigned the owner**. It is an intra-wave partition inconsistency, not a §File Bounds escalation | ⟨cmd⟩ `git show --stat f32e912` → `web/e2e/contrast-pairs.ts` ⊕ `DarkModeToggle.vue` ⊕ `HarmonicLevelGrid.vue`; `.a`'s granted exception was used correctly at `5081b37` (**1+/1−** on `paper-performance.spec.ts`) | `.a`'s dated addendum-beside; ratification or reversal is the wave owner's |
| **D-5** | MINOR | **`.g` wrote `web/package-lock.json` (`cc6c32d`, `f93eacc`), outside the literal `web/package.json` (gates only) grant.** Already escalated at **A-g-8** and correct on the merits, re-verified here: `ci.yml`'s `web-build` and `e2e-tests` jobs both run `npm ci`, which fails outright on a manifest/lock desync — landing the manifest byte without its derivative would have turned **every** CI job RED | ⟨cmd⟩ `grep -n 'npm ci' .github/workflows/ci.yml` → the two jobs | ratification, not a block |
| **D-6** | MINOR | **`G-F4-CENSUS-CELLS`' two uncorrected cells are owner-named in a dated artefact but absent from the close's own "Residuals at close, each with a named owner" register** — the only RED residue that does not appear there. They are in-bounds and curable (an addendum-beside is in both units' writable sets), so this is a not-done rather than a relieved RED. ⊘ Mitigated: `F-W4-ADDENDA-z` §, rows for `raw-findings.json:2968` (`.c`) and the `HLG-8` claim (`.a`), both spelled **NOT CORRECTED** with their owner | ⟨cmd⟩ `grep -l -- '2968' F-W4-ADDENDA-*.md` → `F-W4-ADDENDA-z…` **only**; same for `HLG-8` | `.c` and `.a` land the two addenda-beside; carry them into the residual register |
| **D-7** | INFO | The close's *"`.a` and `.c` returned PARTIAL"* undercounts: **four** unit receipts carry `Status: PARTIAL` (`.a` `:560`, `.c` `:717`, `.b` `:849`, `.f` `:1153`). The distinction the sentence is reaching for is real — `.a`/`.c` have rows named-and-not-landed, `.b`/`.f` have owed hand-offs — but the word says two where the bytes say four | ⟨cmd⟩ `grep -n 'Status: PARTIAL' docs/tranches/X/execution/C/F-W4.md` | none needed; recorded |
| **D-8** | INFO | The closure's §5 receipt *"⟨cmd⟩ `grep -l -- '2968' F-W4-ADDENDA-*.md` → **NONE**"* is **stale by one file at the settled bytes**: `F-W4-ADDENDA-z` now holds the string, because `.z` wrote the miss down in the same act. The **verdict** (NOT CORRECTED) is unchanged and correct; the receipt is a self-count of a set the writing enlarged, which is the exact shape `F-W4.md` §Y.2 item 3 abolished one level up | this seat's re-run, §B row 15 | re-run, never inherit — or exclude the writing file, as the SELF-COUNT law already requires for `INBOX.md` |

⊘ **What this check looked for and did NOT find**, stated so the green is readable: no fabricated GREEN (all ten reproduce at this seat's own commands); no `test.skip`/`test.fixme`/`continue-on-error`/allowlist/`eslint-disable`/`@ts-ignore` added anywhere in 111 paths; no `node_modules` or `patches/` byte; no narrowed assertion (the two axe helpers and the contrast harness were read, not assumed — and `eslint.config.js` **declines** the one option that would have hidden nine findings); no copied producer selector; no commit without its own pathspec and no sibling's staged bytes swept in; no split commit family; no in-place edit of a frozen artefact; no `git stash`, `reset --hard`, `add -A`/`-u` or force-push; `scripts/dev/dev.sh` in **0** commits of either repo; and **no RED relieved by a fabrication** — where the close had no relief it said so in those words.

### §E — HONEST-RED ADJUDICATION, gate by gate

| gate | relief class | cited at | owner | verdict |
|---|---|---|---|---|
| `G-F4-NO-UNUSED` (run leg) | **spec's own routing** — `F-W4.md` §1's EVALUATE-SCOPE bullet grants `BasisCanvas.vue` *"evaluate — read, derive, and judge; the edit surface is unchanged"*. Both findings are on that file; a consumer cure is a write outside §File Bounds, i.e. a gate failure by the standing law | `A-c-1` | the wave owner | **RELIEVED** |
| `G-F4-VUE-TSC-CLEAN` | **same grant**, `ContourEditorCanvas.vue(42,9)`; zero commits of this wave touch either file | `A-c-1` | the wave owner | **RELIEVED** |
| `G-F4-ADMIN-AXE` | **producer-owned** — the four serious findings are one class (`aria-hidden-focus`) on reka/glass-ui FocusScope sentinels emitted by no app byte; §5.1(6) forbids the frontend hack and the standing law forbids the `node_modules` patch. **Zero `color-contrast`**, so the surface the gate was written for is cured | §5.2 GLASS-RELAY; letter **O-32**, rowed | glass-ui BH/BK | **RELIEVED** |
| `G-F4-ANCHORS` | **out of bounds by the spec's own routing** — `fourier/docs/tranches/F/SUBSTRATE-LEDGER.md` is F.W0's; §0/§5.1(1) say in terms that F.W4 *"re-performs neither as its own act"*. Rows 12–18 are published ready-to-append | `F-W4-CLOSURE.md` §4 | the F.W0 `G-11` table's owner | **RELIEVED** |
| `G-F4-CONTRAST-FLOOR` | **bounds ⊕ producer, both cited at the bytes** — `e2e/contrast-pairs.ts` is `.g`'s and `.g` ran first, so the registry still spells pre-cure recipes (verified at one row: `GAB-1[plate/page]` grades `rgb(245 158 11 / 0.04)` while the live `.admin-banner` now paints `color-mix(in srgb, var(--viz-amber) 10%, transparent)`); the residue that is **not** instrumentation is the producer rungs (`--success`, `--tier-featured`) §5.2 already routes to the relay | `.b` §3 · `.d` §B · `.e` `A-e-1` (replacement rows complete) ⊕ GLASS-RELAY | `.g`, or whoever is granted that file next | **RELIEVED** |
| `G-F4-ZERO-CONSOLE` | **environment, measured not assumed** — `nc -z localhost 27017` and `docker info` DOWN at the close seat **and at this one**; the one readable leg is GREEN (5 passed) and 0 app-code errors were seen in either arm | `F-W4-CLOSURE.md` §8 | CI's `e2e-tests` job, which raises its own mongo | **RELIEVED** |
| `G-F4-CARRY-CLOSURE` leg (b) | **partly relieved**: 11 of 17 by the EVALUATE-ONLY grant (`A-c-1`), 1 by the class booking whose instance identity is now written. **5 carry an owner and no relief** (D-1) | `F-W4-CLOSURE.md` §3.2/§3.3/§3.4 | `A-c-1` ⊕ F.W4's repair docket | **RELIEVED-IN-PART · owner-named** |
| `G-F4-CENSUS-CELLS` | **partly relieved**: cell 5 is a condition row, not falsifiable, discharged at `A-z-5`; cell 3 corrected but outside its cure's commit. **2 cells uncorrected, in bounds** (D-6) | `F-W4-CLOSURE.md` §5 ⊕ `F-W4-ADDENDA-z` | `.c` and `.a` | **RELIEVED-IN-PART · owner-named** |

⊘ **The two RELIEVED-IN-PART rows are why this verdict is `CONFORMANT-HONEST-RED` and not `CONFORMANT`, and why they are booked at D-1/D-6 rather than dissolved into the honest-RED set.** Neither is a BLOCKER/CRITICAL/HIGH by the spec's own weight: both are provenance residues, both are named id-for-id with an owner, and neither is a product defect or a laundered green. **An escape a reader can check and a miss a reader can find are the two shapes this program's closure gates were built to produce; an invented relief would have been the failure.**

### §F — Successor conjuncts

| successor | *Opens after* | conjuncts at this seat's reading | state |
|---|---|---|---|
| **F.W3** | F.W1 (except §C.H rows: F.W0 only) | F.W1 **CLOSED**, F.W0 **CLOSED** — **not gated on F.W4** | **lawfully OPEN** |
| **F.W9** | F.W0 → F.W1 → **F.W3/W4** | F.W0 ✓ · F.W1 ✓ · **F.W4 ✓ (this close)** · **F.W3 `planned`** | **lawfully BLOCKED** on the F.W3 conjunct alone |
| **F.W10** | F.W9 | F.W9 `planned` | **lawfully BLOCKED** |
| **F.W7 · F.W8** | F.W0 / F.W5 / F.W1.close / F.W2 / G-F7-1 | none names F.W4 | **unaffected by this close** |

⊘ **F.W4 ceases to be F.W9's blocker with this close; F.W3 remains one.** The twin-seam reciprocals `PAW-4`, `L-26`, `GAB-25`'s vehicle and `M-β1`'s ramp homes are F.W3's to book, declared at §5.2 and at `F-W4.md` §Y.4, and F.W4 grew no leaf on any of them — verified at the bytes by this seat's bounds scan (`.d`/`.e` touched neither `PAW-4`'s row nor `VisualizationView.vue`'s `L-26`).

**CHECK 1 VERDICT: CONFORMANT-HONEST-RED.** 18/18 gate verdicts reproduce · 0 BLOCKER/CRITICAL/HIGH · 1 MEDIUM · 5 MINOR · 2 INFO. **Honest-RED set (8)**: `G-F4-NO-UNUSED` (run leg) · `G-F4-ADMIN-AXE` · `G-F4-CONTRAST-FLOOR` · `G-F4-CARRY-CLOSURE` (leg b) · `G-F4-ANCHORS` · `G-F4-CENSUS-CELLS` · `G-F4-VUE-TSC-CLEAN` · `G-F4-ZERO-CONSOLE`. **VERIFIED remains unstamped** — X·F's sub-tranche release close owns it, and this check does not move it.
