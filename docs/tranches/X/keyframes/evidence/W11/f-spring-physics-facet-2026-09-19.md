...
## §10 Execution record (appended after the family landed — WRITE-THEN-MEASURE)

**Commits (keyframes.js `master`, each by exact pathspec with the session trailer; SELF-COUNT ⟨cmd⟩ `git log --format=%s 0e604af8..HEAD | grep -c 'X.KF.W11.f'` → 5 · 5):**

| sha | meaning | files |
|---|---|---|
| `065008e9` | the field decision + the two-model contract (ONE sha, two files — §9 plan item 1) | `SpringHeatmap.vue` (813 lines changed) · `SpringPhysicsFacet.vue` (the binding + the axes import, 40) |
| `8b0f89ea` | SPF-3 — the symmetric remap on a stated geometry, the rail inset to [0, 1] | `SpringPhysicsFacet.vue` |
| `7ba8e7dc` | the preset cell surface — SPF-4 + SPF-5 + SPF-7 + KF-SS-37 | `SpringPhysicsFacet.vue` |
| `cad6c7a9` | G-KFW11-5's witness, 18 cases | `test/demo/scenes/spring-heatmap-reversibility.test.ts` (create) |
| `d479f394` | §0u ratchet — the port's one diagnostic cured at the root (`NO_PRESET = ""`, not a cast) | `SpringPhysicsFacet.vue` |

Plus value.js `f6d05a5c` (this file's §0–§9, committed BEFORE `065008e9` — the decision preceded the patch).

**Departures from §9's plan, disclosed.** (1) The regime labels moved once after the first live look: the "underdamped · rings" label is RIGHT-aligned under the line and the pips' names sit ABOVE their dots (the first cut put both left/below and "gentle" collided with the label at rail width); the critical tag reads `ζ = 1 · critical` at the line's left end. (2) The ToggleGroup's track plate is reset with `@layer utilities` classes ON the element, not by a scoped rule — measured live: the group's root reaches the DOM through reka's `as-child` and carries NO scope attribute (⟨probe⟩ root attrs = `tabindex dir role aria-label data-slot data-type data-variant data-size class style`; the items DO carry `data-v-…`), so a scoped `.preset-grid` rule never matches it. (3) A fifth commit (`d479f394`) landed for the ratchet — the port raised the count 24 → 25 at `7ba8e7dc` and the cure brought it back to 24 (double-run) with 0 in this unit's rows.

**Gate G-KFW11-5 — BEFORE → AFTER.** Runtime: ⟨cmd⟩ `npx vitest run --project demo test/demo/scenes/spring-heatmap-reversibility.test.ts` → `No test files found` ×2 at open → **18 passed (18)** ×2 at the settled bytes (once at `cad6c7a9`, once after `d479f394`), in `Tests 18 passed | Test Files 1 passed`. L-m-5's three cases (reversibility over 111 + 131 grid values × 2 directions · tolerance ≤ 0.025 on a 10 008-point sweep, tight · the 27-node mix table frozen inline and re-derived from the formula, the field's own `background-image` asserted to carry those 27 bands) + the painter's peaks (pure and on the REAL registered painter's transforms) + the contract (a real parent binds the two models; arrows/pointers) + D-B1's vertical legend + SPF-4 on the REAL producer ToggleGroup + SPF-7/SPF-5 at the bytes. Byte clause: ⟨cmd⟩ `grep -c 'clamp(values\[i\] ?? 0, 0, 1)' demo/scenes/spring/SpringPhysicsFacet.vue` → **1 · 1** at open → **0 · 0**. No `test.skip` / `it.skip` / `.only(` (⟨cmd⟩ `grep -c … spring-heatmap-reversibility.test.ts` → **0**).

**§0u ratchet.** ⟨cmd⟩ `npx vue-tsc --noEmit -p tsconfig.json 2>&1 | grep -c 'error TS'` → open **24 · 24** (0 in this unit's rows) → transient **25 · 25** at `7ba8e7dc` (1 in `SpringPhysicsFacet.vue:81`, TS2379) → **24 · 24** at `d479f394` (⟨cmd⟩ `… | grep -c 'SpringHeatmap\|SpringPhysicsFacet'` → **0 · 0**). The count did not rise past the banked floor at the settled bytes; the 24 that remain are the same 24 files as at open (⟨cmd⟩ `diff <(sorted open) <(sorted close)` → empty), none this unit's.

**Demo suite at the settled bytes.** ⟨cmd⟩ `npm run test:demo` → **Test Files 1 failed | 46 passed (47) · Tests 1 failed | 405 passed (406)** ×2. The one RED is **`.e`'s** `test/demo/scenes/spring-trace-truth.test.ts` (4b), which reads `SpringHeatmap.vue` by regex for a `DAMPING_MIN = <number>` declaration; this unit's contract re-homed that constant as the exported `DAMPING_AXIS.min` (one home for the coordinate space, L-M-5). The test fails EXPLICITLY with `.e`'s own re-bind instruction, exactly as `.e`'s receipt e-R3 foresaw ("`.j` should read such a red as a re-bind, never a skip"). The file is outside this unit's set → **ESCALATION KF11-E(f1)** below; no byte was kept in `SpringHeatmap.vue` to satisfy a sibling's regex (that would be a workaround aimed at a test).

**`npm run check`** → exit 2 on the three `src/**` TS6133 rows KF11-E2 already returned (compositor.ts:79 · waapi.ts:9 · smooth.ts:194) — outside every W11 row, unchanged by this unit. **eslint** over `demo/scenes/spring` + the test → exit 0. **`git diff --check`** clean on every commit.

**The bounded visual probe (§5.2 — one dev-server session, three captures, one DOM read; the two captures are NOT filed — `*.png` is gitignored under `docs/` — so their hashes stand in).** `p4-facet-light.png` (sha256 `1b329f91c4bd…`) · `p4-facet-dark.png` (`83550e8bf6be…`), in this seat's scratchpad, the Physics facet at `#/spring` in a 1280×900 chromium, DPR 2, first mount. Read live: the field 286 × 254 content box, border `color(srgb 0.914 0.9 0.886 / 0.5)` (the 50 % mix), `touch-action: none`, marker transition `0.2s` (the token), pips at (36.4 %, 49.2 %) smooth · (22.7 %, 65.4 %) snappy · (36.4 %, 80.8 %) bouncy · (54.5 %, 38.5 %) gentle; a centre click → readout `0.65 s / ζ 0.85`, ArrowRight → `0.70 s / ζ 0.85`; the group `role=group` / `Spring presets` / `display: grid` / radius 0 / padding 0 / `backdrop-filter: none` / no shadow; the active cell `aria-pressed="true"`, bg `color(srgb 0.947 0.932 0.960)` (the 8 % wash), border transparent, shadow none, outline 65 % dashed; the rail `left 47.4px` of 284 (1/6). Console: **zero errors, zero warnings** on the route. Nothing else was photographed.

**E13 sweep at this seat's clock.** Positional Status-cell scan of `INBOX.md` → **0 · 0** UNREAD over **79** rows; `../glass-ui/docs/tranches/BK/` newest (9 entries, unmoved); no letter newer than 03:00 today on any path but `INBOX.md` itself. Nothing minted; no row touched.
