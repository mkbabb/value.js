// SERVED MODEL: claude-opus-5-5 — KF.W13U.d5 COLD LEG: probe-switch10.mjs with ONE change — the pick is a route write (location.hash), so no scene row is hovered or focused and no warmScene fires; every first visit fetches its chunk cold (plus the dock hold below): 10 scene switches, sampled every frame.
...
for (const dest of route) {
  ...
  await p.evaluate((d) => { location.hash = '#/' + d; }, dest);
  // hold the dock expanded as the open menu does in probe-switch10.mjs: with no menu open the dock's
  // pointer-idle collapse (~2.5 s) would land inside the window, so the pointer jiggles 1 px on the dock
  { const bb = await p.locator('[data-dock-tether=top] .glass-dock').boundingBox(); const cx = bb.x + bb.width / 2, cy = bb.y + bb.height / 2;
    for (let i = 0; i < 6; i++) { await p.mouse.move(cx + (i % 2), cy); await p.waitForTimeout(417); } }
