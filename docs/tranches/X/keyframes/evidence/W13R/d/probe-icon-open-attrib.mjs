// SERVED MODEL: claude-opus-5-5 — KF.W13R.d copy of KF.W13R.v's probe-icon-rest5500.mjs (gate logic unchanged) + one attribution read: the hovered Scene trigger's own running animations (incl. ::before/::after) during the open-site window. KF.W13U.d2 (OA-32): the chosen scene's dock icon, headed real GPU.
...
async function measure(sel) {
  ...
  const trig = await p.evaluate(() => { const t = document.querySelector('[role=combobox][aria-label="Scene"]'); if (!t) return null; const an = document.getAnimations().filter((a) => a.effect?.target === t || t.contains(a.effect?.target)); return { hovered: t.matches(':hover'), running: an.filter((a) => a.playState === 'running').map((a) => (a.effect.pseudoElement || (a.effect.target === t ? 'self' : a.effect.target.tagName.toLowerCase())) + '|' + (a.transitionProperty || a.animationName) + '|' + a.constructor.name) }; });
  return { trig, box0: b0, bboxDelta: Math.max(Math.abs(b1.x - b0.x), Math.abs(b1.y - b0.y), Math.abs(b1.width - b0.width), Math.abs(b1.height - b0.height)), diffInside: inside, diffOutside: outside, outsideAt, ...info };
}
for (const s of (process.env.SCENES || 'cube,amiga,square,easing,spring,sequence').split(',')) {
  ...
  for (const [site, { sel, prep }] of Object.entries(SITES)) {
    ...
    console.log(s, site, JSON.stringify({ trig: r.trig, bboxDelta: r.bboxDelta, diffInside: r.diffInside, diffOutside: r.diffOutside, outsideAt: r.outsideAt, visible: r.visible, layers: r.layers, live: r.live, tag: r.tag, anims: r.anims }));
  }
}
