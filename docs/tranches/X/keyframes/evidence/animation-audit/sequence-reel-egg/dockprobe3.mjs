...
for (const mode of ["reel-then-play", "held-play", "scrub-then-play"]) {
  ...
  else await p.locator('[aria-label^="Play the reel"]').click();
  await p.waitForTimeout(mode === "held-play" ? 200 : 1900);
  ...
  await p.waitForTimeout(mode === "held-play" ? 1900 : 700); out.push("+later " + await st(p));
