...
// reproduce capture.mjs phase D: play ~1.2 s, drag while playing 0.05->0.95, release, 400 ms, Pause, drag paused 0.95->0.05
await p.locator(R + " button", { hasText: /^\s*Play/ }).first().click(); await p.waitForTimeout(1200);
await p.evaluate(() => window.__E.push([performance.now() | 0, "MARK", "drag-playing"]));
const x0 = rail.x + 20, x1 = rail.x + rail.width - 20;
await p.mouse.move(x0, y); await p.mouse.down();
for (let i = 0; i <= 30; i++) { await p.mouse.move(x0 + (x1 - x0) * i / 30, y); await p.waitForTimeout(50); }
await p.mouse.up(); await p.waitForTimeout(400);
await p.locator(R + " button", { hasText: /^\s*Pause/ }).first().click(); await p.waitForTimeout(800);
await p.evaluate(() => window.__E.push([performance.now() | 0, "MARK", "drag-paused"]));
await p.mouse.move(x1, y); await p.mouse.down();
for (let i = 0; i <= 20; i++) { await p.mouse.move(x1 - (x1 - x0) * i / 20, y); await p.waitForTimeout(50); }
await p.mouse.up(); await p.waitForTimeout(300);
const E = await p.evaluate(() => window.__E); fs.writeFileSync(OUT + `emitlog7-${run}.json`, JSON.stringify({ chain, E }));
const mv=E.filter(r=>r[2]==="pointermove"&&r[1]==="DOM").length, sm=E.filter(r=>r[2]==="onSlideMove"&&r[1]==="SliderImpl").length, up=E.filter(r=>r[1]==="Slider"&&r[2]==="onUpdate:modelValue").map(r=>r[3]).join(","), eng=E.filter(r=>r[1]==="ENGINE").map(r=>r[3]).join(","); console.log(`RUN ${run}: DOMmoves=${mv} slideMoves=${sm} updates=${up} engine=${eng}`); const k=E.findIndex(r=>r[2]==="drag-paused"); console.log(E.slice(k).filter(r=>r[1]!=="DOM-bubble-end").slice(0,40).map(r => r.join(" ")).join("\n")); if (false) console.log(E.filter(r=>r[1]!=="DOM-bubble-end").map(r => r.join(" ")).join("\n"));
}
await b.close();
