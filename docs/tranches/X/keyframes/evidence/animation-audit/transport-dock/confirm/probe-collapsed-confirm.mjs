...
const geo = await page.evaluate(() => {
  ...
  const walk = (el, d = 0, out = []) => { if (d > 5) return out; const s = getComputedStyle(el), r = el.getBoundingClientRect();
    ...
    if (el.matches("button")) out.push(`${"  ".repeat(d+1)}BTN h${Math.round(r.height)} w${Math.round(r.width)} br:${s.borderRadius}`); if (!el.matches("button,svg")) [...el.children].forEach((c) => walk(c, d + 1, out)); return out; };
