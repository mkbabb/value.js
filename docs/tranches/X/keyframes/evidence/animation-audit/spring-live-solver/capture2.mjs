...
await record("F2-play-pill", async () => console.log(await pill("Play animation")), 3000);
await record("G2-tap-while-playing", async () => console.log("railY", await tapRail(0.25)), 2200);
await record("H2-pause-then-tap", async () => { console.log(await pill("Pause animation")); await page.waitForTimeout(300); console.log("railY", await tapRail(0.80)); }, 2200);
await record("J-scrub-panel", async () => {
  const s = page.getByRole("slider", { name: "Scrub animation timeline" }); await s.scrollIntoViewIfNeeded(); const bb = await s.boundingBox();
