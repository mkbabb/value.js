...
const d0 = out.darkBtns.find(r => r.w > 0);
if (d0) { await p.mouse.click(d0.x, d0.y); await p.waitForTimeout(2000); }
await p.screenshot({ path: D + "theme-dark.png", clip: rect });
await p.screenshot({ path: D + "theme-dark-full.png" });
out.darkBg = await p.evaluate(() => [getComputedStyle(document.body).backgroundColor, document.documentElement.className, document.documentElement.dataset.theme]);
const lm = await btn("Switch to light mode"); const l0 = lm.find(r => r.w > 0); if (l0) await p.mouse.click(l0.x, l0.y);
out.k1 = ks();
