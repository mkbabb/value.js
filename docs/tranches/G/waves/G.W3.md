...
Per `audit/G-AUDIT-6 §2`: add 1 mobile-walk spec under `e2e/smoke-mobile/` exercising PaneSegmentedControl + dock interactions. Spec count: 35 → 36 files / 36 → 37 blocks.
...
Conjunction of sub-gates A + B + C + D + E + F + G + H. Wave-level:
- 4 new proof scripts at HEAD, all exiting 0 locally + wired in CI.
