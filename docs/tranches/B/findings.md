...
| F | `UnderlineTabs` shipped standalone, not a `<Tabs variant>` prop | Bζ | **B.W2 → re-filed** | B.W2 Lane C verified glass-ui's `<UnderlineTabs>` is **header-only** (no content panels / `data-state`) — it cannot replace `PaletteDialog`'s reka-ui `<Tabs>` provider without hand-rebuilding the 5 content panels + their a11y (a rebuild-by-hand regression). Orchestrator decision: demo keeps reka-ui `<Tabs>` + the `.underline-tabs` override; the gap re-files sharper to glass-ui (underline as a `<Tabs>`-provider variant). `coordination/Q.md §3`, `audit/B.W2-underline-tabs.md §9`. Named cross-repo destination — invariant-B5 valid. |
...
| H | Mandate 12's AND — the value.js `src/` library was never audited | Bε | **B.W3** | Audit `src/` cohesion/coverage; disposition the 5 untracked WIP files; close the custom typecheck cluster (estimated ~155, landed at 86 — vue-tsc 212→126); verify invariant-30 publisher compliance. |
...
| N | A.W7 close-audit fold-ins — found while closing A at B.W0 | A.W7 close audit 2026-05-19 (`docs/tranches/A/audit/W7-*`, `A/FINAL.md §6`) | **B.W1/W2/W4** | (N1) `demo/@/components/ui/alert/` is a hand-rolled re-implementation of a glass-ui primitive — 3 local SFCs + a local cva, 2 consumers → **B.W2** Lane A (convert the barrel to a glass-ui re-export, delete the 3 SFCs). (N2) dead unused `ui/` barrels (`chart`, `table`, `calendar`, …) with zero consumers → **B.W2** Lane A hygiene (invariant-33 grep then delete). (N3) `var(--animation-slide-md)` referenced at `PaletteCard.vue` with no definition anywhere — a phantom token, predates A → **B.W1** Lane C (invariant-32 retirement, alongside `floating-panel-item`). (N4) `useMetaballRenderer.ts:174` resolves `cssColorToRgb` via a 2D-canvas round-trip every frame → routed with `audit/W6-deferred.md` (the demo-abstraction follow-up). (N5) 14 doc-drift items → **B.W4** (its doc-drift lane already owns this scope; `audit/W7-doc-drift.md` is the inventory). |
