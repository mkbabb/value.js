...
4. **Abrogate the e2e suite** (B.W4). The four-lane e2e assay found the 16 Playwright specs are ≈3,510 lines of brittle, largely-superfluous nonsense — one (`color-visual-validation`) is a library unit test routed through a headless browser; ~42 `.lucide-*` selectors and ~132 `waitForTimeout` sleeps make the suite re-break on every restyle; 2 live-API specs never run in CI; 29 tests are `test.skip`'d. Per "abrogate before patch": delete all 16, replace with a 3-spec role/label smoke suite, add the project's first browser CI gate. The per-wave orchestrator live probe stays the primary wave-gate.
...
| **B.W4** | W3 close | value.js library gap audit + WIP disposition + custom typecheck cluster + e2e abrogation | Audit doc for `src/` cohesion/coverage gaps written; 5 untracked WIP files dispositioned (commit/retire/route); custom-component typecheck cluster closes ~155 errors (target vue-tsc count ≤135); all 16 `e2e/*.spec.ts` deleted; `e2e/smoke/` holds exactly 3 role/label specs; `playwright.config.ts` has a `smoke` project and no `mobile` project; CI runs `playwright test --project=smoke` |
...
| e2e abrogation + smoke suite | `e2e/*.spec.ts` (16 deleted), `e2e/smoke/**` (3 new), `playwright.config.ts`, `.github/workflows/node.js.yml` | B.W4 |
...
- **B.W4**: `audit/B.W4-library-gap.md` exists with the value.js cohesion/coverage findings + the WIP disposition decisions; the 5 untracked WIP files are either committed (`?? ` cleared) or removed from `src/index.ts` (no public-API debt); custom typecheck cluster vue-tsc count drops to ≤135 (from 290; ~155 closed); all 16 `e2e/*.spec.ts` deleted (deletion proof); `e2e/smoke/` holds exactly 3 role/label specs (page-load, color-space-switching, view-switch); `playwright.config.ts` has a `smoke` project and no `mobile` project; `.github/workflows/node.js.yml` carries the Playwright smoke step; `npm run test:e2e -- --project=smoke` green; `grep -rE 'lucide|waitForTimeout|page\.evaluate\(' e2e/smoke/` returns nothing.
