...
| **Tabs `underline`** (UPDATED — B.W2 Lane C: glass-ui shipped the WRONG SHAPE) | glass-ui shipped `UnderlineTabs.vue` as a **standalone, header-only** component (`src/components/custom/tabs/UnderlineTabs.vue`; subpath `@mkbabb/glass-ui/tabs`): `:options` array + `v-model`, a `<div role="tablist">` of buttons + a JS-animated underline. **No slots, no content-panel system, no `data-state`.** B.W2 Lane C verified it **cannot** replace the demo's `PaletteDialog` tabs — that surface is a reka-ui `<Tabs>` *provider* hosting 5 `<TabsContent>` panels with `role="tabpanel"`, `aria-labelledby`, roving tabindex, arrow-key nav, and `data-state`-driven `.palette-tab-content` animation. Swapping in a header-only component would force hand-rebuilding the panels + their a11y — a rebuild-by-hand anti-pattern and an a11y regression. **Orchestrator decision: the demo keeps reka-ui `<Tabs>` + the small `.underline-tabs` override.** See `audit/B.W2-underline-tabs.md`. | **PARTIAL — glass-ui shipped the wrong shape; gap re-filed below** | A.W2 (`.underline-tabs` CSS marker) | **NOT consumed.** `.underline-tabs` override stays — a live, documented override; retires only when glass-ui ships the row below |
| `Card` props fail-explicit | A.W1 consumed Q.W2 Lane A's anticipated Card change | unchanged | A.W1 | — |
| **`floating-panel-item`** | `floating-panel.css` comment names it, no CSS rule anywhere; demo applied the class at 6 sites with no styling effect | **RETIRED at B.W1** — phantom class stripped from all 6 sites (`audit/B.W1-floating-panel-item.md`) | (`HARDEN-4 §5.3` said "consider filing"; never filed) | done — no glass-ui action needed |
| **`<Tabs variant="underline">` on the provider family** (SHARPENED — the real ask) | The original A.W2 ask was an underline *variant*. glass-ui shipped a parallel header-only `<UnderlineTabs>` instead, which cannot serve a tabs-with-content surface (see the Tabs-`underline` row above). The demo needs underline as a variant **within** the `<Tabs>` provider family (e.g. `<TabsList variant="underline">` or `<Tabs variant="underline">`) preserving `<TabsContent>`, `data-state`, and the panel ARIA. | **OPEN — re-filed precisely** | — | a glass-ui successor tranche; B consumes once shipped, `.underline-tabs` retires then |
