...
|---|---|---|---|
| **babb.dev spine** (EC2 `34.197.214.67:1022`, `/opt/deploy`) | git-push → adnanh/webhook (`:9000/hooks/<slug>`, HMAC-SHA256, `ref==master`) → per-repo `deploy-hook.sh` on host → Docker compose → Apache TLS → loopback `:81x0` | fourier, **words** (`api.words.babb.dev`, `:8110`), csp-solver/**sudoku** (`api.sudoku.babb.dev`, `:8120`), value.js (`api.color.babb.dev`, `:8130`) | Hardened hook: (1) flock per-repo, (2) dirty-tree-fail-loud, (3) real bounded health-gate (exit = rollback trigger), (4) rollback-on-rollback (reset→REBUILD→re-gate), (5) optional idempotent auto-migration. CF Pages serves frontends. |
| **friday.institute** (NCSU, NXDOMAIN public, VPN-gated, SSH `:1022`) — SEPARATE, app-owned | CF Pages (`deploy-pages.yml`, workflow_run-gated) for frontends + SSH/scp `deploy.sh` for backends | speedtest, slides | App-owned; NOT the adnanh/webhook spine. (**words is NOT friday** — it's babb.dev: `words.babb.dev` + `api.words.babb.dev` on the mbabb server; see the babb.dev row. There is NO `words.friday.institute`.) |
...
**babb.dev `:81x0` backend port-map (4 backends):** `fourier :8100 → api.fourier` · `words/floridify :8110 → api.words` · `sudoku/csp-solver :8120 → api.sudoku` · `value.js/palette :8130 → api.color`. **speedtest is NOT here** (`:8140` retired — it's friday.institute). Single Apache TLS terminator fronts every `api.<app>.babb.dev` → `127.0.0.1:81x0`.
...
**friday.institute (the cardinal correction the spine has NEVER documented — `grep friday.institute deploy/ → 0 hits`):** hosts **speedtest + slides** only. **words is babb.dev, NOT friday (DEC-9 correction 2026-06-03):** `words.babb.dev` + `api.words.babb.dev` on the mbabb server — there is NO `words.friday.institute`. The current `words/scripts/deploy.sh` (SSH to the mbabb server, `DOMAIN=mbabb.friday.institute`, subpath `/words/`) must be re-pointed to `words.babb.dev`/`api.words.babb.dev` (a words-repo fix, ζ-booked); the babb.dev `:8110` spine arm is words' single home. `speedtest/scripts/deploy.sh` is a 3-layer pipeline: CF Pages frontend (`speedtest.friday.institute`, slug `speedtest`, `.pages.dev`=`speedtest-cf1.pages.dev`) + CF Workers edge (`speedtest-edge.fridayinstitute.workers.dev`) + EC2 Docker API (`api.speedtest.friday.institute`, git-pull + `/api/health` gate + alias-swap rollback). slides' `deploy-pages.yml` shares the same CF account.
...
> **RESOLVED 2026-06-03 (user) — DEC-9:** words → **babb.dev** (`words.babb.dev` + `api.words.babb.dev`) on the mbabb server — **NO `words.friday.institute`**; words is a babb.dev spine backend (`:8110`→`api.words`); re-point its `deploy.sh` off `mbabb.friday.institute`. friday.institute = speedtest + slides only · SUM-1 speedtest **stays frozen; its AT tranche AMELIORATES the CLS-P0** so the freeze can lift (the lighthouse gate is the lift criterion) · **NO repo ever rsyncs — git-pull/webhook only**; value.js's "rsync outlier" was a STALE DOC (dispatch.sh + scripts/deploy.sh both git-pull), **FIXED** in `deploy/host/deploy-dir-layout.md` · branch = **always `master`** (slides aligns main→master) **AND parameterize** the vended templates (belt+suspenders) · library publish = **changesets + `--provenance` + lockfile-diff** (maintainer's call). Remaining host action: the 3 `.pages.dev` `wrangler` verifications (P0, pre-sync). The list below is the original framing.
