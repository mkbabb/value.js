...
- **DEC-9 · CI/CD unification verdicts (2026-06-03, user).** (a) **words** = **babb.dev** (`words.babb.dev` + `api.words.babb.dev`) on the mbabb server — **NO `words.friday.institute`**; words is a babb.dev spine backend (`:8110`→`api.words.babb.dev`); re-point its `deploy.sh` off `mbabb.friday.institute`. friday.institute hosts speedtest + slides only. (b) **speedtest SUM-1 freeze stays**, but its **AT tranche must ameliorate** the dial-CLS P0 so the freeze can lift (the lighthouse gate = the lift criterion). (c) **NO repo ever rsyncs — git-pull/webhook only**; value.js's "rsync outlier" was a STALE DOC (dispatch.sh + scripts/deploy.sh already git-pull) — **FIXED** in `deploy/host/deploy-dir-layout.md`. (d) **branch = always `master`** (slides aligns main→master) AND the vended templates **parameterize** `default_branch` (belt+suspenders). (e) **library publish discipline** = changesets + `--provenance` + lockfile-diff gate for glass-ui / keyframes.js / value.js (maintainer's call).
