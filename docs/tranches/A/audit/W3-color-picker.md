...
Findings applied: Ag-1 (φ type scale), Ag-8 (radius aliases), Ag-9 (ColorNutritionLabel hierarchy), Ag-10 (`font-normal` audit), Ag-7 (shadow routing).
...
**No `font-normal` changes**: the `font-normal` at `:40` is on a plain `<span>` (not a heading), so it does not cancel heading weight — no action per Ag-10 rule.
...
**Input radius**: already `rounded-[var(--radius-input)]` — Ag-8 P1 bug was pre-fixed.
...
### Ag-10 — `font-normal` / display tier
| Location | Before | After | Reason |
|---|---|---|---|
...
| `controls/ComponentSliders.vue` | `:40` | `font-normal` retained | On a plain `<span>`, not a heading — does not cancel heading weight per Ag-10 rule |
