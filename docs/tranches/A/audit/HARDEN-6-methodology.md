...
> **B.W4 update.** This section flagged a research-lettering defect; B.W4's `Ag→Ac` rename resolved it. The original finding is preserved below for the record, with the post-rename state noted inline.

At this hardening pass A's research files were `Aa`, `Ab`, `Ad`, `Ae`, `Ag` — five deliverables with visible gaps at `Ac` and `Af` in the Latin sequence. The internal lane IDs compounded the inconsistency: PROGRESS.md §"Round 1" labels the five lanes `Aα Aβ Aγ Aδ Aε` (Greek), and the deliverables themselves used a third scheme — `Aa-runtime-keystone.md` opens with the header `# A.Rα` and `Ad`/`Ae`/`Ag` referred to themselves as `A.Rδ`, `A.Rε`, and so on. So one tranche carried three parallel naming systems for the same five lanes: Latin filenames with gaps (`Aa, Ab, Ad, Ae, Ag`), Greek lane IDs (`Aα..Aε`), and `A.R<greek>` document headers.

glass-ui's peer tranche Q uses one coherent Greek scheme (`Qα..Qζ`). A's filename gaps were not intentional reservations — nothing in A.md, PROGRESS.md, or A-challenge.md explained them, and the lane count is five, not seven. The `Ag` filename was a Greek-letter transliteration (γ → `g`) inconsistent with the Latin run of the others. **B.W4 renamed `Ag-design-tokens-hierarchy.md` → `Ac-design-tokens-hierarchy.md`**, so the Latin filename sequence is now contiguous `Aa..Ae` with no gaps, and the `research/Aa..Ae` shorthand in `A.md` is accurate. The Greek lane IDs and the `A.R<greek>` document headers are left as-is — historical, unambiguous given the rename.
...
Cross-check against the research finding inventory. The five research files carry roughly 67 findings: Aa has 4 (`A-key-1..4`), Ab has 19 (`Ab-1..19`), Ad has 20 (`Ad-1..20`), Ae has 16 (`Ae-1..16`), Ac has 13 (`Ac-1..13`). (At the time of this hardening pass the γ-angle file was named `Ag`; B.W4 renamed it to `Ac-design-tokens-hierarchy.md` so the Latin filename sequence `Aa..Ae` is gap-free and the `research/Aa..Ae` shorthand in `A.md §8`/§10 is accurate. Before the rename that shorthand omitted the `Ag` file — a writing slip this audit flagged. `HARDEN-3 §1` settled the finding count at 13, not 14.)
...
1. **A.md §8 "None" overstates zero-deferral.** Four research findings are un-routed: `Ac-6` (P1 bug — `--glass-opacity-subtle` override targets a glass-ui-retired token), `Ab-16` (PointerDebugOverlay hardcoded colors, research-marked deferrable), `Ad-20` (`SelectContent` widths, research-marked not-worth-fixing), `Ae-12` (Aurora cursor seam, research-marked non-blocker). Fix: add `Ac-6` to W3 Lane A scope (it is a token bug, same class as the W3 shadow/radius work); add an explicit retire-with-rationale line for `Ab-16`, `Ad-20`, `Ae-12` to A.md §8, citing each research finding's own deferral note. §8 then reads "every finding lands, retires with recorded rationale, or has a named destination" honestly.
...
6. **Research lettering is incoherent.** Files `Aa, Ab, Ad, Ae, Ac` have unexplained gaps at `Ac`/`Af`; lane IDs use Greek (`Aα..Aε`); document headers use `A.R<greek>`; the Greek and Latin orders disagree. Fix: rename the five files to one contiguous scheme (`research/A1..A5` recommended, or `Aα..Aε` to mirror Q); update the `# A.R<x>` headers, PROGRESS.md §"Round 1" lane table, and every `research/A<x>` cross-reference in A.md (§3, §4, §10), findings.md (§3, §4), A-challenge.md, the six wave specs (`Source:` lines), and coordination/Q.md §3.

7. **`research/Aa..Ae` shorthand omitted the `Ag` file.** A.md §8 and §10, dispatch/AGENT.md §"Zero deferral", and coordination/Q.md all wrote "`research/Aa..Ae`" — a range that, before the rename, excluded `research/Ag` (the design-tokens file, 13 findings). Resolved by B.W4's `Ag→Ac` rename: the Latin filename sequence is now contiguous `Aa..Ae`, so the shorthand is accurate as written and needs no further edit.
