...
Findings applied: Ag-1, Ag-7, Ag-8, Ag-12
...
**Ag-8** — card surface radius  
- line 4: `rounded-[var(--radius-card)]` → `rounded-card`

**No changes for Ag-1, Ag-7, Ag-12**:
- Ag-1: `text-subheading`, `text-micro`, `text-mono-small`, `fira-code` already in use — no raw Tailwind text sizes present.
- Ag-7: no hand-rolled `box-shadow` literals in this file.
- Ag-12: `Badge variant="secondary" class="text-mono-small"` at line 36 (color count) already correct — kept.

---

### PaletteCardGrid.vue

**Ag-1** — empty-state text  
...
This count sits in a `DropdownMenuItem` sidebar slot, not beside a heading. Ag-12 binding ("count beside a label/heading") does not apply to sidebar counts inside menu items. Kept as `text-caption` span; documented.
...
**Ag-8** — dialog surface radius  
- line 5 (`DialogScrollContent`): `rounded-[var(--radius-dialog)]` → `rounded-dialog`

**Ag-7 skip**: `box-shadow: var(--shadow-modal), 0 0 0 1px var(--border)` in scoped CSS (lines 611, 617) already routes through `--shadow-modal`. This fix landed in W2 (Ab-2). Not re-touched.

---

### PaletteDialogHeader.vue

**Ag-1** — dialog title at display register  
...
**Ag-8**: `DialogContent` has no explicit `rounded-*` (inherits from shadcn component default). No role-bearing raw radius in template. No change.  
...
| File | Element | Reason |
|---|---|---|
| PaletteCardMenu.vue:66 | `<span>` version count in dropdown item | Count in a menu sidebar slot, not beside a heading; Ag-12 Badge rule applies to heading-level counts only |
