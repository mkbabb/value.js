...
axis "what structure is DERIVED FROM"; F5/F6 split the execution axis (strangler vs big-bang). **Pass-4
fold (`design/pass-4/AGGLOMERATION.md`)**: the three gap-closer charters CHARTED the whole formerly-un-charted
owner HALF with RAN exemplars + **9 committed durable instruments** (BOTH auditors re-ran all 9; every number
reproduced) — **δ** complexity-reduction (ADVANCED **72**) · **ε** long-dirs+styles+demo-wide-manifest
(ADVANCED **72**) · **ζ** hygiene (ADVANCED **75**); the composed legs A/B/C HOLD (68/62/68 — ε generalized
α's method demo-wide, closing A's scope cap; β's registry un-back-filled). **BOTH required fresh non-author
adversaries RAN in pass-4**: full-thesis (NOT clean, **80% convergence**, GAP-1..GAP-7) + owner-verbatim
(CLEAN on clean-break, **78% carriage**, §4 gaps 1..5). → **the COMPOSED THESIS rises 60→78%** — the
un-inflated FLOOR of the two lenses (full-thesis 80 / owner-verbatim 78), held at 78 because clause-9
performance-above-all (un-charted anywhere) is a real drag the full-thesis lens under-weighted; never inflated
above the stricter lens. The pass-3 "half the edict un-charted" finding is CLOSED; NG-1/3/4/6 discharged.
Thesis RE-CAPPED at 78% by the surviving gaps: P4-G1/G2 the durable law applied to censuses not codemods (no
committed `--apply` carve; 2 color-SCC carves ride β's un-back-filled registry) · P4-G3 `@src` mislabeled DEAD
(live @ 235 sites, a fresh NG-3 self-violation) · P4-G4 the fork pile GREW to ~13 · P4-G5 performance-above-all
un-charted · P4-G6 the down-metric is ceiling-compliance not net — now THREE OPEN rows with named pass-5
closers (η/θ/ι + the standing durable-codemod law). **Pass-2 rows retained** for the families: F3 48→40,
F4 44→36 (retro overrides); the F4 lattice-mirror SHAPE-half RETIRED. Convergence % are the critics'/auditor's
earned numbers throughout; States/% are never inflated. **States**: ADVANCED (live candidate) · BANKED
(substrate/support) · MERGED (subsumed) · OPEN (enumerated, un-charted — the pass-5 surface) · RETIRED (dead)
· BLOCKED (none fire).
...
| **⇒ THE COMPOSED THESIS (A × B × C × δ × ε × ζ)** | F1-law ∘ F3-placement × F6-vehicle+F2-substrate × F4-dimension+F5-battery × δ-complexity × ε-demo-wide × ζ-hygiene | **ADVANCED — THE CANDIDATE** | **78%** | pass-4: the whole owner edict is now charted; 9/9 analysis instruments durable + reproduce; NG-1/3/4/6 discharged; NG-2 narrowed to the codemod layer. RE-CAPPED at the un-inflated FLOOR **78%** (full-thesis 80 / owner-verbatim 78; clause-9 the binding drag) by P4-G1/G2 (execution not re-runnable), P4-G3 (`@src` scope slip), P4-G4 (fork pile grew to ~13), P4-G5 (performance un-charted). The remaining 22% is ENUMERATED with named η/θ/ι closers. Clock 0 of 2 |
| **⊕ clause 12 — LIBRARY COMPLEXITY REDUCTION** | god-module carve set + a metric that goes DOWN | **⟐ Charter δ — ADVANCED** | **72%** | pass-4 δ CHARTED the sharpest clause: G500 14→11 / OVER500 1576→1075 / MAXLOC 754→658 across 3 RUN carves + a real born-RED gate (FAIL@11 / PASS@14, verified) + NG-4 ruled by measurement (β's merge = relocation, the carve is the win; carve amplifies 5→10). Capped by P4-G1/G2 (10-of-13 SPEC, execution not re-runnable) + P4-G6 (ceiling-compliance ≠ net; LoC ROSE +158) → pass-5 η |
| **⊕ clause 4/3/1 — long-dirs + styles + demo-wide manifest** | `panes/` carve · `style.css` cohesion · per-dir flatten across all 6 `demo/@/` | **⟐ Charter ε — ADVANCED** | **72%** | pass-4 ε CHARTED clauses 4/3/1: panes chassis-carve RAN (16→11+chassis, 0-consumer barrel deleted), `style.css` cohesion-carve RAN (79.5% legitimately global — honest), the 377-site abrogation + 64-file placement census (α-reproduction PASS), NG-6 dispositioned (R1/R2), NG-3 objective stated. Capped by P4-G3 (`@src` mislabeled DEAD) + P4-G4 (OF-ε-1/OF-ε-2 net-new forks) → pass-5 θ |
| **⊕ clause 11 — D4 hygiene (minus the gate-cull)** | PNGs · worktrees · benches · scripts · docs · plugins | **⟐ Charter ζ — ADVANCED** | **75%** | pass-4 ζ CHARTED clause 11: all 6 survey items → booked dispositions with per-row proof + 3 durable instruments (39 PNGs SWEEP · 16 worktrees PROTECT-defer · 11 benches 6KEEP/4RETIRE/1REWRITE · scripts 7KEEP/1CULL · docs KEEP · gates→C); the plugins "delete" contradiction → OF-6 with measured poles (3 poles × 2). Capped by OF-6 undecided + P4-G7 (doc drift) + P4-G8 (adopt-blocked delta) |
| **⊕ P4-G1/G2/G6 — EXECUTION reproducibility + net-metric** | commit `--apply` carve/scatter codemods + back-fill β's registry + a dedup/dead-code scan | **OPEN → pass-5 η** | — | the durable-instrument law applied to CODEMODS not just censuses: no committed `--apply` carve (δ's "3 carves green"/ε's "smoke 154" reset away); `dispatch`/`gamut` ride β's un-re-runnable "2 FAIL→0" registry; add the dedup scan so "no net reduction" is measured not asserted; reconcile the `docs-linkgraph` prose |
| **⊕ P4-G3/G4 — scope-honesty + fork CLOSURE** | fix the `@src`=DEAD mislabel · attach every fork a recommended pole + measured cost | **OPEN → pass-5 θ** | — | correct `@src` (0-in-`demo/@/`; LIVE @ 235 sites + a surviving vite alias; @-abrogation is demo-scoped) → fold into OF-ε-1; drive the ~13-fork pile toward CLOSURE (recommended pole + cost + the one owner-decision each needs) so the owner rules a BATCH at OF-5 and the pile stops growing. θ opens NO new fork |
| **⊕ P4-G5 — the PERFORMANCE limb (clause 9)** | a value.js perf win with a down-metric OR an explicit reconcile-and-route | **OPEN → pass-5 ι** | — | "elegance, simplicity, AND performance above all" — the third limb has ZERO carriage across 4 passes. ι charts it: either a parse/interp/subpath-budget perf win, or name U.W-PERF/Q14/LCP as the carrier + book what value.js owns vs defers. Not survey-only |
...
| 4 (3 CHARTERS + 2 FRESH AUDITS → AGGLOMERATE) | pass-4 fleet (charter-δ/ε/ζ + 9 durable instruments · full-thesis convergence adversary · owner-verbatim/clean-break audit) → agglomerator | DONE — `design/pass-4/AGGLOMERATION.md`. **The three gap-closer charters CHARTED the whole formerly-un-charted owner HALF with RAN exemplars + 9 committed durable instruments (BOTH auditors re-ran all 9 from the repo root; every number reproduced — the pass-3 NG-1/NG-2 reproducibility debt is CLOSED for the analysis surface): δ (complexity, clause 12 — a metric that goes DOWN: G500 14→11 / OVER500 1576→1075 / MAXLOC 754→658 across 3 RUN carves + a real born-RED gate FAIL@11/PASS@14 + NG-4 ruled by measurement) · ε (long-dirs+styles+demo-wide-manifest, clauses 4/3/1 — panes chassis-carve + `style.css` cohesion-carve RAN, the 377-site abrogation + 64-file placement census, NG-6 dispositioned, NG-3 objective stated) · ζ (D4 hygiene, clause 11 — all 6 survey items → booked dispositions, the plugins "delete" contradiction → OF-6 with measured poles).** The composed legs A/B/C HOLD (68/62/68). **BOTH required fresh non-author adversaries RAN**: full-thesis (NOT clean, **80% convergence**, GAP-1..GAP-7) + owner-verbatim (CLEAN on clean-break, **78% carriage**, §4 gaps 1..5). Composed thesis **60→78%** (the un-inflated FLOOR of the two lenses; clause-9 the binding drag — never inflated above the stricter lens). NG-1/3/4/6 discharged. **Gaps survive**: P4-G1/G2 (the durable law applied to the *censuses* not the *codemods* — no committed `--apply` carve; the 2 color-SCC carves ride β's un-back-filled registry) · P4-G3 (`@src` mislabeled DEAD — a live alias @ 235 sites, a fresh NG-3 self-violation) · P4-G4 (the fork pile GREW to ~13: OF-4→7, OF-6→3 poles, + new OF-ε-1/OF-ε-2/NG-6-R1vR2) · P4-G5 (clause-9 performance-above-all un-charted anywhere) · P4-G6 (the down-metric is ceiling-compliance not net — LoC ROSE +158). **→ pass-4 is NOT the clean pass (clock 0 of 2).** OWNER-FORK fold: OF-4 WIDENED (1→7 demo-wide rows) · OF-6 WIDENED (3 poles × 2 plugins) · NEW OF-ε-1 (`@assets`+`@src` survival) · OF-ε-2 (panes per-dir) · NG-6-R1vR2 (hoist-the-type vs ratify). **Next: pass-5 gap-closers — η the execution-reproducibility CODEMOD set (commit `--apply` carve/scatter codemods idempotent from the pinned base + back-fill β's registry + a dedup scan + reconcile the linkgraph prose) · θ scope-honesty (fix the `@src` mislabel) + fork CLOSURE (recommended pole + cost per fork; the pile stops growing) · ι the PERFORMANCE limb (charter clause 9) — under the standing durable-CODEMOD law (every "RAN green" ships a committed idempotent codemod) + a FRESH full-thesis adversary over the expanded surface. Only a zero-gap pass starts the two-clean-pass clock.** |
