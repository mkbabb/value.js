/**
 * CHALLENGE-C probe harness for demo/color-picker/ErrorBoundary.vue.
 * READ-ONLY audit artifact: mounts the SHIPPING component unmodified.
 *
 * Run:
 *   npx vitest run --config \
 *     docs/tranches/V/megatranche/audit/components/ErrorBoundary/probes/vitest.challenge-c.config.ts
 */
...
describe("C-A · the catch path renders and announces", () => {
    it("paints role=alert with the machine-truth detail", async () => {
        const w = mount(ErrorBoundary, { global, slots: { default: () => h(AlwaysThrows) } });
        await nextTick();
        await nextTick();
...
describe("C-B · DIAGNOSTIC BLACKOUT", () => {
    it("no console.error, no console.warn, no app.config.errorHandler", async () => {
        ...
        console.log("[C-B] console.error mentioning the throw:", errCalls.includes("boom-from-render"));
        console.log("[C-B] console.warn mentioning the throw:", warnCalls.includes("boom-from-render"));
...
describe("C-D · FOCUS IS NEVER RESTORED on successful recovery", () => {
    it("activeElement falls to <body> after reset", async () => {
        ...
        console.log("[C-D] activeElement after catch:", ae?.tagName, ae?.getAttribute("role"));

        const btnW = w.find("button");
        (btnW.element as HTMLButtonElement).focus();
        console.log("[C-D] activeElement before reset:", document.activeElement?.tagName);
...
describe("C-F · no live region exists before the error (Safari/VO announcement hazard)", () => {
    it("the aria-live node is created in the same commit as its content", async () => {
        const w = mount(ErrorBoundary, { global, slots: { default: () => h(Healthy) } });
        await nextTick();
        console.log("[C-F] pre-error DOM:", JSON.stringify(w.html()));
        console.log("[C-F] pre-error [aria-live] count:", w.findAll("[aria-live]").length);
        console.log("[C-F] pre-error [role=alert] count:", w.findAll('[role="alert"]').length);
