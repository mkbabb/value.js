...
**Provenance** This document is the merge of **three independent D-axis passes** over the same tree. Pass 1 produced D-B1..D-B3, D-M1..D-M9, D-m1..D-m9, D-i0..D-i3, S-1..S-5. Pass 2 folded in **D-B4, D-M10, D-m10, D-m11** — findings pass 1 did not reach — and filed **two corrections against pass 1's own record**: `D-C1` (D-B2's mechanism is misstated; its verdict survives) and `D-C2` (S-3 rests on a premise the tree falsifies). Pass 3 (this write) folded in **D-M11, D-M12, D-m12..D-m15, S-7**, escalated **D-m7 → MAJOR** on an independent confirmed mechanism (`D-E1`), and filed **two further corrections**: `D-C3` (the repair lane's `α ≳ 0.60` instruction lands *under* the AA floor it cites) and `D-C4` (D-M9's pass-2 proportion note measures a control height the cascade never renders). Every prior pass's text is preserved verbatim; nothing was deleted. L-18 runs both ways, and it runs against this document too — pass 3's single largest contribution is a **falsifier-proof strengthening of D-B3** and the retraction of a defect it set out to file (`S-7`).

**Tally** — 4 BLOCKER · 13 MAJOR · 14 MINOR · 3 INFO (**34 defects**) · 7 SUPERLATIVE (two now qualified, `D-C2` / `D-M11`) · 1 recorded NON-defect **overturned** (`D-i0` → `D-m11`) · 1 severity **escalation** (`D-m7` → MAJOR, `D-E1`) · 4 self-corrections.
...
Pass 3 adds a fourth costume to that same defect and names the pattern beneath all four. Passes 1–2 audited what the file *declares*; pass 3 audited what glass-ui 7.0.0 **already computes underneath it** — `--ui-scale`, `--control-floor`, `--control-h-sm`, `min-block-size` — and found the consumer's own sizing vocabulary is inert against it in both directions. `RIBBON_BUTTON_CLASS` (`RibbonBar.vue:135`) is five tokens: one is a **phantom utility defined nowhere** (D-M11), one is **structurally unreachable** (D-m12, `h-8`), one is a literal no-op (`rounded-full`), and one silently **decouples the label from the system's pointer scale** (D-m13). The design-system-conformance superlative pass 2 awarded that very file (S-6) is therefore qualified: the *props* conform exactly; the *classes* beside them do not resolve. The compensating find is S-7 — the coarse-pointer scale means the adoption delivers conformant 54px touch targets **despite** the call site trying to pin them at 32px, which is the third time in this document that adopting glass-ui bought a property the consumer was working against.
