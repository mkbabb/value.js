...
| 16 | **§B-12 (kf-side)** | K (self) | The owner-checkout reconciliation — one act, `git fetch && git reset --hard origin/master`, untracked V docs unharmed | `FOLD-FORWARD.md` §B-12 | **EXECUTED 2026-09-17 — under the owner's begin-word, by the owner's DELEGATED hand, in the exact three-step reversible order ruled at `COHESION.md` §0j.C KF-OP1.** ⟨*The cell's prior disposition, carried verbatim and not rewritten (E-3): "**OPEN on their side.** Blocked from our side by lane law and by their sacred-checkout rule — value.js must NOT perform it". That reading was correct for every day before the begin-word; the begin-word — which grants "publish, push, and pull whatever items you need" — is what changed, and §0j.C names the hand. A bare reset by value.js remains forbidden; what ran is the ruled order and no other.*⟩ **The act**: (1) `git branch kf-sacred-snapshot-2026-09-17` at HEAD `8281638c`; (2) `checkout && add -u && commit` → snapshot **`6d280ee7`** (226 files, 4483+/6156−) — `add -u` only, so nothing untracked was staged; (3) `checkout master && git fetch origin && git reset --hard origin/master` → **`81a56990`**. **After**: 0 behind · 0 ahead · 0 tracked status rows · 0 frontier-diff · merge-base = `81a56990`. **Rider, carried loudly** (OP-1's own finding): the ruled premise *"untracked V docs unharmed"* holds only for untracked paths **absent** from the target tree — `reset --hard` skips `verify_absent`, so **118 untracked files whose paths exist at `origin/master` were absorbed** (97 of them `docs/tranches/V/`, all keyframes' own V docs) and **6 survived**, including **both** value.js-delivered inbound packets, byte- and mtime-intact. The disposition record is `docs/tranches/X/keyframes/W0/SUBSTRATE-SETTLE-2026-09-17.md`; `:145`'s live-unexecuted-maintenance-act framing above is untouched |
