...
| area core (7 comp) | `wf_66b1fcba-daa` | 17/21 canonical reports · 5 full/1 partial/1 zero | Workflow record says **completed**; physical output does not. Hydrate returned payloads and hash the canonical files before any rerun. Verbatim roster `workflows/args/core.json` |
| area scenes (7 comp) | `wf_dee4c83a-ec2` | 12/21 canonical reports · 4 full/0 partial/3 zero | Workflow record says **completed** while its own result reports only 3/7 run. Treat the record as a claim; hydrate/verify from journals first. Verbatim roster `workflows/args/scenes.json` |
| area picker (4 comp) | `wf_3c8798e8-23e` | 5/12 canonical reports · 1 full/1 partial/2 zero | **KILLED — the only non-complete workflow record.** Harvest/hydrate its journal, then resume with the byte-identical persisted roster only after the other four bands are physically reconciled |
...
| area workbenches (19) | `wf_6edda4a1-192` | 32/57 canonical reports · 10 full/2 partial/7 zero | Workflow record says **completed 19/19**; the return did not recreate 25 declared files. Hydrate complete cached report text, hash it, then rerun only genuinely absent seats |
| area palettes (32) | `wf_22b7a7b7-97b` | 24/96 canonical reports · 8 full/0 partial/24 zero | Workflow record says **completed 32/32**; 72 declared canonical files are absent. Hydrate from cached result/journal payloads before any rerun |
