...
| `audit/visual/` | route + state capture matrices, `owner-marked/` witnesses | STABLE |
...
| **PHASE D safari-real matrix** | `wf_060ffe39-0fd` | 3 seq. Opus seats | **RUNNING** — routes×viewports + inverted/reachable state cells + the dock morph crash arm (glass S0, `--dock-expand-t`), sequential (single safaridriver session), → `audit/visual/safari-real/MATRIX-SAFARI.md`. safaridriver relaunched on :4599 (nohup); dev server :9000 UP API-less. O-11..O-15 packet authoring delegated (Opus seat, from `library-band.md`) |
...
| area shell (12 comp) | `wf_e28d617f-9eb` | 4/72 | **RESUMED 2026-07-27 CHALLENGE-ONLY** (jury stage stripped from component-apotheosis.js per M-12; challenge prompts byte-identical, banked seats replay; roster recovered verbatim from the session transcript into `workflows/args/`) |
| area workbenches (19) | `wf_6edda4a1-192` | 6/114 | **RESUMED 2026-07-27 CHALLENGE-ONLY** (jury stage stripped from component-apotheosis.js per M-12; challenge prompts byte-identical, banked seats replay; roster recovered verbatim from the session transcript into `workflows/args/`) |
| area palettes (32) | `wf_22b7a7b7-97b` | 1/192 | **RESUMED 2026-07-27 CHALLENGE-ONLY** (jury stage stripped from component-apotheosis.js per M-12; challenge prompts byte-identical, banked seats replay; roster recovered verbatim from the session transcript into `workflows/args/`) |
