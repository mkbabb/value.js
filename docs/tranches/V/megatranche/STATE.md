...
| area scenes (7 comp) | `wf_dee4c83a-ec2` | 2/7 comp true | **QUEUED (cap-4).** Resumed 2026-07-28 then TaskStopped ~35min in when the liveness probe proved SIX workflows concurrently alive (the wb/palettes "completed" records were false — their pre-compaction deploys were still running). Banked seats replay on the next resume; re-launch with `resumeFromRunId: "wf_dee4c83a-ec2"` + `workflows/args/scenes.json` VERBATIM when a slot frees |
