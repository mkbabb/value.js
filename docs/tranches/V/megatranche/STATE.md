...
| `audit/visual/` | route + state capture matrices, `owner-marked/` witnesses **OM-1..OM-13** (evening 2026-07-27 batches: OM-3/4 = MT-F029/F030; OM-5 dev-banner-REMOVE = MT-F031; OM-6 blob vibrancy = MT-F032; OM-7 capsule shadows = MT-F033 extends F026; OM-8 ANIMATED-transitions MANDATE = MT-F034; OM-9+OM-10 defect/control pair About-pane misalignment = MT-F035; OM-11+OM-12 palette-card shadow artifacts + NO hover state = MT-F036, ORDERED FIXED, extends MT-F027's mechanism; OM-13 easing readout not a glass input = MT-F037, joins OM-4 in the easing cut) | GROWING |
...
| area core (7 comp) | `wf_66b1fcba-daa` | 2/7 comp true | **RESUMED 2026-07-28 (ACTIVE)** — App + ColorSpaceSelector adjudicated; PaneHeader/EmptyState/ErrorBoundary/PreviewStrip/PreviewRamp unchallenged. Verbatim roster `workflows/args/core.json`. NOTE: a first relaunch this session used a WRONG hand-typed roster (nonexistent paths) — caught by roster diff, TaskStopped inside a minute, relaunched byte-identical |
| area scenes (7 comp) | `wf_dee4c83a-ec2` | 2/7 comp true | **RE-RESUMED 2026-07-28 (ACTIVE)** — promoted from the queue the moment r3's completion freed a slot (harvest → validate → promote, per L-15.8). Earlier same-day stop was the cap-4 restoration; banked seats replay. Verbatim roster `workflows/args/scenes.json` |
| area picker (4 comp) | `wf_3c8798e8-23e` | 1/4 comp true | **QUEUED (cap-4).** Same stop, same reason. Re-launch with `resumeFromRunId: "wf_3c8798e8-23e"` + `workflows/args/picker.json` VERBATIM (roster recovered byte-identical from the persisted run script — the original launch inlined it; both construct child args identically so cached seats replay) |
...
| area workbenches (19) | `wf_6edda4a1-192` | 10/19 comp true | **ACTIVE (pre-compaction 3rd deploy STILL RUNNING — liveness proven by agent-transcript mtimes, its "completed" record is false).** 9 components missing all of D/L/C (extract-controls, mix-pane, gradient-codeeditor, gradient-easingauthoringstage, extract-imagedropzone, generate-pane, extract-pane, mix-animationcanvas, gradient-pane). On its completion notification: harvest → validate → if gaps remain, resume `resumeFromRunId: "wf_6edda4a1-192"` + `workflows/args/demo-workbenches.json` VERBATIM |
| area palettes (32) | `wf_22b7a7b7-97b` | 6/32 comp true | **ACTIVE (same — running, record false).** Journal showed 492 started / 38 returned at last count and is growing again; 26 components still unbanked. Same recipe on completion: harvest → validate → resume `resumeFromRunId: "wf_22b7a7b7-97b"` + `workflows/args/palettes.json` VERBATIM if gaps remain |
...
**The authority on completeness is `node workflows/validate-completeness.mjs`** — run it after
EVERY workflow completion, failure, or harvest, and at every session open. It compares roster ×
{D,L,C} against disk reports ∪ harvest returns ∪ adjudicated apotheoses and writes
`registry/COMPLETENESS-LEDGER.md`; exit 1 = incomplete work exists that no ACTIVE or QUEUED
resume owns (a standing violation — fix immediately). Its ACTIVE/QUEUED sets mirror this table
