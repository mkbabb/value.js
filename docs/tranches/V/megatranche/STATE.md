...
| area core (7 comp) | `wf_66b1fcba-daa` | 19/21 canonical on disk | **RE-RESUMED 2026-07-28 late (ACTIVE)** after the M-16 hydration pass (hydrate-reports.mjs: 131 originals verified + 1 payload materialized + hashes in `registry/HYDRATION-LEDGER.md`); the validator now counts DISK ONLY. Remaining no-payload seats: EmptyState C, PreviewRamp D/L/C. Verbatim roster `workflows/args/core.json` |
| area scenes (7 comp) | `wf_dee4c83a-ec2` | 12/21 canonical on disk | **RE-RESUMED 2026-07-28 late (ACTIVE)** — same hydration discipline; remaining no-payload rows: BlobPane/AboutPane/Katex (D/L/C each), ConfigSliderPane C. Verbatim roster `workflows/args/scenes.json` |
| area picker (4 comp) | `wf_3c8798e8-23e` | 5/12 canonical on disk | **QUEUED (cap-4, first promotion)** — resume with `resumeFromRunId: "wf_3c8798e8-23e"` + `workflows/args/picker.json` (recovered byte-identical from the persisted run script) the moment a slot frees |
