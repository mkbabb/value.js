...
The text-only helper transposition is **done, and measured done at 2026-09-18**: `serializeStylesheetItem`, `reverseAnimationShorthand` and `reverseCSSTime` are each defined exactly once, in `../keyframes.js/src/animation/compile/emit/css-text.ts`, their Keyframes emit/editor callers already resolve there, and value keeps no implementation, export, test or forwarding export for any of them (⟨`grep -rn 'serializeStylesheetItem\|reverseAnimationShorthand\|reverseCSSTime\|formatCSS' src/ \| wc -l`⟩ → **0**, double-run). The count is **three, not four**: `formatCSS` resolves as a bare name in neither tree, and Keyframes' nearest surviving symbol is `formatCSSKeyframeString` in `../keyframes.js/src/animation/compile/emit/format/format.ts`. **No value-side source path is cited for any of them, because none exists** — the parsing directory this paragraph once named was deleted at the 4.0 cut, the tree holds zero `.bbnf` files, and the CSS grammar value ships is `src/css/grammar.ts` (with the 4.1 split products `src/css/rules.ts` and `src/css/serialize.ts` beside it). `serializeStylesheet`, `serializeDeclaration`, `serializeKeyframeSelector`, `stylesheetToString`, `reverseCSSIterationCount`, `parseCSSPercent`, `evaluateMathFunction`, parser objects/constants/errors, registration APIs, parse-that utilities and their public option/sink types are either private grammar detail needed by the retained block or deleted when consumer-less. In particular `CSS_WIDE_KEYWORDS`, `CSSString`, `CSSFunction`, `CSSJSON`, `CSSValues`, `CSSValueUnit`, the nominal `CSSColor`, `CSSParseError`, `registerColorNames`, `clearCustomColorNames`, `getCustomColorNames`, `istring`, `identifier`, `none`, `integer`, `number`, `succeed`, `fail`, `tryParse`, `parseResult`, `ParseCSSValuesOptions`, `JumpTerm`, `StepsArgs`, `SyntaxComponentName`, `ParsedColorUnit`, `NamedTimelineRegistry`, `ParseDiagnostic` and `OnParseError` are not 4.0 exports; `ParseIssue`/`ParseResult`, `JumpPosition`, `CssColor`, the closed AST types above and the final color/value types replace their public roles without aliases. **The parse-that position, recorded against ground truth 2026-09-18 — value.js ships none of it, and adopts none of it here.** The manifest is the whole of it: since the 4.1.0 cut's AM-13 strip, runtime `dependencies` is **absent — the library ships zero runtime dependencies** — and the two peers sit in `devDependencies`, where their rows read exactly `"@mkbabb/glass-ui": "^7.0.0"` and `"@mkbabb/keyframes.js": "^6.0.0"` (the demo and its glass-ui import chain need both at build time; the cut deleted the keyframes row where it had to demote it, and X-W9's Repair 1 restored it there); `@mkbabb/parse-that` appears in neither map; the only two mentions of the name under `src/` are the `math` and `transform` subpath docstrings asserting that very absence. The original research root `../parse-that` stands at `PAUSED_RESEARCH / TERMINAL_SOURCE_RED / NO_ACTIVE_WRITER` — its own words, at the `Status:` line of the tracked authority `docs/tranches/V/megatranche/coordination/PARSER-CSS-PAUSE-HANDOFF-2026-08-02.md`, and its last commit is `ef10d5b`, 2026-07-05 — while every byte of new parser work happens in the fresh, non-overlapping writer root `../parse-that-css-totality-p2` that the fresh-root law of `docs/tranches/V/megatranche/CONSTELLATION-COMMISSION-2026-08-03.md` §2's parse-that block requires (live: last commit 2026-09-18; frozen and preserved roots are never resumed). The `≥10×` admission floor is **retired as law, not merely unmet**, and the arithmetic is why retiring it costs nothing: against the conservative denominator `1,636,680 µs` the 10× budget is `163,668 µs` while the fixed native floor alone is `311,883 µs` — **1.906× over budget** — and the `1,870,633 µs` figure that once carried that law stays uncitable while `P4-EVIDENCE-REPLAY.json` is unreadable (measured absent). Its replacement portfolio — strict-3× (`545,560 µs`), strict-2× (`818,340 µs`), measured break-even (`1,636,680 µs`) — is the frame every budget now restates against, **and not one of its three bars is ratified**: each reads `OWNER-GATED-PENDING-RATIFICATION` in `docs/tranches/X/parse-that/evidence/W1/BAR-LEDGER-2026-09-17.md`, and `docs/tranches/X/COHESION.md` §0j.E OC-1 rules the bench table RECORDED-NOT-GATING with admission decided on correctness. So the portfolio governs the *denominator*, never an admission verdict, and no performance sentence in this file may be read as a bar. Nothing is adopted: `COHESION.md` §0i.1 closes the L1/L5 parser-adoption leg `BLOCKED-ON` the parser proof gate reading GREEN, with X.P.W4's RC-P evaluator returning TRUE at a dated run as the re-trigger, at which point disposition **A** — a successor wave authorized to consume `docs/tranches/X/parse-that/SEAM-CONTRACT.md` at `src/css/**` — is what fires. Until it fires, the sentences above describe the entire parser surface value.js has.
