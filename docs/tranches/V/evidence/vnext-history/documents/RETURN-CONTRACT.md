...
scalar alias. The current canonical authority binds semantic bounds SHA-256
`69d6364137b1cee1196f96048312af9e538d80a84c0bc5217eeebfbb02de5636`,
authority-file SHA-256
`b22f5c351bd1991132508f435270a327f6dc7778e2bd05640692b901c3476b5b`,
and manifest hash
`6af1545ea116a5971a223c765b1fc3057c400142e459552c40b90c8aadf9c3f8`.
That authority authenticates the declared shape, path containment and bounded
package/target/kind scope; it does not claim that every required root or
required subpath is physically present.

Consumer-receipt validation has exactly two internal modes. `live` verifies
the current schemas, resolver, 190-wave registry, authority, physical evidence,
Git identity, root/subpath presence, occurrence observations and bounded
freshness. `immutable` does not rerun live discovery or accept a live clock or
additional-worktree allowance. It requires the exact authenticated
`vnext-consumer-universe-immutable-binding/1` projection containing receipt and
input paths/hashes, semantic universe hash, bounds authority and semantic hash,
input/receipt schema hashes, resolver and registry hashes, observed/resolved
times, the complete epoch and counts, root/edge hashes, resolvability and exact
blockers. A partial projection or legacy `requireFresh` freshness bypass is
invalid. At the return-validator surface, default validation uses `live`; each
offline/historical mode uses the immutable consumer projection while retaining
the distinct global return semantics described above.

Default live validation and plain `--offline` require the exact canonical
`CONSUMER-UNIVERSE-BOUNDS.json` logical path and current authority bytes; plain
...
canonical law. Historical verification reads the materialized file but
projects the original logical binding; a zero-pin or duplicate-pin match,
untracked file, symlink, mutable-checkout-only file or byte/hash drift is RED.

The deterministic source projector recognizes quoted static and dynamic
imports, zero-substitution-template dynamic imports, statement-level and
pure-inline TypeScript type imports/exports, mixed type/value clauses and its
bounded CSS import forms. A computed, escaped, ambiguous or otherwise
unsupported import candidate fails closed. Exit zero and all of the following
are required for a dependency green:
...
receipt epoch hash binds the consumer floor rather than relying on prose. Live
resolution includes a required root when present or marks it unavailable when
physically absent; a required root is never excluded. A required subpath under
an unavailable root is inherited blocked. Under an available root it must
exist, resolve to the declared canonical path and remain strictly contained by
that Git root.

For edges, an unavailable source requires an unavailable disposition and zero
observations. An available source aimed at an unavailable internal target
requires an unavailable disposition and the exact source observations. No
other edge may be unavailable, and an included edge requires an included
source and included internal target. Every unavailable row carries evidence
and an existing-wave retrigger.

C00U **COMPLETE** is legal only when the live resolver exits 0 and the annex is
`resolvable:true` with an empty blocker vector. C00U **BLOCKED** is legal only
when the live resolver exits 2 and the annex is `resolvable:false` with a
nonempty exact blocker vector; every typed unavailable blocker must biject to
the return's exact blocking `routed_remainder`. A blocked C00U never advances
to C05. C05 reruns the resolver against a fresh post-migration census and may
consume only exit 0, `resolvable:true` and no blockers; copying or editing
C00U's earlier receipt is not evidence.
...
authority. In either path, deletion `consumer_scan` carries the complete exact
immutable receipt projection—receipt/input/bounds/schema/resolver/registry,
epoch/counts/root-edge hashes, resolvability and blockers—and is revalidated in
`live` or `immutable` mode as appropriate. A partial three-field binding, stale
copy or freshness bypass cannot authorize deletion judgment.
