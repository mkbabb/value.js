...
for (let index = 2; index < process.argv.length; index += 1) {
    if (process.argv[index] === "--receipt" && process.argv[index + 1]) receiptPath = resolve(process.argv[++index]);
        ...
        process.stderr.write("usage: node validate-keyframes-public-package-proof.mjs (--receipt <path> --snapshot <path> | --return <K23-return>) [--consumer-immutable-capture-authority <path> <file-sha256> <authority-hash>]\n");
        process.exit(2);
    }
}
...
try {
    ...
    if (returnPath) {
        ...
        }, "K23", undefined, "historical-certificate", consumerCaptureAuthorityArgs ? { consumerCaptureAuthorityArgs } : undefined);
        ...
        sourceAuthority = validateHistoricalKeyframesReturn(
            dependencies[0],
            "K22T",
            "K23",
