...
function k23Return({
    ...
    const record = {
        ...
        evidence_inputs: [
            ...
            ...consumerCaptureEvidenceInputs(deletion.consumer, "K23 owner-precut consumer"),
...
try {
    ...
    const sourceConsumerCapture = sourceDescriptor.consumer_capture;
    const sourceCapturePath = realpathSync(sourceConsumerCapture.immutable_capture.path);
    const captureFromRoot = relative(sourceFixtureRoot, sourceCapturePath);
    if (!captureFromRoot || captureFromRoot.startsWith("..") || isAbsolute(captureFromRoot)) {
        throw new Error("K22T emitter immutable consumer capture escapes its dedicated root");
    }
    consumerCaptureAuthority.registerCapture(
        sourceConsumerCapture.receipt_path,
        sourceConsumerCapture.immutable_capture,
    );
