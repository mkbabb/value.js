...
const facilityText = readFileSync(facilityPath);
...
const facility = parseJsonStrict(facilityText);
...
exactKeys(manifest.authority, ["api_contract", "api_contract_sha256", "facility_isomorphism", "facility_isomorphism_sha256", "facility_manifest_sha256", "api_waves", "api_waves_sha256", "closure_waves", "closure_waves_sha256"], "authority");
const expectedAuthority = {
    api_contract: "api-contract.source.json",
    api_contract_sha256: sha256(sourceText),
    facility_isomorphism: "API-FACILITY-ISOMORPHISM.json",
    facility_isomorphism_sha256: sha256(facilityText),
...
const facilityOperationIds = facility.facilities.flatMap(({ operation_ids }) => [...operation_ids.value, ...operation_ids.fourier]).sort();
if (canonicalize(facilityOperationIds) !== canonicalize([...crossTransportIds].sort())) fail("facility/source operation universes differ");
