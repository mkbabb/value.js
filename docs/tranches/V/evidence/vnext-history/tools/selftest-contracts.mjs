...
import { consumerRootSnapshotIndexReference } from "./consumer-root-snapshot.mjs";
...
function expectConsumerInvariantRejected(name, operation, fragment) {
    try {
        operation();
        failures.push(`${name} was not rejected by ${fragment}`);
    } catch (error) {
        if (!error.message.includes(fragment)) failures.push(`${name} failed nonspecifically: ${error.message}`);
        else adversarialRejections += 1;
    }
    consumerReturnRejections += 1;
}
...
try {
    ...
    function deletionConsumerScan(consumerAnnex, authority) {
        ...
        return {
            authority,
            ...projection,
            snapshot_index: structuredClone(consumerAnnex.snapshot_index),
        };
    }
    ...
    const bindConsumerCapture = (record, fixture, purpose = "consumer-universe immutable capture certificate") => {
        record.evidence_inputs.push({
            path: fixture.immutableCapture.path,
            sha256: fixture.immutableCapture.file_sha256,
            purpose,
            class: "generated",
        });
        record.evidence_inputs.push({
            path: fixture.snapshotIndex.path,
            sha256: fixture.snapshotIndex.file_sha256,
            purpose: `${purpose.replace("immutable capture certificate", "root snapshot index")}`,
            class: "generated",
        });
        return record;
    };
    const forkConsumerSnapshotIndex = (fixture, label, mutate) => {
        const index = structuredClone(fixture.snapshotIndexDocument);
        mutate(index);
        const preimage = structuredClone(index);
        delete preimage.index_hash;
        index.index_hash = sha256(canonicalize(preimage));
        const path = join(consumerReturnFixtureRoot, `${label}-consumer-root-snapshot-index.json`);
        writeCanonical(path, index);
        return consumerRootSnapshotIndexReference(realpathSync(path));
    };
    const rebindConsumerSnapshotIndex = (record, original, replacement) => {
        record.annexes["consumer-universe"].snapshot_index = structuredClone(replacement);
        const evidenceInput = record.evidence_inputs.find(({ path, sha256: hash }) => (
            path === original.path && hash === original.file_sha256
        ));
        if (!evidenceInput) throw new Error("consumer root snapshot index evidence binding is unavailable");
        evidenceInput.path = replacement.path;
        evidenceInput.sha256 = replacement.file_sha256;
        return record;
    };
    const offlineConsumerFlags = () => ["--offline", ...consumerReturnMode.captureAuthorityArgs()];

    const c00uGreenFixture = consumerReturnMode.resolve({
        name: "c00u-complete-universe",
        waveId: "C00U",
    });
    const immutableGreenOptions = {
        validationMode: "immutable",
        immutableCapture: c00uGreenFixture.immutableCapture,
        boundsAuthority: c00uGreenFixture.annex.bounds_authority,
        requireResolvable: true,
    };
    const immutableGreen = validateConsumerUniverseReceipt(c00uGreenFixture.receiptPath, immutableGreenOptions);
    if (immutableGreen.receipt.receipt_hash !== c00uGreenFixture.receipt.receipt_hash
        || immutableGreen.receipt_file_sha256 !== c00uGreenFixture.annex.receipt_file_sha256
        || canonicalize(immutableGreen.immutable_capture) !== canonicalize(c00uGreenFixture.immutableCapture)) {
        failures.push("external immutable capture did not reproduce the exact dependency-green receipt");
    } else consumerReturnPositiveControls += 1;

    expectConsumerInvariantRejected(
        "missing external immutable capture",
        () => validateConsumerUniverseReceipt(c00uGreenFixture.receiptPath, {
            validationMode: "immutable",
            boundsAuthority: c00uGreenFixture.annex.bounds_authority,
        }),
        "requires an independently content-addressed external capture",
    );
    expectConsumerInvariantRejected(
        "self-derived immutable binding",
        () => validateConsumerUniverseReceipt(c00uGreenFixture.receiptPath, {
            validationMode: "immutable",
            immutableBinding: consumerUniverseImmutableBindingProjection(c00uGreenFixture.annex),
            boundsAuthority: c00uGreenFixture.annex.bounds_authority,
        }),
        "rejects a caller-supplied self-derived binding",
    );
    expectConsumerInvariantRejected(
        "partial external immutable capture reference",
        () => validateConsumerUniverseReceipt(c00uGreenFixture.receiptPath, {
            ...immutableGreenOptions,
            immutableCapture: { path: c00uGreenFixture.immutableCapture.path },
        }),
        "fields must be exact",
    );
    expectConsumerInvariantRejected(
        "alternate immutable receipt path",
        () => validateConsumerUniverseReceipt(`${c00uGreenFixture.receiptPath}-alternate`, immutableGreenOptions),
        "receipt path differs from the requested receipt",
    );
    expectConsumerInvariantRejected(
        "alternate immutable authority",
        () => validateConsumerUniverseReceipt(c00uGreenFixture.receiptPath, {
            ...immutableGreenOptions,
            boundsAuthority: {
                ...c00uGreenFixture.annex.bounds_authority,
                path: `${c00uGreenFixture.annex.bounds_authority.path}-alternate`,
            },
        }),
        "differs from the external immutable capture",
    );

    const incompleteCapturePath = join(consumerReturnFixtureRoot, "incomplete-immutable-capture.json");
    const incompleteMaterials = structuredClone(c00uGreenFixture.capture.materials);
    delete incompleteMaterials.resolver;
    const incompleteCapture = consumerUniverseImmutableCaptureDocument(
        c00uGreenFixture.capture.binding,
        incompleteMaterials,
    );
    writeCanonical(incompleteCapturePath, incompleteCapture);
    const incompleteCaptureReference = consumerUniverseImmutableCaptureReference(realpathSync(incompleteCapturePath));
    expectConsumerInvariantRejected(
        "missing immutable capture material",
        () => validateConsumerUniverseReceipt(c00uGreenFixture.receiptPath, {
            ...immutableGreenOptions,
            immutableCapture: incompleteCaptureReference,
        }),
        "materials fields must be exact",
    );

    const capturedResolverPath = c00uGreenFixture.materials.resolver.path;
    const capturedResolverBytes = readFileSync(capturedResolverPath);
    try {
        writeFileSync(capturedResolverPath, Buffer.concat([capturedResolverBytes, Buffer.from("\n// corrupted capture material\n")]));
        expectConsumerInvariantRejected(
            "corrupt immutable resolver material",
            () => validateConsumerUniverseReceipt(c00uGreenFixture.receiptPath, immutableGreenOptions),
            "resolver material hash drift",
        );
    } finally {
        writeFileSync(capturedResolverPath, capturedResolverBytes);
    }

    const absentReceiptPath = `${c00uGreenFixture.receiptPath}-absent`;
    const absentInputPath = `${c00uGreenFixture.inputPath}-absent`;
    renameSync(c00uGreenFixture.receiptPath, absentReceiptPath);
    renameSync(c00uGreenFixture.inputPath, absentInputPath);
    try {
        validateConsumerUniverseReceipt(c00uGreenFixture.receiptPath, immutableGreenOptions);
        consumerReturnPositiveControls += 1;
    } catch (error) {
        failures.push(`immutable receipt dereferenced mutable logical receipt or input JSON: ${error.message}`);
    } finally {
        renameSync(absentInputPath, c00uGreenFixture.inputPath);
        renameSync(absentReceiptPath, c00uGreenFixture.receiptPath);
    }

    const capturedConstellation = c00uGreenFixture.constellationRoot;
    const absentConstellation = `${capturedConstellation}-absent`;
    const redirectedConstellation = `${capturedConstellation}-redirect-target`;
    renameSync(capturedConstellation, absentConstellation);
    try {
        validateConsumerUniverseReceipt(c00uGreenFixture.receiptPath, immutableGreenOptions);
        consumerReturnPositiveControls += 1;
        mkdirSync(redirectedConstellation, { recursive: true });
        symlinkSync(redirectedConstellation, capturedConstellation, "dir");
        validateConsumerUniverseReceipt(c00uGreenFixture.receiptPath, immutableGreenOptions);
        consumerReturnPositiveControls += 1;
        rmSync(capturedConstellation);
        rmSync(redirectedConstellation, { recursive: true });
    } catch (error) {
        failures.push(`immutable receipt consulted absent or redirected captured search-root topology: ${error.message}`);
        if (existsSync(capturedConstellation)) rmSync(capturedConstellation, { recursive: true, force: true });
        rmSync(redirectedConstellation, { recursive: true, force: true });
    } finally {
        renameSync(absentConstellation, capturedConstellation);
    }

    const immutableProjection = consumerUniverseImmutableBindingProjection(c00uGreenFixture.annex);
    try {
        assertSameConsumerUniverseImmutableProjection(immutableProjection, structuredClone(immutableProjection));
        consumerReturnPositiveControls += 1;
    } catch (error) {
        failures.push(`identical complete immutable projections were rejected: ${error.message}`);
    }
    const changedHash = (value) => `${value[0] === "0" ? "1" : "0"}${value.slice(1)}`;
    const immutableProjectionMutations = [
        ["receipt_path", (value) => { value.receipt_path += "-alternate"; }],
        ["receipt_file_sha256", (value) => { value.receipt_file_sha256 = changedHash(value.receipt_file_sha256); }],
        ["receipt_hash", (value) => { value.receipt_hash = changedHash(value.receipt_hash); }],
        ["input_path", (value) => { value.input_path += "-alternate"; }],
        ["input_file_sha256", (value) => { value.input_file_sha256 = changedHash(value.input_file_sha256); }],
        ["universe_hash", (value) => { value.universe_hash = changedHash(value.universe_hash); }],
        ["resolver_sha256", (value) => { value.resolver_sha256 = changedHash(value.resolver_sha256); }],
        ["input_schema_sha256", (value) => { value.input_schema_sha256 = changedHash(value.input_schema_sha256); }],
        ["receipt_schema_sha256", (value) => { value.receipt_schema_sha256 = changedHash(value.receipt_schema_sha256); }],
        ["bounds_authority.path", (value) => { value.bounds_authority.path += "-alternate"; }],
        ["bounds_authority.file_sha256", (value) => { value.bounds_authority.file_sha256 = changedHash(value.bounds_authority.file_sha256); }],
        ["bounds_authority.manifest_hash", (value) => { value.bounds_authority.manifest_hash = changedHash(value.bounds_authority.manifest_hash); }],
        ["bounds_sha256", (value) => { value.bounds_sha256 = changedHash(value.bounds_sha256); }],
        ["formation_wave_registry_sha256", (value) => { value.formation_wave_registry_sha256 = changedHash(value.formation_wave_registry_sha256); }],
        ["observed_at", (value) => { value.observed_at = new Date(Date.parse(value.observed_at) + 1000).toISOString(); }],
        ["resolved_at", (value) => { value.resolved_at = new Date(Date.parse(value.resolved_at) + 1000).toISOString(); }],
        ["epoch", (value) => { value.epoch.epoch_sha256 = changedHash(value.epoch.epoch_sha256); }],
        ["counts", (value) => { value.counts.roots += 1; }],
        ["roots_sha256", (value) => { value.roots_sha256 = changedHash(value.roots_sha256); }],
        ["edges_sha256", (value) => { value.edges_sha256 = changedHash(value.edges_sha256); }],
        ["resolvable", (value) => { value.resolvable = !value.resolvable; }],
        ["blockers", (value) => { value.blockers.push("one-sided immutable projection mutation"); }],
    ];
    for (const [field, mutate] of immutableProjectionMutations) {
        const changed = structuredClone(immutableProjection);
        mutate(changed);
        expectConsumerInvariantRejected(
            `one-field immutable projection mutation ${field}`,
            () => assertSameConsumerUniverseImmutableProjection(immutableProjection, changed),
            "differs",
        );
    }

    const capturedAuthoritySource = parseJsonStrict(readFileSync(c00uGreenFixture.materials.bounds_authority.path));
    const expectLexicalAuthorityRejected = (name, mutate, fragment) => {
        const authority = structuredClone(capturedAuthoritySource);
        authority.schema = "vnext-consumer-universe-bounds-fixture/1";
        mutate(authority.bounds);
        authority.bounds_sha256 = sha256(canonicalize(authority.bounds));
        const manifestPreimage = structuredClone(authority);
        delete manifestPreimage.manifest_hash;
        authority.manifest_hash = sha256(canonicalize(manifestPreimage));
        const authorityPath = join(consumerReturnFixtureRoot, `${name}.bounds.json`);
        writeCanonical(authorityPath, authority);
        expectConsumerInvariantRejected(name, () => validateConsumerBoundsAuthority({
            path: c00uGreenFixture.annex.bounds_authority.path,
            file_sha256: hashFile(authorityPath),
            manifest_hash: authority.manifest_hash,
        }, {
            verificationPath: realpathSync(authorityPath),
            requireCanonicalPath: false,
            allowFixtureProfile: true,
        }), fragment);
    };
    expectLexicalAuthorityRejected("nonnormalized-search-root", (bounds) => {
        bounds.search_roots[0].path = `${bounds.search_roots[0].path}/../escape`;
    }, "search root is malformed");
    expectLexicalAuthorityRejected("duplicate-search-root", (bounds) => {
        bounds.search_roots.push(structuredClone(bounds.search_roots[0]));
    }, "duplicate consumer-universe logical search root");
    expectLexicalAuthorityRejected("required-path-coordinate-mismatch", (bounds) => {
        bounds.required_paths[0].path += "-mismatch";
    }, "not the exact typed repository subdirectory");
    expectLexicalAuthorityRejected("required-path-lexical-escape", (bounds) => {
        bounds.required_paths[0].relative_path = "../escape";
    }, "unsafe relative_path");
    expectLexicalAuthorityRejected("duplicate-required-root-identity", (bounds) => {
        bounds.required_roots[1].canonical_realpath = bounds.required_roots[0].canonical_realpath;
    }, "duplicate consumer-universe required root path");
    ...
    bindConsumerCapture(c00uComplete, c00uGreenFixture);
    ...
    const ownerDeletionAnnex = finalizeDeletionAnnex({
        ...
        root_snapshots: structuredClone(c00uGreenFixture.rootSnapshots),
    ...
    bindConsumerCapture(k21Record, c00uGreenFixture, "owner deletion consumer-universe immutable capture certificate");
    ...
    const missingSnapshotIndex = structuredClone(c00uCompleteStructural);
    delete missingSnapshotIndex.annexes["consumer-universe"].snapshot_index;
    expectConsumerReturnRejected(
        "consumer return missing snapshot index",
        run("structural-c00u-missing-snapshot-index", missingSnapshotIndex, offlineConsumerFlags(), {}, consumerReturnMode.validatorPath),
        "snapshot_index",
    );
    const partialSnapshotIndex = structuredClone(c00uCompleteStructural);
    partialSnapshotIndex.annexes["consumer-universe"].snapshot_index = { path: c00uGreenFixture.snapshotIndex.path };
    expectConsumerReturnRejected(
        "consumer return partial snapshot index reference",
        run("structural-c00u-partial-snapshot-index", partialSnapshotIndex, offlineConsumerFlags(), {}, consumerReturnMode.validatorPath),
        "snapshot_index",
    );
    const missingSnapshotIndexEvidence = structuredClone(c00uCompleteStructural);
    missingSnapshotIndexEvidence.evidence_inputs = missingSnapshotIndexEvidence.evidence_inputs.filter(({ path, sha256: hash }) => (
        path !== c00uGreenFixture.snapshotIndex.path || hash !== c00uGreenFixture.snapshotIndex.file_sha256
    ));
    expectConsumerReturnRejected(
        "consumer return snapshot index missing evidence binding",
        run("structural-c00u-snapshot-index-missing-evidence", missingSnapshotIndexEvidence, offlineConsumerFlags(), {}, consumerReturnMode.validatorPath),
        "root snapshot index must be content-addressed exactly once",
    );
    const missingRootIndex = forkConsumerSnapshotIndex(c00uGreenFixture, "missing-root", (index) => {
        index.roots.pop();
    });
    const missingRootIndexReturn = rebindConsumerSnapshotIndex(
        structuredClone(c00uCompleteStructural),
        c00uGreenFixture.snapshotIndex,
        missingRootIndex,
    );
    expectConsumerReturnRejected(
        "consumer return snapshot index missing root mapping",
        run("structural-c00u-snapshot-index-missing-root", missingRootIndexReturn, offlineConsumerFlags(), {}, consumerReturnMode.validatorPath),
        "must biject exactly to included consumer roots",
    );
    const rewrittenRootIndex = forkConsumerSnapshotIndex(c00uGreenFixture, "rewritten-root", (index) => {
        index.roots[0].snapshot_hash = changedHash(index.roots[0].snapshot_hash);
    });
    const rewrittenRootIndexReturn = rebindConsumerSnapshotIndex(
        structuredClone(c00uCompleteStructural),
        c00uGreenFixture.snapshotIndex,
        rewrittenRootIndex,
    );
    expectConsumerReturnRejected(
        "coherent snapshot index rewrite against external receipt",
        run("structural-c00u-snapshot-index-rewritten-root", rewrittenRootIndexReturn, offlineConsumerFlags(), {}, consumerReturnMode.validatorPath),
        "differs from the receipt content snapshot",
    );
    const corruptHistoricalSnapshotSchemaPath = join(consumerReturnFixtureRoot, "corrupt-historical-consumer-root-snapshot.schema.json");
    writeFileSync(corruptHistoricalSnapshotSchemaPath, Buffer.concat([
        readFileSync(c00uGreenFixture.snapshotIndexDocument.snapshot_schema_path),
        Buffer.from(" "),
    ]));
    const corruptSchemaIndex = forkConsumerSnapshotIndex(c00uGreenFixture, "corrupt-schema", (index) => {
        index.snapshot_schema_path = realpathSync(corruptHistoricalSnapshotSchemaPath);
    });
    const corruptSchemaIndexReturn = rebindConsumerSnapshotIndex(
        structuredClone(c00uCompleteStructural),
        c00uGreenFixture.snapshotIndex,
        corruptSchemaIndex,
    );
    expectConsumerReturnRejected(
        "consumer return corrupt historical snapshot schema",
        run("structural-c00u-corrupt-historical-snapshot-schema", corruptSchemaIndexReturn, offlineConsumerFlags(), {}, consumerReturnMode.validatorPath),
        "schema hash drift",
    );
    expectConsumerReturnRejected(
        "offline consumer return without caller capture authority",
        run(
            "structural-c00u-missing-caller-capture-authority",
            structuredClone(c00uCompleteStructural),
            ["--offline"],
            {},
            consumerReturnMode.validatorPath,
        ),
        "requires an external caller-authenticated capture authority",
    );

    const copiedValidatorTranche = resolve(consumerReturnMode.validatorPath, "../..");
    const advancedCurrentPaths = [
        resolve(copiedValidatorTranche, "tools/resolve-consumer-universe.mjs"),
        resolve(copiedValidatorTranche, "consumer-universe.schema.json"),
        resolve(copiedValidatorTranche, "consumer-universe-receipt.schema.json"),
        resolve(copiedValidatorTranche, "CONSUMER-UNIVERSE-BOUNDS.json"),
    ];
    const priorCurrentBytes = advancedCurrentPaths.map((path) => readFileSync(path));
    try {
        writeFileSync(advancedCurrentPaths[0], Buffer.concat([priorCurrentBytes[0], Buffer.from("\n// resolver advanced after immutable capture\n")]));
        writeFileSync(advancedCurrentPaths[1], '{"not":{}}\n');
        writeFileSync(advancedCurrentPaths[2], '{"not":{}}\n');
        writeFileSync(advancedCurrentPaths[3], '{"not":{}}\n');
        const advancedCurrentValidation = run(
            "structural-c00u-advanced-current-tools",
            structuredClone(c00uCompleteStructural),
            offlineConsumerFlags(),
            {},
            consumerReturnMode.validatorPath,
        ...
        expectStructuralConsumerReturnAccepted(
            "unchanged C00U capture after current resolver, schemas, and authority advance",
            advancedCurrentValidation,
            c00uCompleteStructural,
            true,
        );
    } finally {
        for (const [index, path] of advancedCurrentPaths.entries()) writeFileSync(path, priorCurrentBytes[index]);
    }

    const coherentRebindRoot = join(consumerReturnFixtureRoot, "coherent-resolver-rebind");
    mkdirSync(coherentRebindRoot, { recursive: true });
    const reboundResolverPath = join(coherentRebindRoot, "resolve-consumer-universe.mjs");
    writeFileSync(reboundResolverPath, Buffer.concat([
        readFileSync(c00uGreenFixture.materials.resolver.path),
        Buffer.from("\n// coherently rebound resolver identity\n"),
    ]));
    const reboundResolverSha256 = hashFile(reboundResolverPath);
    const reboundReceipt = parseJsonStrict(readFileSync(c00uGreenFixture.materials.receipt.path));
    reboundReceipt.resolver_sha256 = reboundResolverSha256;
    const reboundReceiptPreimage = structuredClone(reboundReceipt);
    delete reboundReceiptPreimage.receipt_hash;
    reboundReceipt.receipt_hash = sha256(canonicalize(reboundReceiptPreimage));
    const reboundReceiptPath = join(coherentRebindRoot, "receipt.json");
    writeCanonical(reboundReceiptPath, reboundReceipt);
    const reboundBinding = structuredClone(c00uGreenFixture.capture.binding);
    reboundBinding.receipt_file_sha256 = hashFile(reboundReceiptPath);
    reboundBinding.receipt_hash = reboundReceipt.receipt_hash;
    reboundBinding.resolver_sha256 = reboundResolverSha256;
    const reboundMaterials = structuredClone(c00uGreenFixture.capture.materials);
    reboundMaterials.receipt = { path: realpathSync(reboundReceiptPath), file_sha256: hashFile(reboundReceiptPath) };
    reboundMaterials.resolver = { path: realpathSync(reboundResolverPath), file_sha256: reboundResolverSha256 };
    const reboundCapture = consumerUniverseImmutableCaptureDocument(reboundBinding, reboundMaterials);
    const reboundCapturePath = join(coherentRebindRoot, "capture.json");
    writeCanonical(reboundCapturePath, reboundCapture);
    const reboundCaptureReference = consumerUniverseImmutableCaptureReference(realpathSync(reboundCapturePath));
    const coherentlyReboundReturn = structuredClone(c00uCompleteStructural);
    Object.assign(coherentlyReboundReturn.annexes["consumer-universe"], {
        receipt_file_sha256: reboundBinding.receipt_file_sha256,
        receipt_hash: reboundBinding.receipt_hash,
        resolver_sha256: reboundBinding.resolver_sha256,
        immutable_capture: reboundCaptureReference,
    });
    coherentlyReboundReturn.evidence_inputs.push({
        path: reboundCaptureReference.path,
        sha256: reboundCaptureReference.file_sha256,
        purpose: "coherently rebound consumer-universe immutable capture certificate",
        class: "generated",
    });
    expectConsumerReturnRejected(
        "coherent resolver/receipt/capture/annex rebind against unchanged caller authority",
        run(
            "coherent-consumer-immutable-rebind",
            coherentlyReboundReturn,
            offlineConsumerFlags(),
            {},
            consumerReturnMode.validatorPath,
        ),
        "candidate capture reference differs from the external caller authority",
    );
    ...
    bindConsumerCapture(c05Green, c05GreenFixture);
    ...
    const c05DeletionAnnex = finalizeDeletionAnnex({
        ...
        root_snapshots: structuredClone(c05GreenFixture.rootSnapshots),
    ...
    bindConsumerCapture(c05Structural, c05GreenFixture);
    ...
    const alternateC05SnapshotIndexPath = join(consumerReturnFixtureRoot, "alternate-c05-consumer-root-snapshot-index.json");
    copyFileSync(c05GreenFixture.snapshotIndex.path, alternateC05SnapshotIndexPath);
    const alternateC05SnapshotIndex = consumerRootSnapshotIndexReference(realpathSync(alternateC05SnapshotIndexPath));
    const splitIndexC05DeletionAnnex = structuredClone(c05DeletionAnnex);
    splitIndexC05DeletionAnnex.consumer_scan.snapshot_index = alternateC05SnapshotIndex;
    finalizeDeletionAnnex(splitIndexC05DeletionAnnex);
    const splitIndexC05DeletionPath = join(directory, "structural-c05-split-snapshot-index-deletion-annex.json");
    writeCanonical(splitIndexC05DeletionPath, splitIndexC05DeletionAnnex);
    const splitIndexC05 = structuredClone(c05Structural);
    const splitIndexDeletionEvidence = splitIndexC05.evidence_inputs.find(({ path }) => path === c05DeletionAnnexPath);
    splitIndexDeletionEvidence.path = realpathSync(splitIndexC05DeletionPath);
    splitIndexDeletionEvidence.sha256 = hashFile(splitIndexC05DeletionPath);
    splitIndexC05.evidence_inputs.push({
        path: alternateC05SnapshotIndex.path,
        sha256: alternateC05SnapshotIndex.file_sha256,
        purpose: "alternate C05 deletion consumer root snapshot index",
        class: "generated",
    });
    splitIndexC05.annexes["deletion-judgment"] = deletionAnnexBinding(
        "C05",
        "c05-rehearsal",
        realpathSync(splitIndexC05DeletionPath),
        splitIndexC05DeletionAnnex,
    );
    refreshImplementationChallenge(splitIndexC05, "structural-c05-split-snapshot-index");
    expectConsumerReturnRejected(
        "C05 top-level and deletion snapshot index split",
        run("structural-c05-split-snapshot-index", splitIndexC05, offlineConsumerFlags(), {}, consumerReturnMode.validatorPath),
        "must use the exact top-level consumer snapshot index",
    );
    ...
    bindConsumerCapture(c00uBlocked, c00uBlockingFixture);
    ...
    bindConsumerCapture(c00uBlockedStructural, c00uBlockingFixture);
    ...
    bindConsumerCapture(c05ExitTwo, c00uBlockingFixture);
