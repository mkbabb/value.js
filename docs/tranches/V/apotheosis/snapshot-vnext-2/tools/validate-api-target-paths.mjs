...
const facilityPath = resolve(root, "API-FACILITY-ISOMORPHISM.json");
const manifest = parseJsonStrict(readFileSync(manifestPath));
const contract = parseJsonStrict(readFileSync(contractPath));
const facility = parseJsonStrict(readFileSync(facilityPath));
const failures = [];
const fail = (message) => failures.push(message);
const sha256 = (value) => createHash("sha256").update(value).digest("hex");

if (manifest.schema !== "vnext-api-target-paths/1") fail("invalid manifest schema");
if (canonicalize(Object.keys(manifest.authority ?? {}).sort(compareCanonicalText)) !== canonicalize(["facilityIsomorphism", "facilityManifestSha256", "operations", "operationsSha256"])) fail("authority fields must be the exact two content-addressed inputs");
...
if (manifest.authority?.facilityIsomorphism !== "API-FACILITY-ISOMORPHISM.json") fail("facility authority must be API-FACILITY-ISOMORPHISM.json");
if (manifest.authority?.facilityManifestSha256 !== facility.manifest_sha256) fail(`facility authority hash ${manifest.authority?.facilityManifestSha256}; expected ${facility.manifest_sha256}`);
if (!manifest.laws?.isomorphism?.includes("146 operations")) fail("target law does not bind the 146-operation facility isomorphism");
...
if (operations.length !== 146 || contract.http.length !== 129 || contract.headless.length !== 17) fail(`operation authority count is ${contract.http.length} HTTP + ${contract.headless.length} headless, expected 129 + 17`);
const facilityOperationIds = facility.facilities.flatMap(({ operation_ids }) => [...operation_ids.value, ...operation_ids.fourier]).sort(compareCanonicalText);
const contractOperationIds = operations.map(({ id }) => id).sort(compareCanonicalText);
if (canonicalize(facilityOperationIds) !== canonicalize(contractOperationIds)) fail("facility/target operation universes differ");
...
process.stdout.write(`${JSON.stringify({
    ...
    facility_manifest_sha256: facility.manifest_sha256,
