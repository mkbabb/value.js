...
const supportedSchemaKeywords = new Set([
    ...
    "oneOf", "allOf", "if", "then", "else", "not", "required", "properties",
    "additionalProperties", "unevaluatedProperties", "propertyNames", "items", "minItems",
    "maxItems", "uniqueItems", "minLength", "maxLength", "pattern", "format", "minimum",
    "maximum", "minProperties", "maxProperties",
]);

export function assertSupportedJsonSchema(schema, pointer = "#") {
    const root = schema;
    const visitedReferences = new Set();
    const walk = (node, at) => {
        if (typeof node === "boolean") return;
        if (!node || typeof node !== "object" || Array.isArray(node)) throw new Error(`${at}: schema must be an object or boolean`);
        for (const key of Object.keys(node)) if (!supportedSchemaKeywords.has(key)) throw new Error(`${at}: unsupported schema keyword ${key}`);
        if (node.format !== undefined && node.format !== "date-time") throw new Error(`${at}: unsupported schema format ${node.format}`);
        for (const [key, child] of Object.entries(node.properties ?? {})) walk(child, `${at}/properties/${key}`);
        for (const key of ["additionalProperties", "unevaluatedProperties", "propertyNames", "items", "if", "then", "else", "not"])
            if (node[key] !== undefined && typeof node[key] !== "boolean") walk(node[key], `${at}/${key}`);
        for (const key of ["oneOf", "allOf"])
            for (const [index, child] of (node[key] ?? []).entries()) walk(child, `${at}/${key}/${index}`);
        if (node.$ref && !visitedReferences.has(node.$ref)) {
            visitedReferences.add(node.$ref);
            walk(resolveReference(root, node.$ref), `${at}/$ref(${node.$ref})`);
        }
    };
    walk(schema, pointer);
}
...
export function validateJsonSchema(value, schema, root = schema, pointer = "") {
    if (root === schema && pointer === "") assertSupportedJsonSchema(schema);
    const errors = [];
    const at = pointer || "/";
    ...
    if (typeof value === "string") {
        ...
        if (schema.maxLength !== undefined && value.length > schema.maxLength) add(`maximum length is ${schema.maxLength}`);
    ...
    if (Array.isArray(value)) {
        ...
        if (schema.maxItems !== undefined && value.length > schema.maxItems) add(`maximum item count is ${schema.maxItems}`);
        ...
        if (schema.minProperties !== undefined && propertyCount < schema.minProperties) add(`minimum property count is ${schema.minProperties}`);
        if (schema.maxProperties !== undefined && propertyCount > schema.maxProperties) add(`maximum property count is ${schema.maxProperties}`);
