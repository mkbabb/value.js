...
The palette API remains at value.js `/api`. Fourier-analysis implements the same applicable protocol laws independently in Python. [`API-OPERATIONS.md`](API-OPERATIONS.md) closes 129 HTTP operations (88 Value, 41 Fourier) plus 17 headless operations. [`API-FACILITY-ISOMORPHISM.json`](API-FACILITY-ISOMORPHISM.json) covers all 146 exactly once by intrinsic job: fork≅derivation and revision≅revision, while Value-only, Fourier-only and D-10/D-12 asymmetries are named rather than erased. Isomorphism never means shared runtime code, identical nouns/routes or invented resources. Both services are explicitly single-tenant: no header, query, path or body field selects a tenant or may alter namespace, authorization, cursor, audit or cache scope. Keyframes.js has no server API.
...
path parser. V29T freezes exactly seven public value paths: `./color`, `./css`,
`./easing`, `./math`, `./path`, `./transform` and `./value`. Every K edge
installs the exact V29T tarball and imports only that manifest; `./quantize`,
deep-source and forwarding paths are rejected. Root absence remains unchanged
under D-1; it is neither added nor represented as a new tombstone.
...
├── transition/{coordinator,flip,view}.ts
├── svg/{draw,morph}.ts
├── preset/core.ts
...
`text/`, `ingest/`, plural `presets/` or generic `internal/` sector. D-6 keeps
the distilled FLIP, native View Transition and ElementMorph jobs; D-7 removes
text ingestion while `input/` accepts only typed Value AST and CSSOM inputs.
The motion-path wrapper, split-text and Oscillator are pruned. `preset/core.ts`
contains only the statically named, intrinsically justified core selected by
K22—no registry, string lookup or eager catalogue. Names inside a module omit
the module prefix. Tests live only in the external isomorphic tree; generated
standards corpora, browser harnesses, package craters and benchmark fixtures
are named support exceptions.
...
### Classification and curation invariant

D-13 retains `(visibility,tier)` and removes only `featured` from the tier enum.

- `tier` is exactly `standard | archived`; create defaults to `standard` and palette patch owns explicit tier transitions.
- All six `visibility × tier` states are legal. Archiving never rewrites visibility, including public or private palettes.
- Legacy `standard` remains `standard`.
- Eligible legacy `featured` becomes `standard` plus featured membership; an ineligible row aborts migration.
- Legacy `archived` remains `archived` with visibility unchanged.
- Archiving, trash, a visibility transition away from public, ineligibility or moderation removes featured membership atomically.
- Unarchiving never recreates featured membership.
- Palette classification and featured membership retain separate CAS streams and emit canonical audit events.
- No `featured` tier value, tier deletion, automatic visibility rewrite, unproved outbox or dual meaning survives.
...
Value keeps pseudonymous users and roles. The client supplies a generated 32-byte recovery secret during registration or rotation; the server never returns or echoes it and stores only a salted digest. A session is an independently generated 32-byte secret whose digest alone is stored and whose raw value appears only in a `Secure`, `HttpOnly`, `SameSite=Strict` cookie. Origin and CSRF enforcement applies to every cookie-authorized mutation. Recovery and session secrets never enter URLs, response bodies, logs, telemetry, idempotency receipts or audit records. A13 retains the complete `/api/shares` facility: a private immutable-share capability exists only in the URL fragment, is never sent as a referrer or query, redeems once into an HttpOnly grant, and is immediately stripped from the address bar.

D-10 preserves D2 §3 P2 for Value. Its 15 keyed commands reserve and replay atomically within one origin process from a bounded LRU; a restart may execute the command again, and no Value migration or gate claims replica/restart durability. Fourier independently retains durable receipts for its 11 accepted commands, including job creation. That asymmetry is binding unless A21J proves a Value need and a fresh adjudication explicitly names D2 §3 P2.
...
| Palette | Create; detail; list; mine; search; patch content/metadata/tier; visibility CAS; trash; purge/tombstone. No restore. |
...
- Any malformed, expired, tampered, query-mismatched or sort-mismatched cursor returns `400 application/problem+json` with type `urn:contract:cursor-invalid`; the server and clients never restart implicitly at page one.
