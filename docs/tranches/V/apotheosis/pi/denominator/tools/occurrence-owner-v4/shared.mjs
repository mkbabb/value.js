...
export function atomicWriteBundle(manifestPath, shardDirectoryPath, manifestBytes, shardBytesByName) {
    const absoluteManifest = resolve(manifestPath);
    const absoluteShardDirectory = resolve(shardDirectoryPath);
    const parent = dirname(absoluteManifest);
    if (dirname(absoluteShardDirectory) !== parent) throw new Error("manifest and shard directory must share a parent for atomic commit");
    mkdirSync(parent, { recursive: true });
    const temporaryRoot = mkdtempSync(join(parent, ".occurrence-owner-v4.tmp-"));
