...
const SHARD_SCHEMA_VERSION = "value.pi.occurrence-owner-shard/v4";
...
export function assembleTables(evidence, discovery) {
    ...
    for (const carrier of carriers) {
        ...
        for (const flag of carrier.red_flags_v4) codeValues.add(flag);
    }
    ...
    for (const row of evidence.v1.owner_scope_inputs.addenda_01_scope_rows) {
        interner.add(row.id);
    ...
    const contextRows = contextKeys.map((key, index) => encodeContext(interner, stableId("ctxv4", [key]), contextsByKey.get(key)));
    ...
    const carrierRows = carriers.map((carrier) => [
        ...
        code(RED.UNREVIEWED),
        carrier.red_flags_v4.map(code).sort((a, b) => a - b),
    ]);
    ...
    const referenceTable = makeTable("references", ["id", "source_i", "start_line", "start_offset", "end_offset", "raw_s", "type_s", "name_s", "canonical_s", "scope_s", "anchor_s", "link_text_s", "normative_s", "targets_carrier_i", "status_c", "flags_c"], referenceRows);
    ...
    const scopeRows = evidence.v1.owner_scope_inputs.addenda_01_scope_rows.map((row) => [
