...
function fail(message) {
    throw new Error(`v4 verification failed: ${message}`);
}
...
export function verifyBundle({ manifest, shardObjects, rawShardBytesByName, manifestSchema, shardSchema, evidence, executionAttestation, validators: suppliedValidators }) {
    ...
    for (const [index, row] of operations.algorithm_opening_rows.entries()) {
        assertIndex(row[0], sourceRows.length, `algorithm opening ${index} source`); const raw = string(row[3], `algorithm opening ${index} raw`); assert(/^<div\b[^>]*>$/i.test(raw), `algorithm opening ${index} raw syntax`); assertIndexes(row[4], candidates.length, `algorithm opening ${index} candidates`, false); red(row[5], `algorithm opening ${index} status`);
    ...
    const booleanAlgorithmCount = operations.algorithm_opening_rows.filter((row) => /\salgorithm(?:\s*=|\s|>)/i.test(string(row[3], "algorithm raw"))).length;
    assert(booleanAlgorithmCount === 293, `authenticated boolean/valued algorithm opening census ${booleanAlgorithmCount}`);
    ...
    for (const type of ["bibliographic", "spec_url", "production", "definition", "idl_or_property", "css_term_double", "css_shorthand_single", "element"]) assert(referenceTypes.has(type), `reference family absent: ${type}`);
    ...
    return { validators, counts: recomputedCounts, boolean_algorithm_openings: booleanAlgorithmCount, scoped_css_references: scopedCss, scoped_idl_references: scopedIdl };
}
