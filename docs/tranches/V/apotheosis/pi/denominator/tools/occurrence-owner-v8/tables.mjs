...
const SHARD_SCHEMA_VERSION = "value.pi.occurrence-owner-shard/v8";
...
export function assembleTables(evidence, discovery) {
    ...
    for (const carrier of carriers) {
        ...
        for (const flag of carrier.red_flags_v8) codeValues.add(flag);
    }
    ...
    for (const disposition of discovery.lexicalDispositions) {
        interner.add(disposition.candidate_kind);
        interner.add(disposition.state);
        interner.add(disposition.disposition);
        interner.add(disposition.reason);
    }
    ...
    codeValues.add(RED.LEXICAL);
    codeValues.add(RED.EXCLUDED);
    ...
    const contextRows = contextKeys.map((key, index) => encodeContext(interner, stableId("ctxv8", [key]), contextsByKey.get(key)));
    ...
    const carrierRows = carriers.map((carrier) => [
        ...
        code(RED.UNREVIEWED),
        carrier.red_flags_v8.map(code).sort((a, b) => a - b),
    ]);
    ...
    if (typedModifierReferences !== 330) throw new Error(`authenticated !!type reference census mismatch: ${typedModifierReferences}`);
    if (datedTrReferences !== 862) throw new Error(`authenticated dated-TR reference census mismatch: ${datedTrReferences}`);
    if (bibliographyModifierReferences !== 5) throw new Error(`authenticated bibliography-modifier census mismatch: ${bibliographyModifierReferences}`);
    ...
    if (bikeshedCodeReferences !== 487) throw new Error(`authenticated Bikeshed-code reference census mismatch: ${bikeshedCodeReferences}`);
    ...
    const colorHrefWitnesses = discovery.references.filter((row) => row.source_path === "css-color-5/Overview.bs" && row.start_offset === 147967 && row.raw === "https://drafts.csswg.org/css-values-4/#combine-integers" && row.processing_context === "HTML_HREF");
    if (colorHrefWitnesses.length !== 1) throw new Error("authenticated CSS Color 5 href witness absent");
    const escapedPromiseLeaks = discovery.references.filter((row) => row.source_path === "web-animations-1/Overview.bs" && row.raw === "[[PromiseIsHandled]]");
    const escapedPromiseExclusions = discovery.lexicalDispositions.filter((row) => row.source_path === "web-animations-1/Overview.bs" && row.start_offset === 79687 && row.reason === "REVIEWED_EXCLUDED_ESCAPED_SHORTHAND");
    if (escapedPromiseLeaks.length !== 0 || escapedPromiseExclusions.length !== 1) throw new Error("authenticated escaped PromiseIsHandled disposition mismatch");
    const rawRunningLeaks = discovery.references.filter((row) => row.source_path === "css-animations-2/Overview.bs" && row.raw === "'running'" && ["BIKESHED_TEXT", "BIKESHED_CODE"].includes(row.processing_context));
    const rawRunningExclusions = discovery.lexicalDispositions.filter((row) => row.source_path === "css-animations-2/Overview.bs" && row.reason === "REVIEWED_EXCLUDED_RAW_PRE_HIGHLIGHTED" && row.candidate_kind === "reference" && row.end_offset_exclusive - row.start_offset === 9);
    if (rawRunningLeaks.length !== 0 || rawRunningExclusions.length < 2) throw new Error("authenticated class-highlighted JavaScript running disposition mismatch");
    const corruptBlendContexts = carriers.flatMap((row) => row.context.sections).filter((row) => row[2].startsWith('\">'));
    const normalBlendWitnesses = carriers.filter((row) => ["compositing-1/Overview.bs", "compositing-2/Overview.bs"].includes(row.source_path) && row.context.sections.some((section) => section[2] === "normal blend mode"));
    if (corruptBlendContexts.length !== 0 || normalBlendWitnesses.length < 2) throw new Error("authenticated compositing normal-blend-mode heading mismatch");
    const displayCommentExclusions = discovery.lexicalDispositions.filter((row) => row.source_path === "css-display-3/Overview.bs" && row.candidate_kind === "excluded_span" && row.reason === "REVIEWED_EXCLUDED_COMMENT");
    const displayCommentOperations = discovery.operations.candidates.filter((row) => row.source_path === "css-display-3/Overview.bs" && row.start_line >= 1241 && row.end_line <= 1310);
    if (displayCommentExclusions.length === 0 || displayCommentOperations.length !== 0) throw new Error("authenticated CSS Display comment exclusion mismatch");
    const referenceRows = discovery.references;
    for (let index = 0; index < referenceRows.length; index += 1) {
        ...
        referenceRows[index] = [
            ...
            reference.target_carrier_ids.map((id) => carrierIndex.get(id)).filter((candidateIndex) => candidateIndex !== undefined).sort((left, right) => left - right),
            code(reference.status),
            reference.red_flags.map(code).sort((left, right) => left - right),
        ];
    }
    ...
    const lexicalIncludedCount = discovery.lexicalDispositions.filter((row) => row.disposition === "INCLUDED").length;
    const lexicalExcludedCount = discovery.lexicalDispositions.length - lexicalIncludedCount;
    const lexicalRows = discovery.lexicalDispositions;
    for (let index = 0; index < lexicalRows.length; index += 1) {
        const row = lexicalRows[index];
        lexicalRows[index] = [
            row.id,
            sourceIndex.get(row.source_path),
            interner.index(row.candidate_kind),
            row.start_offset,
            row.end_offset_exclusive,
            row.raw_sha256,
            interner.index(row.state),
            interner.index(row.disposition),
            interner.index(row.reason),
            row.included_id,
            code(RED.LEXICAL),
            [code(RED.LEXICAL), ...(row.disposition === "REVIEWED_EXCLUDED" ? [code(RED.EXCLUDED)] : [])].sort((a, b) => a - b),
        ];
    }
    const lexicalTable = makeTable("lexical_dispositions", ["id", "source_i", "candidate_kind_s", "start_offset", "end_offset", "slice_sha256", "state_s", "disposition_s", "reason_s", "candidate_id", "status_c", "flags_c"], lexicalRows);
    ...
    const shards = [
        ...
        shard("lexical-dispositions.json", lexicalTable),
    ...
    return {
        ...
        counts: {
            ...
            lexical_included: lexicalIncludedCount,
            lexical_reviewed_excluded: lexicalExcludedCount,
            ...
            css_color_5_href_witnesses: colorHrefWitnesses.length,
            escaped_promise_leaks: escapedPromiseLeaks.length,
            escaped_promise_exclusions: escapedPromiseExclusions.length,
            raw_running_leaks: rawRunningLeaks.length,
            raw_running_exclusions: rawRunningExclusions.length,
            corrupt_blend_heading_contexts: corruptBlendContexts.length,
            normal_blend_heading_witnesses: normalBlendWitnesses.length,
            css_display_comment_operation_leaks: displayCommentOperations.length,
