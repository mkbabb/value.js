...
import { durableWriteBundle, byteSort, canonicalBytes, exactIdentity, sha256 } from "./shared.mjs";
import { assembleTables } from "./tables.mjs";
import { manifestContentDigest, verifyBundle } from "./verify.mjs";

const MANIFEST_SCHEMA_PATH = "docs/tranches/V/apotheosis/pi/denominator/occurrence-owner-formation-v8.schema.json";
const SHARD_SCHEMA_PATH = "docs/tranches/V/apotheosis/pi/denominator/occurrence-owner-formation-v8-shard.schema.json";
const REPLAY_COMMAND = "node --max-old-space-size=650 docs/tranches/V/apotheosis/pi/denominator/tools/occurrence-owner-v8/launch.mjs --source-root \"$PINNED_CSSWG_ROOT\" --keyframes-root \"$KEYFRAMES_PRIMARY_ROOT\" --output \"$OCCURRENCE_OWNER_V8_OUTPUT\"";
const VERIFICATION_MEMORY_LIMIT_BYTES = 943718400;

function assertMemoryBudget(stage) {
    const rss = process.memoryUsage().rss;
    if (rss > VERIFICATION_MEMORY_LIMIT_BYTES) throw new Error(`${stage}: RSS ${rss} exceeds ${VERIFICATION_MEMORY_LIMIT_BYTES}`);
}

function collectGarbage(stage) {
    v8.setFlagsFromString("--expose_gc");
    try { vm.runInNewContext("gc")(); }
    finally { v8.setFlagsFromString("--no-expose_gc"); }
    assertMemoryBudget(stage);
}
...
function makeManifest(evidence, tables, attestation, counterfixtures) {
    const manifest = {
        schema_version: "value.pi.full-source-occurrence-owner-formation/v8",
        date: "2026-07-22",
        ...
        validation: {
            ...
            semantic_shard_regeneration: "PASS_CODES_STRINGS_CONTEXTS_CARRIERS_OPERATIONS_REFERENCES_LEXICAL_DISPOSITIONS_OWNER_SCOPE",
            stable_id_derivation: "PASS_CONTEXT_CARRIER_OPERATION_REFERENCE_LEXICAL_DISPOSITION",
            ...
            mutation_suite: { case_count: 33, rejected: 33, digest_sha256: "0".repeat(64) },
        },
        replay: {
            command: REPLAY_COMMAND,
            arbitrary_output_path: "PASS",
            verify_before_execute: true,
            publication_protocol: "DURABLE_FSYNCED_TWO_RENAME",
            bundle_atomicity: false,
            module_identity_boundary: "PRE_AND_POST_IMPORT_DISK_IDENTITY_WITH_RESIDUAL_PATH_IMPORT_TOCTOU",
            verification_memory_limit_bytes: VERIFICATION_MEMORY_LIMIT_BYTES,
            canonical_json_lf: true,
        },
        credits: { denominator: 0, semantic: 0, owner: 0, operation: 0, compatibility: 0, conformance: 0, parser: 0, feature: 0, cost_lattice: 0, movement: 0, integration: 0, production: 0, owner_edges: [], costed_owner_formation: false },
...
function streamingIdentity(path) {
    const expectedBytes = statSync(path).size;
    ...
    if (bytes !== expectedBytes) throw new Error(`streaming identity size changed during read: ${path}`);
    return { bytes, sha256: hash.digest("hex") };
}

function assertPublishedBundle(output, shardDirectory, manifestBytes, shards, label) {
    const manifestIdentity = streamingIdentity(output);
    if (manifestIdentity.bytes !== manifestBytes.length || manifestIdentity.sha256 !== sha256(manifestBytes)) throw new Error(`${label} manifest mismatch`);
    for (const shard of shards) {
        const actual = streamingIdentity(resolve(shardDirectory, shard.name));
        if (actual.bytes !== shard.bytes.length || actual.sha256 !== shard.identity.sha256) throw new Error(`${label} shard mismatch: ${shard.name}`);
    }
}
...
export async function main(preExecutionAttestation, argv = process.argv.slice(2)) {
    ...
    let discovery = discoverAll(evidence);
    const tables = assembleTables(evidence, discovery);
    discovery.contexts.clear();
    discovery.carriers.length = 0;
    discovery.operations.candidates.length = 0;
    discovery.operations.algorithmOpenings.length = 0;
    discovery = null;
    evidence.v1 = null;
    collectGarbage("post-assembly object release");
    ...
    const expectedSemanticBytesByTable = new Map(["codes", "strings", "contexts", "carriers", "operations", "references", "lexical_dispositions", "owner_scope"].map((table) => [table, tables.shards.find((row) => row.object.table === table).bytes]));
    verifyBundle({ manifest, shardObjects, rawShardBytesByName, manifestSchema, shardSchema, evidence, executionAttestation: preExecutionAttestation, expectedSemanticBytesByTable });
    assertMemoryBudget("baseline relational shardwise verification");
    collectGarbage("pre-mutation object release");
    const mutations = runMutationSuite({ manifest, shardObjects, rawShardBytesByName, manifestSchema, shardSchema, evidence, executionAttestation: preExecutionAttestation, afterCase: collectGarbage });
    assertMemoryBudget("targeted mutation verification");
    collectGarbage("post-mutation object release");
    ...
    if (options.check) {
        assertPublishedBundle(output, shardDirectory, manifestBytes, tables.shards, "repository replay");
        assertMemoryBudget("repository replay verification");
    } else {
        if (existsSync(output) || existsSync(shardDirectory)) throw new Error(`refusing to replace existing output bundle: ${output}`);
        durableWriteBundle(output, shardDirectory, manifestBytes, rawShardBytesByName);
        assertPublishedBundle(output, shardDirectory, manifestBytes, tables.shards, "post-write");
        assertMemoryBudget("post-publication verification");
    }
