...
export function runMutationSuite({ manifest, shardObjects, rawShardBytesByName, manifestSchema, shardSchema, evidence, executionAttestation }) {
    ...
    const relationReject = (name, mutate, reboundTables = [], recomputeDigest = true, ignoredSemanticTables = []) => {
        ...
        let rejected = false;
        try { verifyBundle({ ...bundle, manifestSchema, shardSchema, evidence, executionAttestation, expectedSemanticBytesByTable: expected, validators }); } catch { rejected = true; } finally { restore(reboundTables); }
    ...
    relationReject("D-context-stable-id-derivation", ({ shardObjects: rows }) => { rows.contexts.rows[0][0] = "ctxv7-000000000000000000000000"; }, ["contexts"]);
    ...
    const full = { suite: "occurrence-owner-v7-a-b-c-mutation-counterfixtures", records };
