...
const SEMANTIC_TABLES = Object.freeze(["codes", "strings", "contexts", "carriers", "operations", "references", "owner_scope"]);

function fail(message) {
    throw new Error(`v7 verification failed: ${message}`);
}
...
export function verifyBundle({ manifest, shardObjects, rawShardBytesByName, manifestSchema, shardSchema, evidence, executionAttestation, expectedSemanticBytesByTable, validators: suppliedValidators }) {
    ...
    for (const [index, row] of contextRows.entries()) {
        ...
        assert(row[0] === stableId("ctxv7", [JSON.stringify(decoded)]), `context ${index} stable ID derivation`);
    }
    ...
    for (const [index, row] of intervals.entries()) {
        ...
        assert(row[0] === stableId("opv7i", [string(sourceRows[row[1]][0], "interval source path"), row[4], row[5], row[6]]), `interval ${index} stable ID derivation`);
    }
    ...
    for (const [index, row] of candidates.entries()) {
        ...
        assert(row[0] === stableId("opv7c", [string(sourceRows[interval[1]][0], "candidate source path"), string(row[2], "candidate kind"), interval[4], interval[5], interval[6]]), `candidate ${index} stable ID derivation`);
    }
    ...
    for (const [index, row] of referenceRows.entries()) {
        ...
        assert(row[0] === stableId("refv7", [sourcePath, type, row[3], row[4], raw]), `reference ${index} stable ID derivation`);
    ...
    assert(typedModifiers === 343 && manifest.counts.typed_modifier_references === 343, "authenticated !!type reference census");
    assert(datedTrReferences === 297 && manifest.counts.dated_tr_references === 297, "authenticated dated-TR reference census");
    assert(bibliographyModifiers === 6 && manifest.counts.bibliography_modifier_references === 6, "authenticated bibliography modifier census");
    ...
    assert(bikeshedCodeReferences === 144 && manifest.counts.bikeshed_code_references === 144, "authenticated Bikeshed-code reference census");
    ...
    const recomputedCounts = {
        ...
        references: referenceRows.length, typed_modifier_references: typedModifiers, dated_tr_references: datedTrReferences,
