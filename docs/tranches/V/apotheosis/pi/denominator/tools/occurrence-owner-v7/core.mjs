...
const MANIFEST_SCHEMA_PATH = "docs/tranches/V/apotheosis/pi/denominator/occurrence-owner-formation-v7.schema.json";
const SHARD_SCHEMA_PATH = "docs/tranches/V/apotheosis/pi/denominator/occurrence-owner-formation-v7-shard.schema.json";
const REPLAY_COMMAND = "node docs/tranches/V/apotheosis/pi/denominator/tools/occurrence-owner-v7/launch.mjs --source-root \"$PINNED_CSSWG_ROOT\" --keyframes-root \"$KEYFRAMES_PRIMARY_ROOT\" --output \"$OCCURRENCE_OWNER_V7_OUTPUT\"";
...
function makeManifest(evidence, tables, attestation, counterfixtures) {
    const manifest = {
        schema_version: "value.pi.full-source-occurrence-owner-formation/v7",
        date: "2026-07-22",
        ...
        validation: {
            ...
            semantic_shard_regeneration: "PASS_CODES_STRINGS_CONTEXTS_CARRIERS_OPERATIONS_REFERENCES_OWNER_SCOPE",
            stable_id_derivation: "PASS_CONTEXT_CARRIER_OPERATION_REFERENCE",
            ...
            mutation_suite: { case_count: 30, rejected: 30, digest_sha256: "0".repeat(64) },
        },
        replay: {
...
export async function main(preExecutionAttestation, argv = process.argv.slice(2)) {
    ...
    assertMemoryBudget("baseline independent shardwise verification");
    const expectedSemanticBytesByTable = new Map(["codes", "strings", "contexts", "carriers", "operations", "references", "owner_scope"].map((table) => [table, canonicalBytes(shardObjects[table])]));
    ...
    if (options.check) {
        ...
        verifyBundle({ manifest: existing.manifest, shardObjects: existing.shardObjects, rawShardBytesByName: existing.rawShardBytesByName, manifestSchema, shardSchema, evidence, executionAttestation: preExecutionAttestation, expectedSemanticBytesByTable });
        ...
        verifyBundle({ manifest: written.manifest, shardObjects: written.shardObjects, rawShardBytesByName: written.rawShardBytesByName, manifestSchema, shardSchema, evidence, executionAttestation: preExecutionAttestation, expectedSemanticBytesByTable });
