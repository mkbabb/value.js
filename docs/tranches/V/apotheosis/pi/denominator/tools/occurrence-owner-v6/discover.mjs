...
function carrierRecords(v1Rows, sourceByPath, contexts) {
    ...
    return v1Rows.map((carrier) => {
        ...
        return { ...carrier, context: inherited, red_flags_v6: uniqueSorted(flags) };
...
function operationRecords(evidence, contexts) {
    ...
    const add = (source, kind, extraction, start, end, extraFlags = []) => {
        ...
        seen.add(key);
        candidates.push({ id: stableId("opv6c", [source.exact_path, kind, slice.start_offset, slice.end_offset_exclusive, slice.raw_slice_sha256]), source_path: source.exact_path, kind, extraction, ...slice, context: contextAt(contexts.get(source.exact_path), start), red_flags: uniqueSorted([RED.OPERATION, RED.COMPLETENESS, ...extraFlags]) });
    };
...
function excludedIntervals(text) {
    ...
    for (const match of text.matchAll(tokens)) {
        intervals.push([match.index, match.index + match[0].length, "HTML_TAG_OR_COMMENT"]);
        ...
        if (/^<\//.test(match[0])) {
            ...
            if (index >= 0) {
                ...
                intervals.push([opening.end, match.index, opening.context]);
            }
            continue;
        }
        const highlight = (attribute(match[0], "highlight") ?? "").toLowerCase();
        const raw = ["script", "style", "xmp", "code", "samp"].includes(tag)
            || (tag === "pre" && /^(?:javascript|js|css|html|webidl|idl)$/.test(highlight));
        if (raw) rawStack.push({ tag, end: match.index + match[0].length, context: `RAW_${tag.toUpperCase()}` });
    }
...
export function scanBikeshedReferences(text) {
    ...
    for (let cursor = 0; cursor < text.length;) {
        ...
        found.push({ type, raw: match.raw, character_start: cursor, character_end: match.end, processing_context: "BIKESHED_TEXT" });
        cursor = match.end;
    }
    return found;
}
...
function referenceRecords(evidence, carriers) {
    ...
    for (const source of evidence.sources) {
        ...
        for (const match of scanBikeshedReferences(text)) {
                ...
                const row = { id: stableId("refv6", [source.exact_path, match.type, start, end, match.raw]), source_path: source.exact_path, start_line: lineForOffset(source.lines, start), start_offset: start, end_offset_exclusive: end, raw: match.raw, ...parsed, processing_context: match.processing_context, target_carrier_ids: targetCarrierIds, status, red_flags: uniqueSorted([RED.REFERENCE, status]) };
