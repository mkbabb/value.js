...
const SHARD_SCHEMA_VERSION = "value.pi.occurrence-owner-shard/v6";
...
export function assembleTables(evidence, discovery) {
    ...
    for (const carrier of carriers) {
        ...
        for (const flag of carrier.red_flags_v6) codeValues.add(flag);
    }
    ...
    const contextRows = contextKeys.map((key, index) => encodeContext(interner, stableId("ctxv6", [key]), contextsByKey.get(key)));
    ...
    const carrierRows = carriers.map((carrier) => [
        ...
        code(RED.UNREVIEWED),
        carrier.red_flags_v6.map(code).sort((a, b) => a - b),
    ]);
