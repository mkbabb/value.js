...
const SEMANTIC_TABLES = Object.freeze(["contexts", "carriers", "operations", "references", "owner_scope"]);

function fail(message) {
    throw new Error(`v6 verification failed: ${message}`);
}
...
export function verifyBundle({ manifest, shardObjects, rawShardBytesByName, manifestSchema, shardSchema, evidence, executionAttestation, expectedSemanticBytesByTable, validators: suppliedValidators }) {
    ...
    for (const [index, row] of contextRows.entries()) {
        ...
        assert(row[0] === stableId("ctxv6", [JSON.stringify(decoded)]), `context ${index} stable ID derivation`);
    }
    ...
    for (const [index, row] of intervals.entries()) {
        ...
        assert(row[0] === stableId("opv6i", [string(sourceRows[row[1]][0], "interval source path"), row[4], row[5], row[6]]), `interval ${index} stable ID derivation`);
    }
    ...
    for (const [index, row] of candidates.entries()) {
        ...
        assert(row[0] === stableId("opv6c", [string(sourceRows[interval[1]][0], "candidate source path"), string(row[2], "candidate kind"), interval[4], interval[5], interval[6]]), `candidate ${index} stable ID derivation`);
    }
    ...
    for (const [index, row] of referenceRows.entries()) {
        ...
        assert(row[0] === stableId("refv6", [sourcePath, type, row[3], row[4], raw]), `reference ${index} stable ID derivation`);
    ...
    const recomputedCounts = {
        ...
        bibliography_modifier_references: bibliographyModifiers, terminal_punctuation_urls: terminalPunctuationUrls,
