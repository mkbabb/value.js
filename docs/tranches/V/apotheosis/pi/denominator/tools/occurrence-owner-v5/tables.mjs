...
const SHARD_SCHEMA_VERSION = "value.pi.occurrence-owner-shard/v5";
...
export function assembleTables(evidence, discovery) {
    ...
    for (const carrier of carriers) {
        ...
        for (const flag of carrier.red_flags_v5) codeValues.add(flag);
    }
    ...
    const contextRows = contextKeys.map((key, index) => encodeContext(interner, stableId("ctxv5", [key]), contextsByKey.get(key)));
    ...
    const carrierRows = carriers.map((carrier) => [
        ...
        code(RED.UNREVIEWED),
        carrier.red_flags_v5.map(code).sort((a, b) => a - b),
    ]);
