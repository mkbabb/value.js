...
export function algorithmMarker(opening) {
    return /\salgorithm(?:\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+))?(?=\s|>)/i.test(opening)
        ...
        || attribute(opening, "data-algorithm") !== null;
}
...
function carrierRecords(v1Rows, sourceByPath, contexts) {
    ...
    return v1Rows.map((carrier) => {
        ...
        return { ...carrier, context: inherited, red_flags_v5: uniqueSorted(flags) };
...
function operationRecords(evidence, contexts) {
    ...
    const add = (source, kind, extraction, start, end, extraFlags = []) => {
        ...
        seen.add(key);
        candidates.push({ id: stableId("opv5c", [source.exact_path, kind, slice.start_offset, slice.end_offset_exclusive, slice.raw_slice_sha256]), source_path: source.exact_path, kind, extraction, ...slice, context: contextAt(contexts.get(source.exact_path), start), red_flags: uniqueSorted([RED.OPERATION, RED.COMPLETENESS, ...extraFlags]) });
    };
...
export function parseReference(type, raw) {
    if (type === "bibliographic") {
        ...
        const target = display.target.replace(/^!/, "");
        const hash = target.indexOf("#");
        const name = hash < 0 ? target : target.slice(0, hash);
        const anchor = hash < 0 ? null : target.slice(hash + 1);
        return { type, name, canonical: name.toLowerCase(), scope: null, anchor, link_text: display.link_text, normative };
    }
    if (type === "spec_url") {
    ...
    const scoped = parseScoped(display.target, type === "markup");
    const grammar = type === "production";
    return { type, name: scoped.name, canonical: grammar ? canonicalGrammarName(scoped.name) : canonicalName(scoped.name), scope: scoped.scope, anchor: null, link_text: display.link_text, normative: null };
}

function excludedIntervals(text) {
    const intervals = [];
    for (const match of text.matchAll(/<!--[\s\S]*?-->|<\/?[A-Za-z][A-Za-z0-9:-]*\b(?:[^"'<>]|"[^"]*"|'[^']*')*>/g)) intervals.push([match.index, match.index + match[0].length]);
    return intervals;
}

function delimited(text, start, opening, closing) {
...
export function scanBikeshedReferences(text) {
    const excluded = excludedIntervals(text);
    const found = [];
    let excludedIndex = 0;
    for (let cursor = 0; cursor < text.length;) {
        ...
        for (const [candidateType, opening, closing] of [
            ...
            const candidate = delimited(text, cursor, opening, closing);
            if (candidate !== null && candidate.raw.length > opening.length + closing.length) { match = candidate; type = candidateType; break; }
        }
        if (match === null && text[cursor] === "'" && text[cursor - 1] !== "'") {
            const candidate = delimited(text, cursor, "'", "'");
            if (candidate !== null && /^'@?[A-Za-z0-9_-]+(?:\(\))?(?:\/@?[A-Za-z0-9_-]+(?:\(\))?)?'$/.test(candidate.raw) && text[candidate.end] !== "'") { match = candidate; type = "css_shorthand_single"; }
        }
        if (match === null && /^https?:\/\/(?:drafts\.csswg\.org|www\.w3\.org\/TR)\//i.test(text.slice(cursor))) {
            const raw = /^[^\s<)>'\"]+/.exec(text.slice(cursor))?.[0];
            if (raw !== undefined) { match = { raw, end: cursor + raw.length }; type = "spec_url"; }
        }
        if (match === null) { cursor += 1; continue; }
        found.push({ type, raw: match.raw, character_start: cursor, character_end: match.end });
        cursor = match.end;
    }
    return found;
}
...
function referenceRecords(evidence, carriers) {
    ...
    for (const source of evidence.sources) {
        ...
        for (const match of scanBikeshedReferences(text)) {
                ...
                const row = { id: stableId("refv5", [source.exact_path, match.type, start, end, match.raw]), source_path: source.exact_path, start_line: lineForOffset(source.lines, start), start_offset: start, end_offset_exclusive: end, raw: match.raw, ...parsed, target_carrier_ids: targetCarrierIds, status, red_flags: uniqueSorted([RED.REFERENCE, status]) };
