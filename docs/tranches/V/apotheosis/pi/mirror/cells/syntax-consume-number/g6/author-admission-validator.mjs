...
const schemaIds = Object.freeze({
    "boundary-review.schema.json": "urn:value-js:syntax-consume-number:g6:boundary-review",
    "root-gestalt.schema.json": "urn:value-js:syntax-consume-number:g6:root-gestalt",
    "pre-author-receipt.schema.json": "urn:value-js:syntax-consume-number:g6:pre-author-receipt",
    "author-receipt.schema.json": "urn:value-js:syntax-consume-number:g6:author-receipt",
    "holdout-receipt.schema.json": "urn:value-js:syntax-consume-number:g6:holdout-receipt",
...
function verifyFeatureAndLineage() {
    ...
    for (const terminal of [row.g5_terminal_authority.root_gestalt, row.g5_terminal_authority.rejection, row.g5_terminal_authority.holdout_destruction]) {
        const comparable = { path: terminal.path, sha256: terminal.sha256, bytes: terminal.bytes };
        verifyBoundRow(comparable, `G5 terminal ${terminal.path}`);
    }
    if (row.g5_terminal_authority.holdout_destruction.reuse !== false) fail("G5 secret reuse is not forbidden");
...
function validateHoldout(receipt, ciphertext, formation) {
    ...
    if (receipt.envelope.algorithm !== "AES-256-GCM" || receipt.envelope.magic_ascii_nul !== "VPI6CN1\u0000" || receipt.envelope.magic_bytes !== 8 || receipt.envelope.nonce_bytes !== 12 || receipt.envelope.tag_bytes !== 16) fail("holdout AES envelope declaration mismatch");
    if (!decoded.subarray(0, 8).equals(Buffer.from("VPI6CN1\0", "ascii"))) fail("holdout envelope magic mismatch");
    ...
    for (const statement of sf.statements) {
        if (ts.isImportDeclaration(statement)) {
            ...
            if (module === "@mkbabb/parse-that/core") {
                ...
                if (expectedContractModule !== undefined && module !== expectedContractModule) fail(`${seat}: contract import does not resolve to the frozen G6 contract`);
...
function typeProof(candidatePath, sourceText) {
    ...
    const probePath = join(dirname(absoluteCandidate), ".consume-number-g6-probe.ts");
...
function selfCheck() {
    ...
    const decoded = Buffer.concat([Buffer.from("VPI6CN1\0", "ascii"), Buffer.alloc(12), Buffer.alloc(16), plaintext]);
    ...
    const holdout = {
        ...
        envelope: { algorithm: "AES-256-GCM", magic_ascii_nul: "VPI6CN1\u0000", magic_bytes: 8, nonce_bytes: 12, tag_bytes: 16, aad: { feature_id: featureId, generation, corpus_id: "12345678-1234-4234-9234-123456789abc", formation_sha256: formation.receiptIdentity.sha256, precursor_closure_sha256: formation.receipt.precursor_closure_sha256, plaintext_sha256: identity(plaintext).sha256, plaintext_bytes: plaintext.length } },
    ...
    return { precursor_rows: formation.rows.length, precursor_closure_sha256: formation.receipt.precursor_closure_sha256, topology_negative_controls: 7, schemas: Object.keys(schemaIds).length };
}
...
try {
    ...
    else if (command === "--negative-controls") output = { negative_controls: topologyControls() ? "PASS" : "FAIL", recorded_bypass_digests: [...topologyDenylist] };
