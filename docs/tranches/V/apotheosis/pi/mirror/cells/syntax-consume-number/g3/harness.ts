...
function assertSemanticLeaf(actual: unknown, expected: NumberLiteralExpected, id: string): asserts actual is ConsumedNumber {
    ...
    if (Object.getPrototypeOf(actual) !== Object.prototype) {
        throw new Error(`${id}: result does not have the agreed ordinary Object.prototype`);
    }
    const ownKeys = Reflect.ownKeys(actual);
    if (ownKeys.some((key) => typeof key !== "string")) {
        throw new Error(`${id}: result owns a symbol key`);
    }
    ...
    for (const key of semanticKeys) {
        ...
        if (descriptor === undefined
            ...
            throw new Error(`${id}: ${key} is not an ordinary own writable/enumerable/configurable data property`);
        }
    }
...
export function validatePublicFixtures(fixtures: PublicFixtureFile = publicFixtures): void {
    if (fixtures.feature_id !== "SYNTAX-CONSUME-NUMBER" || fixtures.generation !== 3) throw new Error("fixture identity mismatch");
