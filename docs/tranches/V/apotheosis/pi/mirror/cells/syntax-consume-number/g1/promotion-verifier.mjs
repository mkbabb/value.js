...
function typeProof({ sourceText, candidateIdentity, ts, parseRows, tsRows }) {
    ...
    const probePath = resolve(dirname(candidatePath), ".consume-number-g1.probe.ts");
    ...
    for (const source of program.getSourceFiles()) {
        ...
        if (path === candidatePath) {
            ...
            probeSource = source;
            resolvedInputs.push({ kind: "probe-memory", path: "verifier-generated://consume-number-g1/probe.ts", sha256: sha256(Buffer.from(probe)), bytes: Buffer.byteLength(probe) });
        } else {
...
export async function verifySelf() {
    if (relative(mirrorRoot, verifierFile).split(sep).join("/") !== "cells/syntax-consume-number/g1/promotion-verifier.mjs") throw new Error("verifier is not at its governed path");
    const tools = await loadBoundTools();
    const closure = verifyExecutableClosure(tools.ts);
    const cleanBaseSha256 = verifyCleanBase();
    return { featureId, generation, closure, cleanBaseSha256, toolLedgers: { parseThat: sha256(readFileSync(parseThatLedger)), typescript: sha256(readFileSync(typeScriptLedger)) } };
}
...
if (invoked === verifierFile) {
    if (process.argv[2] === "--self-check" && process.argv.length === 3) {
        ...
        process.stdout.write(`${JSON.stringify(typeProof({ sourceText: text, candidateIdentity: "/__value_number_assay__/consume-number.ts", ...tools }), null, 2)}\n`);
