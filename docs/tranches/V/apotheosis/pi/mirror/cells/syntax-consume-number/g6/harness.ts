...
import controlsJson from "./fixtures/g6-controls.json" with { type: "json" };
import type {
    CompositionFixture,
    ConsumedNumber,
    ConsumeNumberParser,
...
export const g6Controls = controlsJson as unknown as G6Controls;
...
function seedAhead(state: ParserState<unknown>): void {
    state.furthest = state.src.length + g6Controls.diagnostics.preseed_ahead_delta;
...
function assertRawCase(parser: ConsumeNumberParser, testCase: NumberLiteralCase, caseIndex: number): void {
    for (const mode of g6Controls.diagnostics.modes) {
        for (const profile of g6Controls.diagnostics.profiles) {
...
export function validatePublicFixtures(): void {
    ...
    if (g6Controls.feature_id !== "SYNTAX-CONSUME-NUMBER" || g6Controls.generation !== 6) throw new Error("G6 controls identity mismatch");
    ...
    if (JSON.stringify(failureIds) !== JSON.stringify([...g6Controls.failure_case_ids].sort())) {
        throw new Error("failure control IDs do not exactly cover public failures");
    }
    const sizes = g6Controls.work_limit.sizes_utf16_code_units;
...
export function runPublicHarness(parser: ConsumeNumberParser): Readonly<{ assertions: number; failures: number }> {
    ...
    return Object.freeze({
        assertions: cases.length * g6Controls.diagnostics.modes.length * g6Controls.diagnostics.profiles.length + retainedPublicFixtures.composition_cases.length,
        failures: g6Controls.failure_case_ids.length * g6Controls.diagnostics.modes.length * g6Controls.diagnostics.profiles.length,
    });
}
...
export function runWorkLimitHarness(parser: ConsumeNumberParser, now: () => number = () => performance.now()): WorkLimitReceipt {
    ...
    let maxRatio = 0;
    let maxCaseMs = 0;
    for (const construction of g6Controls.work_limit.constructions) {
        const medians: number[] = [];
        for (const size of g6Controls.work_limit.sizes_utf16_code_units) {
            const fixture = materializeHostile(construction, size);
            for (let warmup = 0; warmup < g6Controls.work_limit.warmups; warmup += 1) {
            ...
            for (let sample = 0; sample < g6Controls.work_limit.samples; sample += 1) {
                const state = new ParserState<unknown>(fixture.source, undefined, 0);
                const start = now();
                parser.call(state as ParserState<ConsumedNumber>);
                const elapsed = now() - start;
                if (state.isError || state.offset !== fixture.expected!.end) throw new Error(`${fixture.id}: measured semantic failure`);
                if (!Number.isFinite(elapsed) || elapsed < 0 || elapsed > g6Controls.work_limit.per_case_ceiling_ms) {
        ...
        for (let index = 1; index < medians.length; index += 1) {
            const ratio = Math.max(medians[index]!, g6Controls.work_limit.timing_floor_ms)
                / Math.max(medians[index - 1]!, g6Controls.work_limit.timing_floor_ms);
            maxRatio = Math.max(maxRatio, ratio);
            if (ratio > g6Controls.work_limit.median_pair_ratio_ceiling) {
