...
const guardedSuffixes = [
    ["incomplete-decimal/unsigned", "1.", 0, "1"],
    ["incomplete-decimal/plus", "+7.", 0, "+7"],
    ["incomplete-decimal/minus-exponent-tail", "-7.e2", 0, "-7"],
    ["malformed-exponent/double-plus", "1e++2", 0, "1"],
    ["malformed-exponent/double-minus", "1e--2", 0, "1"],
    ["incomplete-decimal/nonzero-plus", "@@+7.", 2, "+7"],
    ["incomplete-decimal/nonzero-minus-exponent-tail", "@@@-7.e2", 3, "-7"],
].map(([id, source, offset, representation]) => ({ id, source, offset, representation, end: offset + representation.length, ...leaf(representation) }));

const corpus = {
    schema: "value.pi.syntax-consume-number.g13.public-corpus/v1",
    derivation: {
    ...
    guarded_suffix_cases: guardedSuffixes,
...
if (process.argv[2] === "--print") process.stdout.write(rendered);
    ...
    process.stdout.write(`${JSON.stringify({ status: "PASS", generated_successes: signs.length * mantissas.length * exponents.length + signs.length * mantissas.length * 3 + specials.length + guardedSuffixes.length, guarded_suffixes: guardedSuffixes.length, failure_offsets: corpus.failure_offsets.length, signed_integer_exponents: 4 })}\n`);
