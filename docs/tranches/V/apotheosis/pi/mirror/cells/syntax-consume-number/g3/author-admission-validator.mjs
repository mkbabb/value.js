...
function typeProof(sourceText, ts, snapshot) {
    const candidatePath = resolve("/__syntax_consume_number_g3_assay__/consume-number.ts");
    const probePath = resolve("/__syntax_consume_number_g3_assay__/.probe.ts");
...
export function verifySelf() {
    if (slash(relative(mirrorRoot, validatorFile)) !== "cells/syntax-consume-number/g3/author-admission-validator.mjs") {
    ...
    snapshot.close();
    return {
        featureId, generation, status, closure,
...
export function verifyPreAuthorAdmission(manifestDigest) {
    ...
    return {
        ...
        ciphertext, verdicts,
        deferred: ["public", "revealed-holdout", "benchmark", "four-skeptic", "adjudicator", "integration", "projection/default-gates"],
    };
}
...
export function verifyFourSeatAdmission(candidateRootArgument) {
    ...
    return {
        ...
        admitted,
        retainedSkepticProof: ["runtime semantics", "no scanner by alias/indirection", "lineage", "idiom", "KISS"],
    };
}
