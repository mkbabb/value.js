...
const exactLeaf = (actual: unknown, expected: Leaf) => {
    ...
    return Reflect.ownKeys(leaf).map(String).sort().join(",") === "sign,type,value"
        && leaf.sign === expected.sign && leaf.type === expected.type && Object.is(leaf.value, expected.value);
};
...
function runFailure(source: string, priorName: string, diagnostics: string) {
    const prior = predecessor(priorName);
    const state = new ParserState<any>(source, prior, 0);
    const originalSource = state.src;
    const seed = diagnostics === "preseeded-ahead" ? seedDiagnostics(state, 0) : null;
    try { consumeNumber.call(state); } catch (error) { fail(`failure ${JSON.stringify(source)} threw ${String(error)}`); }
    if (state.src !== originalSource || !state.isError || state.offset !== 0 || !Object.is(state.value, prior)) fail(`failure ${JSON.stringify(source)} was not transactional`);
...
function generatedSuccesses(): Success[] {
    ...
    return [...out, ...corpus.special_success_cases];
}
...
let failureRuns = 0;
for (const source of corpus.failure_sources) for (const prior of corpus.predecessor_profiles) for (const diagnostics of corpus.diagnostics_profiles) {
    runFailure(source, prior, diagnostics);
    failureRuns++;
}
...
process.stdout.write(`${JSON.stringify({
    schema: "value.pi.syntax-consume-number.g12.public-result/v1",
    status: "PASS",
