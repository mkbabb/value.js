...
if (receipt.schema !== "value.pi.syntax-consume-number.g12.formation/v1" || receipt.generation !== 12 || receipt.status !== "FROZEN_PUBLIC_PREAUTHOR_FORMATION") fail("formation receipt header drift");
const lines = [];
...
const common = ["contract.json", "public-corpus.json", "public-worker.ts", "public-harness.mjs", "candidate-closure.mjs", "lib.mjs"];
...
const authorHarnessText = ["public-harness.mjs", "public-worker.ts", "candidate-closure.mjs", "lib.mjs"].map((name) => readFileSync(join(root, name), "utf8")).join("\n");
...
process.stdout.write(`${JSON.stringify({
    schema: "value.pi.syntax-consume-number.g12.formation-result/v1",
