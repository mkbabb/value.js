...
type KnownFamily = KnownCssUnit["family"];

const knownUnitFamilies = new Map<string, KnownFamily>();
for (const [family, units] of Object.entries(families)) {
    for (const unit of units) {
        knownUnitFamilies.set(unit, family as KnownFamily);
    }
}
...
export const classifyCssUnit = (unit: string): KnownCssUnit | null => {
    const canonicalUnit = asciiLowercase(unit);
    const family = knownUnitFamilies.get(canonicalUnit);
    return family === undefined ? null : { unit: canonicalUnit, family };
};
