...
type KnownFamily = Exclude<CssUnitFamily, null>;
const knownFamilies = new Map<string, KnownFamily>();
for (const family of Object.keys(unitFamilies) as KnownFamily[]) {
    for (const unit of unitFamilies[family]) knownFamilies.set(unit, family);
}
...
export function classifyCssUnit(unit: string): KnownCssUnit | null {
    const canonical = asciiLower(unit);
    const family = knownFamilies.get(canonical);
    return family === undefined ? null : { unit: canonical, family };
}
