...
export function validateConsumerBoundsAuthority(
    ...
    if (!isAbsolute(binding.path)) throw new Error("consumer-universe bounds authority logical path must be absolute");
    ...
    for (const search of searchRoots) {
        if (!isAbsolute(search.path) || !isAbsolute(search.canonical_realpath) || !Number.isInteger(search.max_depth) || search.max_depth < 1) {
            throw new Error("consumer-universe search root is malformed");
        }
        if (!existsSync(search.path) || !statSync(search.path).isDirectory()) {
            throw new Error(`consumer-universe search root is unavailable: ${search.path}`);
        }
        const canonical = realpathSync(search.path);
        if (canonical !== search.canonical_realpath) throw new Error(`consumer-universe search root realpath drift: ${search.path}`);
        if (canonicalSearchRoots.has(canonical)) throw new Error(`duplicate consumer-universe search root: ${canonical}`);
        canonicalSearchRoots.add(canonical);
    }
    ...
    for (const required of requiredPaths) {
        ...
        if (!isAbsolute(required.path) || !isAbsolute(required.canonical_realpath)
        ...
        if (canonicalPotentialPath(required.path) !== expectedCanonical || expectedCanonical !== required.canonical_realpath
    ...
    for (const field of ["max_entries", "max_files", "max_file_bytes"]) {
        if (!Number.isInteger(manifest.bounds?.[field]) || manifest.bounds[field] < 1) {
