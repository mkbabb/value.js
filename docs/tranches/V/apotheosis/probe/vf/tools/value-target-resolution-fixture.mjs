...
function deletionAnnex({ fixtureRoot, commonEvidence, consumer, truth, waveId, tombstonePath, pattern }) {
    ...
    return { annex, path: realpathSync(path), fileSha256: fileSha256(path), effect, decision };
}
...
function makeOwnerReturn({
    ...
    if (deletion) evidenceInputs.push({ path: deletion.path, sha256: deletion.fileSha256, purpose: `${waveId} owner-precut deletion judgment`, class: "generated" });
    const record = {
...
export function createValueTargetResolutionFixture({ tempRoot, formationTargetPath }) {
