...
for (let index = 2; index < process.argv.length; index += 1) {
    if (process.argv[index] === "--resolutions" && process.argv[index + 1]) resolutionsPath = resolve(process.argv[++index]);
        ...
        process.stderr.write("usage: node validate-value-target-resolutions.mjs --resolutions <path>\n");
        process.exit(2);
    }
}
if (!resolutionsPath) {
...
if (manifest && formation) {
    ...
    if (failures.length === 0) {
        ...
        for (const [index, row] of rows.entries()) {
            ...
            const owner = loadValueTargetOwnerReturn({ wave_id: row.owner, return: row.return }, pointer, fail, ownerCache);
            if (!owner) continue;
