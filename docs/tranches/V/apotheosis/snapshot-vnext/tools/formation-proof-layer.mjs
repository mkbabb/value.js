...
function waveRows(source, failures) {
    ...
    for (const [zeroLine, line] of String(source).split("\n").entries()) {
        ...
        if (cells.length !== 7) failures.push(`waves/G-D.md:${zeroLine + 1}: ${waveId} must have seven cells`);
...
export function validateWaveFormationProof(waveId, cells) {
    ...
    if (!Array.isArray(cells) || cells.length !== 7 || cells[0] !== waveId) {
        return { failures: [`${waveId}: proof-layer projection requires its exact seven-cell wave row`], proof: null };
    }
