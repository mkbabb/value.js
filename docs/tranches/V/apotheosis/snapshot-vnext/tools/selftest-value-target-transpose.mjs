...
function invoke(path) {
    return spawnSync(process.execPath, [validator, "--ledger", path], {
        encoding: "utf8",
...
try {
    ...
    if (positive.status !== 0) failures.push(`positive real transpose rejected:\n${positive.stderr || positive.stdout}`);
        ...
        if (
            ...
            const universalPositive = spawnSync(process.execPath, [returnValidator, universal.path, "--immutable-authority"], {
                encoding: "utf8",
                maxBuffer: 256 * 1024 * 1024,
            });
            ...
            if (universalPositive.status !== 0
                ...
                if (new Set(waves).size !== waves.length || closure.edge_count <= closure.node_count - 1) {
                    ...
                    if (!nonValue) failures.push("V29T fixture does not exercise a cross-band dependency root");
                        ...
                        const nonValueResult = spawnSync(process.execPath, [returnValidator, nonValue.path, "--immutable-authority"], {
                            encoding: "utf8",
                            maxBuffer: 256 * 1024 * 1024,
                        });
                        let nonValueReceipt;
                    ...
                    for (const mode of [[], ["--offline"], ["--immutable-authority"]]) {
                        const symlinkResult = spawnSync(process.execPath, [returnValidator, symlinkPath, ...mode], { encoding: "utf8" });
                    ...
                    const cycleResult = spawnSync(process.execPath, [returnValidator, universal.path, "--immutable-authority"], {
                        encoding: "utf8",
            ...
            const forgedResult = spawnSync(process.execPath, [returnValidator, realpathSync(forgedPath), "--immutable-authority"], {
                encoding: "utf8",
                maxBuffer: 256 * 1024 * 1024,
            });
            ...
            const missingRowResult = spawnSync(process.execPath, [returnValidator, realpathSync(missingRowReturnPath), "--immutable-authority"], {
                encoding: "utf8",
                maxBuffer: 256 * 1024 * 1024,
            });
    ...
    if (universal) {
        ...
        const historical = spawnSync(process.execPath, [returnValidator, universal.path, "--immutable-authority"], {
            encoding: "utf8",
            maxBuffer: 256 * 1024 * 1024,
        });
