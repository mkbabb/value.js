...
function run(name, value) {
    ...
    return {
        input,
        output,
        result: spawnSync(process.execPath, [tool, "--input", input, "--output", output], { encoding: "utf8" }),
    };
}
...
try {
    ...
    if (valid.result.status !== 0) {
        ...
        try {
            ...
            const annex = consumerUniverseAnnexProjection("C00U", canonicalReceiptPath, verified);
    ...
    const staleResult = spawnSync(process.execPath, [tool, "--input", staleInput, "--output", staleOutput], { encoding: "utf8" });
    ...
    const duplicateJsonResult = spawnSync(process.execPath, [tool, "--input", duplicateJsonInput, "--output", duplicateJsonOutput], { encoding: "utf8" });
    ...
    if (!immutableControl || !fixtureReceiptValidationOptions) {
        ...
        const immutableBinding = consumerUniverseImmutableBindingProjection(annex);
        ...
        try {
            ...
            failures.push(`exact immutable binding rejected its frozen receipt after a live root advance: ${error.message}`);
        }
        try {
            validateConsumerUniverseReceipt(canonicalReceiptPath, {
                ...fixtureReceiptValidationOptions,
            ...
            failures.push("immutable validation accepted a receipt without an authenticated binding");
        } catch (error) {
            if (!error.message.includes("requires an authenticated immutable binding")) {
                failures.push(`binding-free immutable validation rejected for unexpected reason: ${error.message}`);
            }
        }
        try {
            validateConsumerUniverseReceipt(canonicalReceiptPath, {
        ...
        try {
            validateConsumerUniverseReceipt(canonicalReceiptPath, {
                ...immutableOptions,
                immutableBinding: { ...immutableBinding, roots_sha256: "0".repeat(64) },
            });
            failures.push("immutable validation accepted an arbitrary authenticated-binding claim");
        } catch (error) {
            if (!error.message.includes("not the exact frozen receipt projection")) {
                failures.push(`arbitrary immutable binding rejected for unexpected reason: ${error.message}`);
            }
        }

        const originalReceiptSource = readFileSync(canonicalReceiptPath);
        const originalInputSource = readFileSync(inputPath);
        const originalUniverse = parseJsonStrict(originalInputSource);
        const writeFrozenForgery = (forgedUniverse, forgedReceipt) => {
            const universePreimage = clone(forgedUniverse);
            delete universePreimage.universe_hash;
            forgedUniverse.universe_hash = sha256(canonicalize(universePreimage));
            const inputSource = `${canonicalize(forgedUniverse)}\n`;
            writeFileSync(inputPath, inputSource);
            forgedReceipt.input.file_sha256 = sha256(inputSource);
            forgedReceipt.input.universe_hash = forgedUniverse.universe_hash;
            const epochPreimage = clone(forgedReceipt.epoch);
            delete epochPreimage.epoch_sha256;
            forgedReceipt.epoch.epoch_sha256 = sha256(canonicalize(epochPreimage));
            const receiptPreimage = clone(forgedReceipt);
            delete receiptPreimage.receipt_hash;
            forgedReceipt.receipt_hash = sha256(canonicalize(receiptPreimage));
            writeFileSync(canonicalReceiptPath, `${canonicalize(forgedReceipt)}\n`);
        };
        const expectStaleBindingRejected = (name, mutate) => {
            try {
                const forgedUniverse = clone(originalUniverse);
                const forgedReceipt = clone(receipt);
                mutate(forgedUniverse, forgedReceipt);
                writeFrozenForgery(forgedUniverse, forgedReceipt);
                try {
                    validateConsumerUniverseReceipt(canonicalReceiptPath, immutableOptions);
                    failures.push(`${name} was accepted with a stale immutable binding`);
                } catch (error) {
                    if (!error.message.includes("immutable binding")) {
                        failures.push(`${name} rejected for unexpected reason: ${error.message}`);
                    }
                }
            } finally {
                writeFileSync(inputPath, originalInputSource);
                writeFileSync(canonicalReceiptPath, originalReceiptSource);
            }
        };
        expectStaleBindingRejected("rehashed frozen receipt", (_universe, forgedReceipt) => {
            forgedReceipt.resolved_at = new Date(Date.parse(forgedReceipt.resolved_at) + 1).toISOString();
        });
        expectStaleBindingRejected("rehashed frozen root head", (forgedUniverse, forgedReceipt) => {
            const changedHead = "f".repeat(40);
            forgedUniverse.roots.find(({ id }) => id === "value").head = changedHead;
            forgedReceipt.roots.find(({ id }) => id === "value").head = changedHead;
            forgedReceipt.epoch.observed_roots_sha256 = sha256(canonicalize(forgedReceipt.roots));
        });
        expectStaleBindingRejected("rehashed frozen evidence", (forgedUniverse) => {
            forgedUniverse.roots.find(({ id }) => id === "value").provenance.origins.evidence[0].sha256 = "f".repeat(64);
        });
        try {
            const forgedReceipt = clone(receipt);
            const reserializedInput = `${JSON.stringify(originalUniverse, null, 4)}\n`;
            writeFileSync(inputPath, reserializedInput);
            forgedReceipt.input.file_sha256 = sha256(reserializedInput);
            const receiptPreimage = clone(forgedReceipt);
            delete receiptPreimage.receipt_hash;
            forgedReceipt.receipt_hash = sha256(canonicalize(receiptPreimage));
            writeFileSync(canonicalReceiptPath, `${canonicalize(forgedReceipt)}\n`);
            try {
                validateConsumerUniverseReceipt(canonicalReceiptPath, immutableOptions);
                failures.push("rehashed frozen input bytes were accepted with a stale immutable binding");
            } catch (error) {
                if (!error.message.includes("immutable binding")) {
                    failures.push(`rehashed frozen input bytes rejected for unexpected reason: ${error.message}`);
                }
            }
        } finally {
            writeFileSync(inputPath, originalInputSource);
