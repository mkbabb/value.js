...
if (!path || process.argv.length > 4 || (process.argv.length === 4 && !offline)) {
    process.stderr.write("usage: node validate-return.mjs <wave-return.json> [--offline|--historical-certificate|--immutable-authority]\n");
    process.exit(2);
}
...
function requireEvidenceInputOnce(binding, pointer) {
    const matches = (record.evidence_inputs ?? []).filter((input) =>
...
function consumerBoundsOptions(binding, pins, pointer, validationMode, immutableBinding) {
    const historicalAuthority = historicalBoundsAuthorityPath(binding, pins, pointer);
    return {
...
if (collectScopeClosure) {
    ...
    const walk = (binding, consumerWaveId, pointer) => {
        ...
        if (immutableAuthority || !offline) {
            const child = spawnSync(process.execPath, [validatorPath, canonicalPath, "--historical-certificate"], {
                encoding: "utf8",
                env: {
                    ...process.env,
    ...
    if (!consumerAnnex) {
        ...
        try {
            const consumerOptions = consumerBoundsOptions(
                ...
                offline ? consumerUniverseImmutableBindingProjection(consumerAnnex) : undefined,
            );
            ...
            const expectedProjection = consumerUniverseAnnexProjection(record.wave_id, consumerAnnex.receipt_path, verified);
            ...
            if (!offline) {
                ...
                try {
                    ...
                    const rerun = spawnSync(process.execPath, [resolver, "--input", consumerAnnex.input_path, "--output", output], {
                        encoding: "utf8",
                        maxBuffer: 128 * 1024 * 1024,
                    });
                ...
                if (baseline?.applicability !== "applicable") {
                    ...
                    if (!binding || !existsSync(binding.path)) {
                        ...
                        if (prior?.receipt_path && existsSync(prior.receipt_path)) {
                            ...
                            fail("/annexes/consumer-universe/baseline: C00U receipt file is unavailable");
                        }
    ...
    if (canonicalDeletionPath) {
        ...
        try {
            ...
            validateDeletionJudgment(deletionBinding.path, {
                ...
                returnValidatorArgs: recursiveReturnValidatorArgs({ historicalGateReplay, offline }),
            ...
            const deletionConsumerOptions = consumerBoundsOptions(
                deletionAnnex.consumer_scan.bounds_authority,
                record.pins,
                "/annexes/deletion-judgment/consumer_scan/bounds_authority",
                offline ? "immutable" : "live",
                offline ? consumerUniverseImmutableBindingProjection(deletionAnnex.consumer_scan) : undefined,
            );
            deletionUniverse = validateConsumerUniverseReceipt(deletionAnnex.consumer_scan.receipt_path, deletionConsumerOptions);
            ...
            if (record.wave_id === "C05" && consumerAnnex && (
                deletionAnnex.consumer_scan.receipt_hash !== consumerAnnex.receipt_hash
                || deletionAnnex.consumer_scan.epoch.epoch_sha256 !== consumerAnnex.epoch.epoch_sha256
            )) {
                fail("/annexes/deletion-judgment/consumer_scan: C05 rehearsal must use its exact frozen consumer-universe census");
            }
        } catch (error) {
