...
export function projectImmutableDependencyClosure(rootWaveId, bindings) {
    const nodes = [];
    const edges = [];
    const identities = new Map();
    const visited = new Set();
    const active = new Set();
    const walk = (binding, consumerWaveId) => {
        if (typeof binding?.path !== "string" || !existsSync(binding.path)) throw new Error(`missing dependency return for ${binding?.wave_id}`);
        const metadata = lstatSync(binding.path);
        if (!metadata.isFile() || metadata.isSymbolicLink() || realpathSync(binding.path) !== binding.path) {
            throw new Error(`noncanonical dependency return ${binding.path}`);
        }
        const bytes = readFileSync(binding.path);
        const fileSha256 = sha256(bytes);
        const returned = parseJsonStrict(bytes);
        const preimage = structuredClone(returned);
        delete preimage.return_hash;
        const returnHash = sha256(canonicalize(preimage));
        const contractSha256 = waveContracts.get(returned.wave_id)?.sha256;
        if (binding.wave_id !== returned.wave_id
            || binding.file_sha256 !== fileSha256
            || binding.return_hash !== returnHash
            || returned.return_hash !== returnHash
            || binding.wave_contract_sha256 !== returned.scope?.wave_contract_sha256
            || binding.wave_contract_sha256 !== contractSha256) {
            throw new Error(`dependency binding does not match ${binding.wave_id}`);
        }
        const policy = requireWaveEdgePolicy(waveEdgePolicy, returned.wave_id, consumerWaveId);
        if (!policy.allowed_statuses.includes(returned.status)) {
            throw new Error(`${returned.wave_id} -> ${consumerWaveId} rejects status ${returned.status}`);
        }
        edges.push({
            from: returned.wave_id,
            to: consumerWaveId,
            return_hash: returnHash,
            status: returned.status,
            role: policy.role,
            allowed_statuses: policy.allowed_statuses,
            edge_policy_sha256: edgePolicySha256(policy),
        });
        const identity = canonicalize({ path: binding.path, file_sha256: fileSha256, return_hash: returnHash });
        const prior = identities.get(returned.wave_id);
        if (prior && prior !== identity) throw new Error(`conflicting dependency histories for ${returned.wave_id}`);
        identities.set(returned.wave_id, identity);
        if (active.has(binding.path)) throw new Error(`dependency cycle reaches ${binding.path}`);
        ...
        active.add(binding.path);
        for (const child of returned.scope?.dependency_returns ?? []) walk(child, returned.wave_id);
        active.delete(binding.path);
    };
    for (const binding of bindings) walk(binding, rootWaveId);
    nodes.sort((left, right) => compareCanonicalText(`${left.wave_id}\0${left.return_hash}`, `${right.wave_id}\0${right.return_hash}`));
    edges.sort((left, right) => compareCanonicalText(`${left.from}\0${left.to}\0${left.return_hash}\0${left.status}\0${left.role}`, `${right.from}\0${right.to}\0${right.return_hash}\0${right.status}\0${right.role}`));
    return { nodes, edges, descendant_closure_hash: sha256(canonicalize({ nodes, edges })) };
}
...
export function loadUniversalWaveReturn(owner, pointer, fail, cache = new Map()) {
    ...
    const validation = spawnSync(process.execPath, [universalReturnValidator, evidence.path, "--immutable-authority"], {
        encoding: "utf8",
    ...
    if (validation.status !== 0) {
        ...
        try {
            ...
            if (receipt.mode !== "offline-immutable-authority"
                ...
                || receipt.historical_gate_replay?.materialized_pin_count !== (returned.pins ?? []).length
    ...
    const result = { returned, file_sha256: fileSha256, return_hash: returnHash, authorization_closure: resultAuthorizationClosure };
    cache.set(key, result);
    return result;
}

export function loadValueTargetOwnerReturn(owner, pointer, fail, cache = new Map()) {
    const waveId = owner?.wave_id ?? owner?.owner;
    const result = loadUniversalWaveReturn(owner, pointer, fail, cache);
