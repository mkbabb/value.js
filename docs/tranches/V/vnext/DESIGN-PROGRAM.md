...
The values below are **probe brackets**, not acceptance thresholds. G00I binds
the design epoch and C00P measures each applicable role on the rendered
surface; G05 may narrow or reject a bracket from those records. No wave may
quote this table alone as proof that its motion is correctly dosed.

| Tier | Purpose | Probe bracket | Rules |
|---|---|---:|---|
...
- The column/visibility minima are layout requirements and the 0.5-physical-pixel bound is a geometric approximation contract; neither is a perceived-precision threshold. C00P's controlled tile-size/stroke/DPR record sets the visual precision threshold before G09 or M08 mutates source.
...
| 7 | Palette curation ownership was orphaned. | Preserved `standard|archived` tier and separated featured-membership authority. |
| 8 | Archived-tier migration was incomplete. | Bound all six visibility×tier states and the independent featured relation. |
