...
URL codec, and one router binding. In Value, the extant `usePaneRouter` idiom is
the ratified route/pane source of truth: this tranche does not reshape it. The
`UrlEnvelope` codec layers over that authority and serializes its shareable
projection. Keyframes follows the same one-router law through its M01 owner.
Components receive projections and dispatch actions; they do not read or write
the URL, storage, history, or API directly. Route state is a shareable
projection of application state, not a second store or parallel router.
...
| `/browse` | `pane:"browse"`; `q:""`; `sort:"featured"`; `tags:[]`; `tier:"standard"`; `curation:"all"` | normalized query, sort, sorted unique tag slugs, tier, curation, cursor only when it is a stable public cursor |
...
### Browse classification and curation join

`tier: "standard" | "archived"` is classification; `curation: "all" |
"featured"` is the independent public featured-membership filter. The exact
default is `{tier:"standard",curation:"all"}`, so both keys are omitted from
its shared envelope. Either nondefault is serialized under its own key and is
never inferred from the other. `archived + featured` refuses as
`invalid-schema`, because archiving removes featured membership. Visibility,
eligibility, membership CAS/version and operator authority are never URL state.
...
|---|---|---|
| Ratified router authority | D01–D02 (`usePaneRouter`), M01–M02 | D25, M11, C07–C09 |
| Shared codec law and generated schemas | D02, M02 | D25, M11, C07–C09 |
| Value route defaults and reducers | D01–D03, D06N–D24 | D25 |
| Browse tier/curation default and share projection | D02, D12F; API owners A07, A07T | D25 |
