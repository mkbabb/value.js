...
| I.W2 — *Soft-delete + grace + restore* | provisional | — | palettes.deletedAt; cascade-delete-with-grace; reaper cron; 410 Gone on GET grace-window palette |
| I.W3 — *Admin idempotency* | provisional | — | idempotent featured setter (`{ "featured": true | false }` body); legacy toggle retired |
| I.W4 — *SOTA envelopes + conformance suite* | provisional | — | problem+json (RFC 7807); ETag + If-Match (412 on mismatch); RateLimit-* headers; Idempotency-Key on POST + PUT; conformance suite at `api/test/conformance/` |
