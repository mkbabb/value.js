...
SPLIT-WORTHY. The same parity note binds `color/dispatch.ts` (522): W1-6's ICtCp fold adds a `color2()` dispatch arm there — its over-522 reading at close is likewise standing-verdict-plus-booked-growth (its no-split cohesion verdict is firm; only a fresh cohesion re-verdict flips it).
