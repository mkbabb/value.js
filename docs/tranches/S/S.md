...
| **X1 — prod api deploy** (R.W7 residue: host webhook dead; prod may serve pre-R api) | maintainer-on-host (exact instructions R `FINAL.md §7`) | CARRIED from R; **W0-3 verifies** (`/health` lineage, `/diff`, `/publish` 200) and repairs + deploys via `scripts/deploy.sh api` if reachable; while prod serves stale api the book stays OPEN |
