# S.W1 — LIBRARY: the 3.0.0 wave (S-24)

**Name**: W1 — Library (the 3.0.0 wave; was "the 2.1.0 wave" — the cut re-stamped to a major per `audit/RATIFICATION-2026-07-05.md §2.1`: the Q2 `logerp` reorder + Q3 `color-soa.ts` excision are public-API breaks)
**Opens after**: S.W0 (runs parallel with S.W2 — round 1; intra-round ordering below is binding).
**Spec of record**: `audit/SYNTHESIS.md §3.3` (items W1-1..W1-9 + gate riders) · §4 library slate · Q2/Q3/Q9 outcomes from `S.md §12` (RATIFIED 2026-07-05: Q2 FLIP reorder-now; Q3 EXCISE; Q9 WIDENED +Jzazbz) · `audit/RATIFICATION-2026-07-05.md §2.1/§2.3` (the 3.0.0 cascade; W1-10 raytrace per Q8's FLIP; W1-11 Jzazbz).
...
**Hard gate**: composite (§Hard gate) — 3.0.0 published + tagged with the by-name breaking-change MIGRATION table · vitest green incl. NEW dark-band + independent-oracle regressions · okhsl dodged band reinstated · caches bounded · cap-check per the census cohesion verdict · W1-10 raytrace agrees with the analytical oracle · W1-11 Jzazbz roundtrip + currency · PT-E dispatched.
**Status**: PENDING (RATIFIED 2026-07-05; gated on S.W0).
...
(SYNTHESIS §3.3 Goal, verbatim; the ratification adds: the `logerp` surface made honest at the
reorder, the raytrace gamut map and Jzazbz built — RATIFICATION §1 Q2/Q8/Q9.)

## §Completion criterion

3.0.0 published + tagged; vitest green incl. NEW dark-band + independent-oracle regressions; the
okhsl dodged sweep band reinstated; parse caches bounded; PT-E letter dispatched. (SYNTHESIS
§3.3 Completion, verbatim; 2.1.0→3.0.0 per RATIFICATION §2.1.)
...
| W1-6 | **New src surfaces (consumed downstream)**: `sampleOKLChSliceBoundary(hue, columns)` (the L×C cusp polyline for W5-8 — the demo never re-derives gamut geometry) · canonical `resolveEasing(string→TimingFunction)` (feeds W5-9; kf/glass-ui converge on it later) · `safeAccentCssString` (retires the demo's hand-denorm blocks, W2) · **ICtCp public space class** (Q9 RATIFIED 2026-07-05; the matrix math already shipped inside `deltaEITP`; cheapest net-new space ever — parsing + `color2()` dispatch + roundtrip goldens + `color2Into` currency; Jzazbz, Q9's widening, is its own item W1-11) | `boundary.ts`; `easing.ts`+`parsing/easing`; `contrast.ts:185`; `difference.ts:143` | design-gradient P1-6; lib-core P2-4; state-color P1-4; deferred-books §4 |
| W1-7 | **Public-surface hygiene** (land as N small commits, not one — the sub-fixes are independent): `color-soa.ts` EXCISED per Q3 (RATIFIED 2026-07-05; orphan API, phantom consumer — zero-consumer evidence recorded; a 3.0.0 break, rides the MIGRATION table) · `reverseCSSIterationCount` wired to its inline twin (keep the export — published API) · **the `logerp` reorder to `(a, b, t)` lands NOW** (Q2 FLIP, RATIFIED 2026-07-05 — "fix now. Breaking changes are fine: always."): all consumers migrated at root, NO compat shim; one of the two breaks that make this cut 3.0.0; rides the MIGRATION table · **`unpackMatrixValues` 2D-branch DEFECT**: the live code emits nonsense atan2-derived rotateX/rotateY for a planar `matrix()` transform (`utils.ts:304-306` — `rotateY = atan2(-c, a)`, `rotateX = atan2(b, d)`); the fix is to return 0 for the two out-of-plane rotations (this bullet is the defect-to-cure, NOT current behavior) · `ch`-unit `getContext` null-narrow (kill the blanket catch masking a null-deref) · `flattenObject` O(N²) flatten-after-loop · `toFixed` trailing-zero consistency · docstring truth (vh/cqw "computed" claim is false — correct or wire) | lib-core P1-1/P1-2/P2-1/-3/-5/-6; legacy-sweep-src P1/P2 | lib-core-value-audit; legacy-sweep-src |
...
| W1-10 | **Raytrace gamut map (R-4)** — NEW at ratification (Q8 FLIP: "Build now"): implemented, tested against the Ottosson analytical mapper as the ORACLE (agreement within stated tolerance on the shared domain); document where raytrace diverges and why that's the point; the R-4 book discharged by construction | `src/units/color/gamut.ts` (the analytical oracle) + new module | RATIFICATION §2.3; deferred-books-census §4 finding 6 |
| W1-11 | **Jzazbz space** — NEW at ratification (Q9 WIDENED: "Jzazbz is to be implemented, too"): full space class + parsing + `color2()` dispatch wrapper + tests; its PQ-variant transfer math is NET-NEW (unlike ICtCp's already-shipped path — scope it honestly); roundtrip goldens + `color2Into` currency; rides the 3.0.0 cut alongside ICtCp (W1-6) | `src/units/color/` (space class, parsing, dispatch arm) | RATIFICATION §2.3 |
...
### Publish note (SYNTHESIS §4, binding; amended per RATIFICATION §2.1/§2.3)

The 3.0.0 publish note names the srgb fix as **output-changing for near-black** (≤2/255, 8-bit
1..10 band) and enumerates the regoldened fixtures. **MIGRATION-table discipline applies: the
3.0.0 cut ships a by-name breaking-change table** — the `logerp` signature reorder (Q2) · the
`color-soa.ts` removal (Q3) · anything the W0-9 dependency sweep excised from the public
surface. ICtCp rides Q9; Jzazbz rides Q9's widening (W1-11); the raytrace gamut map builds per
Q8's FLIP (W1-10). **Deliberately NOT done** (recorded): R-8 gamut-relative spaces (KILLED) ·
sibling-index/count, device-cmyk, ICC · R-7 HCT/CAM16 · Pratt calc() transposition (do NOT pull
forward).
...
|---|---|---|
| color (W1-1, W1-6 boundary/ICtCp, W1-10 raytrace, W1-11 Jzazbz) | `src/units/color/conversions/transfer.ts` · `gamut.ts` · `boundary.ts` · `contrast.ts` · `difference.ts` · `dispatch.ts` (ICtCp + Jzazbz arms) · the raytrace module · the Jzazbz space class/parsing · new goldens/tests | modify/create |
...
1. **3.0.0 published + tagged** (registry `dist-tags.latest=3.0.0`; annotated tag pushed);
   publish note carries the near-black output-change + regoldened-fixture enumeration + the
   by-name breaking-change MIGRATION table (logerp reorder · color-soa removal · any W0-9
   public-surface excision — RATIFICATION §2.1).
...
6. **W1-10**: the raytrace gamut map agrees with the Ottosson analytical oracle within stated
   tolerance on the shared domain; the divergences documented (why they're the point).
7. **W1-11**: Jzazbz roundtrip goldens green + `color2Into` currency holds through it.
8. The `logerp` reorder landed with ALL consumers migrated at root (grep: zero t-first call
   sites); NO compat shim exists (Q2 FLIP).
9. Fresh-build `.d.ts` guard green; the post-W1 cap check per the census cohesion verdict (the
   near-guaranteed `units/utils.ts` ledger row recorded, not silently missed).
10. PT-E letter dispatched (W1-9).
11. `npm run lint` 0 · `npm run typecheck` 0 · `npm test` green · `npm run build` clean.
...
the `logerp` reorder + consumer migration as ONE of them, with body); W1-10 (raytrace + oracle
tests, own commit with body); W1-11 (Jzazbz, own commit); W1-8 per-lift commits,
`stylesheet.ts`/`color.ts` LAST; the 3.0.0 version commit + tag (MIGRATION table in the body);
...
- **`srgbToLinear` decode defect** — FIRES here (the book's own "next output-changing minor"
  trigger; the 3.0.0 major subsumes it); retired at the 3.0.0 publish.
- **R-6 ICtCp** — FOLDS here per Q9 (Q9 RATIFIED-WIDENED: Jzazbz builds too — W1-11).
- **Q2 `logerp` reorder** — NO book: the Q2 FLIP lands it in-wave (W1-7); this 3.0.0 cut IS the
  reorder's vehicle.
- **R-4 raytrace** — DISCHARGED-BY-RATIFICATION: builds here (W1-10).
...
published surfaces. W5-8/W5-9 consume the W1-6 exports — ZERO new demo math. The 3.0.0 cut
