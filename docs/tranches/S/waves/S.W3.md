...
| W3-6 | **Mix clock + Safari-true pour**: ONE authored clock (phase machine reads the canvas's own duration constants, or completion events); `ctx.filter` (unsupported on WebKit) replaced with Safari-true primitives; total ≤1.2s per Q10; reveal lands AT the result plate; generic spinner rows deleted (the animation IS the progress). **Q10 RATIFIED-AMPLIFIED 2026-07-05** (RATIFICATION §1): re-graded from re-author to **first-principles RE-WORK** — the current pour choreography is REPLACED, not repaired; the animation narrates the mix (convergence lands AT the result plate); beauty is a GATE, not a garnish (Fable design lane + the frontend-design bar — the design half binds W5 Lane B's mix recomposition too); Safari-true BY CONSTRUCTION: no `ctx.filter`, no WebKit-unsupported dependency, **NO degraded fallback path — one implementation, every engine**; the ≤1.2s / one-clock gates stand | `useMixingState.ts:86-104`; `useMixingAnimation.ts:65-68,114` | motion §3 P0-M; design-extract-mix F6; RATIFICATION §1 Q10 |
