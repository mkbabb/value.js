...
**Opens after**: tranche ratification (`S.md §12` — **RATIFIED 2026-07-05**). Round 0 — everything else opens after this wave.
**Spec of record**: `audit/SYNTHESIS.md §3.2` (items W0-1..W0-7) · oracle slate §6.1 · book discharges per `deferred-books-census` · `audit/RATIFICATION-2026-07-05.md` (W0-8 per §2.2.2; W0-9 per §2.4) · the `audit/seeds/SEEDS.md` w0 riders (folded into W0-1).
...
**Agents**: 1 serial (W0-1..W0-9 in item order; W0-3's prod probes are read-only and may interleave; W0-8 is research and may interleave). No parallel writers.
**Hard gate**: composite (§Hard gate) — `npm run dev` round-trips palettes locally · smoke-safari in CI · Lighthouse hard-fail · record-only discharges written · the W0-8 genesis brief + W0-9 dependency ledger written · hygiene landed with clean `git status`.
**Status**: PENDING (RATIFIED 2026-07-05 — dispatch open; round 0).
...
|---|---|---|---|
| W0-1 | **Dev-backend truth** (S-11): `npm run dev` delegates to `scripts/dev.sh up` (local api + mongo rs0 + `VITE_API_URL` + dev CORS); explicit `dev:web-only`; origin-honest failure in the availability path (detect the unset-env + localhost + cross-origin-prod precondition → loud dev-misconfig message, never "backend unreachable"); demo/CLAUDE.md + root CLAUDE.md Build section updated. **REJECTED**: adding localhost to prod `ALLOWED_ORIGINS` (recorded so it isn't proposed as a shortcut). **Seed riders (SEEDS.md w0, folded at ratification — the seed proved the mechanism VIABLE)**: (1) surface the `misconfigured` state in an always-mounted dev banner, not only the save-conditional chip (`ApiOfflineChip` mounts only inside `CurrentPaletteEditor` under `v-if="savedColorStrings.length>0"` — the misconfig state is not guaranteed visible at first paint); the unconditional `console.error` is load-bearing either way; (2) optional `predev:web-only` guard (`test -f dist/value.js \|\| npm run build`, mirroring the `prepare` idiom) so the named escape hatch boots on a cold checkout — the honest `dev`→dev.sh entrypoint already one-shot-builds | `package.json:71`, `client.ts:34-35`, `scripts/dev.sh:279-296`, `availability.ts` | api-broken-rootcause; SEEDS.md w0-dev-backend |
...
| W0-8 | **Blob SOTA assay + archaeological survey** (Q7 amplification — RATIFICATION §2.2.2): (a) archaeology — every prior implementation hitherto (value.js SVG-goo era → the demo WebGL2 metaball GooBlob → glass-ui's WebGPU+WebGL2 producer engine), what each did well, what killed it; (b) SOTA assay — the current art (SDF smooth-min fields, marching/analytic metaballs, WebGPU compute, stylized NPR goo, perf envelopes on mobile GPUs); (c) a genesis brief that feeds the JOINT rebuild: the producer engine is glass-ui's (L5), value.js owns the consumer contract + the demo's compositional law. **Artifact**: `docs/tranches/S/audit/blob-genesis.md`. W6's blob items consume it; the glass-ui letter carries it as the shared ground | research (artifact create) | RATIFICATION §2.2.2; blob-greenfield-tech; design-blob-atmosphere-vision |
| W0-9 | **Dependency-excision ledger** (Q3 broadening — RATIFICATION §2.4): every runtime + dev dependency justified by a live consumer or excised (the R-era keyframes-devDep analysis is the model; the CLAUDE.md §3.4 `file:` pin policy is NOT reopened). Artifact: a short ledger in the W0 evidence packet; excisions land in-wave; any public-surface excision rides the 3.0.0 by-name MIGRATION table (§2.1) | `package.json` (deps) | RATIFICATION §2.4 |
...
| Item | Files | Access |
|---|---|---|
| W0-1 | `package.json` (scripts, incl. the `predev:web-only` guard) · `scripts/dev.sh` · `demo/@/lib/palette/api/client.ts` + `availability.ts` (origin-honest failure path) · the always-mounted dev-banner mount site (seed rider 1) · `demo/CLAUDE.md` · `CLAUDE.md` §Build | modify |
...
| W0-8 | `docs/tranches/S/audit/blob-genesis.md` (research artifact) | create |
| W0-9 | `package.json` (dependency excisions) · the dependency ledger in the W0 evidence packet | modify/create |
...
6. The W0-8 genesis brief exists at `audit/blob-genesis.md` (archaeology + SOTA assay + the
   joint-rebuild brief — RATIFICATION §2.2.2); W6's blob items and the glass-ui letter can
   consume it.
7. The W0-9 dependency ledger written (every dep justified-by-live-consumer or excised);
   excisions landed in-wave; public-surface excisions handed to the W1 MIGRATION table.
8. `npm run lint` 0 · `npm run typecheck` 0 · `npm test` green · clean `git status`.
...
artefact path; the W0-8 genesis brief path (`audit/blob-genesis.md`); the W0-9 dependency
ledger; per-item commit hashes.
...
artefact); W0-8 (genesis brief, own commit); W0-9 (dependency ledger + excisions, own commit);
a status commit at close. Scopes name the owned surface (dev / ci-oracles / doc-truth /
dead-surfaces / router-probe / blob-genesis / dep-ledger).
...
record seeds the X1/X2 book rows the close wave (W9) re-verifies. The W0-8 genesis brief feeds
W6's blob items and the glass-ui letter's L5 co-rebuild ground; W0-9's public-surface excisions
(if any) feed the W1 3.0.0 MIGRATION table.
