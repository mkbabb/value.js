...
| **error-injection / thrown-error state** | **build (a11y half → fold U-F56) + gestalt half → U.W-VISUAL census** | The registry §16 named `error-injection` as one of the five untested modalities; an induced thrown-error state IS a reachable UI state. Its a11y half — the error boundary is focus-managed + SR-announced (a live-region/`role="alert"` surface), never a silent dead plate — folds into **U-F56**'s driven-live battery (drive an induced error over the authed/populated surface). The PERCEPTUAL "thrown-error micro-chrome reads coherent" gestalt is the U.W-VISUAL W8-inheritance census's unfiled-pass residue (the T-MARK §3.1 `micro-chrome + thrown-error` pass, VERDICT §5-D3 declined-ledger row). Broader fault-injection resilience testing is out-of-scope (recorded). |
...
| **U-F58** untested-web-modalities | **build-or-out-of-scope PER MODALITY** | RTL + print BUILD (born-RED BR-10/BR-11); PWA/offline + i18n-strings OUT-OF-SCOPE (recorded); long-session/memory FOLD → U.W-PERF; **error-injection/thrown-error** BUILD a11y-half FOLD → U-F56 + gestalt-half → U.W-VISUAL census (the registry §16 fifth modality, restored r6). Zero silent drops per modality. |
