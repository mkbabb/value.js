...
**§28.3 RELAY DISCHARGED (2026-07-12)**: the U→glass-ui BH communiqué LANDED at glass-ui HEAD `17e0f522` (`../glass-ui/docs/tranches/BH/coordination/valuejs-inbox-2026-07-12-u-formation.md`, path-scoped, grep-clean, local-only per the foreign-tree fence) — §1 the mixColors-convention coupling (spectrum-walk co-migrate/hold at cut), §2a U-F4 PRM-dock, §2b the 8 carried-forward T producer reds, §3 the 5.0.0 + parseCSSValue-reshape co-land. The relay half of §28.3 is now REAL. Round 8 (folded into the formation's foundation phase) confirms the structural resolution on this discharged state.
