...
| **U-F30** normalized serialization | **SHARPENED + CONFIRMED** | No denorm seam exists (`toFormattedString` emits the same normalized floats; base.ts:248 calls both "canonical round-trip serializers"). Root: `mixColors` returns a `Color<number>` normalized [0,1] (mix.ts:105,177) and `createColorValueUnit` (color-unit.ts:32) wraps it WITHOUT denormalizing; `toString` (base.ts:202) emits verbatim. **The relative-color path ALSO leaks** (`rgb(from red r g b)`→`rgb(1 0 0)`, should be `rgb(255 0 0)`) — same `createColorValueUnit`-wraps-normalized seam. **FIX-LOCUS CORRECTED by R-2 (§below) / §28**: NOT the shared `createColorValueUnit` wrapper (that BAND-AID double-denorms the working direct path) — the invariant-preserving fix is at the `mixColors`/`resolveRelativeColor` OUTPUT (or, §28-preferred, ONLY the `toString`/serialization layer, holding the raw-channel convention). Zero tests assert a color-mix/relative-color serialization. |
...
**Amelioration consolidation**: U-F30 (+relative-color leak) is a single-locus correctness fix at the **`mixColors`/`resolveRelativeColor` OUTPUT** (physical-range, matching direct-parse) OR — the §28-preferred invariant — at ONLY the `toString`/serialization layer (a per-instance normalization-state brand), holding the raw-channel convention; the design loop picks the invariant. **NOT the shared `createColorValueUnit` wrapper** — R-2 (§below) proved that BAND-AID double-denorms the working direct-parse path (which already stores physical channels), so the shared-wrapper locus is FORBIDDEN, not a candidate. U-F29/U-F31/U-F32/U-F33 are the **serialization/contract CLASS** the owner's E-3 binds together — one library-correctness wave, born-RED per defect, no cherry-picking. The publish/semver decision stays with the owner (§13.5).
...
# ROUND 5 (2026-07-12) — the confirming pass. 1 fresh adversary. Verdict UNSTABLE for ONE bounded scoping gap (R-1/R-2/R-3 all CONFIRMED sound; R-2 STRENGTHENED). Amendment applied → round 6 = final confirm. Budget 30/32.

## §22 THE R-4 AMENDMENT (round-5 finding — VERIFIED by direct read, applied)

R-4/U-F77 scoped U-F29's cross-repo consumers to glass-ui ALONE and asserted "breaks no CONSUMED API (glass-ui uses only parseCSSColor)". **That premise is INCOMPLETE**: **keyframes.js is a co-equal second consumer** —
- pins `@mkbabb/value.js '^3.1.0'` (`../keyframes.js/package.json:268`, VERIFIED) → a 4.0.0 cut strands **TWO** `^3.1.0` floors, not one; **keyframes must co-widen its pin at the cut** (a co-migration R-4 omitted). **[r5 CORRECTION — the block name/framing]:** this keyframes pin lives in the **`dependencies`** (RUNTIME) block (`:267-269`, value.js appears there EXACTLY ONCE), NOT `devDependencies` — it is a **RUNTIME dependency floor**, the STRONGER coupling (a 4.0.0 cut strands keyframes' runtime dep AND every transitive keyframes consumer, not just keyframes' own build). glass-ui's is the peer floor (`peerDependencies` + `devDependencies`); keyframes' is the runtime floor. The prior "TWO peer floors" shorthand mislabeled keyframes' — corrected at the operative loci (U.W-ADOPT/U.W-LIB/FORMATION-VERDICT §72/§9).
- DIRECTLY consumes the reshaped `parseCSSValue` at **3 live sites** (`../keyframes.js/src/animation/resolve/resolve-function.ts:61,159` + `resolve-if.ts:131`, VERIFIED) + `parseCSSSubValue`/`parseCSSValueUnit` at the compile path. The 3 parseCSSValue calls are try/catch-guarded (a throw-shape amelioration degrades gracefully, no crash) but STILL change keyframes' resolved values (multi-token defaults truncate today; after a loud-throw the catch fires → raw-node/DROP; a full-value-default returns a ValueArray the unguarded downstream expects as a single ValueUnit).

**CORRECTION (applied)**: U-F77's disposition (a co-land ordering row) **SURVIVES and is REINFORCED**; only its consumer scoping was wrong. The library-correctness wave's **born-RED gates MUST drive BOTH consumers** (glass-ui's color path via the ^3.1.0 minor auto-adopt of the U-F30 serialization change + keyframes' parse path), and U-F31 (transform single-axis expansion) specifically feeds **keyframes' apply/resolve path** — the concrete "producer-coupling turns build→co-migrate" the charter asks to surface. The version-cut/publish decision (owner-held, §13.5) co-lands against BOTH floors (glass-ui peer + keyframes runtime — see the r5 correction above).

## §23 ROUND-5 CONFIRMATIONS (the reconciliations that held)

- **R-2 SOUND + STRENGTHENED**: verified end-to-end that the direct-parse path stores PHYSICAL channels (toString at base.ts:202 emits `values()` verbatim, no denorm) and the mix/relative paths store NORMALIZED [0,1] (mixColors mix.ts + resolveRelativeColor relative-color.ts:132-163) — so denormalizing at the shared `createColorValueUnit` WOULD double-transform the working direct path (the trap is real); the corrected mix/relative-OUTPUT locus is right and the denorm machinery exists (`serialize.ts convertColorSpaceDenorm`). **STRENGTHENED**: the normalized mix/relative ValueUnit is ALSO wrong when re-fed through `normalizeColorUnit`/`colorUnit2` (double-normalizes 0.3→0.00118) — the EXACT pipeline glass-ui uses; the physical-output fix resolves BOTH the serialization and the re-normalize symptoms at one locus. **Design-loop note**: `resolveRelativeColor` evaluates calc component-exprs against NORMALIZED bindings (relative-color.ts:137-142), so `rgb(from red calc(r + 10) g b)` computes 1+10 normalized then denorms ≠ CSS's 255+10 — **normalize-on-construct handles the calc case cleaner than denorm-on-output** (the invariant pick R-2 already hands the design loop).
- **R-1 SOUND**: glass-ui `morph.css:70-73` derives `--dock-expand-t` from the PRM-frozen `--dock-morph-t` under `[data-morphing]` for BOTH `.expanded` + `.always-expanded`; `overture.css:171-181` is the confirmed red herring. Small over-statement noted (mobile `always-expanded=true` degrades to 244px-not-empty → `always-expanded` might partially mitigate desktop) but non-blocking: the canonical fix is producer-side and a consumer CSS override would breach the glass-ui-first precept, so the fold-into-U-F2 disposition holds.
- **R-3 SOUND**: the shared-surface coordination hazard holds (accepted §10's live CLS=LCP same-element measurement per parsimony).
...
- **W-adopt (the disease wave)** — U-F2 (glass-ui 5.0.0 at the cut) + U-F4 (producer PRM-dock) + U-F68 (lock refresh) + **U-F77 co-land ordering** binding the lib-correctness version cut against BOTH ^3.1.0 floors (glass-ui peer + keyframes runtime `dependencies`) AND glass-ui's spectrum-walk convention co-migration.
...
**U-F77 EXTENDED**: the co-land ordering now binds THREE things at the library-correctness cut — (a) the version decision against BOTH ^3.1.0 floors (glass-ui peer + keyframes runtime `dependencies`), (b) glass-ui's spectrum-walk color-convention co-migration, (c) keyframes' backward-color color-convention co-migration. All relayed per the owner edict.
...
**§28.3 RELAY DISCHARGED (2026-07-12)**: the U→glass-ui BH communiqué LANDED at glass-ui HEAD `17e0f522` (`../glass-ui/docs/tranches/BH/coordination/valuejs-inbox-2026-07-12-u-formation.md`, path-scoped, grep-clean, local-only per the foreign-tree fence) — §1 the mixColors-convention coupling (spectrum-walk co-migrate/hold at cut), §2a U-F4 PRM-dock, §2b the 9 carried-forward T producer reds (T-60/T-52/GAP-L2/GAP-L5/PKT-1/T-38/T-45/P1-R3/GAP-ARM), §3 the 5.0.0 + parseCSSValue-reshape co-land. The relay half of §28.3 is now REAL. Round 8 (folded into the formation's foundation phase) confirms the structural resolution on this discharged state.
