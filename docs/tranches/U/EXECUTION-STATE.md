...
| (queued) | **V DESIGN CAMPAIGN — Pass 2** (per `design/pass-1/AGGLOMERATION.md §3`: stage-0 retro-critics on the un-adversaried F3/F4 → Charter A [F1∘F3 WHAT-tree, lead] ∥ B [F6 vehicle + banked-F2 substrate] ∥ C [F4 diagnostics + the 10→7 battery] → per-charter critics → agglomerate). Pass-1 FOLDED (`88dc0fe`): F1 45 · F3 48 · F4 44 · F6 37 ADVANCED; F2 40 · F5 42 BANKED-as-substrate | **ARMED for 17:32 ET** (2 pass-1 critics died on the 17:30 session wall — reset+2min law; never burn a batch into a known wall). Pass-1 lesson: the `${'$'}{x}`-escape in workflow scripts passes LITERAL text — pass-2 scripts use REAL interpolation | — | on wall: re-arm to next reset+2min |
