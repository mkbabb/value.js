...
| — | **CAMPAIGN PAUSED BY THE OWNER (2026-07-13 ~21:00 ET)** — "once these return, pause, clean up the crons, halt. We\'ll come back to this." Pass-4 FOLDED (`48eb4ff`): NOT clean (clock 0 of 2) but composed thesis **60→78%** (never-inflate floor of the two lenses 80/78); ALL SIX charters ADVANCED (A 68 · B 62 · C 68 · δ 72 · ε 72 · ζ 75); the un-charted owner half CHARTED with **9 durable instruments — every one re-run + reproduced by both auditors from the repo root**. Pass-5 charters NAMED in `design/pass-4/AGGLOMERATION.md`: **η** execution-reproducibility codemods (commit the --apply carve/scatter; back-fill β\'s registry cure) · **θ** scope-honesty + fork CLOSURE (@src is LIVE repo-wide at 235 sites, not DEAD; shrink the fork pile) · **ι** the clause-9 PERFORMANCE limb (un-charted across all 4 passes). **THE HANDOFF OF RECORD: `docs/tranches/V/HANDOFF-2026-07-13.md` (read FIRST).** **RESUME RECIPE**: read DESIGN-CAMPAIGN.md (registry current) + pass-4/AGGLOMERATION.md §pass-5-charters → launch pass 5 on the pass-4 workflow pattern (durable-instrument law binds; owner forks OF-1..OF-6 + OF-ε-1/OF-ε-2 + NG-6-R1/R2 presented-not-decided). Crons KILLED at pause; no live runs; :9000 left up | — | — |
