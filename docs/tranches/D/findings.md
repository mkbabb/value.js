...
| RA-1 | **Mar-2026 `colorUnit2` nesting bug excavated**: bug-commit `35cd9d5` (Jul 17 2024 — `colorUnit2()` shipped without unwrap, latent 19 months across 4 refactors); fix-commit `80cdd59` (Mar 1 2026 — `while (raw instanceof ValueUnit) raw = raw.value` at `src/units/color/normalize.ts:102`). **Fix intact at HEAD**; 10 other read-only defensive peels recorded. Memory's "ONLY accumulation site" claim holds. | `audit/D-REACTIVITY-A-recursion.md §1-2,7` | **D.md §2 new invariant D7** | codified — the unwrap pattern is a tranche-D-binding rule |
| RA-2 | **L8 Color flatten — 6 site-classes at re-introduction risk** under own-property storage (Map-keyed storage was an implicit chokepoint). 4 hardening primitives REQUIRED inside the L8 commit: (a) `ColorChannel<T>` TS brand, (b) DEV-only `assertNotNested` setters, (c) `test/recursion-guard.test.ts` (294-frame replay + clone-no-amplify + depth-3 nest), (d) `clone()` depth-16 ceiling. | `audit/D-REACTIVITY-A-recursion.md §4-6` | **D.W1 Lane L8** (REQUIRED — not optional) | each primitive sketched in the audit doc; the `Color.clone()` rewrite uses per-channel explicit construction (not `new Constructor()` no-args). |
| RB-1 | **Instant-reactivity topology verified clean** — 13 reactivity-graph barriers cataloged; all hot-path barriers are rAF-coalesced, debounced-by-design, or echo-suppressed. The two-tier `shallowRef<ColorModel>` cache (`App.vue` + `useColorModel`) is load-bearing for `defineModel`'s async round-trip and `lastWrittenModel` echo-suppression — **KEEP**. `stableHue` low-chroma guard (`s*v > 0.01`) intact at 3 update sites. | `audit/D-REACTIVITY-B-instant.md §1-2` | — | topology already correct; no fix needed |
| RB-2 | **L8 × Vue reactivity verdict** — trap-cost regression does NOT apply. Zero `Color`/`ValueUnit` instances live inside `reactive()` (the 4 `reactive()` sites hold pure primitives). L8 flatten is strictly cheaper. Add `isReactive(color) === false` regression test inside L8's sub-gate. | `audit/D-REACTIVITY-B-instant.md §3` | **D.W1 Lane L8** sub-gate | regression test sketched; folded |
| RB-3 | **Instant-reactivity wall-clock evidence primitives** — (a) `e2e/smoke/reactivity-instant.spec.ts` (spectrum-drag → docs-pane-update timing ≤ 50 ms median); (b) `useEffectCensus` DEV probe (optional, leak-detection across view switches); (c) `bench/color-channel-access.mjs` L8 microbench as ACCEPTANCE GATE (post-L8 ≥ 5× faster). | `audit/D-REACTIVITY-B-instant.md §7` | **D.W5** (a, b optional) + **D.W1 Lane L8** (c REQUIRED) | both REQUIRED primitives are merge gates per `D-RELEASE-PLAN.md §5` |
| REL-1 | **Merge to master + version-bump strategy** — value.js `0.5.1 → 0.6.0` (minor; L8 storage shape + C1 rename + contract-v2 `development` removal are at least two breaking changes — pre-1.0 semver mandates minor bump). Merge `tranche-b → master` with `--no-ff`; tag `v0.6.0`; post-merge publish + file the keyframes.js consumption-update ask. | `D-RELEASE-PLAN.md §2-3` | **D.W6** close ceremony | release plan authored; D.W6 executes against it |
