...
10. **`bbnf-equivalence.test.ts` misnomer** (per `audit/D-LIB-OPTIMIZATION-SYNTHESIS.md §3 L15`, Dm BBNF section). Verified by CHALLENGE-Dm: the file imports zero BBNF runtime — it snapshots hand-written parsers. Two options: (a) rename to `parser-snapshot.test.ts` (reflects reality); (b) actually wire it to execute the BBNF grammars (`src/parsing/grammars/css-values.bbnf`, `src/parsing/grammars/css-color.bbnf`) and assert equivalence. D.W6's doc-drift lane decides which based on whether the BBNF grammars are intended to be runtime-relevant or documentation-only. Default (KISS): rename. Recorded in `audit/D.W6-doc-drift.md` with the decision rationale.
