...
1. **Out-of-gamut as a designed state** — define four project tokens in `style.css :root` (the sanctioned home, DESIGN.md:11): an ink pair and a paper pair, because the contour must stay legible over *every* (s,v) region of the field — single-ink hatch disappears on dark/saturated substrate (proven in prototype):
   ...
   ```
   The overlay's canvas resolves all four via a computed-style probe and applies them **per contour segment by the field's own luma** — the exact `spectrumDotStyle` logic (the shared `spectrumLuma.ts` helper: luma = v·(1−s/2), flip at `SPECTRUM_LUMA_FLIP` = 0.5 — one shared function, one threshold, so the contour's ink regime and the dot's border regime never disagree about the same region): ink pair over light field, paper pair over dark; the hatch is drawn twice, registered. This is the visual vocabulary of the signature moment.
...
### MICRO-INTERACTIONS
1. **Slider thumb = instrument needle** — share `SpectrumCanvas`'s value-aware border logic (`spectrumDotStyle`, since R.W3 Lane E lifted to `composables/useSpectrumPlateStyle.ts:47-66`; the `luma`-driven black/white contrast is the shared `spectrumLuma.ts` helper, R.W3 Lane B) to the slider thumb so the knob always reads against its ramp. Add a center notch (`::after`, 1px ink line). `ComponentSliders.vue:93` + scoped `.slider-thumb`. *(LANDED at R.W3 Lane C — the thumb is the glass-ui `variant="spectrum"` consume fed by `--slider-thumb-bg`/`--slider-thumb-border-color`.)*
