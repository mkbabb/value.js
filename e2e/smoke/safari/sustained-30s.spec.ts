...
const CONSOLE_FAIL_SUBSTRINGS = [
    ...
    "[stale prop]",
    // S.W0 W0-2(c) + S.W6-5 (safari-truth §4/§P0) — the shader-compile oracle. The
    // aurora WebGL2 shader fails to COMPILE on WebKit's stricter GLSL-ES validator
    // (ANGLE/Chromium is lenient), surfaced verbatim as
    // `[aurora] init failed: Error: [Aurora] shader compile failed: …`. The prior
    // 3-substring guard only watched context-loss / stack-overflow / stale-prop, so
    // this exact P0 sailed through green. `"init failed"` (broader) subsumes the
    // `[aurora] init failed` prefix; both substrings are keyed to the
    // `[aurora]`/`[glass-ui]` deferred-init `onInitError` prefixes (a WebGL producer
    // that cannot compile its shader is never a healthy frame). This is the oracle,
    // NOT a shim: the defect is glass-ui's (L1); the gate makes it VISIBLE — with the
    // L1 producer cure landed it stays silent; a regression (or a fresh WebKit-only
    // GLSL rejection) re-reddens here. Never widen a noise allow-list to swallow it.
