...
import { importPalettes } from "../../services/admin/import.js";
...
router.get("/users", async (c) => {
    ...
    const page = await listUsers(c, limit, offset, q);
    return c.json(page);
});
...
router.get("/users/:slug/palettes", async (c) => {
    const data = await listUserPalettes(c, c.req.param("slug"));
    return c.json(data);
});
...
router.post("/users/:slug/status", async (c) => {
    ...
    await setUserStatus(c, c.req.param("slug"), parsed.data.status);
...
router.delete("/users/:slug", async (c) => {
    const result = await deleteUser(c, c.req.param("slug"));
    return c.json({
        deleted: true,
...
router.delete("/users/:slug/palettes", async (c) => {
    const deleted = await deleteUserPalettes(c, c.req.param("slug"));
