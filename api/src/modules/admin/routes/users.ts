...
router.post("/users/prune-empty", async (c) => {
    const pruned = await pruneEmptyUsers(c.var.services, c.var.adminActor);
...
router.post("/users/:slug/status", async (c) => {
    ...
    await setUserStatus(c.var.services, c.var.adminActor, c.req.param("slug"), parsed.data.status);
...
router.delete("/users/:slug", async (c) => {
    const result = await deleteUser(c.var.services, c.var.adminActor, c.req.param("slug"));
    return c.json({
        deleted: true,
...
router.delete("/users/:slug/palettes", async (c) => {
    const deleted = await deleteUserPalettes(c.var.services, c.var.adminActor, c.req.param("slug"));
