 ...
 *
 * Error semantics follow the typed-throw precedent in `etag.ts`: this
 * middleware never throws on its own; a replayed response is reconstructed
 * directly. Errors thrown by the wrapped handler propagate to `app.onError`
 * and are NOT cached (only successfully-produced responses are captured).
 */
...
export const idempotency: MiddlewareHandler = async (c, next) => {
    ...
    // Only capture a real, body-bearing response (not 204/304). Errors thrown
    // upstream skip this block entirely (they bubble to app.onError).
    const res = c.res;
